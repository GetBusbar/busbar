...
- **An `export` plugin must be rebuilt, and this is a BREAKING plugin-author API change.** The
  export payload schema moves `2` -> `3`: the reply to `deliver` now carries the sink's
  acknowledgement (`durable` / `received` / `retry`) where it used to carry nothing, so a host can
  tell a sink that took a record from one that dropped it. On the wire, a changed reply shape is a
  breaking change, so a v2 sink is refused at LOAD with a clear message rather than mis-called per
  delivery. For plugin AUTHORS: `ExportHandler::deliver` now returns an `ExportAck` instead of `()`
  — an existing sink must return one of the three words (its default now drops the batch and
  answers `Retry`, the honest word for a sink that took the record nowhere). Rebuild against the
  1.6.0 SDK, re-sign, and bump the manifest's `abi_version` to `3`. This affects `kind: export`
