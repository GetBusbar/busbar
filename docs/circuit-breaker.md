...
A tripped tool server answers HTTP **`503`** with a **`Retry-After`** header populated from the cell's own remaining cooldown: an exact number rather than a guess, the same shape budget rejection already uses with `429`. The body is a **JSON-RPC error** with code **`-32001`**, in Busbar's implementation-defined band — JSON-RPC 2.0 §5.1 reserves `-32099`..`-32000` for exactly this, and MCP reserves the `-32099`..`-32020` half of it for codes the specification itself defines, so Busbar's own extensions sit in `-32019`..`-32000` beside `-32000` (`refused`) — carrying structured `data`:
