...
**Read the state of this honestly before you configure anything.** As of this revision the breaker's **trip and fast-fail** are live on the MCP client leg (both transports) and on the A2A relay (all three bindings): every registered tool server and agent has its own cell on the one core breaker, a failing target trips it on the same thresholds the LLM plane uses, a tripped target refuses in milliseconds — MCP answers `503` + `Retry-After` + a JSON-RPC error (never an `isError` tool result: the call never happened), A2A answers the submission `rejected` with a task id to poll — and recovery is the same single-flight half-open probe. And as of this revision the **failover half is live too**: the `pools:` failover pools below (their kind inferred from their `tools:`/`agents:` members) are consulted at dispatch through the one selection loop (`failover::walk`), so a tools/call to a pooled server whose primary is tripped — or cannot even be connected to — is rerouted to its verified twin before the first byte, and a fresh A2A submission to a pooled agent is walked the same way at admission. What stays deliberately narrower than the LLM plane is stated in the safety rules below: after a dispatch has gone out only operator-listed `repeatable:` operations move, and an accepted A2A task is pinned to the member that accepted it. The LLM plane, and everything above this section, is unchanged and has been live for every release since the breaker landed.

**A failure that did not trip the cell does not bench it, and on these planes that is a difference from the LLM plane rather than an omission.** ADR-0002 states the `TransientUpstream` rule as one sentence with two halves — drive trip evaluation *and* re-arm an exponential cooldown; **fail over** to the next candidate. On an LLM pool the cooldown is the *deprioritisation* half: it says "prefer a sibling for a while", and the failover half is what keeps the caller served while it lasts. These planes now ship both halves, but the failover half only reaches a target an operator put in a `pools:` failover pool, and the **unpooled single registration is the canonical case** — so on a degenerate single-member cell the same cooldown would mean "refuse **every** caller of this server for the next 15–120 seconds", handed out after **one** transient blip, on a cell whose own trip predicate had just declined to trip, and rendered to the caller as `-32003` … *"open after repeated failures"*. So the MCP and A2A cells refuse on a **trip** and nothing less: the published predicate, `error_rate >= 0.5` over at least `min_requests` outcomes in the 30-second window. An upstream's own `Retry-After` is still honoured, for as long as the upstream asked for — that is the upstream's backpressure, not Busbar inventing an outage. On the LLM plane, where the walk always has somewhere else to go, the sub-threshold cooldown is unchanged. A **pooled** MCP or A2A cell inherits the plane setting rather than the LLM one — this is a per-breaker choice, not a per-cell one — so `failover::walk` selects past a pooled member only once that member's own cell has actually **tripped**, never on a sub-threshold bench. An **unpooled** registration has no verified twin to select, so a trip there is a refusal rather than a movement: "reroute" on these planes means *the walk chose a different member of a pool you declared*, and it never means Busbar found somewhere else to send a call on its own.
...
**There is one place to configure a breaker, and it is `pools.<pool>.breaker:`.** There is no `breaker:` key under `tools:` or under `agents:`: an earlier version of this page and of the configuration reference said there was, and that was wrong. `tools:` and `agents:` use `deny_unknown_fields`, so a config written against that older text does not start Busbar; it fails at boot naming the key. Nor is there breaker tuning on an MCP or A2A pool: an MCP/A2A failover pool accepts exactly two keys, `members:` and `repeatable:`, and rejects anything else at boot. So on these planes the breaker's thresholds and cooldowns are the built-in defaults, and narrowing them is a config surface that has not been written rather than one you are missing.
...
So these failover pools exist, they are **opt-in**, and declaring none is exactly today's behaviour. They live in the one neutral top-level `pools:` map — the same section the LLM plane uses — and each pool's kind is **inferred from its members**:
...
They are one grammar, in core, over every registry — the same `pools:` the LLM plane uses, so an operator learns the concept once. A pool may not straddle two planes: because a pool's kind is inferred from its members, every member must be the same kind, and a member naming an `agents:` entry alongside `tools:` members refuses boot naming the section the entry really lives in.
...
> **Landed state (1.6.0):** the selection seam, its config vocabulary and its proofs live in `crate::failover`, running on the one `try_admit_breaker` the LLM plane uses — and both call sites are wired: the MCP dispatch path walks the candidate set of a `tools:`-membered `pools:` entry before every leg, and the A2A ingress walks an `agents:`-membered one at submission admission. A configured pool is validated at boot AND consulted at dispatch. Three boundedness rules an operator can hit: a failover pool needs at least **two** members (a one-member pool has nowhere to go and is a typo, not a configuration), a pool holds at most **8** members (the breaker's per-member lane table is fixed at process start), and a pool's name may not collide with a registration id on its own plane (they share the breaker keyspace); all three refuse at boot, naming the reason.

### What a tripped target returns: MCP

This shape is **live**: `crates/busbar-mcp/src/mcp/tests/breaker_fastfail_tests.rs` asserts every token of it against a real dying peer.

A tripped tool server answers HTTP **`503`** with a **`Retry-After`** header populated from the cell's own remaining cooldown: an exact number rather than a guess, the same shape budget rejection already uses with `429`. The body is a **JSON-RPC error** with code **`-32003`**, in Busbar's implementation-defined band — JSON-RPC 2.0 §5.1 reserves `-32099`..`-32000` for exactly this, and MCP reserves the `-32099`..`-32020` half of it for codes the specification itself defines, so Busbar's own extensions sit in `-32019`..`-32000` beside `-32000` (`refused`) — carrying structured `data`:

```json
{ "reason": "upstream_unavailable", "server": "acme", "retry_after_ms": 12000 }
```

The breaker is consulted before any socket, immediately after the authorization gate and before the dispatch loop, and the task-creating path consults it at the same position and *before* the task row is minted (`crates/busbar-mcp/src/mcp/method.rs:1410-1431`, `:1735-1754`; the refusal itself is `:2070-2109`). [MCP](/docs/mcp/#what-a-tripped-upstream-returns) carries the full response.
