...
| `busbar_bucket_tokens` | gauge | `bucket`, `model`, `tier` (+ mint labels on key buckets) | Per-(bucket, model, tier) token counters for the bucket's current budget window, from the token ledger. `bucket` is a virtual-key id or `group:<name>`; `tier` ∈ `tokens_input`\|`tokens_output`\|`tokens_cache_read`\|`tokens_cache_write` — **1.6.0 renamed these from `input`\|`output`\|`cache_read`\|`cache_write`** onto the wire spelling the rate card and `/usage` already use; a dashboard or alert rule matching the old values must be updated. The raw material for any external per-model cost dashboard (multiply by your own catalog). |
