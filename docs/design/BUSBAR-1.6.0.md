...
| 85 | **EVERY plugin response carries a uniform OBSERVABILITY ENVELOPE — `{ result, metrics[], diagnostics[] }`. One shape for every kind, not a per-kind bolt-on. OWNER-LOCKED 2026-09-22.** Owner: *"design the abi correctly and implement it. log changes from 1.5.5 but lets not accept a bad abi."* **THE DEFECT:** the export ABI is an effect-free one-way wire — `ExportResponse::Delivered` is a UNIT variant, and the cold tier has no host-callback vtable — so a sink cannot report the metrics it produced or the diagnostics it raised. That made the four `busbar-export-*` roster crates unbuildable: `file` increments `FILE_LOGS_ROTATED_TOTAL`/`FILE_LOGS_ROTATE_FAILED_TOTAL` and raises five registered diagnostics; `prometheus` renders a process-global recorder fed by ~57 kernel sites; `webhook`'s POST must ride the host's SSRF-guarded egress. None of it crosses the wire. **THIS IS NOT AN EXPORT PROBLEM — IT IS AN ASYMMETRY #3 FORBIDS.** The HOOK kind already has the back-channel (`busbar_api::hooks::HookStatus.metrics`, `crates/api/src/hooks.rs:326`), which the engine validates, bounds and folds into the exposition at `busbar-kernel/src/hooks/scrape.rs`. #3 says *"a plugin is a plugin — two universal rules, identical for every kind"*; one kind having a back-channel and another not is exactly the divergence that law bans. **THE RULE:** every kind's response is `{ result, metrics[], diagnostics[] }` — `result` is the kind-specific answer, the other two are universal. The plugin REPORTS; **the host VALIDATES, BOUNDS and DECIDES.** A plugin never mutates a host counter and never performs a host-owned effect (Part 4 Axis 3: the host always owns the SSRF/pin/breaker/meter chokepoint). **IT CLOSES A LIVE #11 HOLE:** today a COMPILED-IN plugin can reach the process-global `metrics` recorder while the same crate built as a dropped-in cdylib gets its own and silently loses the counters — so compiled-in ≡ dropped-in is asserted but FALSE. Under the envelope that reach is not representable, and the equivalence becomes true. **VERSIONING:** this bumps the ABI, and the bump is the honest outcome — a logged, signed 1.5.5 divergence beats a wire that cannot carry what a plugin did. Note the ABI surface is ALSO carrying eleven separate version constants (`STORE_ABI`, `TRANSPORT_ABI`, `EXPORT_ABI_VERSION`, `HOOK_ABI_VERSION`, `SECRET_ABI_VERSION`, `AUTH_ABI_VERSION`, `ABI_VERSION`, `ABI_MAJOR`, `ABI_MINOR`, `ABI_MAGIC`, `POD_VERSION`) — collapsing those to ONE number is the natural completion of #3 and of #84's *"plugins update only when a new SDK ships"* (a third party should pin one number, not eleven). **That collapse is DESIGNED, not yet ruled** — it is a wide-blast-radius change and wants its own owner word. **THE ORACLE CANNOT SEE ANY OF THIS:** the 1.5.5 golden configures no file/webhook/otlp sink, and `ops.scrape\|metrics\|key` carries 15 families, none of them `busbar_file_logs_*`. Oracle-green here is necessary and nowhere near sufficient — the corpus is OWED cells that configure a file and a webhook sink, or this whole surface ships unwitnessed. | every kind's response carries `metrics[]` + `diagnostics[]`; the host bounds and folds them (hook's `scrape.rs` path is the model); a plugin reaching a process-global recorder is unrepresentable; a compiled-in and a dropped-in build of the SAME crate produce byte-identical `/metrics` exposition (RED-provable — this is #11's real test); the four `busbar-export-*` crates exist and their JSONL + exposition byte-match the compiled-in sinks; new oracle cells configure file and webhook sinks |
| 86 | **HOOKS ARE A FUNCTIONAL FIXED POINT: 1.5.5 is authoritative for the hook kind. The ABI shape may move; the BEHAVIOUR may not. OWNER-LOCKED 2026-09-22.** Owner: *"hooks have not changed in 1.6.0, one of the few things — so look how hooks worked in 1.5.5 and ensure same functionality. if abi changes ok but functionally they have not been touched."* Hooks are among the very few surfaces 1.6.0 was never meant to change, which makes them both the safest kind to reshape under #85's envelope and the most dangerous to get wrong. **THE REFERENCE IS READABLE** — 24 hook paths exist at `v1.5.5` and can be read directly (`git show v1.5.5:<path>`): `crates/api/src/hooks.rs` (the `HookStatus`/`metrics` type #85 models the envelope on), `crates/busbar/src/hooks/{mod,plugin,wire}.rs`, `crates/busbar/src/hooks/scrape.rs` (the validate/bound/fold path whose behaviour is frozen), `crates/busbar/src/proxy/hooks.rs`, and the behavioural contract as written tests — `hooks/tests/{scrape_tests,tests}.rs` and `proxy/tests/hook_opt_in_projection_tests.rs`. **THREE CONSEQUENCES:** (1) **diff 1.5.5's hook behaviour against trunk's before changing anything** — if trunk has ALREADY drifted functionally, that is a defect to report, not drift to preserve; (2) **model #85's envelope on 1.5.5's `HookStatus.metrics` + `scrape.rs`**, not on trunk's copies, unless the two are established identical; (3) **1.5.5's hook tests are the acceptance suite** — port them forward, and **if a 1.5.5 hook test cannot be expressed against the new shape, the SHAPE is wrong**: stop and report, never weaken the test. No other kind carries this freeze — export is where behaviour genuinely changes, hooks are where it must not. | 1.5.5's hook tests pass verbatim against the 1.6.0 hook path; a behavioural diff of 1.5.5 vs trunk hooks is recorded and empty (or its contents reported as defects); the envelope reshape touches hook SHAPE only |
| 87 | **PLANES ALWAYS LEDGER. The billing switch is READ-TIME and KERNEL-SIDE — never a branch inside a plane. OWNER-LOCKED 2026-09-22 (resolves an apparent tension in #42; makes #43/#71 enforceable).** Owner's words: *"planes always ledger"*, *"it's a kernel default all planes run through, no turning things on or off by plane"*, *"rate cards are optional per plane in config"*, and on #42's per-plane scoping — *"AGREED, but it's not an on/off in the plane."* **THE MODEL, three statements that are all true at once:** (1) **A plane ALWAYS appends its raw counts to the ledger** — unconditionally, with no branch and no knowledge of whether billing is on. (2) **`rate_card` is OPTIONAL PER PLANE IN CONFIG** — #47's per-plane reserved key stands unchanged. (3) **The switch is a READ-TIME property of the money VIEW, kernel-side.** "Billing off" means the VIEW reads 0 (#42), *never* that the plane skipped the ledger. So #42's *"optional per plane… now scoped per plane"* describes the CONFIG KEY and what the VIEW does with a plane's counts — it never meant a conditional inside plane code. There was no contradiction to resolve, only a misreading to retire. **MEASURED VIOLATION, 2026-09-22 — the code does not match the paper:** plane crates carry money references where #43 says they must carry none (`busbar-plane-streaming` 41, `-a2a` 30, `-mcp` 29, `-llm` 16, `-decision` 9, matching `rate_card|nanos|price|spend`), and **each `crates/busbar/src/root/units_*.rs` separately reimplements the same rulings** — `units_a2a` (5 accrue / 3 rate_card_version / 37 nanos-hold), `units_llm` (5 / 1 / 37), `units_mcp` (2 / 4 / 26), `units_voice` (7 / 2 / 61). **THE DUPLICATION IS THE DEFECT GENERATOR, and today proved it:** `UnitKey::new(0)` was found and fixed in `units_a2a.rs:1019`, and the IDENTICAL bug still sits at `units_mcp.rs:1481` — independently written, independently wrong, and fixing one did nothing for the other. Four implementations of one ruling are four chances to get it wrong and four places to remember when it changes. **END STATE: ONE kernel-side implementation of accrual, provenance and keying. A plane emits counts per its declared meter class and NOTHING else** — no rate card, no price, no `rate_card_version`, no `UnitKey` minting, no hold arithmetic. That is #43's *"plugins are PRICING-BLIND"* and #71's *"a plane's whole money obligation is to emit ONE fact per unit"*, made true in code rather than asserted in prose. | zero `rate_card`/price/nanos/spend references in any `busbar-plane-*` crate (RED-provable gate); accrual, provenance and unit-keying exist in exactly ONE place kernel-side; a plane appends to the ledger with no billing-state branch anywhere in its path; the money view — not the plane — is what reads 0 when `rate_card` is absent |
