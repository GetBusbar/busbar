...
| WHAT A URL SAYS — the model/action split of the two dialects that keep their model in the request line, the version token their envelope echoes, the framing selector, and their own not-found copy | **RE-KINDED PLANE.** It is `busbar_plane_llm::url` and it is not a deletion at all: reading a dialect's own URL space is the same kind of statement the ladder beside it makes about which dialect an address belongs to. It moved because a caller with no arrival host — a MOUNTED leg — could not reach it where it was, and `units_llm_leg.rs` passed `path: None` for the two dialects whose bodies carry no `model` key as a direct consequence. Pure, host-free, and the plane gained no dependency to take it. | `llm\|` | LANDED (keep-llm-parse-to-plane) |
| `native_ingress::{operation_ingress, ingress_path_model, synthesize_completion}`, `arrival::{PathArrivalFacts, PathModelFacts, gemini_rest, gemini_path_parse, bedrock_path_parse}`, `unit::walk`, `PATH_INGRESS`, `BODY_INGRESS` | **DELETE with the engine twins** — this is `pipeline.rs`/`walk.rs`/`native_ingress.rs`/`arrival.rs`, the two dispatch twins. The engine keeps its OWN spelling of the URL cut above until then, because `kind-isolation:deps` refuses a new `busbar-llm -> busbar-plane-llm` edge outright ("a row may record a PRE-EXISTING not-allowed edge, never introduce one"); the two spellings are pinned equal by a cell in `crates/busbar/src/root/tests/units_llm_leg.rs`, which is the one place that can see both. Explicitly **out of scope for this session.** | `llm\|`, `billing\|`, `teller-route-failover\|` | after the llm leg's late accrual and the default flip |
