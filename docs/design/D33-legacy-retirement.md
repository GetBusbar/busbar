...
| 0 (was 2) | `config::secret::SecretResolver` | **The root no longer spells it.** Both sites were the listener signatures (`serve_thread_per_core` / `serve_listener`), which now take `Arc<dyn busbar_api::SecretResolve>` — the neutral trait the engine's resolver implements and the trait `busbar_substrate::tls::read_pem` already took. The struct's own row: **MOVE → `busbar-core-config`** (§7.2 amends this row: the config layer has no replacement anywhere, so neither does its resolver; `config/secret.rs` is one of only two files in the layer with zero outbound edges, so it is movable the day the crate exists) | `config\|`, `boot\|` | with the config crate |
...
| 5 (now 0) | `tls::{install_crypto_provider, build_server_config, ConnBalancer, serve, serve_plain}` | **MOVED to `crates/busbar/src/root/tls.rs`** (917 lines, verbatim; its 953-line test file with it). The composition root was the only reader — `serve_thread_per_core` / `serve_listener` in `main.rs` — and every name the module reached is neutral: `busbar_substrate::config::sections::TlsCfg`, `busbar_substrate::tls::read_pem`, `busbar_substrate::config::limits::installed` (the slot the two legacy `pub(crate)` timeout accessors read, same defaults), the values leaf's `diag_warn!`/`TLS_ACCEPT_PERSISTENT_FAILURE` and `proxy::max_translate_body_bytes`, and `busbar_api::SecretResolve`. Core's one other reader, the engine test kit's `install_crypto_provider`, inlines the same one-line `ring` install. No re-export back into core. | `boot\|`, `documented-tls\|` | **done** |
| — | `observability` (819 lines) | **STAYS — a root move is blocked by core's own readers.** `export/webhook.rs:21` names `observability::{mask_userinfo, validate_webhook_url}` and `ingress/arrival_host.rs:78` names `observability::percent_decode` (all `pub(crate)`); `crates/busbar` is bin-only, so core cannot name a root module. Moves when `export` and `ingress` leave core, or the three URL helpers find a neutral home first. The root's own readers (`init_logging`, `shutdown_tracing`) are the only ones outside core. | `boot\|`, `ops\|` | blocked on `export/webhook.rs`, `ingress/arrival_host.rs` |
| — | `metrics` (663 lines) | **STAYS — blocked by core's own readers.** `telemetry.rs` (10 sites), `plugin_routes.rs:502,535`, `limits/admission.rs:61`, `governance/mod.rs:1085`, `export/{prometheus,webhook,file}.rs`, `plane_host/vtable.rs`, plus the test corpus (`crate::metrics::init` at ~200 sites). The root names only `metrics::configure` and `metrics::init`. | `ops\|` | blocked on `telemetry`, `export`, `governance`, `limits` |
| — | `telemetry` (419 lines) | **STAYS — blocked by core's own readers.** `state.rs:111` (`AppSlots` is a field of `App`), `ingress/mod.rs:601,616`, `plane/observe.rs:144,157`, `plane_host/mod.rs:585-612`, `test_support/mod.rs:1604`; and it reads `metrics` (above). The root does not name it at all. | — | blocked on `state::App` |
