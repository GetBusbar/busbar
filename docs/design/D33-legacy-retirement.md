...
| 0 | `proxy::reqlog::REQUESTS` | **DELETED.** The model plane is on the uniform audit record: `root/units_llm.rs` seals `busbar_unit_audit::AuditInputs` through both of its terminals, exactly as `units_mcp.rs` and `units_a2a.rs` do. The module it replaced held no durable byte, no golden and no production reader; its three `#[cfg(test)]` sites in `root/units_llm.rs` now read the unit's own record. | — | cut |
