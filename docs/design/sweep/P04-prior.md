...
| crates/busbar-mcp/src/mcp/client/issue.rs | FINDING | file-verdicts.md:1368 LIVE. LEDGER.md BR-S12 (line 1034): three MCP security claims "FLAGGED AS STRONG-SUSPICION, NOT CONFIRMED" — (a) an SSRF-blocked call recorded as refused not dispatched, (b) an unresolvable secret names server-not-source to the caller, (c) an oversized token-endpoint body refused not buffered whole — ledger says explicitly "NOT opened line-by-line... verify before acting." I re-ran `grep -n classify_send_failure crates/busbar-mcp/src/mcp/client/issue.rs`: absent, confirming the ledger's negative citation still holds. Not re-derived further (three separate behavioural reads, ledger's own uncertainty preserved). | X-3301 |
...
### X-3301 · issue.rs — three MCP security claims (SSRF-refused-in-audit-trail, secret-source non-disclosure, oversized-token-body-refused) remain unconfirmed
CLASS:     auth
CERTAINTY: ADJUDICATE
EVIDENCE:  `docs/design/1.6.0-LEDGER.md:1034` (BR-S12), reused verbatim; `grep -n classify_send_failure crates/busbar-mcp/src/mcp/client/issue.rs` → absent, matching the ledger's own negative citation.
ACTION:    Owner/behavioural read of the three specific claims against `issue.rs`'s actual send/refuse/record paths; the ledger explicitly withheld confirmation and this sweep did not re-derive it.
