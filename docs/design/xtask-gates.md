...
| `inventory-coverage.py` | the honest scoreboard for the Appendix B parity claim; reads the oracle golden ledger as data | seg (reserved), sh | yes | `inventory-coverage` | 2 — **DELETED at `c8272b166` with no Rust in this tree; the contract it enforced is recorded in section 1.4 below and is OWED** |
| `inventory-coverage.sh` | 22-line wrapper so the gate is invocable like its `*-gate.sh` siblings | seg, sh | yes | **retire** — the wrapper's whole job (uniform entry point) is what `cargo xtask gate` *is* | — (deleted at `c8272b166`; still NAMED by `qa/segments.toml`'s `inventory-coverage` segment and by design binding PB-0, both of which therefore cite an absent file) |
