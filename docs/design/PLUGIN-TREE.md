...
**The count, and its closure rule.** There are **ten plugin kinds** (§1) — the tenth is `control`,
added by the owner ruling of 2026-09-08 and given its row below. The tree has **eleven rows**:
the tenth-plus-one is `unit`, which is CORE — units are the rules, never loadable, never a plugin —
and is listed here only because it obeys the same declare/register/driven shape and the gate must
read its boundary too. An eleventh *plugin* kind may exist only by walking §6, which `control` did. `loader` and `abi` are not
