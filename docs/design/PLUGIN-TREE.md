...
`:transport-registration`, `:control-path`) and the one the 2026-09-10 all-planes ruling added
(`:step-ownership`, ship twin only); `owed()` returns them all, and each has a RED self-test
...
| `kind-isolation:step-ownership` (ship twin) | Every step is served ONCE, kind-neutrally, for EVERY plane — the half `:plane-steps` does not ask, because a face says a plane declares the step, not who answers behind it. Three finding classes. `unserved`: for every plane × every step of the strict list (READ off `busbar-caps/src/step.rs` × `busbar-contract/src/plane.rs`, so a new step is owed by every plane on the commit that adds it), the root's leg for that plane must name the unit `ARCHITECTURE.md` §2.2 assigns the step. `plane-local-driver`: a task, thread or timer (`tokio::spawn`, `spawn_blocking`, `thread::spawn`, `interval_at`, `interval(`) inside a plane crate or a legacy body, the construct read off the LINE and the step machinery off its NEIGHBOURHOOD (±12 lines). `instance-dispatch`: a kind-neutral or still-serving-legacy crate whose production line carries a decision token AND a quoted literal that is an instance of some kind — the owner's `if x == token_auth else if x == OIDC`, with the instance vocabulary DERIVED from the census rather than listed, and a crate naming its own instance skipped. Not `:vocab`: that row counts mentions against a ledger; this one names implementations and dispatch sites. | Overlay `crates/busbar/src/root/units_admin/mod.rs` with `busbar_unit_scope` removed → `unserved`, naming the step, the plane and the unit. Second RED: a `tokio::spawn` planted in `busbar-plane-mcp` beside the word `probe` → `plane-local-driver`. Third RED: `scheme == "openai"` planted in `busbar-unit-scope` → `instance-dispatch`, naming the kind. GREEN arm: the same comparison against a value with no literal is a face, not a branch. |
