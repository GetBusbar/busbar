...
| `busbar-voice-codec` | `busbar-streams-codec` **(LANDED)**, then `busbar-plane-streams-{openai-realtime,gemini-live,twilio-media-streams}` | the codec crate is renamed with its plane; the D36 split is into the streams plane's three DIALECTS, which are wire vocabularies. `busbar-plane-streams-voice` is struck: voice is a modality, not a dialect. ST-1 handed the rename back BLOCKED — dropping the name un-masks the `legacy` needle `busbar-voice` in `matrix::vocabulary`'s prefix strike, and three of the seven cells it exposes read zero, so a `[[cell]]` row for each would be `minted-row` with no door to admit it. ST-2 landed it by DRAINING those three to zero before the rename (stale prose, a denylist that listed the words it forbids, one path that had not existed for months) and declaring the fourth raise, the composition root's `busbar × legacy` 95 → 158. The prefix strike itself is still there and still hides `busbar-llm`, `busbar-mcp`, `busbar-a2a`, `busbar-contract`, `busbar-substrate` and `busbar-unit-egress` — see `keep-vocabulary-longest-match` |
