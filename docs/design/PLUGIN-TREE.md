...
**The count, and its closure rule.** There are **seven plugin kinds** (§1) — store, secret, auth,
hook, export, plane, transport. Direction (inbound/outbound) is a usage mode, never a kind boundary:
**auth is one kind**, its inbound verification and outbound signing/decoration being two operations of
it — there is no separate egress-auth kind. **transport is one kind, bidirectional**, and a plane
declares its needs as `(transport, auth)` per direction. **A dialect is a thing inside a plane, not a
kind** (no per-dialect crates, no dialect trait), and **admin and oauth2 are compiled-in cleanliness
crates, not a kind**. The tree has **eight rows**: the last is `unit`, which is CORE — units are the
rules, never loadable, never a plugin — and is listed here only because it obeys the same
declare/register/driven shape and the gate must read its boundary too. An eighth *plugin* kind may
...
| **auth** | `busbar-auth-<name>` | `KEY`, `LOCATIONS`, `IO: bool`, issuer config | two operations — inbound `verify` and outbound `decorate`/`sign`; I/O only when `does_io()`, on the blocking pool under a deadline | `kinds::AuthScheme` | contract | kernel | — | universal-config · verify-shape · challenge-rounds · decorate-purity · handshake-round · slot-substitution · deadline |
...
| **secret** | `busbar-secret-<name>` | `KEY`, `REF_GRAMMAR` | key I/O | `kinds::Secret` | contract | the auth / transport-key **units** only | — | universal-config · resolve/watch/sign/seal round-trip · canary grep |
...
**The cleanliness crates, in one paragraph.** admin and oauth2 are compiled-in cleanliness crates,
not a plugin kind: they depend one way on core, sit off the hot path, and are UNMETERED. The metering
is the whole of the distinction: a plane is the metered path and a cleanliness surface is not on it.
A cleanliness crate CAN declare its routes as data (the same claim table a plane declares), own its
...
**The two workflows, and the one place a wire is registered.** A PLANE follows the strict workflow
every plane follows — the ten steps of §2 in order, complete, **never deviating**: a plane that skips a
step or invents one is not a variant of the kind, it is a defect of it. A cleanliness surface follows
the lesser workflow — `verify → admit → audit → answer` — for system-level functions, and has no other
path. **Every plugin kind uses transports**, and no kind of plugin declares or registers a wire of its
own: `http` is declared and registered in exactly ONE spot, `busbar-transport-http`, and planes and
cleanliness surfaces reach it only by declaring routes as data which that transport mounts generically.
Three gate rows state this and are owed alongside Appendix G's rows: **plane step-list completeness**
(every plane declares and answers the whole step list, no deviation), **control path only** (a
cleanliness crate touches no step outside verify/admit/audit/answer), and **single transport
registration** (one registration site per wire; a second declaration of a wire already declared is RED
wherever it appears).
...
Identical for all eight rows. There is no second path in, and the kernel loop is the only place a
plugin acts.
...
4. **BE DRIVEN.** The kernel loop calls the plugin, never the other way. The ten steps —
   ...
   | Route | plane (`route`, `encode_egress`), hook at `Before(Route)` / `After(Route)`, auth (outbound `decorate`), transport (`dial`, `write`), store (`record_*` on a `PlaneRecord` leg) |
...
kind batteries are owed** and are a §8 line item, not a follow-up — a kind whose battery does not
...
|---|---|---|---|
| plane | `busbar-core-contract`, `busbar-core-grammar` | any transport, any unit, any other plane, capabilities, kernel, core, substrate | a plane owns its IR and its dialects, and names a transport only as a claim constant |
| transport | `busbar-core-contract`, `busbar-core-contract-transport`, **lower** `busbar-transport-*` only | any plane, any unit, capabilities, kernel | wires compose over wires; `busbar-transport-tls → busbar-unit-transport-key` is the one live breach (§9 row 6) |
| unit *(core)* | `busbar-core-contract`, `busbar-core-capabilities`, `busbar-core-contract-transport` | any plane, transport, store/auth/secret/hook/export crate | units take facts + principal + clock and nothing else |
| store / auth / secret / hook / export | `busbar-core-contract` (+ the reviewed third-party list) | every other kind's crate, capabilities, kernel, core, substrate, another instance of its own kind | one kind, one crate, no siblings |
...
- **A plane owns its dialects.** A dialect is a wire vocabulary internal to a plane, so a plane crate
  holds its own dialect logic — there are no per-dialect crates and no dialect trait.
  `busbar-plane-streams/src/twilio.rs`, `src/ulaw.rs`, `busbar-plane-llm/src/{dialect,claims,codec}.rs`
  and `busbar-plane-{mcp,a2a}/src/jsonrpc.rs` are the plane's own files. What a plane may NOT name is
  another plane, another plane's dialect, or a sibling instance of any kind.
...
head_keys, verbs}`) and cross into the contract as the plane's own dialect declarations for free.
...
A kind is a kernel change and must look like one. Fifteen lines, walked in full for any candidate kind
...
**The rule.** `busbar-<kind>-<name>`. Kind first, always. A dialect is a thing inside a plane, so it
gets no crate and no name segment of its own; a plane's dialect logic lives as modules in
`busbar-plane-<plane>`, with its codec logic in the per-plane path-dep `busbar-<plane>-codec`
(DECISIONS #6). **A transport is never named for a plane: `busbar-transport-<plane-key>` is a finding,
ceiling 0.** The kind-isolation gate reads the kind from segment two, so directory name and
...
| `busbar-voice-codec` | `busbar-streaming-codec` | renamed with the streaming plane (DECISIONS #18); dialect logic (incl. voice) is modules inside `busbar-plane-streaming` |
...
| — | `busbar-auth-{anthropic,openai}` | outbound `decorate` schemes of the auth kind (§8) |
...
## 8. Migrating the five kinds with zero contract implementors

Five kinds have rules and gate rows but no crate implementing their `busbar-core-contract` trait; the
...
| auth | `auth-admin-tokens` is a 1.5.5-era module on `busbar-api`; `auth-static-plugin` likewise; the outbound `decorate` schemes are hardcoded in `busbar-unit-egress-auth` with a dialect-detection ladder in `busbar-unit-auth` | re-base onto `kinds::AuthScheme` for both operations; the token SIGNER lands in `busbar-unit-auth`, not in a plugin and not in capabilities; create the outbound scheme crates `busbar-auth-anthropic` / `-openai` (the unit keeps the mechanism, the crates keep the vocabulary; `lean-core`'s `max_hits = 21` ratchet closes to 0) | ~250 + ~300 | **R5** `auth/` row; outbound schemes in **R6** |
...
| Audit | Answer here |
|---|---|
| §1.1 codec is an unnamed kind | A codec is not a kind: a dialect is a wire vocabulary **inside a plane** (§1). The plane owns its dialects — dialect logic is modules in `busbar-plane-<plane>`, codec logic is the per-plane `busbar-<plane>-codec` path-dep (DECISIONS #6). There are no per-dialect crates and no dialect trait. |
| §1.2 egress-auth has zero crates | egress-auth is not a kind: outbound signing/decoration is an operation of the auth kind. The outbound scheme crates (`busbar-auth-anthropic` / `-openai`) are sized in §8's auth row, R6. |
| §1.3 five kinds, zero implementors | §8 in full. |
...
| §1.7 "~10" pinned nowhere | Header: seven plugin kinds, eight tree rows (the eighth is core `unit`), closure rule = §6. |
| §1.8 ABI generation for two kinds only | §6 step 4 requires one per kind; only `STORE_ABI` and `TRANSPORT_ABI` exist, so §8 owes one per remaining kind. |
...
| §3 substrate `Transport::{Http, JsonRpc}` | One closed enum spanning both axes is the literal plane-transport shape; `JsonRpc` is a dialect inside its plane, not a transport variant. |
| §4 shipping cross-kind references (9 rows) | Rows 1,7 die with the waist; 2,3 move to the auth kind's outbound scheme crates (§8); 4 becomes D6's keyed code; 5,6 die with substrate; 8 is the ABI POD and takes D6's namespace; 9 is root-scoped and legitimate. |
...
| §8a dialect contract | A dialect is inside a plane, not a kind — no `Dialect` trait and no `DialectMeta`; the plane declares its dialects' surface as data (§5) and owns their logic. |
...
| §8c dialect-specific paths in planes | Legitimate: a plane owns its dialects and holds their logic (§4). Dialect-specific files live in the plane crate. |
| §8d the llm split, sequenced | §10 Q3 and the migration order: **(a)** oracle boot cell pinning the `DECLS` order (`anthropic, openai, gemini, bedrock, responses, cohere` — operator-visible, every metric-family index is a position in it, silently swapped once already) **BEFORE** any reshaping; **(b)** the dialect logic stays as modules inside `busbar-plane-llm` and the codec logic in the per-plane `busbar-llm-codec` path-dep (no per-dialect crates); `chat_handle.rs`/`leaf_codec.rs`/`leaf_handles.rs` are the handle engine. |
| §8e naming | §7: `busbar-<kind>-<name>`, kind first; a dialect gets no crate or name segment of its own. |
...
seven rows below, plus three more — `:plane-steps`, `:transport-registration`, `:control-path` (the
last keeps a cleanliness crate unmetered and control-path only); `owed()` returns them all, and each
has a RED self-test case planted as an `Overlay`.

| Row | Asserts | RED case (`Expect::Red`) |
|---|---|---|
| `kind-isolation:name` | Every crate under `crates/` has a directory name equal to its `package.name`, matching `busbar-<kind>-<name>` (or `busbar-core-*` / `busbar-plugin-*` for core/TCB), and its kind segment equals `Plugin::kind()`. No crate name is `busbar-transport-<plane-key>` for any registered plane key. | Overlay a crate `crates/busbar-transport-mcp/` with `package.name = "busbar-transport-mcp"` → the row names it as a plane-named transport. |
| `kind-isolation:deps` | Each crate's workspace-local `[dependencies]` is a subset of its kind's row in §4. Transport: contract + contract-transport + strictly lower transports. No crate names another instance of any kind. `[dev-dependencies]` out of scope. | Overlay `busbar-plane-llm/Cargo.toml` with `busbar-plane-mcp` in `[dependencies]` → the row names the edge and the two planes it fuses. |
| `kind-isolation:vocab` | No plane crate names ANOTHER plane's dialect or wire-format identifier (a plane owns its own dialects; the §1.3 word list ∪ every OTHER plane's registered dialect key), comments and `#[cfg(test)]` stripped, word-boundary matched. No kernel/unit crate contains any plane key or dialect name (today's `lean-core`, scope extended to `busbar-core-contract*`). No transport crate names a plane. | Overlay `busbar-plane-mcp/src/vendor.rs` containing `const WIRE: &str = "twilio";` (the streams plane's dialect) → the row names file, line and word. |
...
3. **The `unit` row.** Units are core and never loadable (2026-09-06), yet they obey the same
   declare/register/driven shape and the gate must read their boundary. Confirm the tree is eight
   rows, seven of them plugin kinds — or rule units fully out of the tree, in which case §1's last row and
