...
- The **kernel** is the **registry** and the **Teller** plus what the loop needs: frame pump, sessions
  ...
  (`--ceiling busbar-contract,busbar-caps=4765`), which the construction gate runs as
  ...
  — ≤ **2k**, gated as `surface-ceiling:core-policy` and PINNED at its measurement (1,730 with its
  ...
  projection of them, and the subject itself named on the gate's own facts — the ladder took it 1,732 → 1,729 and `GateSubject::subject` put one line back, replacing the `requested_model: None` the neutral gate wrote because it could not name what it was deciding about; 1,732 at [M4c], 2026-09-10; 1,672 the day it was extracted from `busbar-core`, owner ruling 2026-09-08). It names **no plane, no dialect and no transport**, and
...
| `busbar-core-policy` | core | the hook POLICY engine — resolution, gates, rewrites, singleflight, scrape — which the plain-data hook carriers in the substrate are not and never were. The kernel seats hook PLUGINS; this seats their policies. Its manifest names `busbar-contract` and `busbar-substrate` and nothing else; a hook plugin is reached through its own port (`HookSeat`), bound by the composition | its own row, `surface-ceiling:core-policy`, pinned at 1,730 (the grant ladder over the hook subject's argument axis, less the subject member on the gate's facts; 1,732 at [M4c]) |
