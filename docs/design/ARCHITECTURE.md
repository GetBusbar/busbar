...
- The **kernel** is the **registry** and the **Teller** plus what the loop needs: frame pump, sessions
  ...
  (`--ceiling busbar-contract,busbar-caps=4762`), which the construction gate runs as
  ...
  `StoreError`/`SecretError` enums are gone; the surface measured 3479 → 3684 for it. The figure
  is the PIN, not a budget: it is held at zero slack against today's measurement, and it moves up
  only when a new-architecture face lands with a declared raise (`[[gate.ceiling_raises]]` in
  `qa/construction.toml`) that names the face and its measured lines, this figure amended in the
  ...
  kernel never wrote), which paid 21 of it back: 4775 → 4754 — and the plane-neutral HOOK-SUBJECT face (`HookSubject`: where the subject's own name is and where its argument payload is, plus `Plane::hook_subject`, which every plane answers and none defaults; 6 lines), which gives the neutral hook seam a word for *what is being asked for* so it stops borrowing one plane's — MCP's `tool`/`args_json` on the request-admission host ABI, the model plane's `requested_model`/`prompt` on the routing projection: 4754 → 4760 — and its READING (`SubjectFacts`: the same two answers with the locators already resolved, which is the form the firing seam carries because a firing site holds no unit to resolve one against, and on two of the four planes that fire it the subject was settled by operator configuration before the request arrived; 5 lines), which is what let the request-admission host ABI stop taking one plane's word for a callable and its arguments — LESS the three RESERVED routing-projection members that came out with it (`requested_model`, `tool_count`, `system_chars`: no transport projected any of them, so each was a member on a plugin author's face that no hook could ever read, and their catalog twins in `Signal` are now the only form; 3 lines): 4760 → 4762 — and the METERING STEP'S RATE FACE (`ClassRate`: a class and what one unit of its own quantity costs, in micro-units, which is the other half of `UsageLocator` and the vocabulary a rate card is configured in; and the routable member's `price` reading it, which replaces a member cost stated in one dialect's unit of quantity — a price per million TOKENS, on a face a duplex leg priced by the second and a relay priced by the frame could never answer; 12 lines net of the member it deletes): 4762 → 4774. Two crates carry their own surface ceilings beside it, because
  ...
  — ≤ **2k**, gated as `surface-ceiling:core-policy` and PINNED at its measurement (1,726 with its
  grant ladder `no ⊂ ro ⊂ rw` written over the hook subject's ARGUMENT axis and the caller's IDENTITY axis rather than one plane's
  projection of them, and the gate's projection no longer filling the three reserved members that no transport ever read; 1,732 at [M4c], 2026-09-10; 1,672 the day it was extracted from `busbar-core`, owner ruling 2026-09-08). It names **no plane, no dialect and no transport**, and
...
| `busbar-core-policy` | core | the hook POLICY engine — resolution, gates, rewrites, singleflight, scrape — which the plain-data hook carriers in the substrate are not and never were. The kernel seats hook PLUGINS; this seats their policies. Its manifest names `busbar-contract` and `busbar-substrate` and nothing else; a hook plugin is reached through its own port (`HookSeat`), bound by the composition | its own row, `surface-ceiling:core-policy`, pinned at 1,726 (the grant ladder over the hook subject's argument axis, less the three reader-less reserved members the projection stopped filling; 1,732 at [M4c]) |
