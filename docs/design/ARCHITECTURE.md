...
- The **kernel** is the **registry** and the **Teller** plus what the loop needs: frame pump, sessions
  ...
  (`--ceiling busbar-contract,busbar-caps=4805`), which the construction gate runs as
  ...
  kernel never wrote), which paid 21 of it back: 4775 → 4754, and the store kind's COMPLETED FACE (the enforcement half of `busbar_contract::kinds::Store`: the virtual-key directory, the token ledger's ATOMIC ADDITIVE accumulate and the metering book, row-looked-up credentials, the revocation denylist and the two record verbs the record half was missing — 23 bodiless verbs declared with their real semantics rather than folded onto `record_put`, because an absolute set cannot express a fleet-honest accrual and a caller cannot make the key tombstone's test-and-write atomic; 51 lines), 4754 → 4805. Two crates carry their own surface ceilings beside it, because
...
| Store (kind `store`; native ABI **5**; every 1.5.5 dynamic plugin LOADS through an in-tree ADAPTER per kind at the exact 1.5.5 loader windows (store 2, auth 1–2, hook 1, export 2, secret 1), so a 1.5.5 plugins.yaml boots unchanged — parity clause, no `PluginAbiTooOld` for any 1.5.5 plugin) | `append_batch / replay_batch / reserve / release / heads / heartbeat / elect_checkpoint / claim_key / void_claims / replay_put / replay_get / session_put / session_remove / sessions_for / record_put / record_get / record_scan / legacy_cells_read / legacy_cells_write / legacy_audit_head / backup_watermark / purge_before / key_put / key_get / key_list / key_revoke / key_scrub / keys_since / usage_get / usage_put / usage_add / metering_add / metering_list / usage_purge_before / metering_purge_before / credential_put / key_put_with_credential / credential_list / credential_lookup / credential_revoke / credentials_since / denylist_add / denylist_list / record_delete / record_live` (45: the journal, directory and record half above, then the ENFORCEMENT half — the 1.5.5 replay trait's verbs re-declared on the face with their real semantics. `usage_add` is an ATOMIC ADDITIVE accrual and `record_put` is an absolute set, so the money path is not expressible through the record verbs; `key_put` carries a tombstone precondition only a backend can make atomic; `record_live` is a MULTI-USE liveness check and fails CLOSED, unlike every other read on this face. Every verb is bodiless — the accept-and-keep-nothing defaults the 1.5.5 trait carried are COMPATIBILITY behaviour and live in the loader's adapter) | `KEY`, `ABI_FLOOR`, `FLEET_SAFE`, schema versions, measured max sustained record rate |
