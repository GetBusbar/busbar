...
- The **kernel** is the **registry** and the **Teller** plus what the loop needs: frame pump, sessions
  ...
  `busbar-unit-verbs`. **Timing a step is the loop's own face**: the env-guarded stage profiler
  (`busbar_kernel::profile`, `BUSBAR_PROFILE`) attributes time over the loop's ten steps plus the
  transport wait, and every one of its rows is taped from `teller.rs` and from nowhere else. The
  table is `busbar_contract::unit::Step` itself, so a row means the same thing whichever plane
...
| PB-54 | CLI / env / signals / spans cells | the ops inventory's CLI cross-check and `xtask gate` carry cells for every 1.5.5 CLI flag and exit code, env var, SIGHUP-not-handled, the 25 startup steps, the seven `#[instrument]` spans and the request-log record, diffed against the 1.5.5 binary — the 25 `run()` steps are an EXACT ORDERED sequence whose observable output is byte-identical (`busbar starting` stays the first log line; `Bootstrap` / `Migration` / `Policy` / keyset lines are DEBUG-level, never interleaved at INFO on a 1.5.5 config); the pre-runtime phase (CLI flags before any env/file access, the jemalloc purge enable with its two `[warn]` stderr lines, the `busbar-idle-purge` fallback thread); `BUSBAR_PROFILE`'s STAGE NAMES are the one declared exception to the verbatim rule — 1.5.5's 13 named an HTTP egress pipeline's internals and 1.6.0's name the loop's ten steps plus the transport wait (section 1.1), because the profiler is a diagnostic an operator reads and not a surface a client or a config depends on, and the 1.5.5 set had three rows nothing in the tree ever timed. The dump's SHAPE — stderr, every 20 s, `BUSBAR_PROFILE stage={name} n={seen} mean={:.3} p50={:.3} p99={:.3}` in microseconds, leading spaces marking nesting, `BUCKET_CAP = 4096` with Algorithm-R reservoir sampling and `n=` the true observation count — is byte-identical; flags and exit codes as a SUBSET invariant: every 1.5.5 argument behaves identically; `--help` gains lines for the 1.6.0 subcommands (`--data-dir`, `operator keygen`, `keyset import`, `keyset recipient-keygen`, `policy sign`) and those arguments no longer exit 2 — the one additive CLI difference, stated | ops O1/O2/O4, §2.1–2.2 |
