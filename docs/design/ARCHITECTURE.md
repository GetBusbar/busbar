...
kind. There are **ten plugin kinds** — plane, dialect, control, transport, auth, egress-auth, store,
secret, hook, export — and one core row, `unit`, which is never loadable. CONTROL is the tenth, added
by the owner ruling of 2026-09-08 (Appendix A): an UNMETERED served surface, which is the whole of
its split from `plane`. `loader` and `abi` are TCB crates, not kinds. "Rate card" below is CONFIG, not a kind: it has no trait, no `Kind` variant and no crate,
