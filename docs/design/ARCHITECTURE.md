...
| Rate card (config) | unit price = f(lane, meter_class, extras), with a `*` default lane row (used by `SessionAccrual` on sessions that dialed nothing; absent while any bucket declares a `session_seconds` class → boot refusal `MissingDefaultLaneRow`) with no bucket-level tier multiplier — a tier is a scope of the `tariff:` schedule (§4.5); versioned; quantity sources per (plane, transport); permitted lanes per op class; max unit price; lane aliases; `KernelVerb` section (default 0); **bucket chain and cap dimensions** (§4.6) | meter classes, prices, windows, frame selectors, divisors |
...
4  ADMIT         THE DECISION IS 1.5.5's (parity clause): the admission unit evaluates 1.5.5's CHECK-THEN-CHARGE
                 ...
                 budget))⌉ (no chain multiplier: a tier is a SCOPE of the tariff and its amounts are already in the fee term above, so the hold is the summed size of the lines and `TierMismatch` is retired with the key it checked); estimate_c converts bytes to the
...
priced_amount (nano-units, i128 — ONE amount; the pre-tier figure and the multiplier beside it are retired with the tier's multiplier), tariff_scope (`kind:key`, the one scope column), fee_count, currency, rate_card_version,
...
line) is sealed there — and compared to `priced_amount`; the tier is not sealed beside the history any more, because the SCOPE a line was charged at is on the line; divergence
...
- **Numeric contract, five numbered clauses — clauses 1–4 are 1.5.5's pricing law (`cost.rs`;
  ...
  2. *Storage*: `priced_amount` = Σ over the posting's usage lines of quantity × unit price, in
     ...
     exactly as in 1.5.5); currency is a 1.6.0 addition (one per bucket; mixing is a boot refusal). There is no per-chain tier: the scope a line was charged at is recorded on the line.
  ...
  5. *Tier* (**new in 1.6.0**; 1.5.5 has no service tier at all — its per-tier token classes are
     meter classes here, and an upstream-reported per-response tier prices through the extras table):
     a tier is a **SCOPE of the tariff**, not a multiplier. `tariff.tier.<group>` names the same
     absolute fee terms every other scope names, and the one `tier > pool > plane > default` walk
     selects the cell field by field; the posting stores the SCOPE it stopped at
     (`TariffScope::encoded()`, `kind:key` — `tier:gold`, `pool:eu`, `plane:llm`, `default`) beside
     the one amount it charges, and the recompute (§4.2) re-prices at the scope the ROW recorded
     rather than at a configuration that may have been edited since. There is no per-bucket
     `tier_bp`: it was a second pricing mechanism, over the rate card rather than over the schedule
     an operator wrote, and it is retired by name (`RETIRED_CONFIG_KEYS_1_6_0`) with no mechanical
     rewrite — a multiplier does not carry across into absolute amounts without guessing what the
     operator meant.
...
| `tariff.tier.<group>` (per tier) | absent | RETIRES `tier_bp` (per bucket), which was a basis-point multiplier over the rate card; a tier is a SCOPE of the tariff and names the same absolute fee terms every other scope names, resolved `tier > pool > plane > default` field by field. No multiplier, no chain rule and no `TierMismatch`: two groups cannot disagree about a price neither of them holds. The retired key is refused by name with its new home and no mechanical rewrite (a multiplier does not become an amount without guessing). A terms edit is a ledger-endpoint line (PB-16); Appendix A |
...
| `max_hold` (derived) | `max_fanout_recipients` × ⌈(Σ_{direction = Response} `max_response_c` (as resolved) × max price_c + max over the classes that partition the same bytes among {direction ∈ Input, CacheRead, CacheWrite} of (`request_body_max_bytes` ÷ divisor_c) × max price_c + fee + `session_idle_max` × max `session_seconds` price + `max_provider_push` × max price_c — the `Kernel`-direction classes appear ONLY as these explicit terms, never inside the partition max)⌉ + one accrual step, per unit | the largest hold any unit can open; `fee` is the largest fee term any tariff SCOPE names, so the tier is in it through the schedule rather than through a multiplier over the whole sum |
...
- Every boot/reload seals `Policy { binary_digest, resolved policy incl. the defaults table, hash,
  policy_epoch, dual_control, operator_pubkey | unset, escrow }`; boot refusals — ONE list, mirrored verbatim in §10, in two parts. (a) The 228 refusals and warnings of 1.5.5, byte-identical (inventory C2) incl. rate-card completeness, the admin-listener mTLS guard and the serde unknown-key message with its `expected one of` list unchanged (every 1.6.0-additive key lives under the single `fleet:` block, split off before the 1.5.5-shaped parse — PB-40). (b) Refusals that can ONLY fire on 1.6.0-additive keys and therefore never on a 1.5.5 config: overlapping claims across planes; mixed currencies; the retired `groups.<g>.tier_bp` named with `tariff.tier.<group>`; the operator-pinned accrual bound; a secret ref in a plane CONFIG_SCHEMA; `MissingDefaultLaneRow`; `KeysetMissing` and `DataDirNotWritable` (only with `data_dir` configured); `PluginAbiTooOld` (only for a plugin newer than the native ABI); a manifest OUTSIDE its kind's window (the store window is [2,4], PB-11) is refused with 1.5.5's literal and the current printed range (PB-11).
...
`stale_policy`) · amount (usage lines with sources, the one priced nano-unit figure, the tariff scope,
...
- **Effects spec**: cardinality and outcome columns derived mechanically from 1.5.5 where they exist;
  ...
  expired — covers `usage_lines`, `priced_amount`, the tariff scope, `fee_count`, `currency`,
...
adjustments below `adjust_threshold`, `set_overdraft_ceiling` and `grace_slices_per_window` (the two knobs that let money move past a cap), `set_dispute_max_age` (which can hide overdue disputes), and a `tariff` scope's fee terms (which move every posting charged at that scope, a ledger-endpoint line (PB-16)) — journaled, alarmed and shown on the ledger endpoint (PB-16); and on an upgraded
...
| PB-27 | zero-token ends | a response bills ZERO tokens (located and accrued figures internal evidence only) on every 1.5.5 not-billed arm: a stream ending with a terminal error signal (`terminal_error != None`); an SSE transport cut after the first byte (`stream_failed = true`); a buffered cross-protocol 2xx that fails after the upstream 2xx (transport failed mid-read → 502, over the translate cap → 500, untranslatable → 500); the `max_requests` lane unit is refunded on the pre-first-byte cut, on the post-first-byte cut of a NON-SSE body, and on the client disconnect, and NOT on the post-first-byte cut of an SSE body — the discriminator is the content type, never `first_byte_sent`, because `FirstByteBody`'s `else` arm carries the pre-first-byte cut and the non-SSE mid-body cut together and refunds ungated on `had_first`, while the SSE arm touches `budget_spent` nowhere and sets `ended` so the Drop refund is suppressed too (`docs/design/inventory/1.5.5-proxy-hooks.md` §3 :407 "before first byte (or non-SSE mid-body) … budget refunded"; governance 3.8.10 :319 "pre-first-byte / mid-stream transport error"; the §4 no-refund row :435 is the SSE row, and §4 carries no non-SSE post-first-byte row at all); a client disconnect bills the partial tokens and refunds the LANE `max_requests` unit; the flat fee (`billable_requests`) is KEPT, as on every exit after 2xx headers were relayed (`ingress/mod.rs:574-579` refunds only on a non-2xx client status) — each arm one cell | dialects §4.4; proxy-hooks §4 (:430-436; `engine/mod.rs:329-378`, `:552-589`, `response_body.rs:283-345`); governance 3.8.4 |
...
| PB-40 | unknown-key refusal | the serde `expected one of` list is byte-identical to 1.5.5's for EVERY `deny_unknown_fields` struct (all 49: the 44 enumerated at BOOT-P01 plus the five `section_patch!` twins in `config/patch.rs`, `GroupCfg` and `ChildDefault` included): every 1.6.0-additive key — top-level (`data_dir`, `peers`, `keyset_ref`, `wal_capacity`, `outage_grace`, …) AND per-bucket (`currency`, keyed by bucket name) — lives under ONE new top-level block `fleet:`, split off before the 1.5.5-shaped parse | config BOOT-P01 (:515), :67 |
| PB-41 | 1.6.0 warnings on 1.5.5 configs | the store record-rate warning and the `keyset_ref`-unset warning fire ONLY when `data_dir` or `peers:` is written; otherwise they are ledger-endpoint lines; PB-17 holds for every 1.5.5 key incl. `store.module` | config CONF-170, §3 |
...
| PB-44 | `in_flight_reserve` | 0 unless a claimed transport declares `SESSION = true`; a 1.5.5 config sheds at exactly `max_inbound_concurrent`; the reserve is drawn only against session Unit 0 arrivals | config CONF-134 |
...
| PB-48 | stall sweep on `http`/`sse` | the `max_unit_duration` sweep alarms only (`busbar-kernel/src/tick.rs:141`, `Sweep::AlarmOnly`, which `sweep_outcome`/`sweep_settle` end nothing on); the unit's only timers are PB-8's — a stream is cut only by the `upstream_request_timeout_secs` TOTAL deadline (default 300 s, `busbar-substrate/src/config/limits.rs:22`), armed once at send start and covering head and body (`busbar-llm/src/engine/attempt/send.rs:63-76`; at the tag this was reqwest's client-level total timeout, re-provided at the call site by the owned-client cutover); there is no idle timer | proxy-hooks §2.8, ops §3.6 |
...
| PB-82 | usage extraction | 1.5.5's usage read contract is the locator: the cached count is SUBTRACTED from the wire prompt total on openai / responses / gemini (never billed twice); gemini `tokens_out` = `candidatesTokenCount + thoughtsTokenCount`; the forced upstream `stream_options.include_usage` and its two same-protocol hide-back seams (`suppress_same_proto_frame`, `strip_same_proto_usage`) reproduced, so no extra `{choices:[],usage}` chunk reaches an OpenAI client that did not opt in | dialects §4.6 (:706-718), §5 (:780-794) |
...
| PB-86 | usage locators are plane-normalized | the four token classes are read through the plane's usage normalization, not a raw pointer (PB-82); the kernel never re-derives a class from the wire | dialects §5.1 |
...
| PB-88 | dialect pairs never refuse | all 36 ingress×egress pairs are supported with NO pair-level refusal; the 60 bullet-level drop / clamp sites of the dialects inventory (39 with literal warns) plus the 10 universal `prepare_for_egress` steps of :366-381 (`n > 1 → 1` + warn, reasoning / prompt-cache gates, `cache_control` cap, hosted-tool drop, gemini `thoughtSignature` fill, `extra.clear()` — the mechanism behind the `service_tier` trap) degrade-with-warn exactly as at the tag; a plane may not refuse at the PAIR level; untranslatable CONTENT still renders 1.5.5's 400 `invalid_request_error` "We could not process the content of your request." (dialects :969) | dialects §3 (:342), §3.4 (:407-485) |
...
| PB-90 | unmapped 1.5.5 config keys | `advanced.response_headers.*` (PB-73), `limits.max_keys_per_principal`, `auth.key_ttl`, `limits.reasoning_effort_budgets.*`, `limits.max_honored_retry_after_secs`, `limits.hard_down_cooldown_secs` (PB-80), `public_url`, `providers_file`, `identity-providers:`, `pools.<p>.affinity.*` (PB-5), `secrets:` — each lands on its 1.5.5 handler unchanged; the landing map is complete only with this row | config C1 (the ≈ 12 keys with no other mention) |
