# busbar — Architecture v1.29 (parity revision 7)
...
overlap → boot refusal); reflexivity and symmetry asserted; a boot cell per form pair. Masking per
...
**Admin API — byte-identical.** Every 1.5.5 operation (66 at the tag) returns the same bytes for the same state — with the two stated exceptions: `GET /admin/openapi.json` (PB-75, served verbatim) and `GET /admin/info` (ADM-042), whose `version` is `CARGO_PKG_VERSION` and whose `build { auth_modules, hook_plugins, weighted_floor }` reports the running binary, the same shape with the running values —
...
| PB-75 | served OpenAPI document | `GET /admin/openapi.json` (ADM-052, one of the 66) returns the 1.5.5 blob VERBATIM (`info.version` included); the 1.6.0 verbs are described at a new path (`/api/v1/admin/ledger/openapi.json`) — "admin API a superset" means routes, not the legacy document's bytes | routes-admin ADM-052 (:409), :266-284 |
...
| PB-84 | response-stage taps | the `response` stage fires ONCE per request that reaches the forward path (a streaming response taps at response-head time); pre-forward refusals (401/403/429/413/404) never tap; `outcome ∈ {ok, failed, rejected_by_gate}` is the only synthetic field — detached, fire-and-forget, under `MAX_INFLIGHT_TAP_NOTIFICATIONS = 1024` | proxy-hooks §7.2 (`engine/mod.rs:198-221`, `hooks.rs:1407-1417`) |
