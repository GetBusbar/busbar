...
| `streams` (was `voice`; its config section is already `streams:` — the unrelated 1.5.5-frozen `export.<n>.streams` key is a different field and never moves) | openai-realtime, gemini-live, twilio-media-streams | ws, webrtc | a turn; tool calls as provider `OneShot` requests (`Client(AwaitReply)` or `NestedPlane(mcp)`, result as a `SessionUpstream` leg); interrupt fact; pacing fact | audio_tokens_in/out (`Locator`), text_tokens_in/text_tokens_out (`Locator`), cached_tokens, audio_seconds_in (`TransportUnits` on webrtc; cross-checked by `KernelElapsedMono`; `Locator` on ws, the carrier included — the carrier's dialect meters its own uplink from the raw payload before the transform, so no wire has to parse a vendor's envelope to reach the bytes), tool_calls | — | OpenAI Realtime ingress/egress; Gemini Live egress; μ-law↔PCM16 in `encode_ingress_frame`; **raw SIP out of scope** (owner decision); only the FIRST decoded IR event per wire frame is acted on; uplink audio is ASSUMED PCM16 for the `audio_seconds_in` estimate; model-emitted text in a duplex turn prices under `text_tokens_out` — an OUTPUT class, never the input one — because §4.5 clause 2 makes the class a money question and emitted text is output, not input; the `webrtc` leg is §9.3 Phase 0.5 work, not Phase 0; the one-shot transcribe/TTS routes are NOT this plane's — `/v1/audio/speech` and `/v1/audio/transcriptions` are request/response and are claimed by `llm` (rung 14) beside `/v1/audio/translations`, per the owner's 09:0x ruling |
...
- **The voice plane is renamed `streams`** (its config section is already `streams:`) —
  ...
  and have LEFT this plane for the `llm` plane's speech routes. Non-voice dialects are
