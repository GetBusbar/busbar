...
- The **kernel** is the **registry** and the **Teller** plus what the loop needs: frame pump, sessions
  ...
  `busbar-unit-verbs`. **Opening a unit and holding it open is the loop's own face too**: the loop's first six steps
  ...
  (`--ceiling busbar-contract,busbar-caps=3489`), which the construction gate runs as
  ...
  implementor can exist; `docs/design/1.6.0-per-unit-arena.md`) measured 0 lines and declared nothing; the session-parameter projector pair (`plane::SessionParams`, `SessionPlane::session_params` / `adopt_session_params` — the face a composition screens a session OPEN through, carrying the plane's payload as the opaque bytes a deployment's configured gates already match) measured 12 lines and took the pair to **3,489**; the SESSION-OPENING FRAME (`SessionPlane::opening_frames` — the frames a session owes BEFORE any frame arrives, for the wires whose first byte is the SERVER's rather than the client's; every other seam on that trait is driven by an arriving frame, so a plane on such a wire could not say so and a composition that mounted one served an upgraded socket that stayed silent until the client gave up) measured 1 line and took it to **3,490**. The SESSION-OPENING FRAME's companion — WHICH of a dialect's client events is a REQUEST and the terminal each owes when nothing can answer one — is declared on the same dialect row (`Dialect::request_terminal`) and costs this pair nothing: most client events on a duplex wire are notifications the dialect gives a server nothing to send back for, a request is the kind a client BLOCKS on, and a served leg that answered one with silence left that client waiting for as long as the socket was open. The plane marks such a frame and the unit opened on it is REFUSED before the door — past it an ending is a failure and a failure renders no frame — so the wire's own terminal reaches the socket through the refusal path the contract already has. WHICH event a wire opens with is never this face's: it is declared on the dialect's own row (`busbar_plane_streams::dialect::Dialect::opening_event`), rendered by the plane from the posture the session resolved to, through the writer that same row names — so no vendor's event name and no vendor's envelope reaches the contract, the neutral plane or the composition root, and the root writes bytes it did not author exactly as it does for every other frame. Two crates carry their own surface ceilings beside it, because
...
`busbar-plane-<plane>-<dialect>` whose only workspace dependencies are `busbar-contract`, its own
plane, and — until the pre-split `*-codec` halves are split — that plane's own codec half, which is
the shared IR every dialect of the plane meets in and is the same edge the plane itself already
carries; a dialect naming a SIBLING plane, or a sibling plane's codec, is the plane fusion one level
down and is refused; a plane declares its wire surface as data and a transport mounts any plane generically
