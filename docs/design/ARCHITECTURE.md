...
- The **kernel** is the **registry** and the **Teller** plus what the loop needs: frame pump, sessions
  ...
  `StoreError`/`SecretError` enums are gone; the surface measured 3479 → 3684 for it. The figure is the PIN, not a budget: it is held
  at zero slack against today's measurement, and it moves up only when a new-architecture
  ...
  3684 → 3704) is the first, and the deletion that followed it (the never-written `Unit` byte
