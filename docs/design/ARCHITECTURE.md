...
### 4.7 Admin verbs, dual control, config, defaults
...
`export_keyset`, `approve` (the maker-checker approval under `required`: its payload hash must equal
the pending mutation's and its approver must differ from the maker) (12 verbs; keyset import is the
off-node CLI, not a verb), plus `amend_rate_history` — the one money verb, which ADDS a dated rate row
and never edits one (§4.5).
...
**HTTP binding of the 12.** Each of the 12 binds as `<kebab-case-verb>` under the admin prefix: POST for
...
`POST commit-upgrade` · `POST export-keyset` · `POST approve`. `amend_rate_history` binds by the same
rule and is the thirteenth row of the same table: `POST amend-rate-history`. It is a mutation, so it
is a POST; it is not irreducible, so a single admin may run it (§8.1's insider row states the bound).
...
`set_operator_key`; boot cell). Every member is irreducible UNCONDITIONALLY: the two amount-gated
members an earlier draft carried (`adjust` and `resolve_dispute` above `adjust_threshold`) left with
the verbs themselves, and with them the awkwardness of a set whose membership depended on a quantity
no verb name carries. Control: an **operator
...
| `max_unposted_accrual` (per node) | **the ONE formula, from enforced quantities**: `in_flight_cap × (max_hold + max_response_value)`, where `max_response_value` is the largest single response a unit can deliver at the configured rates — accrual since the last Tick checkpoint can never exceed a unit's hold plus its journaled top-ups plus the one response it was already delivering when the tick fell. The ceiling term the earlier formula carried is gone with the ceiling; the substitute is not a knob but a consequence of `max_response` and the card, so nothing has to be armed for the bound to hold; published on the ledger endpoint (PB-16); alarmed above `unposted_alarm`; measured at M2 by the kill-mid-stream cell | asserted at boot/reload only when operator-pinned |
