...
- The **kernel** is the **registry** and the **Teller** plus what the loop needs: frame pump, sessions
  ...
  arena/masking 0.3k; `busbar-caps` + `busbar-contract` ≤ **3,596** **of plugin-visible SURFACE**
  (3,594 as `scripts/loc-surface.py` counts it, 3,596 as `loc-ceilings:caps-contract` does; the two
  differ by two lines of counting rule and are pinned as one decision) — raised from 3.5k on
  2026-09-09 for two faces, each pinned at its measured size, zero slack. (1) THE DIALECT KIND'S
  CONTRACT FACE, 51 lines: `Kind::{Dialect, Control}` with their sealed markers and
  ...
  the contract, so this is the one crate both sides can point at. A future face raises this figure
  by its own declared, measured amount (`[gate.ceiling_raises]` in `qa/construction.toml`) and
  ...
  `scripts/loc-surface.py` (`--ceiling busbar-contract,busbar-caps=3594`), which the construction
