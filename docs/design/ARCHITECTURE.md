...
- The **kernel** is the **registry** and the **Teller** plus what the loop needs: frame pump, sessions
  ...
  — ≤ **2k**, gated as `surface-ceiling:core-policy` and PINNED at its measurement (1,733 at
  the hook-subject wiring, 2026-09-10 — 1,732 at [M4c]; 1,672 the day it was extracted from `busbar-core`, owner ruling 2026-09-08). The ONE line is `GateSubject::subject`: the neutral gate could not name what it was deciding about, so it wrote `requested_model: None` and pointed a reader at a content-projection label no wire has ever carried; the firing plane answers it now, exactly as it already answered `container`. It names **no plane, no dialect and no transport**, and
...
| `busbar-core-policy` | core | the hook POLICY engine — resolution, gates, rewrites, singleflight, scrape — which the plain-data hook carriers in the substrate are not and never were. The kernel seats hook PLUGINS; this seats their policies. Its manifest names `busbar-contract` and `busbar-substrate` and nothing else; a hook plugin is reached through its own port (`HookSeat`), bound by the composition | its own row, `surface-ceiling:core-policy`, pinned at 1,733 (the hook-subject wiring; 1,732 at [M4c]) |
