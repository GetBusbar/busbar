...
| Heap allocations per one-shot unit outside the arena (scope: process allocator on the Teller path; TLS handshakes, pool warm-up, third-party init, and the one boxed transport future per call excluded and listed) | **0** — TWO instruments, because the scope is two paths and one of them was unmeasured. `FORWARD_PASSTHROUGH_MAX_ALLOCS` (1.6.0-dev work, measured 87, gated 107 → ≤ 20 at M4 → 0 at Phase 0.5 exit) is `busbar-llm`'s forward path; `ONE_SHOT_UNIT_MAX_ALLOCS` / `EXIT_MAX_ALLOCS` (measured 21 and 3, `crates/busbar-kernel/tests/alloc_gate.rs`) are the Teller's own — every unit that ends runs `exit()` once, and until 1.6.0 nothing counted what it costs, so the row's stated scope was wider than the instrument enforcing it. Per session as well | `alloc_gate`, both halves built into the gate with a match-count floor each |
