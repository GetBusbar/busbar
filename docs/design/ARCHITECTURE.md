...
- The **kernel** is the **registry** and the **Teller** plus what the loop needs: frame pump, sessions
  ...
  The WAL, group commit and chains live in `busbar-kernel-wal`; the kernel verbs live in
  `busbar-core-admin`, which absorbed the `busbar-unit-verbs` executor (W4.b, DECISIONS #36). **LOC ceilings, gated as one union and per file, by call-graph** (a
...
| `busbar-core-oauth2` | cleanliness crate (not a plugin kind) | the OAuth 2.1 authorization server — metadata document, `/authorize`, `/token`, login, consent, its own signer. Verification is a step and stays in `busbar-kernel-identity` (the crate `busbar-unit-auth` was renamed to); an authorization SERVER is a served surface, compiled in and off the hot path | none |
...
**Kernel verbs** (executed by `busbar-core-admin`, which absorbed the `busbar-unit-verbs` executor in W4.b
(DECISIONS #36) and builds `SecretOnce`; `AdminToken` is held by the composition root) are a closed table **derived mechanically from 1.5.5's `openapi.json`
...
**Admin API — byte-identical.** Every 1.5.5 operation (66 at the tag) returns the same bytes for the same state — with the two stated exceptions, BOTH now version-reporting: `GET /admin/openapi.json` (PB-75, ADM-052: ONE generated admin document describing every served operation, served filtered to the node's configured planes per Law 7, with `info.version` reporting `CARGO_PKG_VERSION`; no side-car document exists) and `GET /admin/info` (ADM-042), whose `version` is `CARGO_PKG_VERSION` and whose `build { auth_modules, hook_plugins, weighted_floor }` reports the running binary, the same shape with the running values —
...
| PB-44 | `in_flight_reserve` | none — the provider reserve is deleted (item 280): every counted unit, a session's provider frame included, draws the one cap and no slot is held back; a 1.5.5 config sheds at exactly `max_inbound_concurrent` | config CONF-134 |
...
| PB-48 | stall sweep on `http`/`sse` | the `max_unit_duration` sweep alarms only (`busbar-kernel/src/tick.rs:141`, `Sweep::AlarmOnly`, which `sweep_outcome`/`sweep_settle` end nothing on); the unit's only timers are PB-8's — a stream is cut only by the `upstream_request_timeout_secs` TOTAL deadline (default 300 s, `busbar-kernel/src/config/limits.rs:22`), armed once at send start and covering head and body (`busbar-llm/src/engine/attempt/send.rs:63-76`; at the tag this was reqwest's client-level total timeout, re-provided at the call site by the owned-client cutover); there is no idle timer | proxy-hooks §2.8, ops §3.6 |
...
| PB-75 | served OpenAPI document | `GET /admin/openapi.json` (ADM-052, one of the 66) serves ONE generated admin document that describes every operation the admin router serves — the 1.5.5 operations and the 1.6.0 kernel verbs alike — filtered to the node's configured planes (Law 7): an operation belonging to a plane the node does not configure is absent from the served document, exactly as its route is. There is no side-car document: nothing is served at `/api/v1/admin/ledger/openapi.json`. OWNER DECISION — `info.version` reports the running binary's version (`CARGO_PKG_VERSION`, 1.6.0), the same rule `GET /admin/info` (ADM-042) already applies, not a 1.5.5-verbatim pin | routes-admin ADM-052 (:409), :266-284 |
...
| PB-87 | non-chat billing classes | `Billing::Flat` (moderation) and the per-op meters for embeddings / image / transcription / speech / rerank are sealed into the migrated card's declared set at `Migration` with 1.5.5's projection, so no class 1.5.5 billed is `Refused(Admit, Unpriced)` and no figure changes — except the rerank search units 1.5.5 discarded: a rerank reporting them bills the counted class `search_units` (item 134, #42/#71: a present card prices it or, silent about it, refuses; no card reads 0), and one reporting none stays `Flat` | dialects §5.3 (:824-830) |
...
| PB-103 | the dated rate-card history, and the `/usage` read path that does not consult it | cards are an APPEND-ONLY DATED HISTORY (owner ruling `docs/design/BUSBAR-1.6.0.md` #79): a config `PUT /config/settings` card APPENDS an entry rather than replacing the price (`crates/busbar/src/root/kernel.rs:262`, `effective_from` — `0` on the first entry and `now_ms` after, so publishing a card never touches the window before its own date; the window recompute is `:291`), and 1.5.5's rate-card completeness refusal is byte-identical (a partial card is still the same 400 `invalid_request` naming every unpriced model with its paste-ready zero-rate stub). The signed back-dated correction is ADDITIVE, never a rewrite of a 1.5.5 surface: `POST /api/v1/admin/ledger/amend-rate-history` (`crates/busbar/src/root/units_admin/mod.rs:679`) is a route 1.5.5 does not carry, so 1.5.5 answers it with the ROUTER's generic 404 `not_found` / "resource not found" — BYTE-IDENTICAL signed and unsigned, because 1.5.5 is refusing the route and never reads `X-Busbar-Signature`. **`GET /api/v1/admin/usage` NOW RESOLVES THROUGH THE HISTORY (#79's owed item, landed):** `busbar_core_admin::v1::service::get_usage` prices each metering row against `card_at(row's own first instant)` — the later of the bucket start and `MeteringRow::priced_from_ms`, the `effective_from` of the entry in force when the counts were accrued — instead of flat off the CURRENT card. That instant joins the accrual key, so a card edit inside a UTC day SPLITS the day's row and each half keeps the card it was earned under; the clip to the bucket start is what keeps the instant inside the row's own wall-clock span so a signed back-dated correction can still reach it. Where there is no history (no composition-root install) or the instant falls in a hole no entry covers, the read FALLS BACK to the previous release's `derive_spend_micros_row` — a fallback, never a silent zero (#42). `spend_micros` still carries full micro-unit precision with NO minor-unit projection (a sub-cent card reads back 18 per row and 54 as an exact sum), and `PostingStamp.rate_card_version` is reporting provenance and is never the pricing input — the key is an instant, which is what keeps the history recoverable backward. The registered D-3 breaking divergence is now SHIPPED behaviour for this read. STILL OWED, and a live inconsistency: the two ENFORCEMENT-ledger reads beside it (`GET /keys/{id}/usage`, `GET /groups/{group}/usage`, both through `GovState::derived_bucket_usage`) derive from the CURRENT card because `UsageLedger`/`ModelTokens` carry no price instant, so after a card edit the two surfaces disagree. `get_usage` also still parses NO `as_of` request parameter (the `as_of` in its response is an OUTPUT, always `store::now()`) | governance 5.4, 5.6.6, 5.6.10, 6.3.2; routes-admin ADM-066; `docs/design/rate-card-history.md` §1.1, §2.3, §9; `docs/design/BUSBAR-1.6.0.md` #79 |
