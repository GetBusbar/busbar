...
- The **kernel** is the **registry** and the **Teller** plus what the loop needs: frame pump, sessions
  ...
  (`--ceiling busbar-contract,busbar-caps=4754`), which the construction gate runs as
  ...
  — ≤ **2k**, gated as `surface-ceiling:core-policy` and PINNED at its measurement (1,729 the day
  its grant ladder `no ⊂ ro ⊂ rw` was written over the hook subject's ARGUMENT axis and the caller's IDENTITY axis rather than one plane's
  projection of them, 2026-09-10; 1,732 at [M4c], 2026-09-10; 1,672 the day it was extracted from `busbar-core`, owner ruling 2026-09-08). It names **no plane, no dialect and no transport**, and
...
| `busbar-core-policy` | core | the hook POLICY engine — resolution, gates, rewrites, singleflight, scrape — which the plain-data hook carriers in the substrate are not and never were. The kernel seats hook PLUGINS; this seats their policies. Its manifest names `busbar-contract` and `busbar-substrate` and nothing else; a hook plugin is reached through its own port (`HookSeat`), bound by the composition | its own row, `surface-ceiling:core-policy`, pinned at 1,729 (the grant ladder written over the hook subject's argument axis; 1,732 at [M4c]) |
