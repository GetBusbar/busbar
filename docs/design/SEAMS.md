...
| G9 | `config-stability-gate` | — | `scripts/config-stability-gate.sh`; generator `scripts/config-schema.py` (SOURCES `:121-168`, `resolve_sources :171`); snapshot `crates/busbar-core/src/config/config-schema.snapshot.json`; waivers `config-schema.waivers`; CI `.github/workflows/ci.yml:730,742,746` | two rules: (1) drift — snapshot byte-equals a fresh render; (2) additive-only against a **git ref, never the working tree** (`:628-657`). Field removed/retyped/newly-required or enum variant dropped = exit 3. Frozen at 1.5.3. |
