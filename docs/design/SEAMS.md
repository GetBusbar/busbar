...
| $4 | `busbar_core::cost::price` | — | **GONE** (keep-core-delete-4). It was `cost.rs:208`, documented "THE ONE ENFORCEMENT PRICER", and `#[cfg_attr(not(test), allow(dead_code))]` — dead outside tests, with the live seam routing around it. | Deleted with `ExtraRates`, `STANDARD_TIER_BP` and the open-label namespace (140 lines). This row stays in the table because the count in §1.11's heading and the verdict below it are about what was FOUND, and a pricer that is deleted is a different fact from a pricer that was never there. `cost.rs` derives no money at all now, and since keep-core-delete-8 holds no chain memo either. |
...
| Unit | Ports | Where |
|---|---|---|
| admission | `Admission :79`, `Cells :201`, `CellStore :219`, `ChainWalk chain.rs:223` (on `busbar_unit_cost::GroupTable`) | `busbar-unit-admission/src/{lib,cells,chain}.rs`. Beside the walk, and not a port: `ChainCache cache.rs` — every chain a resolved table can produce, walked ONCE at boot, plus the capped-bucket-id index, read back as `Chain`/`BucketView` cursors that borrow (keep-core-delete-8). It is here rather than on `GroupTable` because the cache is made of what the walk RETURNS, and the cost unit naming this one is a Cargo library cycle. |
| audit | `Audit :265`, `ChainedRecord legacy/chain.rs:146`, `Clock legacy/entry.rs:228`, `DurableSeam :240` | `busbar-unit-audit/src/` |
| auth | `AuthModule module.rs:29`, `CredentialDigest cache.rs:63`, `HeaderView carrier.rs:32`, `HeaderProbe detect.rs:27`; the signed-key arm and the revocation gate read the contract's `VirtualKeyDirectory` face (`busbar-contract/src/kinds.rs`) — the crate's own `KeyVerifier`/`RevocationView`/`ResolvedKey` are gone | `busbar-unit-auth/src/` |
...
| X3 | `auth_bindings.rs` | `KeyScope`, `KeyFacts` and `VirtualKeyDirectory` moved home to `busbar-contract/src/kinds.rs` (the virtual-keys face; `is_revoked`, `operator_token_hash` per call). Left here: `AuthBindings` (`new`/`without_directory`, `directory()`, `credential_cache`) carrying the face itself — no adapter, the unit takes the face — `GovernanceDirectory` (`impl busbar_contract::VirtualKeyDirectory`), `AdminTokens` over the face | installed `root/kernel.rs:54,190,304,410`; used `main.rs:857`, `units_admin.rs:1356,3127,4190`, `units_mcp.rs:380`, `units_voice.rs:708,766,1984,1997`. `GovernanceDirectory` still names `busbar_core::governance::GovState` — legacy reach. |
...
`VirtualKeyDirectory`, since moved home to `busbar-contract::kinds`) are ports declared at the composition root, which is
