...
| `DELETE /overlay/{section}` | **Revert one overlay section to base `config.yaml` truth** (`section` ∈ `groups` \| `hooks` \| `root` \| `plugin_versions`). Discards ALL overlay mutations for that section — a `groups` reset restores the base limit tree, a `hooks` reset restores base hooks, a **`root` reset discards every durable `PUT /config/settings` override** (rate_card, per_request_fee, security, limits, listen, tls, store, and everything else that block covers), and a `plugin_versions` reset discards every first-party-plugin rollback pin — while leaving the other sections' runtime mutations untouched. `full` scope, `If-Match` optimistic concurrency, audited, versioned, and the cleared overlay is persisted (the revert survives restart). A section with no overlay state is an idempotent no-op (`changed: false`, ETag/version unchanged). Only a section OUTSIDE this four-value set is `400` — `root` and `plugin_versions` are real, consequential targets, not safe no-ops. An ephemeral Busbar with no config files has nothing to revert to and returns `400` |
