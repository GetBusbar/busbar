...
| Class | Budget | Covers |
|---|---|---|
| config | 10/min | `POST /config/apply`, `PUT /config/settings`, `POST /config/reload`, `POST /config/rollback`, `PUT /admin-auth`, `DELETE /overlay/{section}`, `POST /plugins/reload`, `POST /plugins/rollback`, `POST /restart`, and the named-definition-map writes `PUT /identity-providers/{name}`, `PATCH /identity-providers/{name}/settings`, `DELETE /identity-providers/{name}`, `PUT /export/{name}`, `PATCH /export/{name}/settings`, `DELETE /export/{name}`, `PUT /tools/{name}`, `PATCH /tools/{name}/settings`, `DELETE /tools/{name}` (the blast-radius set: each rebuilds and swaps the whole App) |
...
| `DELETE /overlay/{section}` | **Revert one overlay section to base `config.yaml` truth** (`section` ∈ `groups` \| `hooks` \| `root` \| `plugin_versions` \| `identity-providers` \| `export` \| `tools`). Discards ALL overlay mutations for that section — a `groups` reset restores the base limit tree, a `hooks` reset restores base hooks, a **`root` reset discards every durable `PUT /config/settings` override** (rate_card, per_request_fee, security, limits, listen, tls, store, and everything else that block covers), a `plugin_versions` reset discards every first-party-plugin rollback pin, and a **named-definition map reset (`identity-providers` / `export` / `tools`) discards every definition that map was given through the API** — for `tools`, every MCP server registered and approved through the API, reverting the plane to whatever `config.yaml`'s `tools:` block declares — while leaving the other sections' runtime mutations untouched. `full` scope, `If-Match` optimistic concurrency, audited, versioned, and the cleared overlay is persisted (the revert survives restart). A section with no overlay state is an idempotent no-op (`changed: false`, ETag/version unchanged). Only a section OUTSIDE the set above is `400` — `root`, `plugin_versions` and the named maps are real, consequential targets, not safe no-ops. A named-map reset that would leave `config.yaml` naming a definition it removes is refused `409` `conflict` naming the entry and its referents; nothing is cleared. An ephemeral Busbar with no config files has nothing to revert to and returns `400` |
