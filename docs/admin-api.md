...
| `DELETE /keys/{id}` | **DESTRUCTIVE, irreversible.** Hard-purge: the key's metadata row is removed AND its usage/billing history is deleted from the store — there is no tombstone, no rollback, and no way to recover the record afterward. `204 No Content`; the key stops resolving immediately. Honors `If-Match`; `404` for an unknown id. If you want the key to stop authenticating while KEEPING its historical record (budgets, usage, audit attribution) queryable, use `POST /keys/{id}/revoke` instead — that is the safe, non-destructive way to retire a key |
| `POST /keys/{id}/revoke` | **Denylist-only revoke (non-destructive, recommended for normal key retirement)**: the binding's metadata row and all of its usage/billing history are KEPT (unlike `DELETE`, which removes both), but the id is added to the revocation denylist so it stops authenticating immediately, same as `DELETE`. `{revoked: id}` on success, `404` for an unknown id. The distinct verb exists for signed-token keys (1.5.0): a `DELETE` and a `POST .../revoke` land on the SAME denylist, but this endpoint is the one to reach for when you want the historical record (budgets, usage, audit attribution) to keep resolving by id after the key stops authenticating |
| `POST /keys/{id}/rotate` | Re-issue the key's **credential in place**: `200`, same id (budgets, rate windows, usage history, audit attribution carry over), the **previous credential stops authenticating immediately** (fleet-wide — the new binding generation is durable), the new one is shown exactly once. A 1.5.0 signed-token key answers `{token, expires_at}` (every token issued before the rotation is now rejected); a legacy hashed-secret key answers `{secret}`. Rotation never converts a signed-token key into a hashed-secret one. An attached AWS credential is untouched |
