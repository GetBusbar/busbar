...
The check is a simple two-level test: `read-only` satisfies every read; every mutation needs `full`. A `max_admin_scope: read-only` ceiling on an identity-provider DEFINITION caps that provider's principals to reads regardless of what `role_bindings` grants (`none` denies admin entirely).
