...
The operator admin token holds `full`. Role-carrying principals (external modules) get the highest scope their bound roles grant under `auth.role_bindings.<module>`, capped by the module's `max_admin_scope:`; unbound roles grant nothing. Insufficient scope is `403 forbidden` naming the scope that would have sufficed. Unknown HTTP methods fail closed to `full`.
