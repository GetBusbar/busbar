...
| `DELETE /overlay/{section}` | **Revert one overlay section to base `config.yaml` truth** (`section` ∈ `groups` \| `hooks` \| `root` \| `plugin_versions` \| `named_maps`). Discards ALL overlay mutations for that section — a `groups` reset restores the base limit tree, a `hooks` reset restores base hooks, a **`root` reset discards every durable `PUT /config/settings` override** (rate_card, per_request_fee, security, limits, listen, tls, store, and everything else that block covers), a `plugin_versions` reset discards every first-party-plugin rollback pin, and a **`named_maps` reset discards every API-applied named-DEFINITION entry** (`identity-providers:`, `export:`) so those maps revert to what `config.yaml` declares — while leaving the other sections' runtime mutations untouched. `full` scope, `If-Match` optimistic concurrency, audited, versioned, and the cleared overlay is persisted (the revert survives restart). A section with no overlay state is an idempotent no-op (`changed: false`, ETag/version unchanged). Only a section OUTSIDE this five-value set is `400` — `root`, `plugin_versions` and `named_maps` are real, consequential targets, not safe no-ops. **Referential integrity:** a `named_maps` reset is REFUSED with `409 conflict` when base `config.yaml` still references a definition that exists only as an overlay entry (e.g. `auth.chain:` names a provider you created through the API), naming every blocking entry and its referents — the same verdict the per-entry `DELETE /identity-providers/{name}` gives, so the bulk path cannot be used to reach a dangling state the per-entry guard forbids. Remove the reference from `config.yaml` first, or delete the entries individually. An ephemeral Busbar with no config files has nothing to revert to and returns `400` |
