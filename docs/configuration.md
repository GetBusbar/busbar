...
| `otlp` | Traces | `url` (**required**) | OTLP/HTTP trace exporter. Loopback `http://` is allowed (standard collector default); remote endpoints must use `https://`. SSRF-guarded: rejects RFC-1918, link-local, CGNAT, metadata hosts. Traces are flushed on graceful shutdown. At most ONE instance (an `otlp` instance is this process's one trace-export layer, with one queue and one capacity gate behind it; the subscriber itself is installed whether or not you configure a collector). |
