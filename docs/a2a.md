...
The task TTL is reached the same way rather than as a second deadline: a task whose last movement is older than the abandonment ceiling is moved to `canceled` through the normal write path, and `canceled` is terminal, so the sweep that retires the task is the thing that retires its token. (Before any of this, the callback endpoint checked only that the MAC verified and that the row existed — no liveness, no revocation, no deadline — so a captured token replayed indefinitely, and each replay re-notified the caller and appended another entry to the task's durable provenance chain.)
