...
Busbar checks liveness through a neutral store verb whose default is **fail-closed**: a deployment whose store cannot say whether a token is still live refuses the callback rather than accepting it. (Before this, the check compared a timestamp against itself over a verb that defaulted to "yes", so no token was ever refused and a captured one replayed indefinitely.)
