...
- **The GET stream is gone**, and with it resumability. `GET` and `DELETE` answer `405`. The server-to-client channel moved onto a method: `subscriptions/listen` is an ordinary POST whose response is a long-lived stream of notifications (`crates/busbar-mcp/src/mcp/subscribe.rs:1-21`). A principal admitted through `auth.role_bindings` keeps its subscription for as long as a binding still grants it: removing the binding, or narrowing it to no pools, ends the stream with `identity_not_live` on the next frame, and a changed pool or group applies from that frame on.
