...
> Status: accepted. `ADR-0001` is referenced in `crates/busbar-kernel/src/store/in_memory/availability.rs`.
...
`HealthState::select_weighted_for` in `crates/busbar-kernel/src/store/in_memory/mod.rs`):
