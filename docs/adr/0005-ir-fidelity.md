...
> Status: accepted. `ADR-0005` is referenced in `crates/busbar-llm-codec/src/ir/types.rs`
...
Define a **superset IR** (`crates/busbar-llm-codec/src/ir/mod.rs`) that is the union of what the six
...
ADR-0006 in `crates/busbar-kernel/src/proto/mod.rs`; that seam is the *mechanism*, while this ADR is
