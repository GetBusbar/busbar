...
pin-by-sha gate catching this migration's own unpinned action ref (§5, fixed by pinning that ref). 12m53s
...
3. **`R14` (every third-party action pinned by commit sha) caught this migration's own mistake**,
   ...
   not a sha. Fixed by pinning it (`@v1` → `@d0dd21912a57c7435649c77f689b68d348d8a662 # v1`,
...
results are identical to the prior run's) is this slot's real Latchkey execution evidence, read once
