...
// A BATTERY IS WRITTEN FOR THE KIND, NOT FOR TODAY'S CALLERS. `assert_total` is the arm for a unit
// that SERVES its step, and the twelve crates that owe their answer half do not call it yet; an arm
// deleted for being uncalled today is a ruling deleted, and the next crate to grow its fixture
// would have to re-derive it. `dead_code` is allowed HERE and nowhere else in this tree.
#![allow(dead_code)]
...
//! that battery: the helpers live here once, the fourteen blocks of `tests/unit_conformance.rs`
//! call them, and a new ruling added here is asked of all fourteen crates by the same run instead
//! of being hand-copied into fourteen files and drifting.
//!
//! It is a module of the GATE RUNNER's test tree rather than of a shipped helper crate, because
//! the dependency has to run tooling -> plugin. See `tests/unit_conformance.rs`'s own header.
//!
...
//!
//! # Usage, from a crate's block of `tests/unit_conformance.rs`
//!
//! ```ignore
//! use crate::conf;
//!
