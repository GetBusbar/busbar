...
//! integrates by. Every transport's block of `tests/transport_conformance.rs` fills
//! [`TransportDecl`] from its own `TransportMeta` consts and runs [`assert_declaration`]; the
//! answers are then the same question asked seven times instead of seven questions.
//!
//! It is a module of the GATE RUNNER's test tree rather than of a shipped helper crate, because the
//! dependency has to run tooling -> plugin. See `tests/transport_conformance.rs`'s own header.
//!
...
//!
//! So that the battery is about the DECLARATION and not about a type: every wire's block passes the
//! consts its own `TransportMeta` states, and the battery never has to be taught which seven types
//! exist. Adding a wire is adding a block, not editing this file.
