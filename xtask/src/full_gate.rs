...
pub const CARGO_LOCAL: &[&str] = &[
    ...
    "cargo clippy -p busbar -p busbar-core -p busbar-admin --all-targets --features openapi-schema --locked -- -D warnings",
    "cargo test -p busbar -p busbar-core -p busbar-admin --features openapi-schema --locked openapi -- --nocapture",
    "cargo build --locked --bin busbar",
    "cargo test -p busbar --test migration_corpus --locked -- --nocapture",
    "cargo test -p busbar-streaming --features runtime,test-support -p busbar-streaming-codec --features runtime --locked",
...
pub const REGISTRY_NOT_IN_CI: &[(&str, &str, Excuse)] = &[
    ...
    (
        "hot-path-perf",
        "the perf witness, staged ahead of the keystone wave that \
...
fn selftest(
    ...
    match cx.read(fixture) {
        ...
        Ok(ftext) => {
            ...
            let want = "cargo test -p busbar-streaming --features runtime,test-support -p busbar-streaming-codec --features runtime --locked";
            ok(
