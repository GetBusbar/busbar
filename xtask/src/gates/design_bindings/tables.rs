...
pub static NOTES: &[(&str, &str)] = &[
    ...
    ("PB-75", "The binding's side-car clause (1.6.0 verbs described at a separate `/api/v1/admin/ledger/openapi.json` document, the legacy document's bytes left as 1.5.5) is superseded: there is ONE generated admin document describing every served operation, and the node serves it filtered to its configured planes. The served-surface tests pin that; info.version tracks the crate version (owner decision)."),
