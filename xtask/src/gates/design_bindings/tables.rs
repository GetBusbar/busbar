//! THE CURATED TABLES OF THE DESIGN BINDINGS LEDGER — data, moved rather than rewritten.
//!
//! Every entry here was confirmed by a human reading the test or the gate: it asserts the
//! binding's rule, not merely its vocabulary. The conversion transcribed the four tables out of
//! `scripts/design-bindings.py` byte for byte, because a table nobody re-derived is a table
//! nobody can have changed by accident — and `cargo xtask gate design-bindings --write` is
//! proven to reproduce the committed ledger byte for byte on top of them.

/// The curated mappings: binding id -> the checks that PROVE it, each confirmed by reading the
/// test or gate. `proves` names the clause the check pins, because most bindings are omnibus
/// rows and a single test rarely covers all of one. Transcribed from
/// `scripts/design-bindings.py::SEED` unchanged — it is data, and the conversion moved it, not
/// edited it.
/// One curated check: its kind, the ref it names, and the clause it proves.
...
pub static SEED: &[(&str, &[SeededCheck])] = &[
    ...
    ("PB-12", &[
        ("test", "each_plugin_sink_instance_gets_its_own_admission_gate", "the export sink admission gate is per instance (the corrected export-sink config row)"),
        ("test", "a_plugin_sink_sheds_deliveries_beyond_its_inflight_cap", "the fixed MAX_INFLIGHT_PLUGIN_DELIVERIES cap"),
    ...
    ("PB-22", &[
        ...
        ("test", "crates/busbar-kernel/src/governance/tests/limits_tests.rs::chain_and_parent_blocks_child_and_charges_nothing", "a blocked attempt charges nothing on any chain bucket"),
        ("test", "test_group_blocked_429_names_the_budget_group", "the first blocking bucket names the 429 and charges 0"),
        ("test", "crates/busbar-kernel/src/governance/tests/limits_tests.rs::budget_cap_derives_from_ledger_and_rate_card", "the derived + fee lookahead"),
        ("test", "test_group_token_spend_blocks_chain_admission", "tokens is enforced post-hoc via derived spend"),
        ("test", "crates/busbar-kernel/src/governance/tests/limits_tests.rs::concurrent_gauge_holds_and_releases", "the concurrent gauge"),
        ("test", "crates/busbar-kernel/src/governance/tests/limits_tests.rs::disabled_group_freezes_the_chain", "the FREEZE arm"),
    ]),
    ("PB-24", &[
    ...
    ("PB-35", &[
        ("test", "crates/busbar-kernel/src/auth/tests/tests.rs::test_empty_chain_is_open_front_door", "an empty chain is Open with or without a credential"),
        ("test", "crates/busbar-kernel/src/auth/tests/tests.rs::test_nonempty_chain_fails_closed_on_all_pass", "all-Pass without a keys arm is Denied"),
        ("test", "crates/busbar-kernel/src/auth/tests/tests.rs::test_keys_in_chain_sets_flag_not_module", "the keys arm sets keys_in_chain and boxes no module"),
        ("test", "crates/busbar-kernel/src/auth/tests/tests.rs::test_chain_identifies_with_module_and_principal", "the first Identify wins"),
    ...
    ("PB-47", &[
        ("test", "crates/busbar-kernel/src/governance/tests/limits_tests.rs::pool_scoped_accrual_and_refund_mirror_the_charge", "accrual and refund land on the pool the admission charged"),
        ("test", "crates/busbar-kernel/src/governance/tests/limits_tests.rs::budget_block_carries_downgrade_target", "the block names the downgrade pool"),
    ...
    ("PB-57", &[
        ("test", "crates/busbar-kernel/src/store/tests/tests.rs::test_zero_weight_member_is_never_selected", "weight-0 lanes are filtered before selection"),
    ...
    ("PB-68", &[
        ...
        ("test", "crates/busbar-kernel/src/tests/net_guard_tests.rs::the_shared_internal_predicate_covers_every_range_any_plane_ever_checked", "one shared predicate across planes"),
    ]),
    ("PB-73", &[
    ...
    ("PB-75", &[
        ...
        ("test", "openapi_documents_every_served_admin_operation", "ONE generated document describes exactly the served admin operations (plane operations tagged x-busbar-plane, kernel verbs x-busbar-since 1.6.0) and the side-car document does not exist"),
        ("test", "served_openapi_lists_only_the_configured_planes", "with a 1.5.5 config the served document's paths are the 1.5.5 golden's plus the always-served kernel verbs, no plane path; a configured plane's operations are served again"),
        ("test", "admin_corpus_reconciles_with_the_served_router", "every served admin operation answers on a live listener and has admin.ops oracle cells"),
    ]),
    ("PB-76", &[
    ...
    ("PB-79", &[
        ("test", "crates/busbar-kernel/src/governance/tests/limits_tests.rs::missing_group_fails_closed", "MissingGroup fails closed at admission"),
        ("test", "crates/busbar-kernel/src/governance/tests/limits_tests.rs::disabled_group_freezes_the_chain", "a frozen group blocks the chain"),
        ("test", "crates/busbar-kernel/src/governance/tests/limits_tests.rs::chain_and_parent_blocks_child_and_charges_nothing", "a parent block charges nothing"),
        ("test", "crates/busbar-kernel/src/governance/tests/limits_tests.rs::total_window_blocks_without_retry_after", "a block without Retry-After"),
    ...
    ("PB-91", &[
        ("test", "crates/busbar-kernel/src/governance/tests/limits_tests.rs::refund_returns_the_fee_but_never_the_requests_limit_slot", "a refund returns the fee but never the requests slot"),
...
pub static NOTES: &[(&str, &str)] = &[
    ...
    ("PB-48", "Resolved 2026-09-06: max_unit_duration DOES exist -- it is the fourth parameter of `sweep()` at crates/busbar-kernel/src/tick.rs:141, and Sweep::AlarmOnly is the verdict it returns when the protocol declares no idle bound. The old note was wrong on the fact and the row was carried unproven on it. The sweep half is compared by crates/busbar-kernel/tests/recovery_and_ticks.rs::a_slow_unit_is_not_a_lost_one, which drives sweep() past max_unit_duration with idle_bound_applies false, gets AlarmOnly, and shows sweep_settle takes no hold and ends no unit on it; nothing in crates/ calls sweep() on a production path at all, so the alarm is the most that can happen. The stream half is compared on the SHIPPED proxy path by crates/busbar-llm/src/engine/engine_tests/stream_deadline_tests.rs::a_proxied_stream_is_cut_at_the_configured_second_and_by_nothing_earlier: the same black-holed upstream is cut at 7 s under one configuration and at 23 s under another, so the timer that fires is the operator's total deadline and no idle timer exists to fire first. The binding's `main.rs:3179-3181` anchor was dead and is amended in Appendix B to busbar-substrate/src/config/limits.rs:22 and busbar-llm/src/engine/attempt/send.rs:63-76."),
    ...
    ("PB-58", "Resolved 2026-09-06: all three reason codes DO exist -- crates/busbar-caps/src/decision.rs declares OverBudget, OverdraftCeiling and StaleSlice, and OverBudget is constructed six times in production, once at crates/busbar-llm/src/unit/admit.rs:203 as a Decision::refuse at StepName::Admit. The row was carried unproven on a note that was wrong on the fact. Every clause of this binding is a claim that a SHAPE DOES NOT OCCUR, which no behavioural test can settle, so it is settled on the shipped VOCABULARY by crates/busbar/tests/money_never_ends_an_admitted_unit.rs: the set of reasons the tree constructs inside `Abort::Kernel` is exactly {ClientGone, Drain} and contains no money reason; the set it constructs inside `Refusal::new` contains OverBudget (the non-vacuity, and the binding's positive half -- money refuses at a door) and neither OverdraftCeiling nor StaleSlice; and `Overdraft::Ceiling` is named in one production file, the module defining the rule, so no shipped path branches on the ceiling and KernelVerb::SetOverdraftCeiling carries no payload -- flag-only. The behavioural half is where the behaviour is: admit.rs's over_budget_refuses_with_no_charge_and_nothing_to_refund and crates/busbar/src/root/tests/durability.rs's a_spend_past_the_reservation_is_carried_out_as_an_overdraft (the unit settles; the chain holds its counts and no figure; the rebuilt book derives the carry at read)."),
    ("PB-61", "Resolved 2026-09-06: MAX_NEEDMORE_FRAMES is crates/busbar-contract/src/bounded.rs:41 and the pump reads it at crates/busbar-kernel/src/pump.rs:128, where the run of consecutive \"not yet\" answers is kept ON THE SESSION SLOT -- so `Scheduler::ask_again(None)` is the `None => false` arm, and a transport with no session has no run to count. crates/busbar/tests/body_chunk_bounds.rs::the_frame_ceiling_is_unreachable_on_the_two_transports_that_carry_no_session reads SESSION off the SHIPPED HttpTransport and SseTransport (both false) and drives four times the ceiling of NeedMore frames through the pump with no session in hand, and ::a_body_is_refused_at_request_body_max_bytes_whatever_its_chunk_count spools the SAME byte cap twice -- once whole, once one byte per chunk, 256 times the frame ceiling in chunks -- and gets the same SpillBudget refusal on the same byte, never Stalled."),
    ("PB-17", "Resolved 2026-09-06: the binding is a COUNT, and it is now counted. crates/busbar/tests/migration_corpus.rs::no_corpus_config_warns_more_at_boot_than_the_published_1_5_5_did migrates and validates each of the 69 shipped configs in tests/migration-corpus/from-tags and compares the warning lines the current binary emits, per config, against the number the PUBLISHED 1.5.5 binary emitted on the same config -- read out of the pinned golden's own config.migrate|<tag>|validate-migrated recordings, so the baseline is measured and not typed in. 1.5.5 warned 69 times over the 69 configs; this binary warns fewer, having retired the BUSBAR_PROVIDERS deprecation, and a warning that APPEARS is reported with its config and its text. The absence tripwire it used to cite is kept: it proves a different, still-wanted thing about metric series."),
];

/// Bindings whose own note already says nothing was compared. Carrying one as `mapped` because a
/// test fn of some other subsystem happens to bear a matching name is the instrument lying.
///
/// EMPTY TODAY, and that is a measurement rather than a missing table: it mirrors
/// `scripts/design-bindings.py::UNPROVEN_BY_NOTE`, which the owner emptied as each of the four
/// bindings it once held (PB-17, PB-48, PB-58, PB-61) gained a check that compares something. The
/// rule is still live — [`super::verify`] consults this list on every binding — so restoring an
/// entry is a one-row edit, and the self-test plants one to prove the arm can still go RED with
/// nothing in the table on HEAD.
pub static UNPROVEN_BY_NOTE: &[(&str, &str)] = &[];
...
pub static SUGGEST: &[(&str, &str)] = &[
    ...
    ("PB-75", "admin test: the served GET /admin/openapi.json lists exactly the operations the router serves for the configured planes, and info.version equals CARGO_PKG_VERSION"),
...
/// Non-`PB-N` audit-shorthand ids Appendix B's free text still carries, named in words before the
/// row becomes a public artifact.
pub static PUBLIC_WORDED: &[(&str, &str)] = &[("D2", "the terminal-usage settle guarantee")];
