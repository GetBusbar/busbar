...
//! [`external`].
//!
//! THE FILE NAME IS LOAD-BEARING. This is `gates/construction.rs` with its parts in
//! `gates/construction/`, not `gates/construction/mod.rs`, because
//! `scripts/design-bindings.py::xtask_gate_module` resolves a `cargo xtask gate <name>` citation to
//! exactly `xtask/src/gates/<name_with_underscores>.rs` — one mechanical spelling, derived from the
//! gate name rather than maintained beside it, so a gate cannot be cited under a name the runner
//! does not answer to. Four Appendix B bindings cite this gate; under the `mod.rs` spelling every
//! one of them would red on a path that does not exist, for a reason entirely about the rewrite.
