...
fn minted_rows(cx: &Ctx, reg: &super::KindRegistry) -> Vec<String> {
    ...
    for (table, ids, what) in [
        ...
        for key in super::base::row_keys(&now, table, ids) {
            ...
            // A `[[minted]]` ADMISSION IS READ BEFORE THE NEW-COLUMN CARVE-OUT, so that a crate
            // whose rows land in a column that is itself new still lands in `minted_cells` and is
            // reconciled against its row's `cells = N`. Reading the column carve-out first would
            // swallow those rows silently and under-count the admission.
