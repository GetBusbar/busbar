...
//!   of a manifest no rule was reading. On the composition root this is BY DESIGN (`proto-llm`,
//!   `plane-mcp`), and that exemption is now a written rule with a green case rather than
...
///
/// `crates/busbar` carries `proto-llm`, `plane-mcp`, `plane-voice`: the root is the one place
...
pub fn rule_inputs(
    ...
    for c in crates {
        ...
        //
        // THE COMPOSITION ROOT IS EXEMPT, IN WRITING. `proto-llm`, `plane-voice`: the root is
