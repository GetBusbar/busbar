...
impl Gate for InventoryCoverageGate {
    ...
    fn run(&self, cx: &Ctx) -> Verdict {
        ...
        let row_floor = if floor_problems.is_empty() {
            Row::pass(
                ROW_FLOOR,
        ...
        if cx.env().write {
            // The floor is enforced BEFORE anything is written — a `--write` under a collapsed
            // discovery must not launder the collapse into the new baseline.
            ...
            let row_gaps_named = if new_gaps.is_empty() {
                ...
                Row::fail(
                    ...
                    format!(
                        "{} id(s) newly uncovered and not accepted: {}. A gaps file that grows on \
                         its own turns a coverage regression into the record that excuses it; \
                         restore the coverage or accept the loss by hand-editing {GAPS_JSON} in the \
                         same commit.",
                        new_gaps.len(),
                        new_gaps.iter().map(|g| g.id.as_str()).collect::<Vec<_>>().join(", ")
                    ),
                )
            };
        ...
        let row_gaps_named = match named_gap_ids(cx) {
            None => Row::fail(
                ...
                format!("{GAPS_JSON} does not exist — run cargo xtask gate inventory-coverage --write first"),
            ),
            Some(named) => {
