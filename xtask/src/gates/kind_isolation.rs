        ...
        if t.starts_with('[') {
            ...
            reg.errors.push(format!(
                "unknown-table\t{REGISTRY_FILE}:{}\t`{t}` — the file holds `[[transitional]]`, \
                 ...
                 `[[edge]]`, `[[cell]]`, `[[disagreement]]`, `[[patch]]`, `[[minted]]` and \
...
pub(super) struct BaseLedger {
    /// crate -> the kind the BASE announced it as. The `[[minted]]` row's first check.
    pub announced: BTreeMap<String, String>,
    /// (crate, kind) -> the `[[cell]]` count the BASE pinned. The `moved_from` ceiling.
