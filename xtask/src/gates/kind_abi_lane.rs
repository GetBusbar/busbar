...
//! the tree to it.
//!
//! ## WHICH RULE THIS ENCODES — #30, NOT THE SHORTHAND IT SUPERSEDES
//!
//! The spec contradicted itself about this, and that contradiction is the whole reason the gate has a
//! mapping row. `BUSBAR-1.6.0.md:65` USED TO carry the OLD shorthand — "Every plugin has **both**
//! ABIs" — beside the very lane table that cancels it; that sentence has since been rewritten to
//! state #30's rule directly and to stop attributing it to #2. `:354` is the governing text: #30's own
//! closing sentence reads "the 'every plugin has BOTH ABIs' shorthand is SUPERSEDED — the law is
//! 'both LANES exist on the seam; each kind is assigned to one.'" This gate encodes `:354`. Under
//! the shorthand there is nothing to check (every kind is on every lane, so no kind can be on the
//! wrong one), which is exactly the shape of a rule that is green over nothing; under #30 there are
//! two rosters and a kind can be caught on the wrong one. The partition #30 draws is 2 HOT (plane,
//! transport) / 5 COLD (store, secret, auth, hook, export), and it is READ from those two sentences
//! rather than restated here, so when `:65` was corrected the gate followed with no code change.
//!
...
/// WHERE #30 LIVES TODAY. The gate was written against `docs/design/DECISIONS.md`; the decision
/// table moved into the release spec and that file no longer exists, so a gate still pointing at it
/// would have reported "the decision is unreadable" on every run — the honest failure this gate's
/// mapping row is built to give, but a failure about the GATE rather than about the tree. The
/// SYMBOLS survived the move (`HOT/POD lane = …` / `COLD/JSON lane = …` are the same two sentences,
/// verbatim, in row 30 of the decisions table); only the path did not. `plane-abi-neutrality` reads
