...
measure() {
  ...
  local surface_ceilings sc_contract_caps sc_grammar sc_contract_transport sc_core_hooks
  surface_ceilings="$(python3 - "$TOML" <<'PYEOF'
import sys, tomllib
with open(sys.argv[1], "rb") as fh:
    cfg = tomllib.load(fh)
sc = cfg.get("gate", {}).get("surface_ceilings", {})
for key, default in (("contract_caps", 3500), ("grammar", 500), ("contract_transport", 1000), ("core_hooks", 2000)):
    print(sc.get(key, default))
PYEOF
)"
  { read -r sc_contract_caps; read -r sc_grammar; read -r sc_contract_transport; read -r sc_core_hooks; } <<<"$surface_ceilings"
  ...
  for surface_spec in \
    ...
    "core-hooks|busbar-core-hooks|$sc_core_hooks|the hook policy engine's surface"
  do
