...
RULES = [
    ...
    Rule(
        ...
        "an explicit claim that a machine produced the artifact; matched as an ATTRIBUTION SHAPE, so "
        "the vendor names this gateway routes to (Anthropic, OpenAI, Claude, Gemini) never fire, and "
        "a genuine human co-author trailer is untouched",
        [
            Pat(r"co-?authored-by:[^\n]*\b(?:claude|chatgpt|gpt-?\d?|copilot|codex|cursor|gemini|"
                r"llama|anthropic|openai|ai|bot)\b"),
            Pat(r"noreply@anthropic\.com"),
            # The robot-emoji trailer, but only as an ATTRIBUTION: a bare 🤖 is decoration, and the
            # README uses one legitimately to mark an agent-readable resource.
            Pat(r"\bgenerated with\s*\[?\s*(?:claude|chatgpt|copilot|gpt|gemini|ai\b)"),
            Pat(r"\U0001F916[^\n]{0,24}\bgenerated\b"),
            Pat(r"\bAI[- ]generated\b"),
            Pat(r"\bgenerated by (?:an? )?(?:AI|LLM|Claude|GPT|model|assistant)\b"),
            Pat(r"\bwritten by (?:an? )?(?:AI|LLM|assistant|agent)\b"),
        ],
    ),
    Rule(
...
FIXTURES = {
    ...
    "ai-attribution": (
        ...
        # The four vendor-name controls plus the genuine human trailer, in one file: if any rule ever
        # widens into a bare product name, THIS is the fixture that goes loud.
        "// Routes to Anthropic, OpenAI and Gemini upstreams.\n"
        "// `Claude-on-Vertex` is a first-class provider protocol: Claude models served by Vertex.\n"
        "// Claude Code is one of the clients this gateway fronts.\n"
        "Co-authored-by: Jane Doe <jane@example.org>\n"
        "// Generated with schemars from the Rust types; do not hand-edit.\n"
        "// Agent-readable: getbusbar.com/llms.txt  \U0001F916\n",
    ),
    "commit-hash-citation": (
