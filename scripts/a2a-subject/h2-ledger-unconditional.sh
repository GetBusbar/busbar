...
#
# Gating scenario `a2a.battery|h2-ledger-unconditional` -- H2 (ARCHITECTURE.md #2.2 step 6, METER)
# for the A2A plane, and the ONE claim at that step that has nothing to do with money:
#
...
#     rate_card PRESENT  GET /api/v1/admin/usage -> by_model: [ {agent:probe, a2a, requests: 1} ]
#
...
# binary, the same mock agent and the same call, with the card as the ONLY difference:
#
...
WORK_ROOT="${H2_WORK:-${here}/../../target/h2-scratch/a2a-ledger-unconditional.$$}"
...
h2_arm() {
  ...
  h2-oracle:
    ...
    sleep 2
    printf "%s %s\n" "$st" "$(h2_meter_row_field "agent:probe" "a2a" requests)"
  ' _ "$here" "$1"
}
...
if [ "$failures" -eq 0 ]; then
  printf 'PASS\t%s\n' "one served message/send posted its metering row with the rate card absent AND present: the ledger records what happened and the card is a lens applied at read time"
else
  printf 'FAIL\t%s\n' "$detail"
  exit 1
fi
