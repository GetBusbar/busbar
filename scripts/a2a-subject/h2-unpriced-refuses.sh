...
# billing is on; free is an EXPLICIT zero row, never silent." So EITHER the node refuses to boot,
# OR the call is refused -- both are refusals and this leg accepts either. What it does not accept is
# a 200 that quietly charges nothing for the class it hit.
#
# THIS LEG IS A PAIR, AND THE PAIR IS THE POINT. A leg that only asserted "refuse" could be passed
# by a node that refuses everything, and a leg that only asserted "serve" could be passed by a node
# that never bills. So both arms of #42's switch are driven against the same binary, the same mock
# agent and the same call:
#
...
# This one refuses a caller whose class the node cannot put a price on at all. Different question,
# different stage, different evidence: one is arithmetic against a cap, the other is the absence of a
# number. Neither substitutes for the other, and neither is rewritten by the other.
set -uo pipefail
here="$(cd "$(dirname "$0")" && pwd)"

WORK_ROOT="${H2_WORK:-${here}/../../target/h2-scratch/a2a-unpriced-refuses.$$}"
...
case "$arm_on" in
  ...
  *)
    if [ "$on_status" = "200" ]; then
      failures=$((failures+1))
      detail="${detail}arm 2 (billing ON, class 'bytes' unpriced) was SERVED 200 and billed spend_cents=${on_spend} -- the flat fee alone, with the hit class silently priced at zero. #42: a silent 0 is ONLY ever correct when rate_card is ABSENT, and here it is PRESENT; the call owed a refusal (or the node owed a boot refusal, #77(5)); "
      ...
      # unpriced class, and the permissive "anything but 200" let every one of them pass (a2a twin
      # of mcp item 500).
      failures=$((failures+1))
      detail="${detail}arm 2 (billing ON, class 'bytes' unpriced) answered ${on_status}, which is neither a served 200 nor a refusal (want a 4xx call refusal or a boot refusal, #42/#77(5); 000 is no connection, 5xx is a node failure); "
    fi
    ;;
esac

if [ "$failures" -eq 0 ]; then
  printf 'PASS\t%s\n' "billing OFF serves and bills the flat fee alone; billing ON with the declared class 'bytes' unpriced refuses rather than billing it zero"
else
  printf 'FAIL\t%s\n' "$detail"
  exit 1
fi
