...
same helper the MCP leg does, because busbar's token format is one format across both planes."
  ...
  # rig serving both instruments therefore has to hold the credential where a CLIENT holds it: a
  # transparent forwarder that adds `Authorization` only when the request carries none.
  #
