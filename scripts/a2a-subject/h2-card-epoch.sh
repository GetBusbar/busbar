...
# 1.5.5 had no a2a plane, so no recorded cell can exist to diverge from.
#
# THE RULE, stated in the decision's own words: "'Price against the latest rate card' means the
# latest card whose `effective_from` had arrived at the posting's instant -- NOT the latest card ever
# authored ... Publishing a new card never reprices the window before its `effective_from`." The
# resolution key is the posting's own arrival instant (`arrived_ms`), not a version stamped into the
# posting.
#
# THE DIMENSION THIS LEG MOVES IS THE FLAT FEE, and that is a deliberate, decision-backed choice
# rather than a convenience. #44: "A flat/static fee is ONE plane-agnostic pricing dimension on the
# rate card (applied per-request or per-session), identical for every plane, and NEVER rounded." It
# is also the ONLY dimension of this card that an a2a deployment can move at all: the card's other
# dimensions are the four LLM token tiers keyed by `models:` name, and this plane's declared class
# (`bytes`) is neither (see h2-class-price.sh). Never rounded is what makes the arithmetic below
...
#                           Pending; re-running the operator's ceremony is the real path
#   call B                  arrives inside card 1's window
#   GET keys/<kid>/usage
#
# THE THREE HYPOTHESES SEPARATE ARITHMETICALLY, which is the whole reason there are two calls and
# not one. One call could not tell a correct resolution from a resolution that always returns the
# opening entry:
#
...
#
# THE READ IS THE BUDGET CELL (`GET /api/v1/admin/keys/<kid>/usage`), not the metering series, and
# for a stated reason: `derived_bucket_usage` (governance/state.rs:1581) reads a cell that survives
# the live config swap, while the write-behind metering buffer does not -- a leg that spans an apply
# and read `GET /admin/usage` would be measuring the swap as well as the card.
set -uo pipefail
here="$(cd "$(dirname "$0")" && pwd)"
# shellcheck source=h2-lib.sh
source "${here}/h2-lib.sh"

WORK="${H2_WORK:-${here}/../../target/h2-scratch/a2a-card-epoch.$$}"
trap 'h2_stop' EXIT
...
h2_approve_agent || { failures=$((failures+1)); detail="${detail}re-approval after the live apply failed; "; }
...
# errored on one, took the false arm and let the leg PASS on money it never read (a2a twin of mcp
# item 498).
