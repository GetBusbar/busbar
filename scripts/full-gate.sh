...
declare -a SKIP_REASON=(
  ...
  "scripts/construction-gate.sh|its STRICT --check is RED BY DESIGN on HEAD (eight rows over their qa/construction.toml ceilings) while the construction work it measures is in flight; ci.yml runs that form report-only (continue-on-error, verdict printed by the umbrella, not counted). This entry skips the STRICT form only. The gate is NOT unmeasured locally: --selftest runs here (the rule above) and the qa construction-gate segment runs --check-pinned, which reds on any row that is not in [gate].expected_red AND on any pinned row that has started passing. This reason used to say 'three rows', which had been wrong for a while and is exactly the drift the pin now prevents. DELETE this entry when expected_red is empty and the CI job is flipped to blocking."
)
