...
while IFS=$'\t' read -r P_REPO P_DIR _ _ P_SERVICE P_RELGATE P_GATE _; do
  ...
  case "$P_SERVICE" in
    ...
    valkey)
      ...
      DOCKER_CONTAINERS+=("$SUITE_CONTAINER")
      docker run -d --rm --name "$SUITE_CONTAINER" -p 16379:6379 valkey/valkey:8@sha256:495e4fecdc98ee48a20b207726caa5ab6451e0fac3642a9be10d9e70b3068df6 >/dev/null
    ...
    vault)
      ...
      docker run -d --rm --name "$SUITE_CONTAINER" --cap-add=IPC_LOCK \
        -e VAULT_DEV_ROOT_TOKEN_ID=root -p 18200:8200 hashicorp/vault@sha256:5be49781ecf78bfe775c5309c6a4d9f4e9e040b6c885c99eb2b12fb69855e1a2 >/dev/null
