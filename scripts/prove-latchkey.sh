...
# ── THE NO-CONTEXT TRANSPORT: THE TREE IS FETCHED, NOT UPLOADED ──────────────────────────────────
# WHY THIS EXISTS: Latchkey meters an org's uploaded job CONTEXT over a trailing 3-day window (20
# GiB). Every pack-path proof uploads ~43 MB of tree, and under heavy proving that window fills and
# EVERY submission is refused — "a create without context_bytes is not limited by this quota" is the
# refusal's own way out. This transport takes it: `latchkey run --no-context` uploads nothing, and
# the runner obtains the exact tree it would have been handed by CLONING it from origin instead.
#
# IT IS THE SAME TREE, PROVEN BY THE SAME ENGINE. Nothing about the proof changes: the runner builds
# the identical `.latchkey/git` (base + picks + tip, the refs lk_stage_repo pushes), lays down the
# identical working tree (`git archive tip`), and runs the identical lk_onbox_script. The ONLY
# difference is transport — a git fetch of already-pushed objects in place of a 43 MB upload — so the
# assembled, re-pinned tree is byte-for-byte the pack path's (verified locally, --selftest-adjacent).
#
# WHAT TRAVELS is a small CONTROL BUNDLE (the engine, the batch, the runner, the assemble script and
# the pick list) pushed to origin as a git commit under refs/proof/nocontext/<ref>. A git push is
# NOT counted against the context-storage quota, so nothing here re-fills the window.
LK_NO_CONTEXT="${LATCHKEY_NO_CONTEXT:-0}"
LK_GIT_REMOTE="${LATCHKEY_GIT_REMOTE:-$(git -C "$REPO" remote get-url origin 2>/dev/null || echo https://github.com/GetBusbar/busbar.git)}"
LK_BUNDLE_REF=""

# THE FETCH TOKEN, READ NEVER PRINTED. A no-context runner must authenticate to the PRIVATE repo,
# so unlike LATCHKEY_TOKEN (which the CLI reads locally and never ships) this ONE token must reach
# the runner's environment. It is preferred from LATCHKEY_GIT_TOKEN (the owner provisions a scoped,
# read-only deploy token there), falling back to `gh auth token` for a laptop smoke. It is passed
# as `--env LK_GIT_TOKEN=` and read on the runner ONLY by a git credential helper out of the
# environment — never placed in a URL, never on a git command line, never echoed, and no `set -x`
# runs where it is live.
lk_git_token() { # prints the token or nothing (return 1)
  if [ -n "${LATCHKEY_GIT_TOKEN:-}" ]; then printf '%s' "$LATCHKEY_GIT_TOKEN"; return 0; fi
...
# THE ON-RUNNER ASSEMBLE SCRIPT. Emitted from ONE function so --selftest drives the real text. It
# reconstructs the EXACT staging lk_stage_repo/lk_pack_dir build on the laptop — the bare
# `.latchkey/git` with the same refs and the `git archive tip` working tree — but by fetching the
# already-pushed objects from origin rather than unpacking an upload. Then it execs the same runner.
# Fully positional and self-contained (no baked values) so the heredoc stays literal: the token is
# expanded on the runner, never here.
lk_nocontext_assemble() {
  cat <<'ASM'
#!/usr/bin/env bash
# Generated by scripts/prove-latchkey.sh — reassembles the tree on a NO-CONTEXT Latchkey runner.
set -euo pipefail
URL="$1"; TIP="$2"; BASE="$3"; MODE="$4"; BR="$5"; POSTURE="$6"; FAM="$7"; TESTS="$8"; LEGS="${9:-}"
say() { printf '\n== %s  (%s)\n' "$1" "$(date -u +%H:%M:%S)"; }
CH='!f(){ echo username=x-access-token; echo "password=$LK_GIT_TOKEN"; };f'
bare=.latchkey/git

say "reassemble the history from origin (no context was uploaded)"
[ -e .git ] && { echo "prove-latchkey nocontext: RED — cwd already has a .git; refusing to build over it"; exit 2; }
git init -q --bare "$bare"
# receive.shallowUpdate mirrors lk_stage_repo; the fetches below are shallow so the runner clone is small.
git -C "$bare" config receive.shallowUpdate true
git -C "$bare" config gc.auto 0
git -C "$bare" config credential.helper "$CH"

# THE BASE, under the two names the ratchets read (lk_stage_repo writes both).
git -C "$bare" -c protocol.version=2 fetch -q --depth 1 "$URL" "$BASE:refs/proof/base/$BASE" \
  || { echo "prove-latchkey nocontext: RED — base $BASE not fetchable from origin; nothing is proven"; exit 2; }
git -C "$bare" update-ref refs/remotes/origin/integration/oracle-phase0 "$BASE"
git -C "$bare" update-ref refs/heads/integration/oracle-phase0 "$BASE"
...
# THE PICKS, commit AND parent (--depth 2) so the engine's `cherry-pick -x` has the diff base; the
# -x trailer it writes is the provenance the hold-release predicate reads.
if [ -s .latchkey/picks.txt ]; then
  while IFS= read -r h; do
    [ -n "$h" ] || continue
    git -C "$bare" -c protocol.version=2 fetch -q --depth 2 "$URL" "$h:refs/proof/picks/$h" \
      || { echo "prove-latchkey nocontext: RED — the pick $h could not be fetched from origin; nothing is proven without it"; exit 2; }
  done < .latchkey/picks.txt
fi

# THE AUDIT PINS, best-effort, exactly as lk_stage_repo stages them: qa/audit-ledger.json names the
# commit each audit round read and the audit-ledger gate re-derives that tree, so a CLONE that cannot
# resolve a pin reds for a reason that is not the tree's (the MCP-NODE-r7 finding — a clone was
# missing them). They are fetched from origin if the engine pushed them there; their absence costs
# one gate row, never the proof, so this never fails the assemble.
git -C "$bare" -c protocol.version=2 fetch -q --depth 1 "$URL" '+refs/audit-pins/*:refs/audit-pins/*' 2>/dev/null || true

# THE WORKING TREE — exactly lk_pack_dir's `git archive tip` output, laid into cwd.
git -C "$bare" archive --format=tar tip | tar -xf -
echo "   reassembled tip $(printf '%.9s' "$TIP") on base $(printf '%.9s' "$BASE"); $(grep -c . .latchkey/picks.txt 2>/dev/null || echo 0) pick(s), fetched not uploaded"

# HAND OFF TO THE SAME RUNNER the pack path runs — from here every byte is the pack path's.
exec bash .latchkey/.lk-run.sh "$MODE" "$BR" "$POSTURE" "$TIP" "$BASE" "$FAM" "$TESTS" "$LEGS"
ASM
}

# THE ONE-LINE BOOTSTRAP the runner starts with (its whole command). It fetches the control bundle
# (a git commit, not an upload), extracts .latchkey/, drops its bootstrap .git, and execs the
# assemble script. Baked with the remote and bundle ref; the token is expanded on the runner from
# the environment (\$LK_GIT_TOKEN), never here.
lk_nocontext_bootstrap() { # $1..$8 = MODE BR POSTURE TIP BASE FAM TESTS legs
  cat <<BOOT
set -euo pipefail; export GIT_TERMINAL_PROMPT=0
git init -q .
git config credential.helper '!f(){ echo username=x-access-token; echo "password=\$LK_GIT_TOKEN"; };f'
git remote add origin '$LK_GIT_REMOTE'
git -c protocol.version=2 fetch -q --depth 1 origin '$LK_BUNDLE_REF'
git archive FETCH_HEAD | tar -xf -
rm -rf .git
exec bash .latchkey/.lk-assemble.sh '$LK_GIT_REMOTE' '$4' '$5' '$1' '$2' '$3' '$6' '$7' '$8'
BOOT
}

# PUSH THE CONTROL BUNDLE to a proof ref on origin. Uses an isolated index and the repo's object
# store, force-added (`.latchkey` is gitignored in the tree), so the working index and HEAD are
# untouched. Prints the ref it pushed.
lk_nocontext_push_bundle() { # $1 = bundle dir that contains .latchkey/ ; prints the ref
  local bundle="$1" ref="refs/proof/nocontext/$REF" idx tree commit
  idx="$(mktemp "$LAND_TMP/lk-nocontext-idx.XXXXXX")" || return 1
  (
    export GIT_INDEX_FILE="$idx" GIT_DIR="$REPO/.git" GIT_WORK_TREE="$bundle"
    export GIT_AUTHOR_NAME="busbar latchkey prove" GIT_AUTHOR_EMAIL="ci@busbar.invalid"
    export GIT_COMMITTER_NAME="busbar latchkey prove" GIT_COMMITTER_EMAIL="ci@busbar.invalid"
    git read-tree --empty || exit 1
    git add -Af -- .latchkey || exit 1
    tree="$(git write-tree)" || exit 1
    commit="$(git commit-tree "$tree" -m "lk12 no-context control bundle $REF")" || exit 1
    git push -q "$LK_GIT_REMOTE" "$commit:$ref" || exit 1
  ) || { rm -f "$idx"; return 1; }
  rm -f "$idx"
  printf '%s' "$ref"
}

# SUBMIT a no-context job. The token reaches the runner by `--env LK_GIT_TOKEN=` (the one, necessary
# exception to "no --env" — a private clone needs a credential); it is read there only by a git
# credential helper out of the environment and never printed. Prints the job id like lk_submit.
lk_submit_nocontext() { # MODE BR POSTURE TIP BASE FAM TESTS legs ; prints the job id
  local boot tok id
  : >"$LK_SUBMIT_ERRFILE" 2>/dev/null || true
  tok="$(lk_git_token)" || {
    echo "no-context needs a git token (LATCHKEY_GIT_TOKEN, or gh auth token) to fetch the private tree" >>"$LK_SUBMIT_ERRFILE"
    return 1; }
  boot="$(lk_nocontext_bootstrap "$@")"
  id="$("$LK_BIN" run --size "$LK_SIZE" --timeout "$LK_TIMEOUT" --no-context --env "LK_GIT_TOKEN=$tok" --quiet --detach "$boot" 2>>"$LK_SUBMIT_ERRFILE" | tr -d '[:space:]')"
...
if [ "${1:-}" = "--selftest" ]; then
  ...
  # THE LATCHKEY API TOKEN IS AN ENVIRONMENT READ, NEVER AN --env. `latchkey run --env K=V` puts the
  # value in the job's environment where the job's own log can print it; the CLI reads LATCHKEY_TOKEN
  # here. THE ONE, DELIBERATE EXCEPTION is the no-context GIT fetch token: a private clone needs a
  # credential ON the runner, so `--env LK_GIT_TOKEN=` is the only --env this file ships, and it
  # carries LK_GIT_TOKEN and nothing else. What is counted is a `$LK_BIN run` line carrying --env.
  [ "$(grep -cE '"\$LK_BIN" run .*--env' "${BASH_SOURCE[0]}")" = 1 ] \
    && say PASS "exactly one --env is shipped (the no-context git token), never the API token" \
    || say FAIL "unexpected number of --env run lines (expected exactly the no-context git token)"
  run_env_flag="$(grep -E '"\$LK_BIN" run .*--env' "${BASH_SOURCE[0]}" | grep -oE -- '--env "[^"]*"')"
  case "$run_env_flag" in
    '--env "LK_GIT_TOKEN='*) say PASS "  ...and the only --env value is LK_GIT_TOKEN=" ;;
    *) say FAIL "an --env other than LK_GIT_TOKEN is shipped ($run_env_flag)" ;;
  esac
  [ "$(grep -cE '"\$LK_BIN" run .*--env.*LATCHKEY_TOKEN' "${BASH_SOURCE[0]}")" = 0 ] \
    && say PASS "  ...and the LATCHKEY API token is never shipped by --env" \
    || say FAIL "the LATCHKEY API token is shipped to the runner"
  ...
  echo "== prove-latchkey SELF-TEST (the no-context transport uploads nothing and rebuilds the same tree) =="
  # THE REAL EMITTERS, against their own invariants — the assemble script and the bootstrap this
  # file will actually ship, not a description of them.
  asm="$(lk_nocontext_assemble)"
  boot_src="$(LK_GIT_REMOTE='https://github.com/GetBusbar/busbar.git' LK_BUNDLE_REF='refs/proof/nocontext/x' lk_nocontext_bootstrap preprove br dev deadbeef deadbeef '.' '' '')"
  # bash -n on both emitted scripts: a runner that will not parse is exit 70, never a verdict.
  printf '%s\n' "$asm" | bash -n 2>/dev/null \
    && say PASS "the emitted assemble script parses (bash -n)" \
    || say FAIL "the emitted assemble script does not parse"
  printf '%s\n' "$boot_src" | bash -n 2>/dev/null \
    && say PASS "the emitted bootstrap parses (bash -n)" \
    || say FAIL "the emitted bootstrap does not parse"
  # THE JOB CREATES NO CONTEXT. The whole point is the storage-quota bypass: the run line must carry
  # --no-context.
  grep -qE '"\$LK_BIN" run .*--no-context' "${BASH_SOURCE[0]}" \
    && say PASS "the no-context submission passes --no-context (nothing is uploaded)" \
    || say FAIL "the no-context submission does not pass --no-context"
  # THE TREE IS THE SAME TREE. The assemble script builds the SAME refs lk_stage_repo builds and lays
  # the working tree with the SAME `git archive tip`, then execs the SAME runner — so what is proven
  # is byte-identical to the pack path (the transport differs, the tree does not).
  case "$asm" in *'refs/remotes/origin/integration/oracle-phase0'*) say PASS "  ...it writes the integration base ref the ratchets read" ;; *) say FAIL "assemble does not write the base ref" ;; esac
  case "$asm" in *'refs/proof/picks/'*) say PASS "  ...it fetches each pick under refs/proof/picks (the engine cherry-picks -x, so provenance is preserved)" ;; *) say FAIL "assemble does not stage the picks" ;; esac
  case "$asm" in *'archive --format=tar tip | tar -xf -'*) say PASS "  ...it lays the working tree with the same git-archive lk_pack_dir uses" ;; *) say FAIL "assemble does not lay the working tree by git-archive" ;; esac
  case "$asm" in *'exec bash .latchkey/.lk-run.sh'*) say PASS "  ...and hands off to the same on-box runner the pack path runs" ;; *) say FAIL "assemble does not exec the shared runner" ;; esac
  case "$asm" in *'refs/audit-pins/*'*) say PASS "  ...it fetches refs/audit-pins/* so the clone resolves the audit-ledger gate (MCP-NODE-r7)" ;; *) say FAIL "assemble does not fetch the audit pins" ;; esac
  # THE GIT TOKEN NEVER LANDS IN A URL OR A LOG. It is read by a credential helper out of the
  # environment; it must not appear in a clone/fetch URL and must not be echoed except as the
  # helper's `password=` protocol line git itself consumes.
  case "$asm$boot_src" in *'x-access-token:$LK_GIT_TOKEN@'*|*'x-access-token:${LK_GIT_TOKEN}@'*) say FAIL "the git token is embedded in a URL" ;; *) say PASS "the git token is never placed in a URL" ;; esac
  bad="$(printf '%s\n%s\n' "$asm" "$boot_src" | grep -nE '(echo|printf)[^|]*LK_GIT_TOKEN' | grep -vE 'password=' || true)"
  [ -z "$bad" ] \
    && say PASS "  ...and is only ever emitted as the credential helper's password= line, never logged" \
    || say FAIL "the git token is printed outside the credential helper: $bad"
...
# RE-DERIVE THE TRANSPORT SWITCH AFTER THE ENV FILE IS SOURCED. lk_load_token reads the mode-600
# env file (LATCHKEY_GIT_TOKEN lives there), so the operator can adopt the no-context transport by a
# single line in that file; without this re-read the switch would only be honoured from the process
# environment. The values set on the current line default to what was read at source time.
LK_NO_CONTEXT="${LATCHKEY_NO_CONTEXT:-$LK_NO_CONTEXT}"
[ -n "${LATCHKEY_GIT_REMOTE:-}" ] && LK_GIT_REMOTE="$LATCHKEY_GIT_REMOTE"
...
if [ "$LK_NO_CONTEXT" = 1 ]; then
  # ── NO-CONTEXT TRANSPORT: nothing is uploaded; the tree is fetched on the runner ──────────────
  # Only a small control bundle travels, and it travels as a git commit (not context bytes). The
  # runner rebuilds the identical .latchkey/git + working tree from origin and runs the same runner,
  # so the proof is byte-identical to the pack path's — see lk_nocontext_* for the reasoning.
  BUNDLE="$LAND_TMP/latchkey-nocontext-$REF"
  STAGE="$BUNDLE/.latchkey"
  RUNNER=".latchkey/.lk-run.sh"
  cleanup() { rm -rf "$BUNDLE"; }
  trap cleanup EXIT INT TERM
  rm -rf "$BUNDLE"; mkdir -p "$STAGE" || lkdie "could not make the no-context bundle dir $STAGE"
  cp "$LK_ENGINE" "$STAGE/land.run.sh" || lkdie "could not stage the engine ($LK_ENGINE) into the bundle"
  [ -n "$BATCH" ] && cp "$BATCH" "$STAGE/batch.txt"
  printf '%s\n' $PICKS >"$STAGE/picks.txt"
  lk_nocontext_assemble >"$STAGE/.lk-assemble.sh"
  lk_onbox_script "$MODE" "$BRANCH_NAME" "$POSTURE" >"$STAGE/.lk-run.sh"
  chmod +x "$STAGE/.lk-run.sh" "$STAGE/.lk-assemble.sh"
  LK_BUNDLE_REF="$(lk_nocontext_push_bundle "$BUNDLE")" || lkdie "could not push the no-context control bundle to $LK_GIT_REMOTE"
  # THE AUDIT PINS MUST BE FETCHABLE BY THE CLONE. lk_stage_repo pushes them into the ephemeral bare
  # repo; a no-context runner clones from origin instead, so they have to be ON origin for the
  # audit-ledger gate to resolve them (the MCP-NODE-r7 finding). Pushed best-effort under their
  # canonical names; a pin that will not travel costs one gate row, never the proof.
  lk_np=0
  for lk_pin in $(git -C "$REPO" for-each-ref --format='%(refname)' refs/audit-pins/ 2>/dev/null); do
    git -C "$REPO" push -q "$LK_GIT_REMOTE" "+${lk_pin}:${lk_pin}" 2>/dev/null && lk_np=$(( lk_np + 1 ))
  done
  [ "$lk_np" -gt 0 ] && lklog "no-context: $lk_np audit-pin(s) made fetchable on origin (the clone resolves the audit-ledger gate)"
  lklog "no-context: control bundle pushed to $LK_GIT_REMOTE ($LK_BUNDLE_REF); the runner fetches base $(printf '%.9s' "$BASE") + $(printf '%s' "$PICKS" | grep -c . || true) pick(s) — ZERO context uploaded, so the storage quota is bypassed"
else
...
lk_launch() { # $1 = shard name, $2 = the legs it owns ('' = the whole plan); prints nothing
  ...
  if [ "$LK_NO_CONTEXT" = 1 ]; then
    id="$(lk_submit_nocontext "$MODE" "$BRANCH_NAME" "$POSTURE" "$TIP" "$BASE" "$SCOPE_FAM" "$SCOPE_TESTS" "$legs")"
  else
