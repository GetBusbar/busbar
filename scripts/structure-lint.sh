...
# Step 3.7 of 1.6.0 extracts the plane-NEUTRAL trust/audit/plane machinery out of `busbar-core` into
# `busbar-substrate` (a crate `busbar-core` in turn depends on). A symbol that moved there is still
# asserted EXACTLY where its census row expects — the row's scope gains `$SUBSTRATE` so it FOLLOWS
# the symbol to its new home rather than reading zero (which every ban here treats as a false green).
...
# files out of core rather than silently shrinking, and a future non-arm file appearing in one of
# these crates still cannot branch on a foreign axis.
#
# DERIVED, NOT SPELLED, for the same reason the plane roots below are: two constants here is two
# things to remember on the day the third dialect lands, and the failure of forgetting is SILENT —
# the row keeps saying `ok` over a crate it never opened. The rule is mechanical: every
# `crates/busbar-llm/src`, `crates/busbar-mcp/src` and every `crates/busbar-proto-*/src` is a
# protocol crate. ZERO of them is
# a HARD FAILURE and not a skip,
# by the argument this file makes everywhere else — a scan set nobody can locate is a scan set of
# zero files, and zero is the passing answer to every ban below.
# THREE GLOBS, because the naming is not uniform and pretending it is would silently drop a whole
...
# ── WHERE A PLANE LIVES ──────────────────────────────────────────────────────────────────────────
# The SAME argument as the CORE/BIN seam above, one level down. 1.6.0's owner ruling R-E makes MCP
# and A2A PLUGIN CRATES: a crate depending on `busbar-api` alone, which `busbar-core` in turn
# depends on, exactly as it depends on `busbar-auth-admin-tokens` today. So `${CORE}/mcp` and
# `${CORE}/a2a` are a CURRENT FACT, not a permanent one — and every row below that names a plane
# path named it as a literal, which is 14 rows and 22 census/axis scopes that would have gone stale
# on the day the planes moved.
#
# THE ROOT IS DERIVED, NOT DECLARED, and the derivation lives in ONE place: scripts/plane-roots.sh,
# shared with the three scan-floor lints that had the same constant spelled the same wrong way. Its
# rule is "the one directory named after the plane anywhere under crates/", with zero homes and two
# homes both hard failures — a plane nothing can locate is a plane every rule here scans zero files
# of, and zero is the passing answer to every ban in this file.
#
# The failure is folded into this run's `fail` rather than exiting, so a tree with an unresolved
# plane still gets told everything else that is wrong with it in the same pass.
#
# Nothing here weakens a rule. It changes WHERE each rule looks from a constant into a fact about
# the tree, so a rule that is armed today is armed after the move without a human remembering to
# re-point it — and goes LOUD if the tree stops answering the question.
...
# The scan roots for every TREE-WIDE rule (the census and the axis ledger). It is `$CORE + $BIN`
# plus the plane roots, and the plane roots are listed EXPLICITLY rather than assumed to be inside
# `$CORE`: while they are, the extra prefixes are redundant and cost nothing (the scope match takes
# the first prefix that hits and stops); the day a plane becomes its own crate they are the only
# reason the MCP wire-word census rows and the two axis bans keep covering it. Without them a
# census row over `"mcp-protocol-version"` would go from 1 to 0 the moment the constant left
# `$CORE`, which reads as MISSING — loud, but loud AFTER the fact, and the fix would be this line
# anyway.
#
# The extracted protocol crates are in `$TREE` for the same reason and by the same argument: the
# axis rows scope over them (with themselves in the allow list) so the scan FOLLOWS the files out of
# core rather than silently shrinking as step 4 proceeds. They are DERIVED by their own rule rather
# than spelled, and they are a SEPARATE rule from the plane roots because a protocol crate is not a
# plane: `busbar-mcp/src/codec` is MCP-the-PROTOCOL's declaration and `${CORE}/mcp` is the MCP PLANE,
# and both exist at once on this tree — `plane_roots_resolve mcp` finding exactly one home (the
# plane in core, not the codec in `busbar-mcp`) is the proof of that, not a coincidence.
TREE="${CORE}/,${BIN}/,${MCP}/,${A2A}/,${PROTO_ROOTS}"
...
CHOKE_POINTS=(
  ...
  #
  #         LEDGERED EXEMPTION on the `#[no_mangle]` rule for `crates/plane-abi-spike-plugin/src/lib.rs`.
  #         That crate is a P0 SPIKE cdylib whose ENTIRE PURPOSE is to measure the PLT delta of a
  #         hand-declared `extern "C-unwind"` repr(C) POD host-call reached across a real `dlopen`
  #         boundary versus the in-process fn-pointer proxy. It shares NO Rust types with the host —
  #         it re-declares the frozen `#[repr(C)]` `Facts`/`Decision` layout, which IS the ABI under
  #         measurement — so routing its one export through `export_*_plugin!` would wrap the very
  #         seam the benchmark exists to time and destroy the measurement. It is a benchmark artefact,
  #         not a production plugin, and it exports exactly one symbol the bench looks up by name.
  'B-plugin-export|EXPORT-BYPASS|crates/plugin-sdk/src/boundary.rs (via the export_*_plugin! macro)|crates/plugin-sdk/tests/boundary_class.rs::null_out_pointer_never_leaks|define exports via export_*_plugin!, never by hand|#\[(unsafe\()?no_mangle>>hand-rolled #[no_mangle] export>>crates/plugin-sdk/src/lib.rs,crates/plane-abi-spike-plugin/src/lib.rs;#\[(unsafe\()?export_name>>hand-rolled #[export_name] export>>crates/plugin-sdk/src/lib.rs|an unwind or a written-then-failed out-param crossing the C ABI is UB, so no export may skip the wrapper'
  ...
  'I-trust-serve-derivation|TRUST-COMPARISON-BYPASS|'"$SUBSTRATE"'/trust/mod.rs (Approval::serves)|'"$MCP"'/tests/trust_gate_tests.rs::the_routed_gate_answers_exactly_what_the_deleted_inline_decision_answered|ask crate::trust::Approval::serves; never re-derive the answer from the raw registration fields|schema_hash[[:space:]]*\.is_some>>an inline "is there an approved digest?" test;schema_hash[[:space:]]*\.is_none>>an inline "is there no approved digest?" test;\.pin[[:space:]]*\.is_some>>an inline "is this upstream pinned?" test>>'"$SUBSTRATE"'/trust/mod.rs;\.pin[[:space:]]*\.is_none>>an inline "is this upstream unpinned?" test>>'"$SUBSTRATE"'/trust/mod.rs|a second answer to "may this serve?" diverges from the first the moment either is fixed, and the divergence is discovered as an in-flight call that served after the operator revoked it'
...
DECISION_INPUT=(
  ...
  'B10-routing-reads-no-description|DESCRIPTION-ON-ROUTING-PATH|'"$MCP"'/client/dispatch.rs|resolve|description>>a tool description read while deciding a route|route on the bound identity alone — registered server, namespaced tool, approved digest — and never on text the upstream authors|an upstream rewrites its own description at will, so a router that reads one is a router the upstream steers'
  'B10-revalidate-reads-no-description|DESCRIPTION-ON-ROUTING-PATH|'"$MCP"'/client/dispatch.rs|revalidate|description>>a tool description read while re-validating a route|re-validate on the bound identity and the snapshot generation, never on text the upstream authors|the pre-dispatch re-check is the last gate an in-flight call passes; a hostile description reaching it would undo the resolve-time rule one line before the call goes out'
  'B10-visibility-reads-no-description|DESCRIPTION-ON-ROUTING-PATH|'"$MCP"'/client/dispatch.rs|visible_catalogue|description>>a tool description read while deciding what a caller may see|filter the catalogue on the caller grant and the approval, never on text the upstream authors|what a caller is shown is an authorisation answer, and an upstream that could influence it would be choosing its own audience'
)
...
# The two plane roots, RESOLVED (see `plane_home` at the top of this file) rather than spelled.
# `plane_dup_table`/`plane_dup_modules` skip a root that is not a directory — which is right for the
# self-test's fixture corpora and WRONG as a way to lose a plane, so the resolution above is what
# guarantees both roots exist. If one ever does not, `plane_home` has already failed the run and
# every ledger row below reports STALE-LEDGER as well: the ledger cannot be quietly emptied by
# moving a plane out from under it.
PLANE_ROOTS="mcp=${MCP} a2a=${A2A}"
...
observed_pin|DISTINCT|-|mcp/connect.rs observed_pin derives the TransportPin actually presented on the wire: for a cert_spki or mtls declaration it substitutes the peer certificate SPKI read off the accepted handshake, and for any other mechanism the declared pin unchanged. a2a/pin.rs observed_pin extracts the CardPin carried by an agent-card Sighting, returning it only when the card was Seen. Different input (a ServerEntry plus an observed peer SPKI vs a Sighting of a CardPin), different output type (TransportPin vs CardPin), different subject (transport certificate identity vs a signed agent card). Same verb - the pin we actually observed - over unrelated subjects, so this is a signed DISTINCT claim and not a rename.
...
contains|DISTINCT|-|mcp/admin_view.rs contains asks runtime(app).servers.servers whether an MCP SERVER name is registered on this snapshot; a2a/admin_view.rs contains asks agent_cfg(app).agents whether an AGENT name is. Both are the boolean the admin write path reaches through its plane's registry_contains seam so core names no plane runtime type, and each reads its OWN registry off the snapshot — different container, disjoint key space, no shared value. One membership question asked of two unrelated name sets.
get|DISTINCT|-|mcp/admin_view.rs get projects one McpServerDefCfg (the pin mechanism as the module column, the sorted tools/prompts/resources capability NAMES) onto NamedDefView; a2a/admin_view.rs get projects one AgentDefCfg (pin mechanism, fingerprint-pinned bool, reverify TTL) onto the same view. The read half of GET .../tools/{name} versus GET .../agents/{name}: different config type, a different subset of the view's columns populated, no shared field and no shared caller.
list|DISTINCT|-|The vector twins of get and distinct for the same reason: mcp/admin_view.rs list maps runtime(app).servers.servers through mcp_server_view, a2a/admin_view.rs list maps agent_cfg(app).agents through agent_def_view. GET .../tools versus GET .../agents — the same iterate-and-project shape run over two unrelated registries and two unrelated config types, sharing only the verb.
openapi_schemas|DISTINCT|-|mcp/admin_view.rs openapi_schemas registers McpTrustView and McpHealthView and attaches their schema references onto the tools connect, changes and health paths; a2a/admin_view.rs openapi_schemas registers A2aTrustView plus the ApproveReq REQUEST body and attaches them onto the agents connect and approve paths. Each is its plane's half of PlaneDecl::openapi_schemas writing into the SHARED schemars generators — different response types, a request body on one plane and none on the other, different paths and different verbs.
reresolve_gates|DISTINCT|-|mcp/admin_view.rs reresolve_gates rebuilds next.mcp_server_gates from each MCP server's hooks via next.resolve_container_gates; a2a/admin_view.rs reresolve_gates rebuilds next.a2a_agent_gates from each agent's hooks via crate::hooks::resolve_container_gates. Each is its plane's half of the config-swap gate rebuild, moved out of admin::v1::service::reresolve_plane_gates so core names no plane registry type — different gate field written, different registry read, different resolver entry point.
metadata_route|DISTINCT|-|Both serve GET /.well-known/oauth-protected-resource and hand their Metadata to the ONE shared renderer crate::ingress::protocol::metadata, so the RFC 9728 rendering is NOT duplicated — what is left in each is the plane's own wrapper. mcp/envelope.rs specialises to McpWords, reads the resource off the BOUND host seam (resource_of(&ctx.host)) and builds the doc inline (because the core document(&App) method is still &App-typed); a2a/receive.rs specialises to A2aWords, downcasts the engine handle to AppHandle and calls A2aWords::document(&app). Same well-known path, two Words impls, two app-access seams, one shared renderer beneath both.
admin_view.rs|DISTINCT|-|Each plane's admin_view.rs is that plane's own read-side adapter over the SHARED admin::v1 CRUD, reached through PlaneDecl::named_def_list/named_def_get so core admin names no plane config type. mcp/admin_view.rs projects MCP servers (McpServerDefCfg -> NamedDefView) and additionally owns the MCP trust surface (McpTrustView/McpHealthView and the connect/changes/health verbs); a2a/admin_view.rs projects agents (AgentDefCfg -> NamedDefView) with the pin/fingerprint/reverify columns. Same filename because both say the same true and un-actionable thing — this is this plane's admin projection — over disjoint config types, disjoint view columns and disjoint verb sets.
"
...
DECLARATION_CENSUS=(
  ...
  'wire-word-meta-protocol-version|WIRE-WORD-RESPELT|"io\.modelcontextprotocol/protocolVersion"|1|'"$TREE"'|the `_meta` protocol-version key must have exactly one spelling in the tree: the ingress REQUIRES it and the client EMITS it, and two copies disagree in the direction where each side is internally consistent with itself'
  'wire-word-meta-client-capabilities|WIRE-WORD-RESPELT|"io\.modelcontextprotocol/clientCapabilities"|1|'"$TREE"'|the `_meta` client-capabilities key is REQUIRED on the way in and written on the way out; a second copy is a request busbar would send and then refuse'
  'wire-word-header-protocol-version|WIRE-WORD-RESPELT|"mcp-protocol-version"|1|'"$TREE"'|the protocol-version header name is read by the ingress and written by the client from one constant; a second literal is the drift the deleted symmetry test existed to catch'
  'wire-word-header-method|WIRE-WORD-RESPELT|"mcp-method"|1|'"$TREE"'|the method-mirror header name is required inbound and emitted outbound from one constant'
  'wire-word-header-name|WIRE-WORD-RESPELT|"mcp-name"|1|'"$TREE"'|the target-name header is required inbound on the three addressed methods and emitted outbound from one constant'
  ...
  'trust-serve-decision|TRUST-DECISION-RESPELT|fn[[:space:]]+serves[[:space:]]*\(|1|'"$TREE"','"$SUBSTRATE"'/|there is exactly ONE may-this-serve comparison in busbar; a second is two answers to one question and they diverge the first time either is fixed, and zero means the dispatch gate was routed somewhere this row cannot see'
  ...
  'refusal-word-identity-not-live|REFUSAL-WORD-RESPELT|"identity_not_live"|1|'"$TREE"','"$SUBSTRATE"'/|the ordered validator'"'"'s identity refusal has one spelling, defined in crate::audit::vocab and re-exported; a second definition is two homes for one vocabulary, which is what this unit'"'"'s own merge conflict was'
  'refusal-word-not-serving|REFUSAL-WORD-RESPELT|"not_serving"|1|'"$TREE"','"$SUBSTRATE"'/|the registration-level refusal has one spelling; a plane may render it more finely under its own words, but it may not respell this one'
  'refusal-word-artifact-drifted|REFUSAL-WORD-RESPELT|"artifact_drifted"|1|'"$TREE"','"$SUBSTRATE"'/|the rug-pull refusal has one spelling: it is the one word that indicts the UPSTREAM rather than the config or the grant, and a second copy is two answers to who is at fault'
  'refusal-word-generation-moved|REFUSAL-WORD-RESPELT|"generation_moved"|1|'"$TREE"','"$SUBSTRATE"'/|the lifecycle-race refusal has one spelling across both planes; two copies is the in-flight-outliving-an-approval story told two ways'
  ...
  'refusal-word-no-upstream-left|REFUSAL-WORD-RESPELT|"no_upstream_left"|1|'"$TREE"','"$SUBSTRATE"'/|the nowhere-left-to-send refusal has one spelling across every plane that fails over; a second copy is one outage recorded under two words'
  'refusal-word-not-interchangeable|REFUSAL-WORD-RESPELT|"not_interchangeable"|1|'"$TREE"','"$SUBSTRATE"'/|the pins-disagree refusal has one spelling: it is the one word that indicts the CONFIGURATION rather than the caller or the upstream, and a second copy is two answers to who is at fault'
  'refusal-word-not-repeatable|REFUSAL-WORD-RESPELT|"not_repeatable"|1|'"$TREE"','"$SUBSTRATE"'/|the safety rule'"'"'s refusal has one spelling; it is the one outcome an operator may deliberately want to change, and a second copy is a change they would make in one place and not the other'

  'the-one-ordered-request-validator|VALIDATOR-RESPELT|fn[[:space:]]+validate_request[^a-zA-Z0-9_]|1|'"$TREE"','"$SUBSTRATE"'/|there is exactly ONE ordered request validator in busbar and every protocol reaches it; a second is a protocol that has acquired its own order, and zero means the order was inlined back into a plane where nothing owns it'
  ...
  'the-one-generation-source|GENERATION-SOURCE-FORKED|fn[[:space:]]+next_generation[^a-zA-Z0-9_]|1|'"$TREE"','"$SUBSTRATE"'/|every versioned snapshot in the process takes its generation from one monotonic source; a second source is two numbering schemes that compare equal by accident, and zero means nothing is versioned'
  ...
  'one-parse-time-plane-boundary|PLANE-BOUNDARY-RULE-RESPELT|fn[[:space:]]+refuse_cross_plane_reference[[:space:]]*\(|1|'"$TREE"','"$SUBSTRATE"'/|there is exactly ONE parse-time answer to "does this hook reference reach onto another plane"; a second is two answers that drift the first time either is fixed, and zero means the rule left the tree and every dotted reference is now silently accepted'
  'one-section-attach-validator|PLANE-BOUNDARY-RULE-RESPELT|fn[[:space:]]+validate_section_hooks[[:space:]]*\(|1|'"$TREE"'|the SECTION-level attach list must be judged by the same rule one entry is; a second copy is the place a looser rule grows, and it is the list an operator uses to attach a control to everything'
  ...
  'protocol-name-resolution|PROTOCOL-LOOKUP-RESPELT|fn[[:space:]]+decl_for[[:space:]]*\(|1|'"$TREE"'|there is exactly ONE by-name protocol resolution in busbar, and a second one is a second answer to which protocols exist'
  ...
  'protocol-builtin-table|PROTOCOL-TABLE-RESPELT|static[[:space:]]+BUILTIN_DECLS|1|'"$TREE"'|the set of built-in protocols is declared in exactly one place, as data; a second table or none at all is the match coming back'
)
