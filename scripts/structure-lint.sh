...
runtime|DISTINCT|-|mcp/mod.rs runtime returns &McpRuntime from the ALWAYS-PRESENT MCP_RUNTIME_SLOT and .expects both the lookup and the downcast, because the MCP runtime exists on every generation the plane is compiled into; a2a/mod.rs runtime returns Option<&A2aPlane> from PLANE_DECL.key and is None exactly when agents: is unconfigured this generation. Both downcast a type-erased plane_slots entry inside their own plane so core stays free of the concrete type, but they name different runtime types, different slot keys, and OPPOSITE presence contracts (infallible versus config-conditional).
