...
#
# Gating scenario `mcp.rig|h2-exit-terminal` -- H2 (ARCHITECTURE.md #2.2 step "exit") for the MCP
# plane. Proves the exit path settles EXACTLY ONCE per unit and never double-posts: two served
# `tools/call` requests give exactly two usage-request deltas and exactly two new admin audit rows
...
WORK="${H2_WORK:-${here}/../../target/h2-scratch/exit-terminal.$$}"
trap 'h2_stop' EXIT
...
audit_rows="$(h2_audit_rows_since "$seq_before" "mcp_tool.call" "mcp_tool:probe_ping")"
[ "$audit_rows" -eq 2 ] || { failures=$((failures+1)); detail="${detail}new mcp_tool.call audit rows=${audit_rows}(want exactly 2, one settle per unit); "; }

if [ "$failures" -eq 0 ]; then
  h2_verdict PASS "two served tools/call units settle to exactly two usage deltas and exactly two audit rows -- one terminal per unit, never doubled"
else
