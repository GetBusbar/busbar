...
# site_token_state -> present | fork-pr | missing   (the Q39 fork-PR ruling, 2026-09-24)
# A pull_request from a FORK is given no secrets by GitHub, so an empty token there is expected and
# says nothing about the site: the caller records the site rows as a VISIBLE `not-run` (a SKIP with
# a ::warning::), never a fail and never a pass. Every other empty token -- push, release, dispatch,
# workflow_call, schedule, a same-repo PR -- is `missing` and FAILS. The fork bit reaches the shell
# as PR_HEAD_FORK (job env from github.event.pull_request.head.repo.fork), never spliced into script
# text; GITHUB_EVENT_NAME is the runner's own. Only the literal pair (pull_request, true) is fork-pr.
site_token_state() {
  if [ -n "${SITE_VERIFY_TOKEN:-}" ]; then
    echo present
  elif [ "${GITHUB_EVENT_NAME:-}" = pull_request ] && [ "${PR_HEAD_FORK:-}" = true ]; then
    echo fork-pr
  else
    echo missing
  fi
