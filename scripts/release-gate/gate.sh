...
selftest() {
  ...
  # CASE 7 (the container boot rows) is not here: `start_container` and `is_running` are defined by
  # the docker-checks change, which has not landed on this branch. The case comes back with them.
...
STUB
  ...
  # Six columns, with the VERSION column carrying the release under test: a row about another
  # release is refused before the short-list floor is ever reached, so a five-column row here would
  # red this case for the wrong reason.
  {
    printf 'release:exists\tPASS\tok\t\t9.9.9\t-\n'
    printf 'meta:openapi\tPASS\tok\t\t9.9.9\t-\n'
    printf 'docker:label\tPASS\tok\t\t9.9.9\t-\n'
