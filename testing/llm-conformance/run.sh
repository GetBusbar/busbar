...
# One file, two sections: the FAIL -> named-gap list (`gaps`) and the SKIP ceiling
# (`expected`/`accepted`). Both loaders below read it, so both flag spellings point here.
...
while [ $# -gt 0 ]; do
  case "$1" in
    ...
    --named-gaps|--gaps) GAPS="$2"; shift 2 ;;
