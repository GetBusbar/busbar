...
# (h2-lib.sh excluded): six per plane, one per Teller step this plane gates — MCP
# authenticate/verify/admit/meter/audit/exit, A2A verify/admit/route/meter/audit/exit. The fold-driven
# legs carry 1, the honest minimum — a suite that enumerated NOTHING has not run. RAISE these
# deliberately as scenarios are added; a drop is the finding, not the fix.
run_leg run_mcp         "mcp.rig|"        1 "MCP official-subject"
run_leg run_h2_mcp      "mcp.rig|h2-"     6 "MCP H2 gating scenarios"
run_leg run_a2a_battery "a2a.battery|"    1 "A2A independent battery"
run_leg run_h2_a2a      "a2a.battery|h2-" 6 "A2A H2 gating scenarios"
