...
if [ "$SELFTEST" = 1 ]; then
  ...
  run_selftest() {
    ...
    # RED 3: a DECLARED voice leg that reports no row must be named as a FAIL. Drives the REAL
    ...
    # RED 4: the SHIPPED baseline must carry a row for every SHIPPED voice leg. A leg absent from the
