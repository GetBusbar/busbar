...
[ -s "$tarball" ] || fail "fetch-plugin reported success but $PLUGIN's tarball is empty or missing"
...
"$ORACLE_BIN" capture-exec "$rc" "$W/stdout" "$W/stderr" --strip-path "$W" --strip-path "$BIN" \
  ...
  || fail "the capture pipeline failed (the oracle capture-exec or the triple-scrubber exited non-zero)"
