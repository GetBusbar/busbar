...
# shellcheck disable=SC2034  # LEG_KIND/LEG_STATUS/LEG_SLICES are read by streaming-conformance.sh on source
#
...
#
# Before this leg, NOTHING in `busbar-streaming` ever called `JournalHost::audit_record` — a session could
...
# (`StreamingRuntime::audit_session`, a no-op on the pre-host/dev-default runtime with nothing to journal
...
#
#   * a clean session open lands EXACTLY ONE row, shaped `("streaming.session.open", "voice:<call-id>",
