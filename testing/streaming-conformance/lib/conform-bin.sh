...
#
# SHARED HELPER for the streaming-conformance legs — NOT a leg (it lives outside legs/, so the runner's
# `legs/*.sh` glob never discovers it). Sourced by each `legs/<name>.sh` to locate the REAL Rust
# conformance harness (`busbar-streaming`'s `streaming-conform` bin) the legs shell out to. The legs reuse the
# crate's production codecs + runtime through this bin; they never reimplement a codec in shell.

# Resolved from THIS file's location (testing/streaming-conformance/lib/): the battery dir, repo root,
# fixtures, and the cross-dialect map. Computed once at source time.
VC_LIB_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
VC_DIR="$(cd "$VC_LIB_DIR/.." && pwd)"          # testing/streaming-conformance
VC_ROOT="$(cd "$VC_DIR/../.." && pwd)"          # repo root
VC_FIXTURES="$VC_DIR/fixtures"
VC_MAP="$VC_ROOT/docs/design/voice-cross-dialect-map.json"

# The trees the harness IS. A `streaming-conform` older than any file under these is a binary that would
...
_vc_harness_trees() {
  printf '%s\n' \
    "$VC_ROOT/crates/busbar-streaming/src" \
    "$VC_ROOT/crates/busbar-streaming-codec/src" \
    "$VC_DIR/legs" \
    "$VC_DIR/lib"
}
...
# Echo the path to the built `streaming-conform` binary, building it once if necessary.
...
voice_conform_bin() {
  if [ -n "${VOICE_CONFORM_BIN:-}" ]; then
    if [ ! -x "$VOICE_CONFORM_BIN" ]; then
      printf 'streaming-conform: VOICE_CONFORM_BIN=%s is not an executable file\n' \
        "$VOICE_CONFORM_BIN" >&2
      return 1
    fi
    local newer
    if newer="$(_vc_newer_than "$VOICE_CONFORM_BIN")"; then
      printf 'streaming-conform: VOICE_CONFORM_BIN=%s is older than %s — it would report conformance about source it was not built from\n' \
  ...
  local bin="${CARGO_TARGET_DIR:-$VC_ROOT/target}/debug/streaming-conform"
  cargo build -q --manifest-path "$VC_ROOT/Cargo.toml" \
    -p busbar-streaming --features runtime,test-support --bin streaming-conform >&2 || return 1
  [ -x "$bin" ] || return 1
  printf '%s' "$bin"
}
