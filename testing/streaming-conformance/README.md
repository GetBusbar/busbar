...
is **exercised by `streaming-conformance.sh --selftest`** rather than asserted by a
...
Nothing in `streaming-conformance.sh`, in the verdict emitter, or in
`.github/workflows/streaming-conformance.yml` changes. The runner immediately holds
...
- `testing/streaming-conformance/fixtures/{openai,gemini}/` — captured transcripts
...
bash testing/streaming-conformance/streaming-conformance.sh --selftest

# run everything and emit the honest verdict (the default)
bash testing/streaming-conformance/streaming-conformance.sh --verdict

# one leg (what the workflow's per-leg jobs run)
bash testing/streaming-conformance/streaming-conformance.sh --leg spec-per-dialect --slice openai
bash testing/streaming-conformance/streaming-conformance.sh --leg governance

# what is declared
bash testing/streaming-conformance/streaming-conformance.sh --list
```

## Layout

```
streaming-conformance.sh     the runner: --selftest | --verdict | --leg | --list
...
The CI workflow is `.github/workflows/streaming-conformance.yml`: `gate-selftest`
