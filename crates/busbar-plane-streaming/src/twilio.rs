...
//!
//! `busbar-streaming` has no dialect codec for this wire (it is not one of its two duplex dialects), and
//! the one Twilio-shaped module that exists in that crate's source tree
//! (`busbar_streaming_codec::topology::twilio`) is gated behind busbar-streaming's `runtime` cargo feature, which
...
//!
//! The events this module models onto the shared [`busbar_streaming_codec::ir`] vocabulary: a `media` event
//! becomes an [`busbar_streaming_codec::ir::media::IrAudioFrame`] (direction `Up`, format
//! [`busbar_streaming_codec::ir::media::AudioFormat::G711Ulaw`]) carrying the base64-decoded µ-law bytes
...
//
// Written independently for this crate; `busbar_streaming_codec::topology::twilio` (runtime-gated, not in this
