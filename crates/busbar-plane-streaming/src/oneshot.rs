...
//!
//! `busbar-streaming`'s own IR is duplex/session-only: per its crate-root documentation, Plane 4 is "the
//! duplex / live-voice plane," and nothing in `busbar_streaming_codec::ir` models a request with no session at
