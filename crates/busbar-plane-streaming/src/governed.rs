...
//! tool in-process through `busbar_streaming::runtime::ToolExecutor`, and authors the `function_call_output`
...
//! is that telling, dependency-inverted the same way `busbar_streaming::runtime::ToolExecutor` is, because a
