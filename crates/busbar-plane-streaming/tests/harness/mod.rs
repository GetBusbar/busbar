...
/// A WebSocket transport stack that publishes a request target as a fact — the convention every
/// plane harness uses for its own `path` fact.
#[derive(Debug)]
pub struct WsStack {
    path: String,
}
