...
//!
//! The voice plugin held two things behind one name: these codecs — the plane-4 duplex/session
...
//! `busbar-plane-streams` adapts the codecs and must not link the runtime: a plane is a PURE kind
...
//! and `bytes`. The runtime half depends on this crate and re-exports every module that moved
//! under its old path, so the `ir::…` and `topology::twilio::…` paths over there resolve exactly
...
/// crate declares a feature at all. The runtime half's `topology` is gated on `runtime` (OFF by default),
...
/// with the module: that crate's `runtime` forwards here, and the `#[cfg]` resolves to exactly
