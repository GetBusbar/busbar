...
//! LAYER 3 — MEDIA / AUDIO-FRAME (VERBATIM by default — an identity IR). Design `plane4-duplex-session.md` §2.4.
//!
//! Audio frames (`input_audio_buffer.append` up; `response.output_audio.delta` down) are byte-relayed
//! VERBATIM by default. Per `plane4-duplex-session.md` §2.1 this is still an IR — the tap point for meter and audit, and the seam
...
/// THE NEGOTIATED PCM WIRE FORMAT (`plane4-duplex-session.md` §2.3 audio-format field). Two dialect-normalized shapes:
...
/// barge-in truncate math (`plane4-duplex-session.md` §2.3) — because raw PCM carries no timestamps.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
...
/// THE PURE BARGE-IN TRUNCATE HELPER (`plane4-duplex-session.md` §2.3). Given the total bytes of downlink audio the plane has
...
/// THE NEUTRAL AUDIO-FRAME IR (`plane4-duplex-session.md` §2.4). `media` is OPAQUE — the identity transform by default; the IR
