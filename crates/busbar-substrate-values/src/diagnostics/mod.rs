...
// THE NEUTRAL VOCABULARY, RE-EXPORTED FROM ITS NEW HOME. `Class`, `Severity`, `Diagnostic` and its
// `Banner` — the plane-agnostic diagnostic vocabulary types — moved DOWN into `busbar-contract` (the
// leaf every plane already names) so a PURE plane adapter can name them without reaching into this
// crate (the #39 -host fold prerequisite). This is a type-home relocation only: the items are
// byte-identical, and this glob re-export makes every historical path resolve unchanged — an outside
// caller's `busbar_substrate::diagnostics::Class` and this crate's own `Diagnostic { … }` const
// literals below both bind here. `busbar-contract` names only `busbar-grammar`, and these types pull
// nothing but `std::fmt`, so the move adds no dependency to the contract and names no plane instance.
// The neutral built-in catalog ([`REGISTRY`]), the plane-install seam ([`install_diagnostics`]), the
// runtime union ([`all`]/[`by_code`]) and the cross-crate emit macros stay HERE.
pub use busbar_contract::diagnostics::*;
