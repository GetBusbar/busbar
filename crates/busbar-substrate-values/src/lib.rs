...
// over `busbar-api`/`busbar-plugin` leaves, and the ONE RELAY that reads the `groups:` tree into
// the neutral limit vocabulary `busbar-contract` owns. It opens nothing, which is why it is on
