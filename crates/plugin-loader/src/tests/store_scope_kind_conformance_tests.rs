...
fn register_like_boot() {
    busbar_api::register_scope_kind("mcp_server");
    busbar_api::register_scope_kind("mcp_tool");
}
    ...
    if candidate.is_none() && std::env::var_os("CI").is_some() {
        panic!(
            "the store example plugin cdylib is not built under CI: `cargo test --workspace` must \
             build busbar_store_example_plugin. Refusing to silently skip DECISIONS #11's \
             compiled-in vs dropped-in scope-kind equivalence."
        );
    }
    candidate
}
...
fn linked_and_dropped_in_store_round_trip_a_plane_scope_grant_identically() {
    ...
    let linked = busbar_store_example_plugin::open("{}").expect("compiled-in ctor");
    ...
    assert!(back.scope_allowed("mcp_tool", "filesystem_read_file"));
    assert!(back.scope_allowed("mcp_server", "filesystem"));
