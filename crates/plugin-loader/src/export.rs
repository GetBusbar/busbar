...
impl DynExport {
    ...
    /// Hand one batch for `stream` across the ABI and RELAY WHAT THE SINK SAID ABOUT IT. A transport
    ...
    /// otherwise. It now answers with the sink's own word, so a composed sink and a loaded one are
    /// the same object on this question too.
    pub fn deliver(
        &self,
        stream: ExportStream,
