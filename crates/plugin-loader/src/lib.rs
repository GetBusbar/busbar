...
mod tests {
    ...
    // against a REAL Vault dev-mode server — mirroring the discipline `load_and_exercise_auth_oidc_plugin`
    // brought to kind:auth (a real JWKS fetch, not a stub) and store-postgres/store-redis bring to
    // kind:store (a real database, not a mock).

    /// Locate the secret-vault-plugin cdylib, mirroring `secret_example_plugin_path`/
    /// `auth_oidc_plugin_path` exactly: same target-dir derivation, same CI-hard-fail-instead-of-
    /// silent-skip policy so this, the only over-the-ABI coverage of the real Vault backend, cannot
    /// quietly vanish.
