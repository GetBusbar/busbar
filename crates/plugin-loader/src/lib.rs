...
mod tests {
    ...
    /// Mirrors `load_and_exercise_secret_vault_plugin`'s fail-closed discipline for `kind: store`.
    #[test]
    ...
    // ── kind:secret real-Vault ABI-crossing coverage (busbar-secret-vault-plugin) ──────────────────
    //
    // `load_and_exercise_secret_example_plugin` above proves the DynSecret dlopen seam works at all
    // (a hermetic in-memory fixture). This proves the REAL Vault-backed plugin over that SAME seam,
    // against a REAL Vault dev-mode server — mirroring the discipline store-postgres/store-redis
    // bring to kind:store (a real database, not a mock). auth-oidc's own equivalent real-ABI proof
    // (a real JWKS fetch, not a stub) now lives in its own repo (GetBusbar/auth-oidc), which brings
    // 100% of its own logic + adapter rather than reaching into this crate's tests.

    /// Locate the secret-vault-plugin cdylib, mirroring `secret_example_plugin_path` above exactly:
    /// same target-dir derivation, same CI-hard-fail-instead-of-silent-skip policy so this, the only
    /// over-the-ABI coverage of the real Vault backend, cannot quietly vanish.
    fn secret_vault_plugin_path() -> Option<std::path::PathBuf> {
        let candidate = (|| {
            let exe = std::env::current_exe().ok()?;
            let profile_dir = exe.parent()?.parent()?;
        ...
        if candidate.is_none() && std::env::var_os("CI").is_some() {
            panic!(
                "the secret-vault plugin cdylib is not built under CI: `cargo test --workspace` \
                 must build busbar_secret_vault_plugin. Refusing to silently skip the only \
    ...
    /// endpoint, then `resolve()` it back through the `DynSecret` wrapper over the real C ABI — both
    /// field-addressing forms, and the fail-closed 404/403 paths. Gated on `BUSBAR_TEST_VAULT_ADDR` +
    /// `BUSBAR_TEST_VAULT_TOKEN`, same hard-fail-under-CI policy as every other live-backend test in
    /// this crate/workspace.
    ///
    /// Start a real Vault first:
    /// ```sh
    /// docker run --rm -p 8200:8200 --cap-add=IPC_LOCK -e VAULT_DEV_ROOT_TOKEN_ID=root hashicorp/vault
    /// BUSBAR_TEST_VAULT_ADDR=http://127.0.0.1:8200 BUSBAR_TEST_VAULT_TOKEN=root cargo test -p busbar-plugin-loader
    /// ```
    #[test]
    fn load_and_exercise_secret_vault_plugin() {
        let Some(path) = secret_vault_plugin_path() else {
            eprintln!("skip: secret-vault plugin cdylib not built (run under --workspace)");
            return;
        };
