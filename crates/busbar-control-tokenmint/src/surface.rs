...
/// Why the plane could not be built. Distinct from [`crate::config::AsCfgError`] because these are
...
impl TokenIssuer {
    ...
    pub fn build(
        ...
        Self::assemble(identity, key_material, protected_resources, fetch)
...
impl From<AsBuildError> for PluginError {
    fn from(e: AsBuildError) -> Self {
        let (class, code) = match &e {
            AsBuildError::SigningKey(_) => (ErrorClass::Malformed, catalog::BUILD_SIGNING_KEY),
            AsBuildError::SigningKeyMaterial(_) => {
                (ErrorClass::Malformed, catalog::BUILD_SIGNING_KEY_MATERIAL)
            }
            AsBuildError::Service(_) => (ErrorClass::Internal, catalog::BUILD_SERVICE),
        };
        let words = e.to_string();
        catalog::refuse(class, code, words.clone()).with_param("error", ParamValue::Str(words))
    }
}
