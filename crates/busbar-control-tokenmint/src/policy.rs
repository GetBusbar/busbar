...
///
/// `pub(crate)` because [`crate::cimd`] applies the SAME refusal to a metadata document's
...
pub(crate) fn name_impersonates_the_deployment(name: &str) -> bool {
...
pub(crate) fn default_grant_scopes(
    ...
    // `expect` is sound and is not a shortcut: `Identity::from_section` already refused every entry
    ...
    oauth_as::scope::ScopeSet::from_tokens(identity.default_grant())
        .expect("Identity::from_section refuses a default_grant entry that is not a scope token")
}
