...
fn make_root_cfg(
    ...
    config::RootCfg {
        tool_defs: busbar_substrate::plane::config::ToolsSection::default().0,
        agent_defs: busbar_substrate::plane::config::AgentsSection::default().0,
...
fn test_validate_rejects_bad_protocol() {
    ...
    for proto in busbar_substrate_values::proto::known_protocols() {
        assert!(
            msg.contains(proto),
...
fn the_empty_set_the_validator_refuses_is_the_one_an_empty_registry_produces() {
    let empty_registry = busbar_substrate_values::proto::Registry::new(
        busbar_substrate_values::proto::merged_boot_decls(&[], &[]),
    );
