...
//! the downlink-facing [`StreamingSession`] (tool results / barge-in cancel) and the [`UplinkForwarder`]
...
pub struct TelephonyProxy<C> {
    ...
    downlink_plane: Arc<StreamingSession<C>>,
    ...
    C: DuplexReader + DuplexWriter + Send + Sync + 'static,
{
    ...
    let downlink_plane = Arc::new(StreamingSession::new(Arc::clone(&core)));
