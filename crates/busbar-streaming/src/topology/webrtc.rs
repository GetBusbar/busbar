...
use crate::runtime::session::{SessionCore, StreamingSession};
...
/// token to hand the browser, the [`StreamingSession`] plane to serve over the sideband WSS, its shared
...
pub struct Attached<C> {
    ...
    /// socket of its own.
    pub session: StreamingSession<C>,
    ...
    M: TokenMinter,
{
    ...
    let session = StreamingSession::new(Arc::clone(&core));
    Ok(Attached {
        token,
        session,
        core,
