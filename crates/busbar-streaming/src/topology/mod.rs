...
//! THE TWO TOPOLOGIES the streaming runtime exposes (design `plane4-duplex-session.md` §5-6), behind the `runtime` feature.
//!
...
//!
//! Both are assembled from a [`crate::runtime::StreamingRuntime`] via [`begin_session`], which opens the D2
//! metering lease (fail-closed on a refused budget) and the durable [`SessionHandle`] before a frame
//! flows.

pub mod minter_https;
pub mod telephony;
/// MOVED to `busbar-streaming-codec`: the Twilio Media Streams messages are a GRAMMAR, not a dial — a
...
/// `busbar_streaming::topology::twilio::…` resolve unchanged. The Twilio DIAL (the outbound call the
/// webhook answers) is `telephony`, and it stayed.
pub use busbar_streaming_codec::topology::twilio;
pub mod webrtc;
...
/// THE STREAMING PLANE'S BREAKER-CELL KEY for one provider dial target. The `stream:` prefix is the
/// plane-qualified keyspace rule (the `tool:` / `agent:` precedent — a streaming cell can never collide
/// with an MCP tool cell, an A2A agent cell, or a bare LLM pool name), and the id is the operator's
/// provider/pool name that a refusal names. Formed HERE (not in the neutral substrate) so the streaming
...
/// one door to an outbound duplex socket, and the close of the egress-audit finding that streaming's
...
///
/// THE BREAKER RIDES BENEATH THE DIAL (the streaming-client cell): before any socket, the `(pool, lane)`
...
> {
    ...
    // ADMITTED: count the dispatch attempt on the streaming-client leg (both labels operator-configured
...
/// THE STREAMING PLANE's [`GauntletPlane`] for a SESSION open — its contribution to the shared open-pass
    ...
    C: DuplexReader + DuplexWriter + Send + Sync + 'static,
{
    ...
    // (the pre-host/dev-default runtime). Streaming posts no separate "kernel-floor" line at Admit the way
