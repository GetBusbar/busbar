...
pub struct SessionCore<C> {
    ...
    /// (`host.meter_ledger` + `host.meter_series`). `None` on an ungoverned deployment. Streaming keeps
            ...
            for ev in events {
                match ev {
                    ...
                    IrServerEvent::Usage(u) => {
                        ...
                        // ledger, attributed to the presenting key (the streaming twin of the LLM plane's
                        // `ledger_and_meter`). Streaming keeps no meter of its own; THIS is the Meter step.
...
/// already has — streaming spawns no boot task, and a sweep that outlived the socket it was sweeping for
...
impl<C> DuplexPlane for StreamingSession<C>
where
    C: DuplexReader + DuplexWriter + Send + Sync + 'static,
{
    fn classify(&self, _frame: &[u8]) -> Option<WireCallRef> {
        ...
        // call busbar issued — correlation is done at the IR layer (streaming `CallRef`), not here.
        None
    }
