...
//! `busbar_substrate::plane_host::SessionScope` (design `plane4-duplex-session.md` §4). One streaming session owns ONE durable handle
...
/// The durable-audit kind stamped on a streaming session's records — matches `PLANE_DECL.audit_kind`.
const STREAMING_SESSION_KIND: &str = "streaming_session";
...
/// THE OPAQUE DURABLE ROW for one streaming session — the neutral engine stores it as `Arc<dyn Any>`; the
...
pub struct StreamingSessionRow {
...
/// A STREAMING SESSION'S DURABLE BINDING — the [`SessionScope`] plus the plane's row shape. Open it at
...
impl SessionHandle {
    ...
    pub fn open(&self, now: u64) -> Result<(), HandleEngineError> {
        ...
        self.scope.open(
            ...
            // No sweep-time abandon transition for a streaming session; teardown is explicit.
    ...
    pub fn get(&self) -> Option<StreamingSessionRow> {
        self.scope
            .get()
            .ok()
            .and_then(|row| row.downcast_ref::<StreamingSessionRow>().cloned())
    }
    ...
    pub fn bump_turn(&self, now: u64) -> Result<u64, ScopedMutateError> {
        let row = self.scope.mutate(|row, _pos| {
            let cur = row
                .downcast_ref::<StreamingSessionRow>()
...
/// BOOT-REHYDRATE the durable `streaming_session` working-set from `store` into `engine` — the
/// [`crate::PLANE_DECL`] `hydrate` step, mirroring the A2A task-set restore. Reads every persisted
/// `streaming_session` row through the neutral [`DurableHandleEngine::rehydrate`] seam and, per row:
...
pub fn rehydrate_sessions(
    ...
    engine.rehydrate(store, STREAMING_SESSION_KIND, |_store, body| {
        ...
        match serde_json::from_slice::<StreamingSessionRow>(body) {
            Ok(row) if row.terminal => Ok(RehydrateOutcome::Terminal),
            Ok(row) => Ok(RehydrateOutcome::Active {
                id: row.id.clone(),
                meta: row.meta(),
                row: row.clone().arc(),
                // Streaming sessions carry no resumable event chain beyond genesis in this build.
...
fn mutation_for(next: StreamingSessionRow) -> Mutation {
