...
//!
//! A live streaming carrier cannot be priced after the fact the way a one-shot request is: the session is
...
/// Streaming has NO meter and NO budget of its own (the vetted doctrine: "no separate meter, no separate
/// budget"); this routes each turn's token usage onto the ONE ledger, attributed to the presenting
/// virtual key, so a streaming session's spend shows up on `usage_for(key)` / the admin usage series just
...
pub struct TurnMeter {
    ...
    /// The pool label for the metering series (the streaming front-door pool).
    pool: &'static str,
...
impl TurnMeter {
    ...
    /// streaming twin of the LLM plane's `ledger_and_meter`. The budget-chain accrual (`meter_ledger`)
...
/// cost lease over the neutral [`MeteringHost`] seam (core's `EngineHostImpl`), so a live streaming carrier
