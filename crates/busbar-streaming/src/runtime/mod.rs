...
//! THE T2 STREAMING SESSION RUNTIME — the live duplex engine (design `plane4-duplex-session.md` §8). Behind the
...
// spells `busbar_streaming::runtime::GovernedCalls` resolves exactly what it always did. The split is a
...
pub struct StreamingRuntime {
    ...
    /// seam the host's other planes journal through (see [`StreamingRuntime::audit_session`]), without the
...
impl StreamingRuntime {
    ...
    /// Land ONE admin-audit row for a streaming-plane mutation through the live host's `JournalHost` leg —
...
/// (type-erased as `Arc<dyn Any + Send + Sync>`), the seam the composition root composes the streaming
...
/// before any request), so a live streaming session reserves / settles / exhausts against the caller's
...
/// The shared composition body: assemble the per-generation [`StreamingRuntime`] over the given metering
...
// THE STREAMING D2 LEASE BILLING ORACLE — the streaming money-path byte/scalar-pinned regression backstop, the
...
#[cfg(test)]
#[path = "streaming_d2_billing_oracle.rs"]
mod streaming_d2_billing_oracle;
