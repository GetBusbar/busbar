...
//! THE STREAMING CONFORMANCE HARNESS — the real driver behind the `testing/streaming-conformance/` battery.
//!
...
use busbar_streaming::runtime::{
    build_runtime_hosted, Carrier, EchoToolExecutor, HostMeteringPort, LeaseState,
    LocalMeteringPort, MeteringPort, SessionCore, StreamingRuntime,
};
...
fn cfg_essentials(config: &busbar_streaming::ir::SessionConfig) -> Norm {
    Norm::Config(format!(
        "{:?}|{:?}|{}|{:?}",
        config.instructions,
        config.voice,
...
// ══════════════════════════════════════════════════════════════════════════════════════════════════
// LEGS 5-7 — composition: the three seams that decide whether a MOUNTED streaming door can actually
...
fn probe_provider_credential() -> (&'static str, String) {
    if busbar_streaming::mount::provider_composed() {
        return (
            "FAIL",
    ...
    if busbar_streaming::mount::compose_provider("https://api.example.com", &undeclared, &resolver)
        .is_ok()
        || busbar_streaming::mount::provider_composed()
    {
        return (
            "FAIL",
    ...
    if busbar_streaming::mount::composed_provider_base_url() != Some("https://api.example.com") {
    ...
    if busbar_streaming::mount::compose_provider("https://other.example.com", &reference, &resolver)
        != Ok(false)
        || busbar_streaming::mount::composed_provider_base_url() != Some("https://api.example.com")
    {
...
impl busbar_streaming::runtime::GovernedCalls for ProbeCalls {
    fn replied(
        &self,
        session: u64,
        call_id: &str,
    ) -> Result<(), busbar_streaming::runtime::ReplyRefusal> {
        let mut open = self.open.lock().unwrap();
        if !open.iter().any(|c| c.session == session) {
            self.refused.lock().unwrap().push(call_id.to_string());
            return Err(busbar_streaming::runtime::ReplyRefusal::NoSuchSession);
        }
        match open
            .iter()
            .position(|c| c.session == session && c.call_id == call_id)
        {
            ...
            None => {
                self.refused.lock().unwrap().push(call_id.to_string());
                Err(busbar_streaming::runtime::ReplyRefusal::UnknownCall)
            }
        }
    }
...
impl busbar_streaming::runtime::ToolExecutor for ServesNothing {
    ...
    C: DuplexReader + DuplexWriter + Send + Sync + 'static,
{
    ...
    .with_governed(busbar_streaming::runtime::GovernedSession {
        session: 7,
        calls: Arc::clone(&table) as Arc<dyn busbar_streaming::runtime::GovernedCalls>,
    });
...
fn probe_tool_reply() -> (&'static str, String) {
    ...
    (
        ...
        format!(
            ...
            busbar_plane_streaming::plane::TOOL_REPLY_DEADLINE_SECS
        ),
    )
}
...
fn probe_metering_lease() -> (&'static str, String) {
    ...
    let cap = busbar_streaming::runtime::cap_nanos_from_buckets(&chain);
    if cap != Some(4_000) {
        return (
            "FAIL",
...
/// key valid for the streaming audience opens a session, and the declared vocabulary is inert.
fn probe_session_scope() -> (&'static str, String) {
    ...
    if !busbar_streaming::mount::session_scope_allowed(&wildcard) {
        return ("FAIL", "a wildcard principal was refused a session".into());
    }
    // An explicit grant on the streaming pool admits.
    if !busbar_streaming::mount::session_scope_allowed(&key_with_scopes(
        "vk-granted",
...
//
//   * the dispatch slot CLAIMS a Gemini-labelled base distinct from the OpenAI one (`streaming_claims`)
//   * the plane still ADMITS exactly one audience for both dialects (`streaming_admission`)
//   * a Gemini WS-accept arrival is actually declared, keyed to this plane's own slot (`streaming_ws_arrivals`)
...
// spec/cross-parity battery, but no ingress route spoke it — `streaming_claims`/`streaming_ws_arrivals` named
...
fn probe_gemini_live_route() -> (&'static str, String) {
    ...
    let Some(slot) = busbar_streaming::mount::streaming_build(&ctx) else {
        return ("FAIL", "streaming_build produced no dispatch slot".into());
    };
    ...
    let Some(admission) = busbar_streaming::mount::streaming_admission(slot.as_ref()) else {
        return (
            "FAIL",
    ...
    let arrivals = busbar_streaming::mount::streaming_ws_arrivals();
    ...
    if gemini.slot_key != busbar_streaming::PLANE_DECL.key {
        return (
            "FAIL",
            format!(
                "the Gemini arrival is keyed to '{}', not the plane's own slot '{}'",
                gemini.slot_key,
                busbar_streaming::PLANE_DECL.key
            ),
        );
    }
...
fn probe_provider_dial() -> (&'static str, String) {
    ...
    rt.block_on(async move {
        ...
        let (mut provider_in, _provider_out) = match busbar_streaming::topology::dial_provider(
            &host,
...
/// Assemble a per-generation [`StreamingRuntime`] rebound onto `host`'s D2 lease — the harness's own
/// [`build_runtime_hosted`] call, shared by every probe below that needs a governed session.
fn hosted_runtime(host: Arc<dyn EngineHost>) -> StreamingRuntime {
    let base = StreamingRuntime::new(
        Arc::new(busbar_substrate::plane::handle_engine::DurableHandleEngine::new()),
        Arc::new(LocalMeteringPort),
        Arc::new(EchoToolExecutor),
...
fn probe_admit_refusal() -> (&'static str, String) {
    ...
    if fixture.ledger_usage("spent-caller").is_some() {
        return (
            "FAIL",
            "a refused admission still posted a ledger entry -- streaming's design posts no separate \
...
fn main() {
    ...
    let usage = || -> ! {
        eprintln!(
            "usage:\n  streaming-conform spec <openai|gemini> <fixtures_dir>\n  streaming-conform replay <fixtures_root>\n  streaming-conform cross <oo|og|go|gg> <openai_dir> <gemini_dir> <map.json>\n  streaming-conform governance <checkpoint>\n  streaming-conform composition <provider-credential|metering-lease|session-scope|gemini-live-route|provider-dial|admit-refusal|route-failover|audit-record|exit-terminal|tool-reply>"
        );
        std::process::exit(2);
    };
