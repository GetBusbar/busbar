...
//! THE `streams:` SECTION — the streaming plane's config grammar, and the one place its values are read.
//!
//! ## The section IS the plane
//!
//! `streams:` is the fourth plane noun beside `pools:` / `tools:` / `agents:`, and — like them —
//! there is no `plane:`/`bind:`/`target:` selector: writing a `streams:` block IS declaring the streaming
//! plane's configuration. It is a SINGULAR typed section, not a named-definition map: a deployment has
//! ONE live-streaming posture, so the section is one object (the locked session defaults + the three
...
//! live-streaming CEILINGS cannot be retyped or removed without the gate flagging it.
...
/// THE `streams:` SECTION — the streaming plane's owned config. Its VAD/session/media shape IS the GA
...
pub struct StreamsCfg {
    /// The locked session defaults every live session opens with (media formats, streaming, instructions,
...
impl busbar_substrate::plane::config::PlaneCfg for StreamsCfg {
    /// The streaming plane's `streams:` section carries NO secret reference — the exhaustive destructure
...
/// streaming dispatch slot is built from `public_url` alone, and nothing calls the plane's runtime-build
...
/// the streaming grammar declares no credential field of its own and gains none here.
#[must_use]
