...
//! busbar-streaming — the DUPLEX / LIVE-STREAMING plane (Plane 4), as ONE plugin crate.
//!
...
//! do NOT change to accept it, and the crate is strong-form DELETABLE (`git rm -r crates/busbar-streaming`
//! leaves the neutral crates compiling) — proven by `scripts/plane-delete-test.sh streaming`.
//!
...
//! write, so `Plane::has_superset_ir("streaming")` is true — DERIVED from the length-2 `STREAMING_WIRE_FORMATS`
...
/// THE DUPLEX IR AND THE DIALECT CODECS, RE-EXPORTED FROM `busbar-streaming-codec`.
///
/// The plane-4 session intermediate representation (media, control, events, tools, session config,
/// usage), the shared duplex reader/writer and the Gemini Live dialect live in `busbar-streaming-codec`
...
/// are re-exported HERE, under their old name, so every caller that spells `busbar_streaming::ir::…`
/// resolves exactly what it always did. The split is a MOVE: no item changed shape crossing it.
pub use busbar_streaming_codec::ir;

/// THE `streams:` CONFIG SECTION — the streaming plane's owned config grammar ([`config::StreamsCfg`]) and
...
/// THE STREAMING PLANE'S PLANE-CONTRIBUTED DIAGNOSTICS — the `&'static [&'static Diagnostic]` the
/// composition root installs via `busbar_substrate::diagnostics::install_diagnostics`, re-exported at
/// the crate root so the `busbar` binary names one stable path (`busbar_streaming::DIAGNOSTICS`), exactly
/// as `busbar_mcp::DIAGNOSTICS` / `busbar_a2a::DIAGNOSTICS`. Not yet booted by the binary — streaming
...
/// THE DATA-ROUTE MOUNT — the streaming plane's `PLANE_DECL` `routes` / `claims` / `admission` / `build`
...
/// `PLANE_DECL` (the hooks stay empty/`None`, wired through the [`STREAMING_CLAIMS`] / [`STREAMING_ADMISSION`] /
/// [`STREAMING_BUILD`] / [`STREAMING_ROUTES`] cfg-split consts, exactly as [`STREAMING_BUILD_RUNTIME`] is).
#[cfg(feature = "runtime")]
pub mod mount;
...
const STREAMING_BUILD_RUNTIME: Option<
    fn(
        &dyn std::any::Any,
...
// ── THE DATA-ROUTE MOUNT HOOKS — cfg-split exactly as `STREAMING_BUILD_RUNTIME`, so the default
...
// `busbar-streaming/runtime`) both mounts and admits, keeping the ratchet's "mounted ⇒ admitted" true.

/// `PLANE_DECL.build` — construct the per-generation dispatch slot from `public_url` (the audience),
/// `crate::mount::streaming_build` with the feature on; `|_| None` (no slot) with it off.
#[cfg(feature = "runtime")]
const STREAMING_BUILD: fn(
    &busbar_substrate::plane::registry::BuildCtx,
) -> Option<std::sync::Arc<dyn std::any::Any + Send + Sync>> = mount::streaming_build;
#[cfg(not(feature = "runtime"))]
const STREAMING_BUILD: fn(
    &busbar_substrate::plane::registry::BuildCtx,
...
/// `PLANE_DECL.hydrate` — boot-rehydrate the durable streaming-session working-set before any listener
/// binds ([`mount::streaming_hydrate`]); `None` off-feature so the default decl is byte-unchanged.
#[cfg(feature = "runtime")]
const STREAMING_HYDRATE: Option<busbar_substrate::plane::registry::BootHook> =
    Some(mount::streaming_hydrate);
#[cfg(not(feature = "runtime"))]
const STREAMING_HYDRATE: Option<busbar_substrate::plane::registry::BootHook> = None;
...
/// THE DIALECT NAME this plane speaks first — OpenAI's bidirectional Realtime streaming API. Named once
...
/// [`ir::GeminiLiveCodec`]; adding it to `STREAMING_WIRE_FORMATS` is what EARNS the plane its superset IR
...
/// the A2A discipline — a plane earns a superset at its SECOND wire format and not before.
const STREAMING_WIRE_FORMATS: &[&str] = &[OPENAI_REALTIME, GEMINI_LIVE];

/// THE STREAMING PLANE'S PROVIDER-FACING BEARER — the ONE construction that plans the provider-facing
/// `Authorization: Bearer <credential>` from the RESOLVED provider-credential string, and NEVER a
/// caller/governance token passed through. Both [`streaming_egress_auth_headers`] (the
...
pub(crate) fn streaming_provider_bearer(
    key: &str,
...
fn streaming_egress_auth_headers(
    key: &str,
    _ctx: &busbar_substrate::proto::SigningContext,
) -> Vec<(axum::http::HeaderName, axum::http::HeaderValue)> {
    streaming_provider_bearer(key)
}

/// THE STREAMING PLANE'S DECLARATION — a `&'static PlaneDecl` the composition root installs at boot so the
/// `busbar` binary names one stable path (`busbar_streaming::PLANE_DECL`). It declares the plane's identity
...
pub const PLANE_DECL: busbar_substrate::plane::registry::PlaneDecl =
    busbar_substrate::plane::registry::PlaneDecl {
        ...
        fallback: false,
        // THE DUPLEX / LIVE-STREAMING PLANE'S DECLARING SECTION is `streams:` (1.6.0 config-seam KEYSTONE
        ...
        // (`"streaming"`), so key ≠ config_section here exactly as the MCP plane's key is `"mcp"` while its
        ...
        // core names no `streams`/`streaming` parse target and streaming can register `streams:` and boot.
        ...
        // TWO dialects ⇒ superset IR, DERIVED from this list's length (see STREAMING_WIRE_FORMATS).
        wire_format_names: || STREAMING_WIRE_FORMATS,
        ...
        // boot is installed under `plane-streaming` (⇒ `busbar-streaming/runtime`), so it always both mounts and
        // admits — the ratchet's "mounted ⇒ admitted" holds by construction.
        claims: STREAMING_CLAIMS,
        admission: STREAMING_ADMISSION,
        build: STREAMING_BUILD,
        routes: STREAMING_ROUTES,
        admin_routes: None,
        openapi: None,
        // BOOT hooks — rehydrate the durable session working-set before the listener, then the
        // post-listener readiness step. Wired behind `runtime` (see [`STREAMING_HYDRATE`] / [`STREAMING_START`]);
        // `None` off-feature so the default decl is byte-unchanged.
        hydrate: STREAMING_HYDRATE,
        ...
        on_swap: None,
        // config-seam: streaming PARSES its owned `streams:` section through its own typed `StreamsCfg`,
        // so `DeployCfg` names no `busbar_streaming` type (the MCP `mcp_parse_section` / A2A
        ...
        // feature (see [`STREAMING_BUILD_RUNTIME`]); `None` when the feature is off so the default build is
        ...
        build_runtime: STREAMING_BUILD_RUNTIME,
        viewer: None,
        ...
        // config-seam: streaming OWNS the `streams:` grammar. `"streams"` is NOT in core's
...
/// THE STREAMING PLANE'S PROTOCOL DECLARATION — a `ProtocolDecl` with `codec: None`, re-exported at the
/// crate root so the `busbar` binary names one stable path (`busbar_streaming::DECLS`). Like MCP / A2A, it
...
pub static DECLS: busbar_substrate::proto::ProtocolDecl = busbar_substrate::proto::ProtocolDecl {
    ...
    // [`streaming_egress_auth_headers`]). LANE-CONSTANT: the builder is a pure function of the resolved
    ...
    egress_auth_headers: Some(streaming_egress_auth_headers),
