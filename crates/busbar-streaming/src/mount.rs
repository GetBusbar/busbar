...
//! THE STREAMING PLANE'S DATA-ROUTE MOUNT (behind the `runtime` feature) — the structural surface that
...
//!
//! Streaming contributes a DISPATCH slot ([`StreamingMount`]) through [`PlaneDecl::build`], carried in
//! `plane_slots` under the plane's decl key (`"streaming"`). It is the object [`streaming_claims`] /
//! [`streaming_admission`] / [`streaming_routes`] all read, and the SAME object the core route adapter hands a
...
//! for and the one busbar demands cannot drift apart) plus a live [`StreamingRuntime`] the route handlers
...
/// so the streaming plane emits into the SAME family without reaching into `busbar-core` (which owns the
...
/// The bounded `pool` label the streaming FRONT DOOR (the streaming-server cell) emits under — a constant, not
/// a caller value, so the series count stays bounded. The outbound provider dial (the streaming-client
...
/// The session-open method label the hook gate/tap projection carries — the single operation a streaming
...
/// THE HOOK CONTAINER the streaming plane's session-open gate/tap fire under — the plane's SINGULAR config
/// section noun (`streams`), since streaming declares no per-registration container (its config is one
...
///
/// The value asked about is the streaming front door's pool ([`FRONT_DOOR_POOL`]), which is the one pool a
/// streaming session is ever served on, so an operator narrows a key to streaming by granting
/// `{ kind: session, value: streaming-server }` and nothing wider. A key with NO scope list at all is the
...
/// plain-text shape every other streaming refusal takes, and BEFORE any hook, lease, durable row or dial.
...
/// composition root's one write of the streaming plane's egress credential.
///
...
/// THE STREAMING PLANE'S RFC 8707 RESOURCE PATH — the segment the plane's canonical audience carries and
/// the base every streaming ingress route sits under. A token presented on any `/v1/realtime/*` path must
...
/// THE GEMINI DIALECT'S CLAIMED BASE — a path distinct from [`MOUNT_PATH`] so [`streaming_claims`] can name
...
/// The RFC 9728 protected-resource metadata path for the streaming resource (the `resource_metadata` a
...
/// THE STREAMING PLANE'S DISPATCH SLOT — the per-generation object `build` erases into `plane_slots` under
...
pub struct StreamingMount {
    /// The RFC 8707 audience every inbound streaming token must carry — `<public_url>/v1/realtime`.
    audience: String,
...
impl StreamingMount {
    /// This plane's admission facts — the audience a token at the streaming door must carry and where a
...
/// `public_url` carrying a path of its own cannot produce a second spelling of the streaming resource. The
...
/// [`PlaneDecl::hydrate`] — BOOT-REHYDRATE the durable streaming-session working-set BEFORE any listener is
...
/// store IS configured, it restores the persisted `streaming_session` rows through the neutral engine seam
...
pub fn streaming_hydrate(ctx: &dyn PlaneBootCtx) -> Result<(), String> {
    ...
    // A configured store: restore the durable `streaming_session` working-set into a fresh engine and
...
/// [`PlaneDecl::start`] — the post-listener boot step. Streaming opens no background boot task: a live
...
pub fn streaming_start(_ctx: &dyn PlaneBootCtx) -> Result<(), String> {
    tracing::debug!(
...
/// [`PlaneDecl::build`] — construct the streaming DISPATCH slot for one config generation. Reads the
...
/// rebinds it onto the live host lease (see [`StreamingMount::runtime`]).
fn dispatch_runtime() -> StreamingRuntime {
...
/// [`PlaneDecl::claims`] — the ONE audience-checked region the streaming plane answers on, spoken in its
...
#[must_use]
pub fn streaming_claims(slot: &dyn Any) -> Vec<(String, &'static str)> {
    match slot.downcast_ref::<StreamingMount>() {
...
/// [`PlaneDecl::admission`] — the audience a streaming token must carry and where a refused caller is sent
...
/// THE STREAMING PLANE'S INBOUND WS-ACCEPT ARRIVALS — the browser-sideband + telephony media legs,
...
/// plane's decl `key`. Empty when the plane has no receiving side, exactly as [`streaming_routes`].
#[must_use]
pub fn streaming_ws_arrivals() -> Vec<WsArrivalSpec> {
...
/// THE SHARED GOVERNED OPEN every streaming route funnels through — the ONE choke point. In order:
///
...
///
/// The front-door request is COUNTED on `busbar_plane_requests_total` (plane = streaming), and the answer
...
/// (`plane = streaming`, the streaming-server cell) lands on the neutral `busbar_plane_requests_total` family;
...
/// provider hop authenticates ONLY through [`crate::streaming_provider_bearer`] — the same lane-constant
...
async fn serve_sdp(
    ...
    for (name, value) in crate::streaming_provider_bearer(&p.api_key) {
...
/// Read the streaming dispatch slot off the neutral `PlaneReqCtx::slot`, resolve the caller + a session id,
/// and drive the shared governed open. `slot` is the SAME `StreamingMount` `build` erased (the mount is
...
async fn serve(
    ...
    let Some(mount) = ctx.slot.downcast_ref::<StreamingMount>() else {
        return refusal(
            axum::http::StatusCode::INTERNAL_SERVER_ERROR,
...
/// the dialect `codec` (the second-dialect route): [`streaming_ws_arrivals`] instantiates it once per dialect
...
{
    let Some(mount) = arrival.slot.downcast_ref::<StreamingMount>() else {
        return refusal(
            axum::http::StatusCode::INTERNAL_SERVER_ERROR,
            ...
            match ingress {
                ...
                Ingress::Sideband => {
                    if let Ok((core, _handle, _guard)) = open_admitted_session(
                        ...
                        serve_with_sweep(
                            Arc::clone(&core),
                            serve_messages(stream, sink, Arc::new(StreamingSession::new(core))),
                        )
                        .await;
                    }
                }
                Ingress::Mint | Ingress::Sdp => {
                    ...
                    // `streaming_ws_arrivals`); Mint/Sdp are the one-shot HTTP passes (`streaming_routes`).
                }
            }
        },
    )
...
// BEHAVIORAL billing proof: drive a streaming turn's usage through the SHIPPED Meter seam over a REAL
