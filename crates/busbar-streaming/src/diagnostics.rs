...
//! Streaming plane diagnostics — the `STREAMING_*` catalog entries this crate OWNS.
//!
...
//! plane (`streaming-…`) and no other plane or dialect noun.
//!
...
//! names one stable path: `busbar_streaming::DIAGNOSTICS`. Streaming is not yet booted by the binary
...
/// The D2 governance outcome: a live streaming session was HARD-CLOSED when its metering lease reached
...
pub const STREAMING_SESSION_LEASE_EXHAUSTED: Diagnostic = Diagnostic {
    code: 7050,
    class: Class::Plane,
    slug: "streaming-session-lease-exhausted",
    title: "Streaming session hard-closed on metering-lease exhaustion",
    severity: Severity::BenignRecurring,
    summary: "A live streaming session reached its metering lease's real cap and was hard-closed rather \
...
pub static DIAGNOSTICS: &[&Diagnostic] = &[&STREAMING_SESSION_LEASE_EXHAUSTED];
