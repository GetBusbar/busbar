...
//! THE HOOKS-GATE CELL — `hooks-gate × {streaming-client, streaming-server}` (one wiring, both directions).
...
fn an_open<'a>(rt: &'a StreamingRuntime, host: Arc<dyn EngineHost>) -> GovernedOpen<'a> {
    GovernedOpen {
        rt,
        host,
        provider: None,
...
fn a_slot() -> Arc<dyn std::any::Any + Send + Sync> {
    ...
    crate::mount::streaming_build(&ctx).expect("streaming_build yields a slot for a public_url")
}
