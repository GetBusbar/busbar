...
//! GOVERNANCE-BUDGET capability cells (streaming-client + streaming-server) — GATE-VALID location.
//!
...
/// streaming-client cell: a session lease settled PAST its cap hard-closes and refuses further spend —
...
/// streaming-server cell: the session lease bills the presenting key and refuses past the cap with a hard
