...
use crate::runtime::{EchoToolExecutor, LocalMeteringPort, SessionHandle, StreamingRuntime};
...
fn runtime_for(model: &str, deny: &[&str]) -> StreamingRuntime {
...
fn build_binds_the_audience_from_public_url_and_none_without() {
    ...
    let claims = streaming_claims(slot.as_ref());
    assert_eq!(
        claims,
        vec![
...
fn the_five_ingress_doors_mount_audience_checked_across_the_http_and_ws_seams() {
    ...
    let http: Vec<(String, RouteMethod, RouteAuth)> = streaming_routes(slot.as_ref())
        .into_iter()
    ...
    let ws: Vec<(String, RouteAuth, &'static str)> = crate::mount::streaming_ws_arrivals()
        .into_iter()
    ...
    assert!(
        streaming_routes(&()).is_empty(),
        "a slot that is not a StreamingMount mounts no routes"
    );
}
...
fn hydrate_is_a_noop_under_the_ephemeral_posture_and_start_confirms_ready() {
    ...
    assert!(
        streaming_hydrate(&ctx).is_ok(),
        "hydrate under no store is a clean no-op"
    );
    assert!(streaming_start(&ctx).is_ok(), "start confirms readiness");
}
...
fn hydrate_rehydrates_the_durable_session_working_set() {
    ...
    assert!(
        streaming_hydrate(&FakeBootCtx { store: Some(store) }).is_ok(),
...
async fn duplex_session_runs_in_process_through_the_gauntlet_after_hydrate() {
    ...
    assert!(streaming_hydrate(&FakeBootCtx { store: None }).is_ok());
