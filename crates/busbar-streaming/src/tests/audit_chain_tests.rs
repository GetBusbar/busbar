...
//! AUDIT-CHAIN capability cells (streaming-client + streaming-server) — GATE-VALID location.
//!
...
/// streaming-client cell: an opened session seals a genesis event on the durable chain and survives a
...
#[test]
fn an_opened_streaming_session_seals_a_genesis_event_and_survives_reattach_on_the_durable_chain() {
...
/// streaming-server cell: an inbound session-open (the front door) lands a sealed genesis event — the
