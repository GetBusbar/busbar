...
//! THE FRONT-DOOR SESSION-OPEN COUNT on a real scrape — the `metrics × streaming-server` cell. A governed
//! session-open lands on the neutral `busbar_plane_requests_total` family with `plane="streaming"`, read
...
#[tokio::test]
async fn a_streaming_session_open_increments_the_plane_labelled_counter() {
    ...
    // The plane-labelled family carries a `plane="streaming"` series on the real exposition.
    let exposition = metrics_capture::render();
    let counted = exposition.lines().any(|l| {
        l.starts_with("busbar_plane_requests_total")
            && l.contains("plane=\"streaming\"")
            && !l.starts_with('#')
    });
    assert!(
        counted,
        "a streaming session-open must appear on busbar_plane_requests_total under plane=\"streaming\". \
         Exposition:\n{exposition}"
    );
}
