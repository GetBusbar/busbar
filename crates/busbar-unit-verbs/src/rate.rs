...
impl MutationClass {
    ...
    /// EXPLICITLY, and this is the one place that matters: none of them has a legacy row, so the
    /// row lookup below cannot see them, and without the name they would fall through to `Crud` and
    /// spend a mutation slot per read. A read that consumes a mutation budget refuses an operator's
    /// config change because they looked at a balance — or ran `verify`, or hit `/healthz` — first,
    /// which is the opposite of what a read is for. [`crate::verbs::required_scope`] already calls
    /// every one of these `ReadOnly` for exactly this reason; the check is repeated as data here
    /// (rather than calling that function) because `for_verb` must stay free of any dependency on
    /// the executor module it is classifying inputs
    /// for.
