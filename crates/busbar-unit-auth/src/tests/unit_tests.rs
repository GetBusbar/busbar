...
fn the_tier_the_key_is_bound_to_is_what_the_step_hands_forward() {
    // The DIRECTORY the chain reaches the key through is the contract's face, and the tier is a
    // field of the binding row it answers with — the same row the id and the label come out of.
    let bound = |tier: Option<&str>| super::OneKey {
        token: "cred",
        aud: None,
    ...
    let established = auth
        .resolve(&request(), None, Some(&bound(Some("gold"))), None, &token)
        .into_result(&seal)
    ...
    let auth = Auth::new(AuthChain::new(Vec::new(), true));
    let untiered = auth
        .resolve(&request(), None, Some(&bound(None)), None, &token)
        .into_result(&seal)
        .expect("a resolved key is an identity");
    assert_eq!(
