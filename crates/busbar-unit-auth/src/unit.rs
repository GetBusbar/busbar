...
impl Auth {
    ...
    /// it, which is what stops a step reading another step's answer. That seal is not being weakened
    /// here. What a step downstream needs is not the decision — it is the key's own GRANTS, which
    /// the chain resolved and then dropped on the floor, because until now nothing above the chain
    /// could read them.
    ///
    /// So the key travels beside the decision, to the ONE caller that asked for it, and the sealed
    /// answer is unchanged. `resolve` above is this function with the sink thrown away.
    ///
    ...
    /// has a grant list to read. A boxed module's answer type cannot carry one, and the open door
    /// resolved nobody at all. Both leave the sink `None`, and a caller enforcing a grant must read
    /// that as "no grants to enforce against" rather than as "unrestricted": the fail-closed
    /// reading of it is the caller's to take.
    ///
