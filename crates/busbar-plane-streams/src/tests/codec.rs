...
/// NO CLAIM OF THIS PLANE IS A REQUEST-AND-ANSWER ROUTE.
///
...
fn the_claim_table_names_no_route_that_opens_no_session() {
    for path in ["/v1/audio/speech", "/v1/audio/transcriptions"] {
        assert_eq!(
            crate::claims::dialect_for(path),
            None,
            "`{path}` is a request-and-answer route and this plane must not claim it"
        );
    }
    for c in crate::claims::DIALECT_CLAIMS {
        assert_eq!(
            c.claim.transport,
            crate::claims::WS_TRANSPORT,
            "the claim naming `{}` is not on the duplex transport",
            c.dialect
        );
    }
    assert_eq!(crate::claims::DIALECT_CLAIMS.len(), 3);
}
