...
//! DIALECT UNIT TESTS — the OpenAI Realtime GA wire, round-tripped through the neutral IR.
//!
...
#[test]
fn cached_input_tokens_extract_as_a_subset_of_the_input_figure() {
    ...
    // The IR carrier stays wire-faithful: extraction only, the raw provider figures. The BILLING
    // fold that nets the subset out is the IR's own (`IrDuplexUsage::to_billing_usage`) and is
    // asserted beside it, with no dialect in the room.
