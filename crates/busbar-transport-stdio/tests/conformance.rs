...
fn key_is_the_declared_meta_key_and_is_lowercase() {
    let key = subject().key();
    assert_eq!(key, <StdioTransport as TransportMeta>::KEY);
    assert!(!key.is_empty());
...
fn status_namespace_is_present_exactly_when_a_status_class_is() {
    ...
    // names neither.
    assert_eq!(
        <StdioTransport as TransportMeta>::STATUS_CLASS.is_some(),
        <StdioTransport as TransportMeta>::STATUS_NAMESPACE.is_some(),
    );
}
...
fn composition_claim_is_the_registration_claim() {
    ...
    assert_eq!(
        <StdioTransport as TransportMeta>::COMPOSES_OVER,
        busbar_transport_stdio::claims::COMPOSES_OVER,
    );
}
