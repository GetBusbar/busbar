...
/// Re-export the `kind: hook` WIRE FACE at the path this SDK has always published it under. The one
/// definition is `busbar_contract::hook`'s now — see `docs/design/1.6.0-one-face-per-kind.md` §2. This
/// SDK keeps the C glue and the op-dispatch match ([`hook_abi_version`], [`dispatch_hook`],
/// [`hook_dispatch`], [`HookHandle`]) and nothing else.
pub use busbar_contract::hook::HookHandler;
