...
) -> Result<busbar_plugin::cold::SecretResponse, PluginError> {
    match req {
...
pub unsafe fn secret_dispatch(handle: *mut c_void, bytes: &[u8]) -> BoundaryOutcome {
    ...
    match dispatch_secret(module.as_ref(), request) {
        ...
        Err(e) => {
            // The frozen pair a host that predates the structured error reads — the class's
            // LOSSY wire token (`wire_token_for`) and the developer message — with the structured
            // error itself riding beside them for a host that can read more. The message channel
            // must never carry secret material; the face's own rule, restated on the wire.
...
/// error rides beside them for a host that can read more.
pub fn wire_token_for(class: ErrorClass) -> busbar_plugin::cold::SecretErrorKind {
    use busbar_plugin::cold::SecretErrorKind as Wire;
    match class {
        ErrorClass::NotFound => Wire::NotFound,
        ErrorClass::Unavailable | ErrorClass::Timeout | ErrorClass::Exhausted => Wire::Unavailable,
        ErrorClass::Denied => Wire::Denied,
        ErrorClass::Malformed | ErrorClass::Rejected => Wire::Invalid,
        ErrorClass::Conflict | ErrorClass::Integrity | ErrorClass::Internal => Wire::Internal,
    }
}
