...
// ── NEUTRAL SPINE ALIASES ───────────────────────────────────────────────────────────────────────
// This crate carved the hook-DISPATCH core out of `busbar-core` (DECISIONS #19). The dispatch code
// below still spells `crate::config::…`, `crate::session::…`, `crate::ir::…`, `crate::limits::…` — the
// paths it had inside `busbar-core`. Those types now live on the neutral spine
// (`busbar_substrate` / `busbar_api`), so we re-export them under local aliases of the SAME names.
// Every `crate::…` path in the moved code resolves through these, unchanged, WITHOUT this crate ever
// naming `busbar-core` (the reverse edge Cargo refuses).
// Re-export shims: which names are referenced depends on `hooks-ranking`/`test` cfg, so tolerate a
// re-export that a given feature-permutation build does not consume.
#[allow(unused_imports)]
pub(crate) mod config {
    // The `hooks:` config SHAPES + the reserved on_error terminal + the frozen policy-timeout const.
    pub(crate) use busbar_substrate::config::hooks::{
        on_error_terminal, HookCfg, HookKind, HookStage, PromptAccess, UserAccess,
        DEFAULT_POLICY_TIMEOUT_MS,
    };
    // The `pools:` config SHAPES named by the dispatch/ranking code.
    pub(crate) use busbar_substrate::config::pools::{PoolCfg, PoolPolicy};
    // The resolved on_error/on_empty terminal enum.
    pub(crate) use busbar_substrate::config::PolicyOnError;
    // The neutral plugin-settings secret classify/resolve helpers (relocated to the substrate secret
    // home); the engine-specific concrete `SecretResolver` is NOT named by the dispatch core — it is
    // held behind the neutral `busbar_api::SecretResolve` trait (see `HookEnv::secret_resolver`).
    pub(crate) mod secret {
        pub(crate) use busbar_substrate::config::secret::{
            classify_setting, resolve_settings, SettingShape,
        };
        // TEST-ONLY: a byte-faithful builtins-only / plugin-closure resolver, so the dispatch tests
        // build a `HookEnv` exactly as they did against `busbar_core::config::secret::SecretResolver`
        // — without this crate depending on `busbar-core`. Behaviour copied VERBATIM from the engine
        // resolver (`env`/`file` built-ins, fail-closed plugin arm, UTF-8/empty checks).
        #[cfg(test)]
        pub(crate) use crate::tests::TestSecretResolver as SecretResolver;
    }
}

pub(crate) mod session {
    pub(crate) use busbar_substrate::session::{OwnerKey, SessionKey, SessionStore};
}

#[allow(unused_imports)]
pub(crate) mod ir {
    pub(crate) use busbar_substrate::ir::{facts, invoke};
}

pub(crate) mod limits {
    /// Global default routing-policy timeout (ms). Reads the SAME process-global limits slot the
    /// engine installs (`busbar_substrate::config::limits`), falling back to the frozen default const
    /// when uninstalled — byte-identical to `busbar_core::limits::default_policy_timeout_ms`.
    pub(crate) fn default_policy_timeout_ms() -> u64 {
        busbar_substrate::config::limits::installed()
            .map(|l| l.default_policy_timeout_ms)
            .unwrap_or(busbar_substrate::config::hooks::DEFAULT_POLICY_TIMEOUT_MS)
    }
}

// TEST-ONLY neutral aliases for the operation vocabulary + the warn-capture layer the dispatch tests
// reach for at their historical `crate::` paths.
#[cfg(test)]
pub(crate) mod operation {
    pub(crate) use busbar_api::operation::*;
}
#[cfg(test)]
pub(crate) mod test_support {
    pub(crate) use busbar_substrate::testkit::warn_capture;
}
...
// seam returns it without naming core. Re-exported by identity here so `crate::RequestedSignals`
...
/// `crate::{ResolvedPolicy, FallbackHook}` paths (and the `App` gate maps) are unchanged.
...
// alias) so `crate::TapEntry` and every consumer are unchanged.
