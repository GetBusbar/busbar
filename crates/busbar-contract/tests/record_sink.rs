...
//! different shapes, and the only trait that named the narrow one lived on the kernel's record leg,
//! in a crate a store-kind plugin may not name. The three verbs are their own trait here, and
//! `Store` declares them by BEING one.
//!
