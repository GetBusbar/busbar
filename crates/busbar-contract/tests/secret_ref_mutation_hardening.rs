...
//! Mutation-hardening additions for `busbar_contract::secret_ref` (the former `busbar-secret-ref`
//! crate, merged into the contract by DECISIONS #83). The module's existing suite in
//! `src/tests/secret_ref_tests.rs` already covers its behavior; this file instead locks in the one
//! invariant that stands apart from ordinary behavior coverage: `SecretRef` must never leak a
//! secret VALUE through `Debug`/`Display`/logging.
//!
//! Its own top-level `tests/` file (rather than a case added to `src/tests/secret_ref_tests.rs`)
//! because it stands for a SECURITY invariant, not a unit of `secret_ref.rs`'s own behavior — see
//! the repo's test-locality convention (`docs/code-layout.md`): the existing single inline test
//! module stays exactly the src-level unit-test file it was.
//!
...
//! `src/tests/secret_ref_tests.rs`; this file adds the distinctive-marker variant the task calls
//! for and confirms no Display impl exists to leak through.
//!
...
/// API is exercised only through `Debug`; nothing else formats it. (A `serde::Serialize` impl
/// exists instead, tested separately below — Serialize is not string-formatting and carries no
/// log-echo risk on its own.)
#[test]
fn secret_ref_debug_output_never_contains_an_inline_literal_a_caller_tried_to_smuggle_in() {
    ...
    // the numeric/bool/string cases `src/tests/secret_ref_tests.rs` already covers.
