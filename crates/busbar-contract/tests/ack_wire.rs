//! Red-before-green guard for the single `kinds::Ack` export acknowledgement wire (OWNER-RULED
//! 2026-09-21).
//!
//! The ONE `Ack` type must (a) deserialize as well as serialize, and (b) use `snake_case` tokens.
//! Before this pass, `Ack` derived `Serialize` only with no `rename_all`, so it could not be read
//! back off the wire and would have emitted PascalCase (`"Durable"`) instead of the `durable` /
//! `received` / `retry` this test pins. This test fails on either regression.
