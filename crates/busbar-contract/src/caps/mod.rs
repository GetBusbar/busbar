...
// Folded from the former `busbar-caps` crate (#37/#38), which did not enable this contract-wide
// lint; the capability types travel byte-identically, so the lint is relaxed for the module rather
// than adding Debug impls that would change them.
...
mod honesty_table {
    ...
    /// this test exists at all: the table below is a doc comment the gate never reads.
    ///
