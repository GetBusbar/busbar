...
/// store-kind plugin, and a store-kind plugin may not name the crate the kernel's record leg lives
/// in. So the trait that names the sink has to live HERE, in the contract both halves are allowed
/// to name — a store implements it, and the composition root hands it to the kernel's runner. A
/// second copy of these three verbs declared anywhere else would be a second protocol, and the
/// node would have two answers to "what is a record sink".
///
