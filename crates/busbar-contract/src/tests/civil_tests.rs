...
//! Tests for the proleptic-Gregorian civil-date split in `crates/busbar-contract/src/civil.rs`.
//!
//! These assertions previously sat in `crates/busbar-kernel/src/tests/civil_tests.rs`, which no
//! `mod` declaration reached and cargo therefore never compiled: the `#[path = "tests/civil_tests.rs"]`
//! in `civil.rs` resolves against THIS crate's `src/`, not the kernel's, so the two files are not the
//! same file and only this one is built. The kernel copy held the only executed coverage the split
//! ever had while this one was a placeholder with zero `#[test]`, which is how `civil_from_days` came
//! to ship with no assertion behind it. Restored here, against the functions that actually serve.
