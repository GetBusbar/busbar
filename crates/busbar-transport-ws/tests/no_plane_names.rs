...
//!
//! The mount in this crate serves any plane's declared surface, as a DUPLEX SESSION: one upgrade,
//! many frames, both directions. The whole value of that is that it does not know which plane, and
//! the whole value is lost the first time a protocol's name appears here — one `if` on a dialect,
//! one frame kind spelled out because "it is only this one protocol", one dependency added because a
//! codec was convenient. Every one of those is a small, reasonable-looking diff, and the thing they
//! add up to is the coupling this generic mount exists to remove.
//!
//! The pull is stronger on a duplex wire than on a request/answer one, which is why this file
//! matters more here. Session state has to live somewhere, and the shortest way to write it is
//! inside the transport that already holds the socket — at which point the transport knows what a
//! session accumulates, which is a plane's knowledge wearing a wire's clothes. It lives behind the
//! driver's session handle instead, and nothing in this crate can name it.
//!
...
//! `busbar_contract_transport::session::SessionDriver` — the duplex sibling of the one-shot
//! `UnitDriver` — handed to it at listen by the root that implements it.
//!
