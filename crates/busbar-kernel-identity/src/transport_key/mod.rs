...
//! # transport_key — TLS secret→config provisioning, kernel-side (#36, #40)
//!
//! Folded from the retired transport-key crate. #36 names this step the one exit from the unit
//! collapse — "the TLS secret→config provisioning → kernel-side machinery (never a plugin;
//! the tls plugin gets an opaque config handle over the ABI, #40)" — and it lives in this crate
//! because it fits this crate's definition (roster def 3, #83): what it resolves is the node's own
//! TLS credential — the certificate and key a listener signs its handshake with, and the client CA a
//! presented client certificate is verified against — which is #36's "verify/sign credentials, in
//! or out" for `busbar-kernel-identity`, with no direction in the name.
//!
...
//! [`TransportKeyHandle`] (`busbar-contract`) rather than as bytes — nothing downstream of this
//! step can turn a handle back into key material. Given a [`SecretSource`] (the abstract "read this
//! named secret" seam a real deployment's secret plugin sits behind) and an [`AccessJournal`] (the
//! "record that this secret was read, and why" seam the audit trail sits behind), it resolves TLS
//! key material and builds the `rustls::ServerConfig` a listener actually needs. The built config
//! reaches the transport only as the contract's opaque [`TransportConfigHandle`], delivered through
//! the transport's [`TransportConfigSink`] (#40(b)): no transport names this crate.
//!
//! ## What is in here
//!
//! - [`SecretSource`] / [`AccessJournal`] — the two seams a real deployment wires to the secret
//!   plugin and the journal; this module depends on neither directly.
...
//!   (mutual TLS when a client CA was resolved, server-only TLS otherwise), ported from
//!   `busbar-core`'s `tls::build_server_config`.
//! - [`issue_handle`] — the thin wrapper over `TransportKeyHandle::issue` this step is the one
//!   caller of.
//!
//! ## What is deliberately absent
//!
//! The listener accept loop, the hyper/axum connection-serving machinery, and the end-to-end
//! TLS wire tests that drive them with a real TCP client are a LISTENER concern, not a
//! key-resolution concern. See the test module doc for the exact list of `busbar-core::tls` tests
//! this leaves unported and why.
...
pub fn provision_server(
    ...
    sink.register_config(TransportConfigHandle::issue(
        token,
        slot.index,
        ConfigRole::Listen,
        Arc::new(config),
