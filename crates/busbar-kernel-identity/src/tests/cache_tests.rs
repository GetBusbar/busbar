...
/// jitter hex SHA-256 would for a fixed credential, so swapping in the real digest
...
fn sha256_digest_jitter_matches_1_5_5s_hex_sha256() {
    ...
    // 1.5.5's hex SHA-256 (the former hex SHA-256), computed inline so this crate does
    // not name the retiring legacy contract crate (#35): hex-encoded SHA-256 is exactly what it was.
    let reference = {
    ...
    assert_eq!(
        ours, reference,
        "Sha256Digest must be byte-for-byte 1.5.5's hex SHA-256"
    );
