//! The `openai` dialect of the `llm` plane — one wire vocabulary, and nothing else.
//!
...
//! vendor's, the row that says where this vendor keeps the model, the client's response ceiling and
//! the four metered quantities, the credential its clients present and the egress scheme its
//! upstreams expect. All of it is a constant, all of it is read once at registration, and none of it
//! is spelled anywhere in `busbar-plane-llm`.
//!
...
//! A dialect crate that reimplemented `read_request` would be a second OpenAI Chat Completions
//! implementation, and the only thing that could ever be said about it is that the two agreed on
...
//!
//! ## Why `openai` and `responses` are two crates and not one
//!
//! They are two dialects, and the tree says so in three places that agree. The plane's ladder gives
//! them different rungs (7 and 14 here, 10 there) matching different paths. Their location rows
//! differ in every field that can differ — the conversation is at `/messages` here and `/input`
//! there, the response ceiling is `/max_tokens` (plus a newer spelling) here and
//! `/max_output_tokens` there, and the four metered quantities are at four different pointers. And
//! the codec crate carries them as two sibling modules with two readers and two writers. One crate
//! spanning both would be a crate with a vendor-shaped branch in it, which is the shape the kind
//! exists to remove. `busbar-plane-llm-responses` is the second crate, and it follows this one.
...
/// The `openai` dialect, holding the plane it translates for.
///
