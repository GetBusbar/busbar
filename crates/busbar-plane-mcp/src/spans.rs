...
//! identifier, the parameters, the tool name and its arguments. A pointer that names an array
//! element resolves to nothing rather than to a guess.
//!
...
/// "the client sent no arguments" and "the client sent empty arguments" are different facts, and the
/// loop settles them differently, so they must not arrive here as the same answer.
#[must_use]
...
mod tests {
    ...
    fn the_request_pointers_resolve() {
        let body = br#"{"jsonrpc":"2.0","id":7,"method":"tools/call","params":{"name":"search","arguments":{"q":"x"}}}"#;
        let found = resolve(
            body,
            &["/method", "/id", "/params/name", "/params/arguments"],
        );
        let at = |p: &str| {
            found
                .iter()
        ...
        assert_eq!(at("/method"), Some(&br#""tools/call""#[..]));
        assert_eq!(at("/id"), Some(&b"7"[..]));
        assert_eq!(at("/params/name"), Some(&br#""search""#[..]));
        assert_eq!(at("/params/arguments"), Some(&br#"{"q":"x"}"#[..]));
    }
    ...
    fn an_absent_pointer_is_absent() {
        ...
        let found = resolve(body, &["/params/name"]);
    ...
    fn an_array_element_does_not_answer_for_an_object_member() {
        let body = br#"{"result":{"content":[{"name":"inner"}]}}"#;
        let found = resolve(body, &["/params/name"]);
        assert!(found.is_empty());
    }
    ...
    fn the_same_bytes_give_the_same_spans() {
        let body =
            br#"{"jsonrpc":"2.0","id":"a","method":"resources/read","params":{"uri":"file:///x"}}"#;
        let first = resolve(body, &["/method", "/id", "/params/uri"]);
        let second = resolve(body, &["/method", "/id", "/params/uri"]);
