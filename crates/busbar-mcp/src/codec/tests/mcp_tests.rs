...
/// The registry row names the plane's own key.
///
/// The other half of this agreement — that the plane kind's `KEY` is that same string — is
/// asserted in `busbar-plane-mcp`'s `src/tests/claims.rs::the_plane_key_is_the_codecs_own`. One
/// constant, named once in the plane crate, read here.
#[test]
fn the_declaration_registers_under_the_planes_key() {
    assert_eq!(DECL.name, busbar_plane_mcp::PLANE_KEY);
}

// THE NOTIFICATION VOCABULARY AND ITS TESTS MOVED TO `busbar-plane-mcp` — a notification is a wire
// dialect, which definition 16–20 puts in the plane crate. They are `src/tests/codec_tests.rs`
// there, under the same test names, reading the same `McpNotification` this crate no longer holds.
