...
//! live in [`busbar_kernel::net_guard`], because they were written twice — once here and once for the A2A
...
//! rendering of [`busbar_kernel::net_guard::AddressRefusal`] rather than a second decision about it.
//!
...
impl SsrfPolicy {
    ...
    /// that already sent a credential. The body ceiling is `busbar_kernel::proxy`'s streaming cap, applied
...
pub(crate) enum SsrfRefusal {
    ...
    /// it refuses the first 3xx outright and streams its body under `busbar_kernel::proxy`'s cap.
    ///
...
/// [`busbar_kernel::net_guard`]'s; what is here is the URL this path starts from and the wording it reports
/// in.
pub(crate) async fn pin_upstream(
    ...
    Ok(net_guard::resolve_and_pin(
        &host,
        port,
        https,
        &net_guard::SystemResolver,
        policy.guard(),
    )?)
}
...
/// restated here.
// NO PRODUCTION CALLER: the pin path judges through `busbar_kernel::net_guard` directly, which is the whole
