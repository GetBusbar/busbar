...
//! `busbar_kernel::trust::Approval::state` disagreeing with the last observation, which is the shared trust
...
use busbar_kernel::trust::{Approval, Observation, PinnedArtifact, Sighting, TrustState};
...
/// it (`busbar_kernel::trust` explicitly does not interpret `mechanism()`), so a per-mechanism special case
...
impl busbar_kernel::trust::declared::Declares for TransportPin {
...
pub(crate) struct ServerCatalogue {
    ...
    pub(crate) ledger: busbar_kernel::trust::reverify::Ledger,
}
...
impl ServerCatalogue {
    ...
    pub(crate) fn registered(id: ServerId) -> Self {
        Self {
            ...
            ledger: busbar_kernel::trust::reverify::Ledger::default(),
        }
    }
    ...
    #[allow(dead_code)]
    pub(crate) fn drift(&self) -> busbar_kernel::trust::Drift {
