...
//! has to close it.
//!
//! The gate is [`busbar_kernel::egress_auth::gate::authorise`], reached here through
...
/// THE EGRESS GATE FOR THIS PLANE: [`busbar_kernel::egress_auth::gate::authorise`] with MCP's grant kind and
...
/// [`busbar_kernel::egress_auth::gate`] — the same extension point the A2A plane uses for its own — so the
...
pub(crate) fn authorise_server_egress(
    ...
    busbar_kernel::egress_auth::gate::authorise(caller, subject, 0)
        .map(|_witness| ())
        .map_err(|refusal| {
