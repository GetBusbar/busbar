...
///
/// The boot replay exists so that a quarantine survives a restart and a restart does not silently
/// re-open it. The replay decoded each durable row and, on a row that would not decode, ran
/// `continue` — no log line, no count. So corrupting ONE row was a supported and untraceable way to
/// un-quarantine a drifted upstream across the next restart, and the durable store is exactly the
/// surface an attacker with write access reaches.
///
