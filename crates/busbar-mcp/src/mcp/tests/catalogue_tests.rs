...
fn grant_of(pairs: &[(&str, &str)]) -> busbar_contract::records::VirtualKey {
...
fn seeing(key: &busbar_contract::records::VirtualKey) -> busbar_kernel::catalogue::Caller<'_> {
...
fn two_grants_see_two_different_catalogues_and_a_third_sees_none() {
    ...
    let names = |g: &busbar_contract::records::VirtualKey| {
...
/// not a pin.
///
/// The rule is [`busbar_kernel::trust::declared`]'s now, and it used to be this plane's alone: the sibling
...
fn a_call_resolved_under_generation_n_is_refused_when_the_live_generation_is_n_plus_1() {
    ...
    // ordered validator in `busbar_kernel::trust::validate` compares it LAST, deliberately: "something
...
fn no_principal_sees_another_principals_prompts_resources_or_templates() {
    ...
    type ReadSurface = fn(&Catalogue, &busbar_kernel::catalogue::Caller<'_>) -> Vec<String>;
...
///
/// the engine's neutral catalogue (`busbar_kernel::catalogue`) refuses an item that requires NO grant, and every MCP entry type requires two.
...
fn a_key_that_is_no_longer_live_sees_nothing_on_any_surface() {
    ...
    for (what, mutate) in [
        (
            "deleted",
            (|k: &mut busbar_contract::records::VirtualKey| k.deleted_at = Some(1))
                as fn(&mut busbar_contract::records::VirtualKey),
        ),
        (
            "disabled",
            |k: &mut busbar_contract::records::VirtualKey| k.enabled = false,
        ),
        ("expired", |k: &mut busbar_contract::records::VirtualKey| {
            k.expires_at = Some(1)
        }),
    ] {
        let mut gone = live.clone();
        mutate(&mut gone);
        let asked = busbar_kernel::catalogue::Caller {
            key: Some(&gone),
            now: 100,
            generation: busbar_kernel::trust::validate::Generations::at_admission(1),
        };
        ...
        assert_eq!(
            busbar_kernel::catalogue::judge(cat.tools_for(&seeing(&live))[0], &asked, &(),)
                .unwrap_err()
                .audit_reason(),
            busbar_contract::vocab::REASON_IDENTITY_NOT_LIVE,
