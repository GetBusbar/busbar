...
/// `busbar-plane-mcp` so a pure plane kind can read the dispatch list, and this name is on it. The
...
pub(crate) use busbar_plane_mcp::codec::METHOD_SUBSCRIPTIONS_LISTEN;
...
/// [`busbar_contract::records::VirtualKey::scope_allowed`] — which reads `allowed_scopes` alone and looks at
...
fn caller_of<'a>(
    host: &std::sync::Arc<dyn busbar_kernel::plane_host::EngineHost>,
    key: Option<&'a std::sync::Arc<busbar_contract::records::VirtualKey>>,
    generation: u64,
...
pub(crate) fn listen(
    ...
    // `crate::codec::subscribe` states: a hand-read `params.notifications.toolsListChanged` accepts
