...
impl Transport {
    ...
    /// [`busbar_contract::transport::transport::Transport`] is the engine's dispatch axis. Collapsing them would tie a
    /// wire word an operator has already written to an enum the engine is free to reshape.
    pub(crate) fn axis(self) -> busbar_contract::transport::transport::Transport {
        match self {
            Transport::StreamableHttp => busbar_contract::transport::transport::Transport::Http,
            Transport::Stdio => busbar_contract::transport::transport::Transport::Stdio,
        }
    }
}
...
/// closed set is silently DROPPED — [`super::callerask::CallerAsk::capability_key`] returns `None`
/// and the round's filter removes the entry. So an operator who typed `elicitation/created` did not
/// get a broken confirmation gate, they got NO confirmation gate: the destructive tool the gate was
...
///
/// Boot is the only place an operator is present to be told, and #42 is the rule: an unknown is
/// REFUSED, never silently defaulted. The set is [`super::callerask::ASK_METHODS`] — the same three
/// strings dispatch matches on, READ rather than re-spelled, so a fourth method the protocol grows
    ...
    for (round, entries) in rounds.iter().enumerate() {
        for (key, entry) in entries {
            if !super::callerask::ASK_METHODS.contains(&entry.method.as_str()) {
                return Err(format!(
                    "{at}: `{family}.{capability}.{field}[{round}].{key}.method` is `{}`, which is \
                     not one of the three client-side methods an ask may name ({}). A method \
                     outside that set names no capability a caller could declare, so it would be \
                     dropped at dispatch and this confirmation gate would never be shown.",
                    entry.method,
