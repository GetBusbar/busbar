...
impl crate::trust::net::Resolver for Answering {
...
impl crate::trust::lane::BreakerView for AllAdmitted {
    ...
    fn try_admit(
        ...
        _lane: usize,
        _now: u64,
    ) -> Result<(), crate::trust::lane::Unavailable> {
        Ok(())
    }
}

/// The query those tests ask through.
pub(crate) fn admitting() -> crate::trust::lane::BreakerQuery<'static> {
    static BREAKER: AllAdmitted = AllAdmitted;
    crate::trust::lane::BreakerQuery {
        breaker: &BREAKER,
        pool: "p",
        now: 0,
    }
...
impl KindFacts for AllYes {
    fn net_guard_passes(&self, dest: &DestinationFacts) -> bool {
        ...
        !matches!(
            crate::trust::net::check_destination_facts(
                dest,
                &[],
                &Answering(address),
                crate::trust::net::GuardPolicy::default(),
                &crate::trust::net::Denylist::default(),
            ),
            Err(crate::trust::net::NetworkRefusal::MetadataDenied(_)
                | crate::trust::net::NetworkRefusal::Guard(_))
        )
    }
...
fn an_upstream_needs_every_conjunct_of_its_rule() {
    ...
    let at = crate::trust::lane::BreakerQuery {
        breaker: &open,
        pool: "p",
        now: 7,
    };
