...
//! Clause two, three and five: what a posting stores, how it projects, and the one amount it
//! charges.

use super::*;
use crate::{cents_of, micros_of, LaneClass, RateCard, FEE_CLASS};

/// The priced amount is the sum over the posting's lines INCLUDING the fee line, and each line
/// records the rate it was priced at. Three input tokens at two thousand nano-units and four
/// output tokens at five thousand is twenty-six thousand; a three-cent fee is thirty million more.
///
/// ONE FIGURE AND NOT TWO. There is no pre-tier sum to compare this against any more: a tier is a
/// SCOPE that selects the terms the fee line is built from, so the sum over the lines IS the
/// charge.
#[test]
fn the_priced_amount_is_the_sum_of_the_lines_including_the_fee() {
    let c = card("m", 2.0, 5.0, 3);
    let posted = priced(&c, "m", &usage(&[(INPUT, 3), (OUTPUT, 4)]), 1);
    ...
    assert_eq!(posted.priced_nanos, 30_026_000);
...
fn with_no_card_every_class_prices_at_zero_and_the_fee_still_posts() {
    ...
    assert_eq!(posted.priced_nanos, 150_000_000, "five fees of 3 cents");
...
fn a_class_a_present_card_does_not_name_prices_at_zero_and_is_flagged() {
    ...
    let posted = priced(&c, "m", &usage(&[(INPUT, 3), ("web_search", 3)]), 0);
    assert_eq!(posted.priced_nanos, 6_000, "the unnamed class adds nothing");
...
fn a_lane_absent_from_a_present_card_prices_at_zero_and_reports_it() {
    ...
    let posted = priced(&c, "mystery", &usage(&[(INPUT, 1_000_000)]), 1);
    assert!(posted.lane_unpriced);
    assert_eq!(posted.priced_nanos, 20_000_000, "only the flat fee remains");
}
...
fn an_adversarial_class_name_cannot_collide_with_the_fee_line() {
    ...
    let posted = priced(&c, "m", &usage(&[(INPUT, 3), (FEE_CLASS, 5)]), 1);
    // 3 x 2000 + 5 x 10 + one 3-cent fee.
    assert_eq!(posted.priced_nanos, 6_000 + 50 + 30_000_000);
    assert_eq!(
        posted.lines.len(),
        4,
...
fn each_class_bills_against_its_own_rate() {
    ...
    assert_eq!(posted.priced_nanos, 15_000_000_000);
...
fn the_oracle_rate_card_reproduces_its_pinned_figure() {
    ...
    let posted = priced(&c, "oracle", &usage(&[(INPUT, 11), (OUTPUT, 7)]), 0);
    assert_eq!(posted.priced_nanos, 2_500_000_000, "two and a half units");
...
fn the_cent_projection_truncates_toward_zero() {
    ...
    let at = |tokens: u64| priced(&c, "m", &usage(&[(INPUT, tokens)]), 0).minor();
...
fn both_projections_saturate_rather_than_wrap() {
    ...
    let posted = priced(&c, "m", &usage(&[(INPUT, u64::MAX)]), 0);
...
fn the_nano_scale_keeps_sub_micro_precision() {
    ...
    let posted = priced(&c, "m", &usage(&[(INPUT, 8)]), 0);
...
fn an_explicit_zero_rate_is_known_and_bills_nothing() {
    ...
    assert_eq!(posted.priced_nanos, 0);
...
fn the_estimated_mark_travels_onto_the_posting() {
    let c = card("m", 1.0, 0.0, 0);
    let reported = priced(&c, "m", &usage(&[(INPUT, 10)]), 0);
    let floored = priced(&c, "m", &estimated_usage(&[(INPUT, 10)]), 0);
    assert!(!reported.estimated);
    assert!(floored.estimated);
    assert_eq!(floored.priced_nanos, reported.priced_nanos);
}
