...
use crate::{KeyBudgetView, LaneClass, RateCard, RateCardVersion, NANOS_PER_CENT};
...
fn correcting_the_rate_moves_the_answer_without_touching_the_ledger() {
    ...
    let l = lines(&[(INPUT, 1_000_000)]);
    let v = KeyBudgetView::capped("group:team@month", 10_000);
    let cheap = RateCard::from_micro_rates(
        RateCardVersion::new("v1"),
        [(LaneClass::new("blue", INPUT), 10.0)],
        0,
    );
    let dear = RateCard::from_micro_rates(
        RateCardVersion::new("v2"),
        [(LaneClass::new("blue", INPUT), 20.0)],
        0,
    );
    assert_eq!(
