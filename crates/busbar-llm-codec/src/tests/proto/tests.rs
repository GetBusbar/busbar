...
// The `residual_dialect_for_path` conformance tests (the `/v1/models/{id}` colon disambiguation and
// the "names no dialect → None" cases) RELOCATED to `busbar-llm` (`tests/detect_tests.rs`): they name
// dialects, and they now exercise the generic residual fold through the plugin's own
// `residual_claims` predicates.
...
fn synth_anthropic_request_id_msd_is_uniform() {
    ...
    for _ in 0..N {
        ...
        for &msd in &[bytes[0], bytes[12]] {
            total += 1;
            if crate::dialect::BASE62_ALPHABET[..29].contains(&msd) {
                low += 1;
            }
        }
    }
...
fn test_requires_max_tokens_per_protocol() {
    for (name, want) in [
        ...
        assert_eq!(
            busbar_kernel::proto::decl_for(name).is_some_and(|d| d.requires_max_tokens),
            want,
...
fn test_roundtrip_identity() {
    let protocol = crate::proto_codec::protocol_for("anthropic").expect("anthropic should exist");
...
fn cache_breakpoints_gated_by_lane_capability_on_bedrock() {
    ...
    assert!(busbar_kernel::proto::decl_for("bedrock").is_some_and(|d| d.cache_markers_model_gated));
    assert!(
        busbar_kernel::proto::decl_for("anthropic").is_some_and(|d| !d.cache_markers_model_gated)
    );
    ...
    let prep = |allowed: bool| busbar_contract::ir::egress_prep::EgressPrep {
    ...
    let mut req = ir;
    crate::chat_handle::chat_prepare_for_egress(&mut req, &prep(false));
    let ir = req;
    ...
    let ir = anthropic.reader().read_request(&body).unwrap();
    let mut req = ir;
    crate::chat_handle::chat_prepare_for_egress(&mut req, &prep(true));
    let ir = req;
    let wire = bedrock.writer().write_request(&ir);
    assert!(
        contains_cache_point(&wire),
...
fn test_registry_resolves_anthropic() {
    ...
    assert!(crate::proto_codec::protocol_for("nonexistent").is_none());
}
...
fn test_irerror_bridge() {
    ...
    assert_eq!(
        busbar_kernel::breaker::classify(&ir_error),
...
fn test_registry_has_both_protocols() {
    // Both should exist
    assert!(
        crate::proto_codec::protocol_for("anthropic").is_some(),
        "anthropic should exist"
    );
    assert!(
        crate::proto_codec::protocol_for("openai").is_some(),
        "openai should exist"
    );

    // Verify openai writer path
    let openai = crate::proto_codec::protocol_for("openai").expect("openai should exist");
    assert_eq!(openai.writer().upstream_path(), "/v1/chat/completions");

    // Verify anthropic writer path
    let anthropic = crate::proto_codec::protocol_for("anthropic").expect("anthropic should exist");
...
/// ingress with NO canonical prefix, so `crate::proto_codec::native_tool_id_prefix("cohere")` must be `None` (like
...
fn cohere_tool_ids_pass_through_verbatim_no_decode() {
    ...
    assert_eq!(crate::proto_codec::native_tool_id_prefix("cohere"), None);
...
fn write_sse_frame_pins_both_framings_byte_for_byte() {
    ...
    let mut out = Vec::new();
    crate::dialect::write_sse_frame(&mut out, "x", &data);
    assert_eq!(out, b"event: x\ndata: {\"a\":1}\n\n");

    let mut out = Vec::new();
    crate::dialect::write_sse_frame(&mut out, "", &data);
...
mod ir_property_tests {
    ...
    fn test_anthropic_request_decode_assertions() {
        ...
        // fixture cannot fake
        let protocol =
            crate::proto_codec::protocol_for("anthropic").expect("anthropic should exist");
        let reader = protocol.reader();
    ...
    fn test_openai_write_response_event_error() {
        let writer = OpenAiWriter;
        let err = busbar_contract::protocol::IrError {
    ...
    fn assert_stripped_ok(input: &str) -> String {
        let out = crate::dialect::strip_top_level_usage_member(input)
    ...
    fn strip_usage_never_corrupts_string_content() {
        ...
        assert!(
            crate::dialect::strip_top_level_usage_member(input).is_none(),
    ...
    fn strip_usage_no_usage_key_returns_none() {
        ...
        assert!(crate::dialect::strip_top_level_usage_member(input).is_none());
    }
}
