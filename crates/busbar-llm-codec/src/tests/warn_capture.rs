...
//! THIS CRATE'S OWN WARN-CAPTURE FIXTURE (#83a SD-3): the plane's tests assert a coded diagnostic
//! fired without reaching the host crate for a test kit. Test-only (`#[cfg(test)]` at its one `mod`
//! line). No other capture layer is ever built in this crate's test binary, so the process-global
//! capture gate below is one gate per binary.
//!
