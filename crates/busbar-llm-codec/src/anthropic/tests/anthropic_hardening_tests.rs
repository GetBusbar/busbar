...
// `status_word_golden` (class -> derived `error.type` word) removed: it pinned the
// `stream_error_type` class-derivation that M16 removes from the mid-stream error writer. The
// derived word is no longer written to the wire for `IrStreamEvent::Error` — 1.5.5 parity requires
// the provider signal verbatim (see `write_response_event_error_type_carries_the_signal_verbatim`
// and `write_response_event_error_null_type_when_signal_absent`, which pin the CORRECTED behavior).
