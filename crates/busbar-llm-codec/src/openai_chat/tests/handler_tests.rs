...
fn transcription_usage_duration_round_trips() {
    ...
    assert_eq!(
        ...
        Some(Billing::Duration {
            seconds: busbar_contract::Count::parse("1").expect("1 is exact"),
        })
    );
    let back: Value = serde_json::from_slice(
...
fn transcription_usage_tokens_round_trips() {
    ...
    let ir = crate::ir::audio::TranscriptionResp {
        text: "hi".into(),
        usage: Some(Billing::Tokens(busbar_contract::billing::TokenUsage {
            input: 11,
            output: 3,
            ..Default::default()
        })),
...
fn speech_write_response_decodes_b64_payload_to_raw_bytes() {
    ...
    let b64 = busbar_contract::media::base64_encode(raw);
