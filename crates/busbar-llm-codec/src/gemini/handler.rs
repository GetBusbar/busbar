...
pub fn write_transcription_request(r: &crate::ir::audio::TranscriptionReq) -> Bytes {
...
pub fn write_transcription_response(r: &crate::ir::audio::TranscriptionResp) -> WireBody {
    let mut body = json!({
        "candidates": [{
...
pub fn write_speech_request(r: &crate::ir::audio::SpeechReq) -> Bytes {
...
pub fn write_image_request(r: &crate::ir::image::ImageReq) -> Bytes {
...
pub fn read_speech_response(wire: &[u8]) -> Result<crate::ir::audio::SpeechResp, CodecError> {
    ...
    if let Ok(v) = serde_json::from_slice::<Value>(wire) {
        ...
        // pointing at index 0 missed the audio for that shape and refused, as "carries no
        // inlineData", a response that did carry synthesis. Both spellings are accepted for the same
        // reason `read_transcription_request` accepts both.
        let inline = v
            .pointer("/candidates/0/content/parts")
            .and_then(Value::as_array)
            .and_then(|parts| {
