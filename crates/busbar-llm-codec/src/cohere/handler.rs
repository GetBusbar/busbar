...
pub fn write_rerank_request(r: &crate::ir::rerank::RerankReq) -> Bytes {
    let mut body = json!({
        "model": r.model,
        "query": r.query,
...
/// `(rerank, cohere)` key — G6 A4b option-a). Byte-identical to the pre-cutover inline write.
pub fn write_rerank_response(r: &crate::ir::rerank::RerankResp) -> WireBody {
...
pub fn read_rerank_results(v: Option<&Value>) -> Vec<crate::ir::rerank::RerankResult> {
    v.and_then(Value::as_array)
        .map(|a| {
            a.iter()
                .filter_map(|x| {
