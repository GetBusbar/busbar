...
//!
//! [`drain_frames_checked`](crate::eventstream::drain_frames_checked) is the production DECODER — just enough to pull `(event_type, payload)`
...
//! `InternalServerException` mid ConverseStream). For those frames [`drain_frames_checked`](crate::eventstream::drain_frames_checked) returns the
...
//!
//! [`encode_frame`](crate::eventstream::encode_frame) is the production ENCODER (the exact inverse of [`drain_frames_checked`](crate::eventstream::drain_frames_checked)) used for
...
/// gated to this crate's test builds (the Bedrock dialect's tests are its callers) to avoid an
/// unused-function warning in a shipped binary.
#[cfg(test)]
