...
/// Rungs 7 and 14 are one carved-out dialect's now, rung 10 a second one's, rungs 2, 4 and 11 a
/// third's and rungs 3, 5 and 6 a fourth's, declared in `busbar-plane-llm-openai`,
/// `busbar-plane-llm-responses`, `busbar-plane-llm-anthropic` and `busbar-plane-llm-gemini` and
/// merged back in at registration, so reading the constant would assert about a third of the
/// ladder and call the gaps it found correct.
///
...
///
/// ASKED OF THE PLANE, not of this crate's own constant: nine of the fourteen rungs below are a
/// registered dialect's, and the answer a request gets is the merged walk's answer. That the answers are
