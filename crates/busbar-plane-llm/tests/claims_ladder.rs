...
/// Rungs 7 and 14 are one carved-out dialect's now, rung 10 a second one's and rungs 2, 4 and 11 a
/// third's, declared in `busbar-plane-llm-openai`, `busbar-plane-llm-responses` and
/// `busbar-plane-llm-anthropic` and merged back in at registration, so reading the constant would
/// assert about part of the ladder and call the gaps it found correct.
///
...
///
/// ASKED OF THE PLANE, not of this crate's own constant: six of the rungs below are a registered
