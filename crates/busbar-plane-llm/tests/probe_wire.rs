...
use busbar_contract::unit::{Clock, Ctx, TransportView};
use busbar_plane_llm::{LlmPlane, Upstream};

/// The stack under a probe: none.
///
/// A probe is raised by the node's own clock, not by bytes that arrived, so there is no connection
/// beneath it and no request target it was read off. Handing the plane a wire fixture here would
/// state the opposite — and it would also be the one thing in this file that could make the answer
/// depend on something a probe does not have. It names no wire, which is the point: the reason this
/// file spells no transport's name is that a probe genuinely has none, not that the name was
/// avoided.
struct NoArrival;

impl TransportView for NoArrival {
...
/// A context with the clock the harness uses and nothing under it but the arena.
fn probe_ctx<'u>(
    arena: &'u harness::LeakArena,
    config: &'u harness::EmptyConfig,
    transport: &'u NoArrival,
    labels: &'u Labels<'u>,
) -> Ctx<'u> {
    Ctx::new(
        Clock {
...
fn the_probe_this_plane_writes_is_the_probe_the_engine_sent() {
    ...
    let transport = NoArrival;
    let labels = Labels::default();
    let ctx = probe_ctx(&arena, &config, &transport, &labels);
