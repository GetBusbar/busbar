...
fn key_is_the_declared_meta_key_and_is_lowercase() {
    let key = subject().key();
    assert_eq!(key, <GrpcTransport as TransportMeta>::KEY);
    assert!(!key.is_empty());
...
fn status_namespace_is_present_exactly_when_a_status_class_is() {
    ...
    // names neither.
    assert_eq!(
        <GrpcTransport as TransportMeta>::STATUS_CLASS.is_some(),
        <GrpcTransport as TransportMeta>::STATUS_NAMESPACE.is_some(),
    );
}
...
fn composition_claim_is_the_registration_claim() {
    ...
    assert_eq!(
        ...
        busbar_transport_grpc::claims::COMPOSES_OVER,
    );
}
