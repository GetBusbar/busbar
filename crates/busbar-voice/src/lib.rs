...
/// THE DUPLEX IR AND THE DIALECT CODECS, RE-EXPORTED FROM `busbar-streams-codec`.
///
/// The plane-4 session intermediate representation (media, control, events, tools, session config,
/// usage), the shared duplex reader/writer and the Gemini Live dialect live in `busbar-streams-codec`
