...
/// THE NEGOTIATED PCM WIRE FORMAT (§2.3 audio-format field). Two dialect-normalized shapes:
...
/// barge-in truncate math (§2.3) — because raw PCM carries no timestamps.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
...
/// THE PURE BARGE-IN TRUNCATE HELPER (§2.3). Given the total bytes of downlink audio the plane has
