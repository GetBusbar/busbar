...
/// the message is core's, so only the message stayed. The NAME is unchanged, so every call site and
/// every design-inventory citation still reads; what moved is the qualifier.
///
/// TWO PIECES OF SHIPPED PROSE STILL SPELL IT `Scope::parse_ceiling` — this module's own
/// `v1/json/openapi.json`, in the ceiling token's field description, and `busbar-substrate`'s
/// `api.rs` doc comment — and both are LEFT ALONE on purpose. The first is a frozen v1 body byte
/// that the recorded operator-surface families replay, and a landing that is a move does not move a
/// recorded byte; the second is inside the frozen crate. Neither is a call site, and the error TEXT
/// they both quote is produced here and is unchanged to the byte.
...
/// string precisely so it carries no web-framework dependency at all, and turning one into the other
/// is a transport adapter's job rather than a second reading of the rule.
