...
//
// WHY THE PARAGRAPH ABOVE IS FROZEN, AND WHY THIS ONE IS NOT A DOC COMMENT. This type's doc comment
// IS the `description` of its schema in the served `openapi.json`, and that document is a 1.5.5
// wire surface judged as a JSON SUPERSET of the published one: a description that grows is not a
// new path beside the old, it is an existing string leaf whose value changed, and no register entry
// can forgive that without forgiving every other description with it. So the published paragraph
// above stands byte-for-byte and the 1.6.0 story lives down here, where maintainers read it and no
// client does.
//
...
// (`phase` + `fires_at`) close that. All three are declared AFTER `global`, at the END of the
// struct, because the schema's `required` array follows declaration order and the published
// `required` must stay a PREFIX of this one for the growth to read as growth.
#[derive(Debug, Clone, Serialize)]
#[cfg_attr(feature = "openapi-schema", derive(schemars::JsonSchema))]
pub(crate) struct HookView {
    ...
    // FROZEN PUBLISHED PROSE — see the note on the struct: this one line is the field's
    // `description` in `openapi.json`, so the 1.6.0 reading guidance lives here instead of in the
    // doc comment. `at` is the LEGACY single-valued tap stage, kept for back-compat and NOT the
    // field to read: `null` here does NOT mean "a gate" and does not mean "unscoped". Every hook
    // written in the current top-level `hooks:` grammar has `at: null` by construction
    // (`config::hook_cfg_from_def` never sets it, and `--migrate-config` rewrites a legacy `at:`
    // into `phase:`), so this field is `null` for essentially every hook a running deployment has.
    // Read `fires_at`. 1.6.0 owner rule: a 1.5.5 client reading this view must see this field
    // byte-for-byte, so it is RESTORED alongside `phase`/`fires_at`.
    ...
    // ── 1.6.0 GROWTH, DECLARED LAST ───────────────────────────────────────────────────────────
    // Everything above this line is the published 1.5.5 view, in the published order. Everything
    // below is new, and it is below precisely so the schema's `required` array grows only at its
    // tail: an entry inserted mid-list moves every following name's index and stops being growth.
