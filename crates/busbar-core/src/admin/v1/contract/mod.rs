...
/// The built-in authorization scopes and the effective authority a principal holds.
///
/// DEFINED IN `busbar-unit-scope`, not here. Deciding whether a held grant satisfies a requirement
/// is what a unit does, and this crate held a second, independent encoding of the same two-rung
/// chain — the same variants, the same `allows`/`meet`/`dominates` arithmetic, the same `Grants`
/// bitset — which the unit's own doc comment described as a verbatim port of this one. Two encodings
/// of one authorization rule is a rule that can be tightened in one place and not the other, on the
/// one surface where that is least survivable, and the two had in fact already drifted (the unit
/// asked whether the method was `POST` before granting the dry-run exemption; this copy never did).
///
/// Re-exported under the names every in-crate caller already spells, so the deletion moved where the
/// rule LIVES without moving a single call site.
pub use busbar_unit_scope::{Grants, Scope};
...
/// used to disagree — the API accepted any string (the write path only ran the `serde` type-check,
/// and `Option<String>` accepts every string), persisted it, answered 200, and the gateway then
/// refused to BOOT on the next restart with "unknown max_admin_scope". A successful admin write that
/// leaves the deployment unbootable is the failure mode a second copy of the accepted-value list
/// buys you; there is now only one copy.
///
/// A free function here rather than a method on `Scope`, because the ACCEPTED VALUES are the unit's
/// (`Scope::parse`, one copy) while the MESSAGE names this crate's own configuration default. Only
/// the message is core's, so only the message stayed.
pub(crate) fn parse_scope_ceiling(subject: &str, token: &str) -> Result<Scope, String> {
...
/// collapse): every read (`GET`/`HEAD`) plus the two stateless dry-run paths (`config/validate`,
...
///
/// THE MATRIX ITSELF IS THE UNIT'S. What is left here is the one thing that is genuinely this
/// crate's: the `axum::http::Method` this surface's handlers are handed. The unit takes a plain
/// string precisely so it carries no HTTP-framework dependency, and turning one into the other is a
/// transport adapter's job rather than a second reading of the rule.
...
/// DEFINED IN `busbar-contract`, not here — as `PluginError`, the ONE structured error a face
/// carries when it refuses a caller.
///
/// The frozen v1 taxonomy is ten variants, ten `code` strings, ten statuses and the two-key envelope
/// they render into, and it used to be split: the SHAPE was rendered in the administrative plane
/// (whose own doc comment said the shape lived there and nowhere else, precisely so a second
/// rendering could not drift from the first) and the CONTENT was this enum, here, which rendered the
/// shape a second time out of its own `json!`. Shape in one crate and content in another is that
/// same split with an extra seam in it: the `code` a condition renders under is as much the frozen
/// contract as the brace placement is, and neither half means anything without the other.
///
/// The two halves go together, and they go to the CONTRACT rather than to the plane. A taxonomy
/// homed in `busbar-plane-admin` would make this crate name a plane crate to refuse a caller, which
/// is plane-specific knowledge in the one crate this release exists to empty; the plane-purity rule
/// refuses it by name and correctly. Homed in the contract, the engine, the plane and the
/// composition root all reach one taxonomy and none of them reaches another.
///
/// Re-exported under the name every in-crate caller already spells.
pub use busbar_contract::PluginError as AdminError;
...
/// has never needed anything else about it and the contract may not name a unit crate to obtain one.
/// This is the one line that bridges the two: the scope model is the unit's, the token is what the
