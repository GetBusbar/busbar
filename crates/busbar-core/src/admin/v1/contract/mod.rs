...
/// config, config_validate) can name it without reaching into the admin surface. Re-exported here
/// so every in-admin caller (and the admin-worded parts below that build on it) is unchanged.
///
