...
//!
//! `<section>` is `/identity-providers` or `/export` today. **Adding `tools:` (1.6.0 MCP) or
//! `agents:` (1.6.0 A2A) is ONE variant on [`NamedMapSection`]** — [`routes`] mounts from
...
pub(crate) struct MutatedDefView {
    #[serde(flatten)]
    def: crate::admin::v1::contract::AnyNamedDefView,
