...
///
/// It used to be the one place the envelope was CONSTRUCTED, and it was not: the admin plane renders
/// the same two keys for every refusal the composition root's loop answers, out of its own
/// `refusal::envelope_of`, whose doc comment says the shape lives there and nowhere else. It did not.
/// A `json!` here was a second copy of a wire shape a client pinned, free to move without the first
/// moving — and the two had already begun to differ in what they did with a `"` in the message.
///
/// So the BODY is the error's own (`AdminError::envelope`, in the plane, beside the code and message
/// it wraps) and what happens here is the part that is genuinely this crate's: the status, the
