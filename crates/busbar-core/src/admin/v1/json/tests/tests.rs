...
/// The bytes this surface answers a refusal with are the plane's envelope, byte for byte.
///
...
/// and the content type around bytes the taxonomy renders beside the shape. This asks the two
/// directly, over the whole taxonomy INCLUDING messages a caller wrote, because the caller-supplied
/// half is where a second copy of a rendering rule shows up first.
///
...
#[tokio::test]
async fn a_refusals_bytes_are_the_planes_envelope() {
    use http_body_util::BodyExt;
    let taxonomy = [
    ...
    for error in &taxonomy {
        ...
        assert_eq!(
            bytes.as_ref(),
            busbar_plane_admin::refusal::envelope_of(error.code(), &error.message()).as_bytes(),
            "the surface\u{2019}s bytes for {error:?} are not the envelope the plane renders"
        );
