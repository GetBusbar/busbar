...
//! published cap (Cohere: 5). Before this fix, `ProtocolWriter::write_request` (via
//! `busbar_core::proto::clamp_stop`) silently truncated the list to the cap and forwarded the
//! partial set upstream with only a `tracing::warn!` — a caller relying on their 6th+ stop
...
//! same-protocol Cohere->Cohere request relays verbatim (never rebuilt from the IR), so an
//! over-cap same-protocol request is untouched and left to Cohere's own native 400.
