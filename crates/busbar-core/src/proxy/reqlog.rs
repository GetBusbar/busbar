...
//! NO tamper-evident chain — while the MCP plane chained every `tools/call` (`mcp::calllog`) and the
...
//! bounded unit is the presenting key, exactly as `mcp::calllog` is. That is also the unit an
...
//! `a2a::pushdeliver`'s pin map and `mcp::calllog` under `store: memory` are documented with, it is
...
//! [`LlmRequestLog::records_for`] have no admin verb, exactly as `mcp::calllog`'s equivalents do
...
pub(crate) struct LlmRequestLog {
    ...
    /// way `mcp::calllog`'s is, and bounded in practice by the same thing: the principals are minted
...
/// and [`crate::mcp::calllog::CALLS`], and for the same reason: a config apply must not reset the
...
impl LlmRequestLog {
    ...
    /// Poison-recovering locks, for the reason `mcp::calllog` gives: the critical sections only
    ...
    ///
    /// NO PRODUCTION CALLER — the same real gap `mcp::calllog::verify_principal_chain` names, and it
