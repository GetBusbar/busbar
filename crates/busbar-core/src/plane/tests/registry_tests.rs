...
use crate::plane::registry::{build_dispatch, builtin_plane_decls, plane_decl_for, PlaneDecl};
...
fn boot_fold(
    ...
    let facts = |rows: &[&'static PlaneDecl]| -> Vec<PlaneDeclaration> {
        rows.iter().map(|d| d.declaration()).collect()
    };
...
fn a_same_key_registration_is_skipped_and_the_first_copy_wins() {
    ...
    assert_eq!(
        *a2a_entry,
        A2A_FROM_THE_CRATE.declaration(),
...
fn every_plane_key_answers_from_its_declaration() {
    ...
    for decl in builtin_plane_decls() {
        ...
        assert_eq!(*resolved, decl.declaration());
...
pub(crate) mod seeded {
    ...
    use busbar_substrate::plane::registry as table;
