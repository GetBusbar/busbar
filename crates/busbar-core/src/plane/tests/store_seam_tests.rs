...
use busbar_api::{McpCallRecord, McpDemotionRow, StoreResult, TaskEventRow, TaskRow};
use std::sync::Arc;
...
// below is the authoritative statement of what `PlaneStore` carries. Fourteen methods, and the
// exact fourteen a plane needs to persist its own trust state.
#[allow(clippy::type_complexity)]
fn plane_method_set_is_exactly_the_plane_methods() {
    let _: fn(&dyn PlaneStore, &TaskRow) -> StoreResult<()> = <dyn PlaneStore>::put_task;
    let _: fn(&dyn PlaneStore, &str) -> StoreResult<Option<TaskRow>> = <dyn PlaneStore>::get_task;
    let _: fn(&dyn PlaneStore) -> StoreResult<Vec<TaskRow>> = <dyn PlaneStore>::list_tasks;
    let _: fn(&dyn PlaneStore, u64) -> StoreResult<u64> = <dyn PlaneStore>::purge_tasks_before;
    let _: fn(&dyn PlaneStore, &TaskEventRow) -> StoreResult<()> =
        <dyn PlaneStore>::append_task_event;
    let _: fn(&dyn PlaneStore, &str) -> StoreResult<Vec<TaskEventRow>> =
        <dyn PlaneStore>::list_task_events;
    let _: fn(&dyn PlaneStore, &McpCallRecord) -> StoreResult<()> =
        <dyn PlaneStore>::append_mcp_call;
    let _: fn(&dyn PlaneStore, &str) -> StoreResult<Vec<McpCallRecord>> =
        <dyn PlaneStore>::list_mcp_calls;
    let _: fn(&dyn PlaneStore) -> StoreResult<Vec<String>> =
        <dyn PlaneStore>::list_mcp_call_principals;
    let _: fn(&dyn PlaneStore, u64) -> StoreResult<u64> = <dyn PlaneStore>::purge_mcp_calls_before;
    let _: fn(&dyn PlaneStore, &McpDemotionRow) -> StoreResult<()> =
        <dyn PlaneStore>::put_mcp_demotion;
    let _: fn(&dyn PlaneStore) -> StoreResult<Vec<McpDemotionRow>> =
        <dyn PlaneStore>::list_mcp_demotions;
    let _: fn(&dyn PlaneStore, &str) -> StoreResult<()> = <dyn PlaneStore>::clear_mcp_demotion;
    let _: fn(&dyn PlaneStore, &str, u64, u64) -> StoreResult<bool> =
        <dyn PlaneStore>::redeem_ask_state;
}
...
impl busbar_api::Store for RecordingStore {
    ...
    // The two plane methods the forwarding test observes.
    fn put_mcp_demotion(&self, row: &McpDemotionRow) -> StoreResult<()> {
        self.demotions.lock().unwrap().push(row.clone());
        Ok(())
    }
    fn list_mcp_demotions(&self) -> StoreResult<Vec<McpDemotionRow>> {
        Ok(self.demotions.lock().unwrap().clone())
    }
...
fn plane_store_view_forwards_to_the_real_backend() {
    ...
    assert!(
        store.list_mcp_demotions().unwrap().is_empty(),
        "a fresh backend holds no demotions"
    );
    store
        .put_mcp_demotion(&McpDemotionRow {
            server: "srv-1".to_string(),
            reason: "drift".to_string(),
            recorded_at: 100,
        })
        .unwrap();
    let rows = store.list_mcp_demotions().unwrap();
    ...
    assert!(
        store.redeem_ask_state("nonce-a", 1_000, 10).unwrap(),
