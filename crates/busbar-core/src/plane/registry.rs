...
// THE POPULATION HALF — the boot fold, the process plane list, the by-key/by-section resolutions and
// the ABI index bijections — RELOCATED to `busbar_substrate::plane::registry`, so the config layer can
// ask the registry which planes exist without an edge back into core. Nothing about the mechanism
// changed: the registry is still populated only by DATA handed to `install_planes` (production) or
// `register_test_plane` (the test surface), and the neutral crate names no plane crate and no dialect.
// Core keeps the two halves it cannot hand over — [`BootCtx`], whose phase fields hold the core-live
// `App`/`AppHandle`, and [`build_dispatch`], which folds core's `PlaneDispatch` — plus one shim per
// spelling below, so every `crate::plane::registry::…` call site resolves unchanged.

/// HAND THE NEUTRAL REGISTRY CORE'S OWN BUILT-IN ROWS, in core's `#[cfg(test)]` binary only.
///
/// Core's own test binary still boots the shipped `[llm, mcp, a2a]` process list (the plane crates are
...
/// the neutral-purity lint excludes) and installed into the neutral registry through its
/// `install_builtin_planes` seam — which is what fixes the CANONICAL LAYERING ORDER regardless of the
/// order sibling test-kits happen to register in. Idempotent, and driven off every population shim
/// below so no read can beat it. In production and under `test-support` this is a no-op and the
/// registry's built-in row set is empty, exactly as it was before the relocation.
#[cfg(test)]
pub(crate) fn ensure_host_builtins() {
    static ONCE: std::sync::Once = std::sync::Once::new();
    ONCE.call_once(|| {
        busbar_substrate::plane::registry::install_builtin_planes(
...
/// The built-in declarations, at core's historical spelling. Read by the registry's own tests to build
/// a list with ONE MORE declaration in it — which is the whole of what a loader will do differently.
pub(crate) fn builtin_plane_decls() -> &'static [&'static PlaneDecl] {
    ensure_host_builtins();
...
/// INSTALL PLANE DECLARATIONS — the composition root's one write into the plane axis, at core's
/// historical spelling. The seam itself is
/// [`busbar_substrate::plane::registry::install_planes`]; `pub`, because the `busbar` binary crate is
/// the composition root and calls this from `main` (`register_planes`).
pub fn install_planes(decls: &'static [&'static PlaneDecl]) {
    ensure_host_builtins();
    busbar_substrate::plane::registry::install_planes(decls)
}

/// THE BOOT FOLD, at core's historical spelling — see
/// [`busbar_substrate::plane::registry::merged_boot_plane_decls`]. Core's own production code folds
/// nothing itself (it reads the folded process list through [`plane_decls`]); the fold's order and
/// skip rules are driven directly by core's registry tests, which is the one caller of this spelling.
#[cfg(test)]
pub(crate) fn merged_boot_plane_decls(
    installed: &[&'static PlaneDecl],
    builtins: &[&'static PlaneDecl],
) -> Vec<&'static PlaneDecl> {
    busbar_substrate::plane::registry::merged_boot_plane_decls(installed, builtins)
}

/// THE TOP-LEVEL CONFIG SECTIONS THE CONFIG DOCUMENT STILL OWNS CONCRETELY, at core's historical
/// spelling — see [`busbar_substrate::plane::registry::CORE_OWNED_CONCRETE_SECTIONS`]. Read by core's
/// registry tests, which drive the dup-claim guard against the reserved set directly.
#[cfg(test)]
pub(crate) use busbar_substrate::plane::registry::CORE_OWNED_CONCRETE_SECTIONS;
...
/// The process plane list, at core's historical spelling — see
...
/// THE ABI PLANE-KEY for a plane's stable decl `key`, at core's historical spelling — see
/// [`busbar_substrate::plane::registry::plane_key_index`].
pub(crate) fn plane_key_index(key: &str) -> u8 {
    ensure_host_builtins();
    busbar_substrate::plane::registry::plane_key_index(key)
}

/// THE SCOPE-KIND at ABI scope-kind index `idx`, at core's historical spelling — see
/// [`busbar_substrate::plane::registry::scope_kind_at`].
pub(crate) fn scope_kind_at(idx: u32) -> Option<&'static str> {
    ensure_host_builtins();
    busbar_substrate::plane::registry::scope_kind_at(idx)
}

/// THE ABI SCOPE-KIND INDEX for a kind string, at core's historical spelling — see
/// [`busbar_substrate::plane::registry::scope_kind_index`].
pub(crate) fn scope_kind_index(kind: &str) -> Option<u32> {
    ensure_host_builtins();
    busbar_substrate::plane::registry::scope_kind_index(kind)
}

/// The stable decl `key` of the plane at ABI registration index `idx`, at core's historical spelling —
/// see [`busbar_substrate::plane::registry::plane_key_at`].
pub(crate) fn plane_key_at(idx: u8) -> Option<&'static str> {
    ensure_host_builtins();
    busbar_substrate::plane::registry::plane_key_at(idx)
}

/// RESOLVE A PLANE DECLARATION BY KEY, at core's historical spelling — see
/// [`busbar_substrate::plane::registry::plane_decl_for`].
pub(crate) fn plane_decl_for(key: &str) -> Option<&'static PlaneDecl> {
    ensure_host_builtins();
    busbar_substrate::plane::registry::plane_decl_for(key)
}

/// RESOLVE A PLANE DECLARATION BY ITS CONFIG SECTION, at core's historical spelling — see
/// [`busbar_substrate::plane::registry::plane_decl_for_config_section`].
