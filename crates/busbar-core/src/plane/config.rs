...
/// EVERY TOP-LEVEL CONFIG SECTION a bare hook reference could be reaching onto — the CONTRACT's
/// section fold over the process plane list, with the substrate's [`NAMED_MAP_SECTIONS`] as the
/// trailing half: the plane sections in layering order, then the 1.5.3 named-definition maps,
/// deduped. The FOLD is the contract's; this is the process wrapper, and it is here rather than in
/// `busbar-substrate` for the reason the behaviour table is (the substrate is frozen; this glue dies
/// with `busbar-core`) and rather than in the composition root for a reason of its own: core's
/// config layer reads it (`config/mod.rs`, twice), and core cannot name a root module.
pub fn config_sections_folded() -> Vec<&'static str> {
...
// Bound for the reason the registry binds the list's readers: core's own `cfg(test)` binary seeds
// its built-in rows on read, and has no bootstrap that runs before an arbitrary test.
#[cfg(test)]
pub(crate) use crate::plane::registry::registry_tests::seeded::config_sections;
#[cfg(not(test))]
pub(crate) use config_sections_folded as config_sections;
