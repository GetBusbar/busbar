        ...
        match opened {
            Ok(mut file) => {
                if let Err(e) = writeln!(file, "{line}") {
                    busbar_substrate::diag_warn!(busbar_substrate::diagnostics::FILE_LOG_APPEND_FAILED, path = %sink.path, error = %e, "request-log file append failed; this log was dropped");
                }
            }
            Err(e) => {
                busbar_substrate::diag_warn!(busbar_substrate::diagnostics::FILE_LOG_OPEN_FAILED, path = %sink.path, error = %e, "request-log file open failed; this log was dropped");
            }
        }
    });
}
...
fn rotate(path: &str) {
    ...
    if std::path::Path::new(&oldest).exists() {
        if let Err(e) = std::fs::remove_file(&oldest) {
            busbar_substrate::diag_warn!(
                busbar_substrate::diagnostics::FILE_LOG_RETENTION_FAILED,
    ...
    for i in (1..ROTATE_ARCHIVE_LIMIT).rev() {
        ...
        if std::path::Path::new(&from).exists() {
            ...
            if let Err(e) = std::fs::rename(&from, &to) {
                busbar_substrate::diag_warn!(
                    busbar_substrate::diagnostics::FILE_LOG_SHIFT_FAILED,
    ...
    match std::fs::rename(path, &archive) {
        ...
        Err(e) => {
            busbar_substrate::diag_warn!(
                busbar_substrate::diagnostics::FILE_LOG_ROTATE_RENAME_FAILED,
                path = %path, error = %e,
