...
/// The engine's window answers, unchanged. Since the arithmetic moved to
/// the admission unit's `window` module these are no longer a test OF the computation — that suite is
/// the unit's `tests/window.rs` now, including the civil-date table this file used to carry. What
/// they pin is the thing the move had to preserve: `crate::governance::budget_window` and
/// `window_end` still answer exactly what they answered when this crate computed them itself, for
/// every window word and for the corrupt-row fall-safe. Every bucket id in this crate is built
