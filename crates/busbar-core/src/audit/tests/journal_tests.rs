...
//! Tests for the generic scope-keyed [`crate::audit::journal::Journal`], over its NEUTRAL durable
//! path — the one the shipped streams actually take (the host serves many streams and learns each
//! `kind` at register time, so it names no plane type).
//!
...
//! that this machinery is plane-agnostic.
//!
...
//!  3. **Reframe is per-record.** An undecodable stored row is skipped and counted, and an
//!     enumerated-but-empty scope is distinguished from one whose rows were all unreadable.
...
use busbar_unit_audit::legacy::chain::{
    frame_prelude, ChainLabels, ChainedRecord, Digest, Framing,
};
