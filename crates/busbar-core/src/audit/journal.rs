...
//! framing — [`busbar_unit_audit::legacy::chain::ChainedRecord`]) plus the neutral store envelope its
//! rows cross the seam in (a [`NeutralRecord`] carrying an opaque pre-framed content suffix).
//!
//! It names no plane noun: the type parameter is `R`, the scope is a `&str`, and the store is the
//! narrow [`PlaneStore`] seam. A plane's record type implements [`NeutralRecord`] plane-side, so the
//! coupling points AT core, never out of it.
//!
//! ## The write-ordering invariant, stated once
//!
//! [`Journal::append_scoped`] appends to the in-RAM chain to MINT the sequence and link, writes the sealed
...
//
// The DURABLE SEAM a plane reaches over the host vtable must NAME NO PLANE TYPE: the host serves many
// streams at once and learns each stream's `kind` at REGISTER time (runtime DATA, never a const). So
// this surface takes the `kind` as a `&str`, persists a NEUTRAL `{seq, prev_hash, hash, content}` body
// (the plane's `content` is an opaque pre-framed suffix carried verbatim), and turns a stored body back
// into a record through a plane-side REFRAME callback rather than `serde`-decoding it into a type core
// would have to name. The chain authority, the position cache, the LRU and the write-ordering are the
// SAME `Journal` machinery above.
