...
//! `busbar_substrate::trust::verify::VerifyGate` is the async, clock-independent single-flight coalescer that
...
use busbar_substrate::trust::{reverify, TrustState};
...
/// THE reverify-`due` REACH, wrapped once — returns the full [`reverify::Due`] REASON
...
/// `reverify::due` itself post-extraction — only this host veneer does. `operator_sync`
...
pub(crate) fn verify_decide_due(
    ...
    let ledger = reverify::Ledger {
        last_checked_ms,
        ..Default::default()
    };
    let policy = reverify::Policy {
        ttl_ms,
        recovery_backoff_ms: 0,
    };
    reverify::due(&ledger, &policy, now_ms, operator_sync)
}
...
/// `reverify::due` arithmetic crosses here. Marshals the FULL [`reverify::Due`] REASON
...
/// demote-only [`TrustState::Quarantined`]. The write is fire-and-forget at the primitive
...
#[allow(dead_code)]
pub fn quarantine_settle_over(app: &crate::state::App, subject: &str, state: TrustState) -> bool {
...
/// Marshal a [`TrustState`] into the neutral u8 mirror the drift path carries in
...
#[allow(dead_code)]
pub(crate) fn trust_state_u8(state: TrustState) -> u8 {
    match state {
...
/// Reconstruct a [`TrustState`] from the neutral u8 mirror in [`Key::drift_state`].
/// `0`/absent (a sender that predates the field, guarded out by `size`) and any unknown value fail
/// SAFE to [`TrustState::Quarantined`] — the pre-extension demote-only disposition, so
/// a drift the caller could not name still records rather than silently clearing.
fn trust_state_from_u8(v: u8) -> TrustState {
    match v {
