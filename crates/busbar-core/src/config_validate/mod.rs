    ...
    if let Some(auth) = cfg.auth.as_ref() {
        ...
        for entry in auth.chain.iter().chain(auth.admin_auth.iter()) {
            if let Some(scope) = entry.max_admin_scope.as_deref() {
                // The SAME check the admin named-map write path runs
                // (`busbar_substrate::config::auth::parse_max_admin_scope`, which
                // `Scope::parse_ceiling` is the admin-side map of), so the API can never accept a
                // ceiling this rule would refuse to boot.
                match busbar_substrate::config::auth::parse_max_admin_scope(
