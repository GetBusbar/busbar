...
// fetch list — is plain serde data whose ONE READER is the plugin subsystem: the fetch list is
// `busbar-plugin-loader`'s and the trust policy is `busbar-plugin-sign`'s. Nothing in the config
// layer, and nothing in the substrate beneath it, reads a word of it. So BOTH halves moved to the
// crate that reads them: the GRAMMAR is `busbar_plugin_loader::config` and the RESOLUTION is
// `busbar_plugin_loader::{fetch_specs, trust_policy}`, one `mod` apart, reached through the
// `busbar-core -> busbar-plugin-loader` edge this crate already has. Re-exported at their historical
// `config::` path.
pub use busbar_plugin_loader::config::{
    default_plugins_dir, EnvFetch, GithubFetch, PluginFetch, PluginPublisher, PluginsCfg,
    PluginsTrustCfg, UrlFetch,
};
