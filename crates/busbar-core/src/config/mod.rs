...
pub use crate::breaker::status_class_from_str;
                ...
                if !named.is_empty() {
                    errors.push(format!(
                        "pools.{pool_name}: `{}` {} written on a pool whose kind INFERS to \
                         ...
                         {} from this pool, or move these members into a `models:` pool, whose \
                         kind does read {}.",
                        named.join("`, `"),
...
// the `pub` resolved fields (`limits.default_max_tokens`, `reasoning_effort_budgets`) that an
