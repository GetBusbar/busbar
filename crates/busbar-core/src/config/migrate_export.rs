...
/// this module carries" (see `busbar_plugin::cold::export::projection::resolve_projection`), so this
/// is a TEACHING rewrite, not a semantic one — the migrated document shows the operator the key they
/// will narrow with `fields:`, and the ledger says so. The streams are asked of the SINK, through
/// `super::builtin_export_streams` — the SAME single resolution the validator asks, so the migrator
...
fn migrate_one_export_projection(
    ...
    let Some(streams) = super::builtin_export_streams(&module) else {
        todos.push(format!(
