...
//!
//! `crate::plane::tests::metrics_tests` states the rule this file follows: the failure guarded
...
//! by `crate::build_router`. A test that called `telemetry::upstream_attempt_on` directly would pass
...
async fn scrape(app: &std::sync::Arc<crate::state::App>) -> String {
...
async fn a_tool_call_counts_an_upstream_attempt_naming_the_registration_it_was_issued_to() {
    ...
    let attempts = series_for(&exposition, crate::metrics::UPSTREAM_ATTEMPTS_TOTAL, SERVED);
    assert!(
        !attempts.is_empty(),
    ...
    assert!(
        series_for(&exposition, crate::metrics::UPSTREAM_FAILURES_TOTAL, SERVED).is_empty(),
...
async fn an_upstream_that_cannot_be_reached_is_counted_as_a_transient_failure_beside_its_attempt() {
    ...
    let exposition = scrape(&app).await;
    let attempts = series_for(&exposition, crate::metrics::UPSTREAM_ATTEMPTS_TOTAL, DEAD);
    assert!(
        !attempts.is_empty(),
        "a leg busbar actually attempted must be counted even when it failed, or the error rate has \
         no denominator. Exposition:\n{exposition}"
    );
    let failures = series_for(&exposition, crate::metrics::UPSTREAM_FAILURES_TOTAL, DEAD);
    assert!(
        !failures.is_empty(),
