...
//! `crate::trust::verify`'s own batteries; this file proves the WIRING — that `tools/call` actually
...
fn boot(peer: &Peer, verify_ttl: &str) -> Arc<crate::state::App> {
...
async fn read(app: &Arc<crate::state::App>) -> (u16, serde_json::Value) {
    call(
        app,
        &granted(),
        "tools/call",
