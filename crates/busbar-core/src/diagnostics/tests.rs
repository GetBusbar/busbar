...
const MIGRATED_FILES: &[&str] = &[
    ...
    "src/preflight.rs",
    "src/telemetry.rs",
    // `tls.rs` DELETED (TLS-1): its cert/key/CA parsing duplicated the transport-key unit's own
    // (now the one live copy) and its accept/serve loop MOVED BY IDENTITY to the composition
