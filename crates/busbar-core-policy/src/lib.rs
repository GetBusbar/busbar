...
// ── THE VOCABULARY MODULES — spellings, not policy ───────────────────────────────────────────────
//
// See the crate header. Each of these is the name the bodies above were written with, pointed at the
// neutral substrate item it always meant. Nothing here decides anything.

/// The CONFIG vocabulary the engine reads: the `hooks:` and `pools:` leaf structs, the access and
/// terminal enums, and the secret resolver that turns a hook's `settings:` refs into values before
/// they cross the ABI. All plain serde data in `busbar_substrate::config`; none of it names a plane.
pub(crate) mod config {
    ...
    pub(crate) use busbar_substrate::config::pools::PoolCfg;
...
/// THE ONE PROJECTION a gate screens a request through. `busbar_substrate::ir::facts` — a trait, a
/// borrowed content item, a provenance slot and a shape count. No request TYPE is nameable here.
pub(crate) mod ir {
    pub(crate) use busbar_substrate::ir::facts;
}

/// The neutral per-session substrate. The gate's incremental screen cache is one TENANT of it (owner
/// key `gate.screen`); the substrate itself is nobody's.
pub(crate) mod session {
    pub(crate) use busbar_substrate::session::{OwnerKey, SessionKey, SessionStore};
}

/// The two shared primitives the engine reads: the wall clock the scrape's staleness is measured on,
/// and the hash the screen-cache session identity is derived with.
pub(crate) mod store {
    pub(crate) use busbar_substrate::store::{fnv1a_u64, now};
}
...
pub(crate) mod limits {
    ...
    pub(crate) fn default_policy_timeout_ms() -> u64 {
        busbar_substrate::config::limits::installed()
            ...
            .unwrap_or(super::config::DEFAULT_POLICY_TIMEOUT_MS)
    }
}
