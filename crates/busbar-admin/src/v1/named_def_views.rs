...
// admin view for its own section(s) and is reached through the plane's `named_def_list`/
// `named_def_get` seam, so this core module names no plane-specific config or view type.
