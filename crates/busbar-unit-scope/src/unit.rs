...
///
/// The lift is the composition root's own, verbatim (`crates/busbar/src/root/units_voice.rs`):
/// `Ok(())` proceeds with the default facts, `Err` refuses with `ScopeDenied`. One plane
/// (`units_a2a`) additionally pushes the draft's resource onto the facts before proceeding; that is
/// the PLANE's fact about its own request, not the scope rule, so it stays where it is and this
