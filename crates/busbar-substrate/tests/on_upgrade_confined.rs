...
//! MECHANICAL PREVENTION for the admit-before-upgrade invariant: a bare `on_upgrade` binds the socket
//! BEFORE the session can be admitted or refused, so it must exist in EXACTLY ONE place — the neutral
//! acceptor `busbar-substrate/src/ingress/duplex_ws.rs`, where `accept`/`accept_coded` own the single
//! audited call. Every plane's WS-accept arrival receives a `WsArrival` and reaches the socket ONLY
//! through the ONE root-composed session seam: the root's `SessionLoopDriver::open` admits the session
//! on the kernel session seam (drawing its one node slot + in-flight lease ONCE, uniformly for every
//! transport) and only THEN binds the socket through `accept_coded`; none may call `on_upgrade` itself.
//! This test scans every `.rs` in the workspace's `crates/` tree and fails RED if a `.on_upgrade(` CALL
//! appears in any file other than `duplex_ws.rs`. Doc/line comments that merely NAME `on_upgrade` are
//! not calls and are excluded, exactly as the neutrality lints strip comments before judging a token.
...
fn on_upgrade_appears_in_no_file_except_the_neutral_acceptor() {
    ...
    assert!(
        ...
        "a bare `on_upgrade(` call escaped the neutral acceptor ({}): it must be reached ONLY through \
         the root-composed session seam (SessionLoopDriver::open, then accept_coded) so the session is \
         admitted before the socket binds:\n{}",
        ALLOWED_PATH_SUFFIX,
        offenders.join("\n")
    );
    assert!(
...
/// The ungated socket-bind wrappers (`accept`/`serve`) live in the neutral acceptor: they bind a socket
/// WITHOUT first opening the session on the kernel session seam, so PRODUCTION plane code must never
/// call them directly — a WS-accept reaches the socket ONLY through the root's session arrival, which
/// admits the session on the seam (`SessionLoopDriver::open`) and only THEN binds through `accept_coded`.
/// Test code may stand up a loopback echo server with the bare `serve` (no session to open), so this
/// gate exempts test files; it fails only on a NON-TEST caller of the ungated wrappers outside the
/// acceptor. This closes WS-F5: the `on_upgrade` confinement above pins WHERE the socket binds; this
/// pins that every production bind is SESSION-SEAM-ADMITTED.
...
fn ungated_ws_accept_serve_wrappers_have_no_production_caller() {
    ...
    assert!(
        ...
        "production code calls an UNGATED WS wrapper (`accept`/`serve`) that binds a socket before the \
         session is opened on the kernel session seam — route it through the root's session arrival \
         (SessionLoopDriver::open, then accept_coded) instead:\n{}",
        offenders.join("\n")
    );
}
