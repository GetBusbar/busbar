...
//! leaf [`busbar_substrate_values::proto`] and is re-exported here whole, so every historical
...
//! no longer belongs in `busbar-core`, because the type it was said to be held by — the "core-only
//! `Arrival`" — relocated to [`crate::ingress::arrival::PathIngress`] and core's spelling of it was a
//! `pub use` of this one. With both halves neutral the fold is neutral, so it sits beside them and
//! the composition root names no retiring crate to register a protocol.
