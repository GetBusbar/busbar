...
use crate::diag_warn;
use crate::diagnostics::TLS_ACCEPT_PERSISTENT_FAILURE;
...
/// process-wide `crate::config::limits` install. A function (not a `const`) so the configured value
/// is read per accepted connection; falls back to the historical 10s when limits aren't installed.
fn handshake_timeout() -> Duration {
    // Byte-identical to busbar-core's former `limits::tls_handshake_timeout_secs()` accessor: read
    // the installed value, else the historical default const — both live in the substrate's own
    // `config::limits` install (the same process-global slot core reads through).
    Duration::from_secs(
        crate::config::limits::installed()
            .map(|l| l.tls_handshake_timeout_secs)
            .unwrap_or(crate::config::limits::DEFAULT_TLS_HANDSHAKE_TIMEOUT_SECS),
    )
}
...
fn body_read_timeout() -> Duration {
    // Byte-identical to busbar-core's former `limits::request_body_read_timeout_secs()` accessor.
    Duration::from_secs(
        crate::config::limits::installed()
            .map(|l| l.request_body_read_timeout_secs)
            .unwrap_or(crate::config::limits::DEFAULT_REQUEST_BODY_READ_TIMEOUT_SECS),
    )
}
...
fn total_body_deadline() -> Duration {
    let cap_bytes = crate::proxy::max_translate_body_bytes() as u64;
...
use crate::config::sections::TlsCfg;
...
///
/// The ONE turn-a-`SecretRef`-into-TLS-PEM function lives NEUTRALLY in this module's parent
/// ([`super::read_pem`]) and is used by this listener's cert/key/CA load sites
/// (`load_cert_chain`/`load_private_key`/`load_client_roots`) — the same seam a plane's OUTBOUND
/// client identity resolver names. One place in the tree turns a `SecretRef` into TLS PEM; a second
/// would be a second place for the "never echo what you read" rule to be forgotten.
use super::read_pem;
...
impl AcceptBackoff {
    pub const FIRST: std::time::Duration = std::time::Duration::from_millis(5);
    pub const CAP: std::time::Duration = std::time::Duration::from_millis(250);

    // `pub` only so busbar-core's relocated TLS tests reach it through the re-export shim; there is
    // one true construction path (`AcceptBackoff::new` inside the two serve loops), so a `Default`
    // impl would be a second, meaningless spelling of it.
    ...
    pub fn next_delay(&mut self, e: &io::Error) -> Option<std::time::Duration> {
        if matches!(
            e.kind(),
...
/// never false-share).
#[repr(align(64))]
pub struct PaddedCount(pub std::sync::atomic::AtomicU32);
...
pub struct ConnBalancer {
    // `counts`/`txs`/`me` are `pub` only so busbar-core's relocated TLS balancer tests can assert
    // per-worker placement counts and force a full-channel hand-off through the re-export shim; `rx`
    // stays private (nothing outside the serve loops touches it).
    pub counts: Arc<[PaddedCount]>,
    pub txs: Arc<[tokio::sync::mpsc::Sender<(std::net::TcpStream, SocketAddr)>]>,
    pub me: usize,
...
impl ConnBalancer {
    ...
    pub fn place_local(&self) -> ConnCountGuard {
        use std::sync::atomic::Ordering;
    ...
    pub fn adopt(&self) -> ConnCountGuard {
        ConnCountGuard {
            counts: self.counts.clone(),
            idx: self.me,
        }
...
/// `pub` (fields private) only so it can be the return type of the now-`pub` `place_local`/`adopt`
/// the relocated busbar-core balancer tests drive through the re-export shim.
pub struct ConnCountGuard {
...
pub fn hardened_conn_builder() -> ConnBuilder<TokioExecutor> {
