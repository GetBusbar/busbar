...
/// THE ONE OPEN-PASS ADMISSION GATE both gauntlet siblings share — the plane's own
/// `verify_destination` in its verify-STRICTLY-before-charge position, so NOTHING may reject an
/// already-charged request. Returns [`Admitted`] on a pass, or the plane's OWN finished,
...
/// ORDER.
///
/// IT IS AN ORDERING SEAM AND NOT A LOOP, which is the whole of what this function says now. It used
/// to run a nine-step scaffold — a second copy of the request loop, with its own token family and
/// its own audit doors — over an adapter that answered seven of those steps with a bare proceed, an
/// EMPTY hold and an empty posting. Nothing was charged, held, metered or posted by any of it: the
/// plane's admission, routing, metering and finish were all inside `drive` the whole time, so the
/// scaffold's entire observable behaviour for every caller it ever had was the two lines below. The
/// one loop a unit runs on is the kernel's, and that is where the ten steps live.
///
...
/// is already resolved (threaded via `req.gov`); this runs the plane's `verify_destination` in the
/// correct PRE-ADMISSION position, and only if it proceeds the plane's `drive` (its own
/// byte-identical engine + metering). Returns the plane's (possibly streaming) response verbatim.
/// The plane owns admission/route/metering/finish; this seam owns solely the order (nothing may
...
pub async fn run_gauntlet(
    ...
    match admit_open(&req, plane.as_ref()) {
        // The gate cleared: the plane's own engine runs the request, as it always did.
        Ok(_admitted) => plane.drive(req).await,
        // The plane's own finished refusal, verbatim — nothing was charged, because nothing ran.
        Err(refusal) => refusal,
    }
}
...
// ─────────────────────────────────────────────────────────────────────────────────────────────
// AND THE OUTCOMES THEMSELVES, kept from the deleted scaffold's own cells: what each sibling hands
// back on a proceed and on a refuse, unchanged across the collapse.
// ─────────────────────────────────────────────────────────────────────────────────────────────
#[cfg(test)]
#[path = "tests/gauntlet_outcome_tests.rs"]
mod gauntlet_outcome_tests;
