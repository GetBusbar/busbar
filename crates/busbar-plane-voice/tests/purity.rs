...
//!
//! Copied in shape from `busbar-plane-a2a`/`busbar-plane-mcp`'s own `tests/purity.rs` (the
//! `walk`/`is_comment`/forbidden-list structure), pointed at this crate. The determinism tests are
//! driven over this plane's one-shot decode path (`decode_one_shot`, reached with no session) rather
//! than a JSON-RPC method table, because that is the one ingress shape this plane answers with no
//! session state to thread through a scaffold.
...
///
/// Driven over the one-shot transcribe path (no session): `decode_one_shot` reads the `path`
/// transport fact to pick the dialect, and every dialect this plane can decode with no session is a
/// one-shot HTTP operation.
#[test]
fn the_decode_step_is_deterministic() {
    let plane = VoicePlane::EMPTY;
    let body = b"";
    let mut answers = Vec::new();
    for _ in 0..8 {
        let scaffold = Scaffold::with_path("http", "/v1/audio/transcriptions");
        let ctx = scaffold.ctx_without_session();
        let frames = vec![frame(body)];
        let mut cursor = FrameCursor::new(&frames);
        let ingress = plane
            .decode_ingress(&mut cursor, None, &ctx)
            .expect("a one-shot dialect path decodes");
...
fn the_encode_step_is_deterministic() {
    let plane = VoicePlane::EMPTY;
    let answer = b"some rendered dialect bytes";
...
fn the_answer_does_not_move_with_the_clock() {
    ...
    for unix_secs in [2_000_000_000_u64, 1_000_000_000] {
        ...
        let Ok(Ingress::OneShot(draft)) = plane.decode_ingress(&mut cursor, None, &ctx) else {
            panic!("a one-shot dialect decodes as one shot");
        };
