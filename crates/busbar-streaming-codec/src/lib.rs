...
//!
//! `busbar-streaming` held two things behind one name: these codecs — the plane-4 duplex/session
...
//! and `bytes`. `busbar-streaming` depends on this crate and re-exports every module that moved under
//! its old path, so `busbar_streaming::ir::…` and `busbar_streaming::topology::twilio::…` resolve exactly
...
/// crate declares a feature at all. `busbar_streaming::topology` is gated on `runtime` (OFF by default),
...
/// with the module: `busbar-streaming`'s `runtime` forwards here, and the `#[cfg]` resolves to exactly
