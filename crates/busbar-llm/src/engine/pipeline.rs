...
pub(crate) async fn forward_with_pool_parsed_inner(
    ...
    let mut cands: Vec<WeightedLane> =
        match filter_op_supported_candidates(rt, op, ingress_protocol, cands) {
            Ok(kept) => kept,
            Err(resp) => return resp,
        };
    // `v` is the PRISTINE parsed request body (parsed once by the caller). Never mutated after this
    // point: each failover hop derives a fresh per-hop `hop_v` (see `derive_hop_body`) so a
    // cross-protocol hop never re-translates a body already rewritten into a previous lane's shape.
    let (wants_stream, client_include_usage, client_has_stream_options) =
        derive_stream_intent(&v, op);

    if let Err(resp) = apply_rewrite_pass(
        host,
        &mut v,
        &mut body,
        pool_name,
    ...
    let gemini_json_array = derive_gemini_json_array(ingress_protocol, op, &v);

    let affinity_key_hash: Option<u64> = derive_affinity_hash(affinity_key, &v, op);

    let (max_cap, breaker_cfg, mut request_ctx) =
        setup_failover_context(rt, pool_name, request_id, client_fwd, &mut cands);

    // ── ROUTING-POLICY SEAM ── decision gates then the pool's routing policy (see the extracted
    // `apply_decision_gates` / `resolve_route_order` for the full contract). ZERO-COST DEFAULT: a
    // `route: weighted` pool builds no projection and leaves `policy_order` `None` (pure SWRR).
    let gate_order = match apply_decision_gates(
        host,
        rt,
        &mut cands,
        &mut request_ctx,
    ...
    .await
    {
        Ok(gate_order) => gate_order,
        Err(resp) => return resp,
    };

    let mut chosen_policy_name: Option<&'static str> = None;
    let policy_order: Option<Vec<usize>> = match resolve_route_order(
        gate_order,
        host,
        rt,
        &mut cands,
    ...
    // whole dispatch (invariant across every failover hop — the pool does not change mid-request).
    // Same value the accessor returns (pool override else all-pools default), same passthrough-40x.
    let upstream_creds = EngineTables::new(rt).pool_upstream_creds(pool_name);

    // PREPARE ends here (dispatch loop begins); from here on `v` IS the first-hop body.
    drop(_prep);
    // Box::pin: the failover-loop future transitively holds the attempt state; awaited inline it
    // unioned into this fn's coroutine past the pinned size bound (the same future-shrink pattern the
    // exhaustion/policy paths use). Byte-identical: a boxed future runs the same steps in order.
    Box::pin(run_failover_loop(
        ...
        chosen_policy_name,
        &stage_shape,
        // Why the PREVIOUS attempt failed — feeds the routing-stage tap payload; `None` on hop 1.
        None,
        usage_sink,
        max_cap,
    ))
...
/// The failover DISPATCH LOOP: walk the candidate lanes up to `max_cap` hops, running one attempt per
/// hop, until an attempt delivers/bails a response or the walk exhausts. A no-usable-lane hop (or a
/// walk that runs out of hops) applies the configured exhaustion mode (Status503 / FallbackPool /
/// LeastBad). Extracted verbatim from `forward_with_pool_parsed_inner`'s tail — same order, same
/// effects; the in-loop exhaustion clones `usage_sink` and the after-loop exhaustion moves it, exactly
/// as before (only one ever runs per request).
#[allow(clippy::too_many_arguments)]
async fn run_failover_loop(
    ...
    mut last_failure: Option<&'static str>,
    mut usage_sink: Option<UsageSink>,
    max_cap: usize,
) -> Response {
    for attempt_no in 0..=max_cap {
        match dispatch_one_attempt(
            host,
            rt,
            cands,
            request_ctx,
        ...
        .await
        {
            HopStep::Return(resp) => return resp,
            HopStep::Continue => continue,
            // No usable lane — whether the members were tripped before this request arrived or
            // excluded during its failover attempts, apply the configured exhaustion mode
            // (Status503 / FallbackPool / LeastBad) with loop prevention.
...
/// One trip through `forward_with_pool_parsed_inner`'s failover loop body: the loop decides what to do
/// next from this. `Return` is a terminal response (delivered/bailed attempt, deadline, empty pool, or
/// a pre-dispatch bail); `Continue` advances to the next candidate (a failed attempt); `Exhausted`
/// means no usable lane — the loop applies the configured exhaustion mode.
enum HopStep {
    Return(Response),
    Continue,
    Exhausted,
...
/// ONE failover-loop iteration: check the deadline, pick a lane (+permit/probe), fire the routing
/// stage tap, count the attempt, derive the fresh per-hop body, then run THE ONE ATTEMPT and classify
/// its outcome for the walk. Extracted verbatim from `forward_with_pool_parsed_inner`'s loop body —
/// same values, same order, same effects; the early `return`s become `HopStep::Return`, the `continue`
/// becomes `HopStep::Continue`, and the no-usable-lane exhaustion return becomes `HopStep::Exhausted`
...
async fn dispatch_one_attempt(
    ...
    if request_ctx.expired(now()) {
        return HopStep::Return(ingress_error(
            ingress_protocol,
    ...
    drop(_asetup);
    // Derive a FRESH per-hop body for translation. REQUEST SHORT-CIRCUIT WITHOUT A DOM: hop 1 of a
    // SAME-protocol JSON dispatch whose head projection PROVES no same-proto invalidator fires re-emits
    // the retained bytes verbatim — the exact bytes the translate seam's own pristine short-circuit
    // would emit — without ever materializing the `Value` tree. `head_provably_pristine` is one-sided:
    // any doubt falls through to the unchanged materialize-and-translate path, so the wire bytes are
    // byte-identical on every branch.
    let head_pristine = ingress_protocol == egress_name
        && v.as_ref()
            .is_some_and(|l| head_provably_pristine(rt, i, l.probe()));
    let hop_v: Option<Value> = match derive_hop_body(v, head_pristine, body_is_json, body) {
        Ok(hv) => hv,
    ...
    .await;
    match outcome {
        AttemptOutcome::Response(resp) | AttemptOutcome::Bail(resp) => HopStep::Return(resp),
        AttemptOutcome::Failed {
            ...
            if matches!(disposition, Disposition::ContextLength) {
                exclude_shared_context_lanes(rt, cands, request_ctx, i);
            }
...
/// first). Extracted verbatim from `forward_with_pool_parsed_inner`'s loop body.
#[allow(clippy::too_many_arguments)]
...
/// A ContextLength failure means the request is too large for THIS model's context window: exclude
/// every candidate whose known `context_max` is at or below the failed lane's (they share or undercut
/// the limit that just failed), so failover lands on a larger-context or unknown-context member. An
/// unknown limit on the failed lane excludes only the failed lane itself (already excluded above).
/// Extracted verbatim from `forward_with_pool_parsed_inner`.
fn exclude_shared_context_lanes(
    rt: &Arc<NativeRuntime>,
    cands: &[WeightedLane],
    request_ctx: &mut RequestCtx,
    i: usize,
...
/// starting from the ORIGINAL request, never from a previous hop's egress-shaped body: hop 1 consumes
/// the carried `v` (the memoized DOM, else ONE parse of the validated bytes), failover hops 2+ re-parse
/// the retained pristine `body` bytes (SIMD parse). A head-provably-pristine hop-1 short-circuit and an
/// opaque body both skip the parse. `Err(())` is the (infallible-in-practice) parse failure the caller
/// turns into a pre-dispatch bail. Extracted verbatim from `forward_with_pool_parsed_inner`.
fn derive_hop_body(
    v: &mut Option<LazyBody>,
    head_pristine: bool,
    body_is_json: bool,
    ...
    if head_pristine {
        ...
        *v = None;
        ...
        Ok(None) // opaque ingress body: byte-level relay/translate; nothing to re-parse.
    } else {
...
/// Resolve the per-request failover walk context: the failover deadline + hop cap (pool override
/// else global default), the shared breaker config, and a fresh `RequestCtx` carrying the ingress
/// client-header allowlist; then apply the pool's configured member-exclusion blocklist to `cands`.
/// Returns `(max_cap, breaker_cfg, request_ctx)`. Extracted verbatim from
/// `forward_with_pool_parsed_inner`.
///
/// Before-first-byte failover boundary: failover is allowed ONLY until the first upstream byte
/// reaches the client. After that point an upstream failure must NOT trigger failover because the
/// client already has a partial response; instead an SSE stream emits an `error` event and terminates
/// and the breaker failure is recorded for that lane, and the client must restart the request itself.
fn setup_failover_context(
    rt: &Arc<NativeRuntime>,
    pool_name: &str,
    request_id: u64,
    client_fwd: Vec<(axum::http::HeaderName, axum::http::HeaderValue)>,
    ...
    (max_cap, breaker_cfg, request_ctx)
}

/// Gemini ingress streaming WITHOUT `?alt=sse`: the native client expects a JSON-array streamed body,
/// not SSE. The route layer signals this via a router shim key (read here; stripped from the body
/// unconditionally before forwarding). GATED on `uses_array_stream_shim()` (true only for
/// GeminiWriter): only a genuine Gemini client can want JSON-array response framing. Without the gate
/// a body-model client (openai/cohere/responses) that sent `{"__busbar_gemini_json_array":true}` in
/// its own fully-controlled body would have its SSE stream silently reframed as a JSON array under
/// `Content-Type: application/json` — undecodable by the official SDK and a router behavior no native
/// backend exhibits. False for every other protocol and for the `?alt=sse` gemini variant.
/// Additionally gated on `op.streaming()`: a non-streaming operation never frames a JSON-array stream
/// (chat streams, so this is a no-op for chat — `true && x == x`). Extracted verbatim from
/// `forward_with_pool_parsed_inner`.
fn derive_gemini_json_array(
    ingress_protocol: &str,
    op: busbar_substrate::handlers::Op,
    v: &Option<LazyBody>,
) -> bool {
...
/// per-request `String` allocation. Prefer the supplied header key; else fall back to the operation's
/// body-derived key (chat: the top-level `system` string; other ops: no body affinity). `None` = no
/// sticky preference. Extracted verbatim from `forward_with_pool_parsed_inner`.
fn derive_affinity_hash(
    affinity_key: Option<&str>,
    v: &Option<LazyBody>,
    op: busbar_substrate::handlers::Op,
) -> Option<u64> {
...
/// ── REQUEST IR + GLOBAL TAP (observe) FIRE ── Parse the EFFECTIVE request (post-rewrite) into the
/// IR, once, here — before the taps, before the gates, and before a lane (egress protocol) is chosen
/// (a content hook may reroute across protocols). Then fire the global request-stage `kind: tap`
/// hooks FIRE-AND-FORGET. ZERO COST when no content hook / no tap is configured. Extracted verbatim
/// from `forward_with_pool_parsed_inner` — same gating, same order, same effects.
#[allow(clippy::too_many_arguments)]
fn run_ir_and_global_taps(
    host: &Arc<dyn EngineHost>,
    v: &mut Option<LazyBody>,
    body: &Bytes,
    req_content_type: &str,
...
/// ── STAGE TAPS: candidate + routing shape ── Capture the stage-tap shape ONCE (scalars only, so it
/// survives `v` moving into the first hop) and fire the `candidate` taps now: the decision reconcile
/// and base ordering above produced the FINAL candidate set for dispatch. Returns the captured shape
/// (rode into the failover loop for the per-hop routing taps). ZERO COST when no stage tap is
/// configured. Extracted verbatim from `forward_with_pool_parsed_inner`.
#[allow(clippy::too_many_arguments)]
...
/// ── ROUTING-POLICY SEAM ── Resolve this pool's routing order for the failover loop. A phase-2
/// `gate_order` overrides everything; else the pool's resolved routing policy (when non-default)
/// projects the body, runs its (Box::pin'd, cold-on-default) decision bounded by its timeout, and
/// coerces the outcome to a ranked order (or `None` ⇒ SWRR) per `on_error`. Sets `*chosen_policy_name`
/// to the policy that produced the order (for the `x-busbar-route-policy` header); `Err(response)` is
/// a policy reject / restrict fail-close. ZERO-COST DEFAULT: a `route: weighted` pool returns `None`
/// with no projection/async. Extracted verbatim from `forward_with_pool_parsed_inner`.
#[allow(clippy::too_many_arguments, clippy::result_large_err)]
async fn resolve_route_order(
    ...
    caller_token: Option<&str>,
    resolved_gov_key: Option<&std::sync::Arc<busbar_api::VirtualKey>>,
    chosen_policy_name: &mut Option<&'static str>,
    ...
    let resolved = match host.pool_policy(pool_name) {
    ...
    match outcome {
        ...
        PolicyOutcome::Weighted => Ok(None),
        ...
        // carrying one of `tags_any`, then let SWRR pick among the survivors.
        PolicyOutcome::Restrict {
            tags_any,
            name,
            on_empty,
        } => {
            apply_policy_restrict(
                tags_any,
                name,
                on_empty,
                cands,
...
/// The routing-policy RESTRICT verb: intersect `cands` with members carrying one of `tags_any` and,
/// on a non-empty intersection, commit the shrink (persists across every failover hop — the
/// compliance guarantee) and record the policy name. An EMPTY intersection is fail-closed
/// (`on_empty` default reject → `Err`), unless `on_empty: weighted` (advisory escape — leaves `cands`
/// as the full pool → SWRR). Extracted verbatim from `forward_with_pool_parsed_inner`.
#[allow(clippy::too_many_arguments, clippy::result_large_err)]
fn apply_policy_restrict(
    tags_any: Vec<String>,
    name: &'static str,
...
/// ── PHASE-2 DECISION GATES (concurrent at t0) ── Fire the GLOBAL decision gates and this pool's OWN
/// gates for a verdict on this request, BEFORE pool routing. All gates fire CONCURRENTLY against the
/// same t0 candidate set — reject and restrict COMMUTE (veto; intersect), and an order is re-validated
/// against the FINAL (post-restrict) set — then the outcomes reconcile deterministically over ONE
/// chain, sorted by ascending `priority` (stable: globals before pool gates on ties, then config
/// order): (1) any reject ⇒ reject wins; (2) else restricts INTERSECT, applied in chain order; (3)
/// else the LAST ordering gate wins, filtered to the surviving set. The restriction persists across
/// failover (hops select from the shrunk `cands`). ZERO COST when no gate is configured. Returns the
/// reconciled `gate_order`; `Err(response)` is a gate reject. Extracted verbatim from
/// `forward_with_pool_parsed_inner`.
#[allow(clippy::too_many_arguments, clippy::result_large_err)]
async fn apply_decision_gates(
    host: &Arc<dyn EngineHost>,
    rt: &Arc<NativeRuntime>,
    cands: &mut Vec<WeightedLane>,
    request_ctx: &mut RequestCtx,
    ...
    if !host.global_gates().is_empty() || !pool_gates.is_empty() {
        ...
        // Reconcile 1: REJECT WINS.
        if let Some(resp) = reconcile_gate_rejects(&outcomes, pool_name, ingress_protocol) {
            return Err(resp);
        }
        // Reconcile 2: RESTRICTS INTERSECT.
        if let Some(resp) = reconcile_gate_restricts(
            &outcomes,
            cands,
            request_ctx,
            rt,
...
/// Decision-gate reconcile 1: REJECT WINS. The first rejecting gate in chain order surfaces — that is
/// the `priority` tie-break when several gates reject at once. `Some(response)` is the surfaced
/// rejection. Extracted verbatim from `forward_with_pool_parsed_inner`.
fn reconcile_gate_rejects(
    outcomes: &[PolicyOutcome],
    pool_name: &str,
    ingress_protocol: &str,
) -> Option<Response> {
    // A deliberate RejectRequest was status-clamped 400..=499 + message-sanitized at the producing
    // seam; a fail-closed errored gate (`on_error: reject`) is a 503, never a silent proceed.
    for outcome in outcomes {
        match outcome {
            PolicyOutcome::RejectRequest {
                status,
                ...
                return Some(gate_rejected(ingress_error(
                    ingress_protocol,
                    StatusCode::from_u16(*status).unwrap_or(StatusCode::FORBIDDEN),
                    reject_kind_for_status(*status),
                    message,
            ...
            PolicyOutcome::Reject => {
                ...
                return Some(gate_rejected(ingress_error(
                    ingress_protocol,
                    StatusCode::from_u16(busbar_substrate::hooks::REQUIRED_HOOK_UNAVAILABLE_STATUS)
...
/// Decision-gate reconcile 2: RESTRICTS INTERSECT, in chain order (intersection commutes — the final
/// set is order-independent; the chain order only decides WHOSE on_empty applies first when the set
/// empties). Shrinking `cands` makes the restriction persist across every failover hop and keeps any
/// ordering inside the eligible set. `Some(response)` is a fail-closed restrict reject. Extracted
/// verbatim from `forward_with_pool_parsed_inner`.
fn reconcile_gate_restricts(
    outcomes: &[PolicyOutcome],
    cands: &mut Vec<WeightedLane>,
    request_ctx: &mut RequestCtx,
    rt: &Arc<NativeRuntime>,
...
/// ── GLOBAL REWRITE (transform) PASS ── Fire the global `prompt: rw` gates (compression/redaction)
/// BEFORE dispatch AND before the routing decision, so the decision + upstream both see the rewritten
/// body. Priority-ordered transform chain; fail-safe throughout (a broken hook is skipped, a non-chat
/// body is untouched). ZERO COST when no rewrite hook is configured. The pool's own rewrite chain
/// fires AFTER the globals. `Err(response)` is a rewrite REJECT / fail-closed exit; on `Ok(())` `body`
/// carries any committed rewrite. Extracted verbatim from `forward_with_pool_parsed_inner`.
#[allow(clippy::too_many_arguments, clippy::result_large_err)]
async fn apply_rewrite_pass(
    host: &Arc<dyn EngineHost>,
    v: &mut Option<LazyBody>,
    body: &mut Bytes,
    pool_name: &str,
...
/// rewrites `v`. Returns `(wants_stream, client_include_usage, client_has_stream_options)`. Extracted
/// verbatim from `forward_with_pool_parsed_inner` — same head-projection reads, same order.
fn derive_stream_intent(
    v: &Option<LazyBody>,
    op: busbar_substrate::handlers::Op,
) -> (bool, bool, bool) {
...
/// 404 in the CALLER's dialect, never a silent dispatch. Dormant while all six protocols serve chat;
/// load-bearing the moment one is removed (the deletion test). Extracted verbatim from
/// `forward_with_pool_parsed_inner`'s prologue.
///
/// (`result_large_err`: the `Err` is the plane's OWN finished by-value `Response`, as in `assemble`.)
#[allow(clippy::result_large_err)]
fn filter_op_supported_candidates(
    rt: &Arc<NativeRuntime>,
    op: busbar_substrate::handlers::Op,
    ingress_protocol: &str,
    cands: Vec<WeightedLane>,
