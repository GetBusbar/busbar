...
/// The STREAMING (or same-protocol non-stream) delivery tail of [`deliver`]: build the
/// first-byte-tracking body wrapper that owns the permit, the mid-stream breaker recording, the usage
/// tap and the budget refund, then assemble the client response headers around it. Extracted verbatim
/// from `deliver`'s async body — same values, same order, same effects. Synchronous: this path holds
/// no `.await` (the buffered/cross-protocol path returns before it).
#[allow(clippy::too_many_arguments)]
fn serve_streaming_response(
    ...
    tap: TapCell,
    _rb_pre: Option<busbar_substrate::profile::Timer>,
) -> Response {
    let (host, rt, i, pool) = (hop.host, hop.rt, hop.lane, hop.pool_cell);
