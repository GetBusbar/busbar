...
//!      400 contract exactly (it goes through `busbar_substrate_values::json::parse`, so the depth security floor and
...
fn captured_head_keys() -> &'static [&'static str] {
    busbar_kernel::proto::registry().head_keys()
}
...
pub struct LazyBody {
    ...
    /// neutral [`busbar_substrate_values::handlers::TranslateCodec::read_facts_value`] entrypoint) and memoized here so
    /// one request costs one read. Held behind the [`busbar_substrate_values::ir::facts::IrFacts`] projection, NOT the
    ...
    /// than a precaution.
    ir: Option<Box<dyn busbar_substrate_values::ir::facts::IrFacts + Send + Sync>>,
}

impl LazyBody {
    /// Validate `bytes` as JSON and capture the head projection — WITHOUT building a DOM. Goes
    /// through `busbar_substrate_values::json::parse` so the depth security floor and the malformed-body reject set
    ...
    pub fn parse(bytes: &Bytes) -> Result<Self, sonic_rs::Error> {
        let head: Head = busbar_substrate_values::json::parse(bytes)?;
        Ok(LazyBody {
            body: Body::Head {
    ...
    pub(crate) fn ensure_dom(&mut self) -> Result<&mut Value, ()> {
        ...
        if let Body::Head { bytes, .. } = &self.body {
            let v: Value = busbar_substrate_values::json::parse(bytes).map_err(|_| ())?;
    ...
    /// [`busbar_substrate_values::ir::facts::IrFacts`], and return it.
    ///
    /// The facts are read by the ingress operation's own handler through the single
    /// [`busbar_substrate_values::handlers::TranslateCodec::read_facts_value`] entrypoint, so it is the SAME parse the
    ...
    ) -> Option<&(dyn busbar_substrate_values::ir::facts::IrFacts + Send + Sync)> {
        use busbar_substrate_values::handlers::TranslateCodec;
        if self.ir.is_none() {
            let handler = busbar_substrate_values::handlers::request_handler(ingress_protocol)
