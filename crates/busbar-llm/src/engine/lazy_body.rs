...
//!      400 contract exactly (it goes through `busbar_substrate::json::parse`, so the depth security floor and
...
pub struct LazyBody {
    ...
    /// neutral [`busbar_substrate::handlers::TranslateCodec::read_facts_value`] entrypoint) and memoized here so
    /// one request costs one read. Held behind the [`busbar_substrate::ir::facts::IrFacts`] projection, NOT the
    ...
    /// than a precaution.
    ir: Option<Box<dyn busbar_substrate::ir::facts::IrFacts + Send + Sync>>,
}

impl LazyBody {
    /// Validate `bytes` as JSON and capture the head projection — WITHOUT building a DOM. Goes
    /// through `busbar_substrate::json::parse` so the depth security floor and the malformed-body reject set
    ...
    pub fn parse(bytes: &Bytes) -> Result<Self, sonic_rs::Error> {
        let head: Head = busbar_substrate::json::parse(bytes)?;
        Ok(LazyBody {
            body: Body::Head {
    ...
    pub(crate) fn ensure_dom(&mut self) -> Result<&mut Value, ()> {
        ...
        if let Body::Head { bytes, .. } = &self.body {
            let v: Value = busbar_substrate::json::parse(bytes).map_err(|_| ())?;
    ...
    /// [`busbar_substrate::ir::facts::IrFacts`], and return it.
    ///
    /// The facts are read by the ingress operation's own handler through the single
    /// [`busbar_substrate::handlers::TranslateCodec::read_facts_value`] entrypoint, so it is the SAME parse the
    ...
    pub(crate) fn into_value(self) -> Result<Value, ()> {
        match self.body {
            Body::Dom(v) => Ok(v),
            Body::Head { bytes, .. } => busbar_substrate::json::parse(&bytes).map_err(|_| ()),
        }
    }
}
