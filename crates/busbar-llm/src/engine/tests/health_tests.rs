...
async fn a_shortened_interval_takes_effect_on_the_inherited_schedule() {
    ...
    let make_cfg = |interval: u64| {
        let mut c = crate::test_support::cfg_with_provider_api_key(
            busbar_contract::secret_ref::SecretRef::none(),
        );
    ...
    spawn_probers(&busbar_kernel::test_support::engine_host(&app));
    ...
    spawn_probers(&busbar_kernel::test_support::engine_host(&app2));
