...
fn head_matches_dom_for_captured_keys() {
    ...
    for raw in bodies {
        ...
        let dom: Value = busbar_llm_codec::json::parse(&bytes).unwrap();
...
fn head_parse_rejects_iff_dom_parse_rejects() {
    ...
    for raw in inputs {
        ...
        let dom_ok = busbar_llm_codec::json::parse::<Value>(&bytes).is_ok();
...
fn head_pristine_matches_translate_output() {
    ...
    for (proto, name, lane_model, body) in cases {
        let app = TestApp::new()
            ...
            .build();
        let hop_bytes = Bytes::from(busbar_llm_codec::json::to_vec(body).unwrap());
...
fn ensure_dom_materializes_and_probe_tracks_mutation() {
    ...
    assert_eq!(
        *dom,
        busbar_llm_codec::json::parse::<Value>(&bytes).unwrap()
    );
    dom.as_object_mut()
        .unwrap()
