...
async fn unsupported_sub_op_rejects_with_404_naming_op_and_model() {
    ...
    let resp = ingress_reject_response(
        "openai",
        &busbar_substrate_values::handlers::IngressReject::UnsupportedSubOp {
            op: busbar_contract::operation::OpVerb::IMAGE,
            model: "dall-e-2".into(),
        },
    );
...
async fn bad_request_reject_keeps_the_unchanged_generic_400() {
    ...
    let resp = ingress_reject_response(
        "openai",
        &busbar_substrate_values::handlers::IngressReject::BadRequest("x".into()),
    );
