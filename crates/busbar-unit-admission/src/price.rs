...
impl Pricer {
    ...
    pub fn resolved_at(
        ...
        // Clamped here for the same reason the other two constructors clamp, and it is the same
        // clamp: the derivation ADDS the fee times the billable count to the token spend, so a
        // negative fee subtracts and a bucket already over its cap on tokens alone derives back
        // under it and is admitted. A third way into this struct is a third way past that.
        let price_per_request_cents = price_per_request_cents.max(0);
