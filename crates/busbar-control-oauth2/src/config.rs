...
//! path. A config that cannot produce a [`crate::surface::OAuth2Control`] does not boot: an authorization server that is
...
/// [`crate::surface::OAuth2Control`]. This is the config half, so it can be compared, logged and
...
/// material arrives at `OAuth2Control::build` already resolved, as `Option<&str>`.
...
impl Identity {
    ...
    pub fn from_section(cfg: &Section) -> Result<Self, PluginError> {
        Self::derive(cfg).map_err(PluginError::from)
    }

    fn derive(cfg: &Section) -> Result<Self, AsCfgError> {
...
/// The refusal as the contract's one structured error: the class the host keys its handling off,
/// the `oauth2.config.*` code the catalog templates, the value as a parameter, and the operator
/// sentence above as the developer message.
impl From<AsCfgError> for PluginError {
    fn from(e: AsCfgError) -> Self {
        let (code, param) = match &e {
            AsCfgError::MissingIssuer => (catalog::CONFIG_MISSING_ISSUER, None),
            AsCfgError::IssuerNotAbsolute(v) => {
                (catalog::CONFIG_ISSUER_NOT_ABSOLUTE, Some(("issuer", v)))
            }
            AsCfgError::IssuerHasQueryOrFragment(v) => (
                catalog::CONFIG_ISSUER_HAS_QUERY_OR_FRAGMENT,
                Some(("issuer", v)),
            ),
            AsCfgError::IssuerHasTrailingSlash(v) => (
                catalog::CONFIG_ISSUER_HAS_TRAILING_SLASH,
                Some(("issuer", v)),
            ),
            AsCfgError::ScopeNotAToken(v) => {
                (catalog::CONFIG_SCOPE_NOT_A_TOKEN, Some(("scope", v)))
            }
        };
        let err = catalog::refuse(ErrorClass::Malformed, code, e.to_string());
        match param {
            Some((k, v)) => err.with_param(k, ParamValue::Str(v.clone())),
            None => err,
        }
    }
}
