...
impl CimdFetch for Recording {
    /// The stub's grammar is the production one's, spelled out rather than delegated: HTTPS, a
    /// host, no fragment. It has to agree with the node's guard or the store-seam tests below would
    /// be gating on a different rule from the one production gates on.
    fn names_a_document(&self, client_id: &str) -> bool {
        client_id.starts_with("https://")
            && !client_id.contains('#')
            && client_id.len() > "https://".len()
    }
