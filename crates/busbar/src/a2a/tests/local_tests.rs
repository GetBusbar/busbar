...
/// A CONFIG BUSBAR WILL NOT HONOUR IS REFUSED, NOT STORED. The delivery carries no credential by
/// decision, so accepting an `authentication` block would tell the caller its receiver will see an
/// `Authorization` header that never arrives.
#[tokio::test]
async fn a_config_naming_credentials_is_refused_because_the_delivery_sends_none() {
