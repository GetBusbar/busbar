...
impl<'r, S: CellStore> A2aUnits<'r, S> {
    ...
    /// naming a second kind of resource would have it enforced here without a line changing, and a
    /// locator of a kind the key was never granted under refuses, because that is what the
    /// cross-kind rule says. Reading the kind and then checking only the ones this file recognised
    /// would be the fail-open version of the same code.
    ///
    ...
    ///
    /// EVERY OTHER ADDRESS with no resolved key REFUSES. That arm is reached by a caller an external
    /// auth module identified, or by one the open front door admitted on a credentialed address:
    /// neither has a grant list, and "no grants to read" is not "every grant". The alternative
    /// reading is the one that would hand a caller with no agent entitlement whatsoever every agent
    /// this deployment fronts.
