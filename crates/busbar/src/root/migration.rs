...
///
/// The root is the only thing that holds both the resolved cost model and the process's rate-card
...
///   ([`busbar_kernel::governance::WINDOW_TOTAL`]), the window the store's key rows are addressed by.
...
///   head is [`busbar_kernel_ledger::cost::HistorySeq::OPENING`] — a real resolution of the real
///   history, so the day an operator's card edit lands before the seal, the marker records the entry
///   it was actually opened under rather than a hardcoded sentinel.
#[must_use]
pub fn config_from(cost: &busbar_kernel::cost::CostModel, now: u64) -> MigrationConfig {
    let group_buckets = cost
        ...
        .map(|b| {
            (
                b.bucket_id.clone(),
                busbar_kernel::governance::budget_window(b.window, now),
            )
        })
        .collect();
    MigrationConfig {
        ...
        group_buckets,
        metering_days: vec![busbar_kernel::governance::metering_bucket(now)],
        rate_card_version: crate::root::kernel::ROOT_CARD.pin().map_or(
            busbar_kernel_ledger::cost::HistorySeq::OPENING.get(),
            |pinned| pinned.seq().get(),
        ),
    }
}
