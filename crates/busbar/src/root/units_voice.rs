...
impl Units for VoiceUnit<'_> {
    ...
    fn evidence(&self, ctx: &UnitCtx) -> Evidence {
        Evidence {
            // What the upstream reported, where it reported anything — and nothing at all on the
            // half of a tool call that is still waiting. That half delivered the question; whatever
            // figure it happens to be carrying belongs to the unit that finishes the call, and
            // locating it here would post it a second time on the resume.
...
mod tests {
    ...
    fn a_session_that_ends_closes_once_and_takes_its_open_calls_with_it() {
        ...
        // A turn empties the lease. It was delivered, so it is metered and settled like any other;
        // what the exhaustion decides is the frame after it.
        let turn = VoiceUnit::new(&node, UnitShape::Turn, 7, 1_700_000_000)
            .charging_through(ungoverned())
            .reporting(TurnUsage {
                audio_tokens_out: 1,
        ...
        let Ended::Settled { .. } = run(&kernel, &turn) else {
            panic!("the turn settles");
        };
        assert_eq!(
            *counting.closes.lock().expect("lock"),
            0,
            "the turn that emptied the lease was still delivered; the session ends on the next door"
        );
