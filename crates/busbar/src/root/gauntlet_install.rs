...
pub fn install() {
    ...
    // Routed through `PLANE_DECLARATION.key` rather than a fresh `busbar_llm::PLANE_KEY` reach:
    // `main.rs` already names `busbar_llm::PLANE_DECLARATION` to install the plane, and its `key`
    // field IS the same capability key (see its definition) — so this reads the seam the plane
