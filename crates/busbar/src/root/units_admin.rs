    ...
    axum::Router::new().fallback(axum::routing::any(
        move |req: axum::http::Request<axum::body::Body>| {
            ...
            async move {
                ...
                let (Some(_resolved), true) = (resolved, claimed) else {
                    return inner.oneshot(req).await.unwrap_or_else(|e| match e {});
                };
                // What the resolve decides here is only that this pair IS one of the plane's
                // operations. WHOSE exit path it is comes off the unit key drawn below, which the
                // guard at the bottom releases against — so the table is consulted once, and the
                // answer at the bottom cannot end up being about a different request.
...
mod tests {
    ...
    /// A store that counts what reached it, so a test can assert an absence rather than infer one.
    #[cfg(feature = "root-admin")]
    #[derive(Debug, Default)]
    ...
    impl Counted {
        ...
        fn hit(&self) -> Result<(), busbar_unit_verbs::StoreError> {
            self.0.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
            Ok(())
        }
    }

    /// The counter behind the trait the loop hands a store through.
    #[cfg(feature = "root-admin")]
    #[derive(Debug, Clone)]
    struct CountingStore(Arc<Counted>);

    #[cfg(feature = "root-admin")]
    impl busbar_unit_verbs::store::Store for CountingStore {
        fn chain_break(
            &self,
            _admin: &busbar_caps::AdminToken,
        ) -> Result<(), busbar_unit_verbs::StoreError> {
    ...
    /// THE THREE CEREMONY VERBS DO NOT REACH THIS COMPOSITION'S STORE, and what an operator running
    /// one on a healthy node therefore gets is the mounted surface's own answer — the same 404 those
    /// paths gave before this wrap existed, because the surface has no route for them.
    ///
    /// Asserted because the opposite reads as true from the composition: `admin_only_sharing` binds
    /// a store that refuses, `chain-break`, `store-restore` and `reseal-epoch-floor` are declared
    /// rows routed into the loop, and the verbs unit has a dedicated store-backed method for each.
    /// What decides is that all three are NEW verbs, and `Verbs::execute` sends a new verb to the
    /// GOVERNANCE seam; the store-backed methods are reached only by a caller that names them, which
    /// this root does not. So the bound store is inert here, and an operator sees the surface rather
    /// than an unavailability this composition invented.
    ///
    /// The second half is what makes that safe to depend on: the day a caller DOES name them, an
    /// unwired store answers "there is nothing here" and not "the store failed" — 404 and not the
    /// 503 that tells an operator to retry something that will never succeed.
    #[cfg(feature = "root-admin")]
    #[test]
    fn the_ceremony_verbs_reach_the_surface_and_not_this_compositions_store() {
        struct Sealed(Option<(PostureCtx, ApprovalState)>);
        impl PostureView for Sealed {
            fn resolve(
                &self,
        ...
        for path in [
            ...
            route(
                &binding,
                Arc::new(CountingStore(Arc::clone(&store))),
                &admin,
                &token,
                &ctx,
                &busbar_kernel::teller::AccrualMeter::new(),
            ...
            .unwrap_or_else(|refusal| {
                panic!("{path} was refused by the loop rather than answered: {refusal:?}")
            });
            binding.units.close(key);
        }
        ...
        assert_eq!(
            store.reached(),
            0,
            "a ceremony verb reached this composition's store; the shipping answer for these three \
             is the mounted surface's, and a store bound here decides nothing about it"
        );

        // And the unwired store's own answer, for the day one of them is routed to it.
        let refusal = busbar_unit_verbs::StoreError::Unconfigured.into_refusal();
        assert_eq!(
            answer_for(Outcome::Failed(
                busbar_caps::StepName::Route,
