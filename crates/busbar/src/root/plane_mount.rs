...
//! is why [`busbar_transport_ws::WsTransport::serve_upgrade`] and
//! [`busbar_transport_ws::WsTransport::pump_session`] are two calls. The loop below races the stop
//! against the WAITING half only. Losing that race drops a pending accept, which tells nobody
//! anything; winning it yields a [`busbar_transport_ws::OpenSession`], and once this loop is holding
//! one the stop cannot take it back, because finishing it is the only thing that can be done with
//! a value that must be moved into the pump.
//!
...
use busbar_contract::transport::session::{SessionDriver, SessionEnd};
...
pub struct Mounted<'m> {
    /// The wire that takes the upgrades. Already composed, already over its lower layer.
    pub wire: &'m WsTransport,
...
/// reach past the point where one is held, because there is nothing to do with an
/// [`busbar_transport_ws::OpenSession`] except move it into the pump.
///
...
pub async fn serve_until<S: Stop>(
    mounted: Mounted<'_>,
