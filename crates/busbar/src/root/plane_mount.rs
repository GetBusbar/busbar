...
struct MountedNode {
    ...
    /// mount's: a gauge or a canary made here would count one plane's traffic against an empty
    /// node, and a second mounted plane doing the same would make two nodes out of one process.
    chain: Arc<PlaneChain>,
}
...
impl MountedNode {
    ...
    fn claims(&self, path: &str, method: &str) -> bool {
        claims_a_path(self.leg.claims(), path)
            || self.chain.claims_the_target(normalise(path), method)
    }
