//! Tests for `units_streaming.rs`. Lifted out of the implementation file so its line count
...
fn the_emitted_audio_class_is_the_one_the_plane_declares() {
    let declared = <StreamingPlane as busbar_contract::plane::PlaneMeta>::METER_CLASSES
        .iter()
...
fn node(io: StreamingIo) -> StreamingNode {
...
fn node_governed_by(io: StreamingIo, groups: busbar_unit_admission::GroupTable) -> StreamingNode {
    node_behind(
        io,
        groups,
...
fn node_behind(
    ...
    StreamingNode::new(StreamingNodeParts {
        plane: StreamingPlane::new(UPSTREAMS),
        groups,
...
fn run(kernel: &Kernel, unit: &StreamingUnit<'_>) -> Ended {
...
fn unit_zero_runs_every_station_and_settles_exactly_once() {
    ...
    let unit = StreamingUnit::new(&node, UnitShape::SessionOpen, 7, 1_700_000_000);
...
fn a_credential_the_door_does_not_accept_ends_the_session_at_authenticate() {
    ...
    for aud in asked.iter() {
        assert_eq!(
            aud.as_deref(),
            Some(<StreamingPlane as busbar_contract::plane::PlaneMeta>::KEY)
        );
    }
}
...
fn a_caller_who_holds_less_than_the_class_requires_is_refused() {
    ...
    let decide = |shape: UnitShape, held: Grants| -> bool {
        let unit = StreamingUnit::new(&node, shape, 7, 1_700_000_000).holding(held);
...
fn a_detached_node_refuses_the_session_at_the_door() {
    let kernel = Kernel::new();
    let node = node(StreamingIo::default());
    let unit = StreamingUnit::new(&node, UnitShape::SessionOpen, 7, 1_700_000_000);
    let Ended::Settled { end, .. } = run(&kernel, &unit) else {
        panic!("a refused unit settles through the same exit");
    };
...
fn a_turn_meters_the_classes_the_plane_declares_with_text_split_by_direction() {
    let kernel = Kernel::new();
    let node = node(serviceable());
    let unit = StreamingUnit::new(&node, UnitShape::Turn, 7, 1_700_000_000)
...
fn an_undeclared_operation_class_is_refused_before_the_door() {
    ...
    let unit =
        StreamingUnit::new(&node, UnitShape::Turn, 7, 1_700_000_000).charging_through(ungoverned());
...
fn each_dialect_dials_its_own_composed_endpoint() {
    let node = node(serviceable());
    let realtime = StreamingUnit::new(&node, UnitShape::SessionOpen, 7, 0);
    assert_eq!(
        realtime.target().map(|t| t.url),
        Some("wss://api.openai.com".to_string())
    );
    let live = StreamingUnit::new(&node, UnitShape::SessionOpen, 8, 0).on_dialect(Dialect::GeminiLive);
    assert_eq!(
...
fn every_dial_takes_the_fail_closed_guard_posture() {
    let node = node(serviceable());
    let unit = StreamingUnit::new(&node, UnitShape::SessionOpen, 7, 0);
...
fn the_opening_unit_seals_under_the_declared_operation_class() {
    ...
    assert_eq!(
        ...
        busbar_unit_audit::record::OpClassId::new("streaming.session.open"),
...
fn a_session_whose_lease_runs_dry_is_closed_and_its_next_frame_refused() {
    ...
    let node = priced_node(StreamingIo {
        dial: Box::new(OpenDial),
    ...
    let turn = StreamingUnit::new(&node, UnitShape::Turn, 7, 1_700_000_000)
        .charging_through(ungoverned())
        .reporting(TurnUsage {
    ...
    let other =
        StreamingUnit::new(&node, UnitShape::Turn, 8, 1_700_000_000).charging_through(ungoverned());
    ...
    let reopened = StreamingUnit::new(&node, UnitShape::SessionOpen, 7, 1_700_000_000);
...
fn plan(kernel: &Kernel, node: &StreamingNode, key: u64, call_id: &str, now_ms: Millis) -> Ended {
...
fn plan_on(
    ...
    let unit = StreamingUnit::new(node, UnitShape::ToolCall, session, 1_700_000_000)
        .charging_through(ungoverned())
        .calling(call_id)
        .at_ms(now_ms);
    let cell = busbar_caps::HoldCell::new(busbar_caps::Hold::open(
...
fn an_unanswered_tool_call_ends_at_the_deadline_its_leg_declared() {
    ...
    let deadline = u64::from(busbar_plane_streaming::plane::TOOL_REPLY_DEADLINE_SECS) * 1_000;
    assert!(
...
fn a_tool_call_that_minted_no_identifier_is_refused_rather_than_entered() {
    ...
    let node = node(serviceable());
    let unit =
        StreamingUnit::new(&node, UnitShape::ToolCall, 7, 1_700_000_000).charging_through(ungoverned());
    let Ended::Settled { end, .. } = run(&kernel, &unit) else {
        panic!("the exit settles it");
    };
    assert!(
...
/// spelling `busbar-streaming` asks them in. Before this port existed the runtime had no way to ask.
#[cfg(feature = "plane-streaming")]
#[test]
fn the_runtimes_port_reaches_the_nodes_own_table() {
    use busbar_streaming::runtime::{GovernedCalls, ReplyRefusal};
...
///
/// The first is that the root composes at all. `mount_root_streaming` seals the registry and then
...
fn the_served_composition_has_no_ungoverned_session_left_in_it() {
    use busbar_streaming::runtime::{Carrier, MeteringPort, SessionCore};
    ...
    crate::compose_streaming_governed_calls();
    let bound = busbar_streaming::mount::served_governed_session()
        .expect("after the root has mounted, every session the door opens is bound to a table");
    let next = busbar_streaming::mount::served_governed_session()
    ...
    assert_eq!(
        ...
        Err(busbar_streaming::runtime::ReplyRefusal::NoSuchSession),
    ...
    let lease = busbar_streaming::runtime::LocalMeteringPort
        .reserve(1_000, 0, None)
        .expect("an uncapped lease always opens");
    let core = SessionCore::new(
        busbar_streaming::ir::codec::OpenAiRealtimeCodec,
        lease,
        None,
        std::sync::Arc::new(busbar_streaming::runtime::EchoToolExecutor),
        Carrier::sideband(),
        None,
    )
    .with_governed(busbar_streaming::runtime::GovernedSession {
        session,
...
fn priced_node(io: StreamingIo) -> StreamingNode {
...
fn a_three_turn_session_pays_one_flat_fee() {
    ...
    let opening = StreamingUnit::new(&node, UnitShape::SessionOpen, 7, 1_700_000_000);
    let Ended::Settled {
        fee: opening_fee, ..
    } = run(&kernel, &opening)
    else {
    ...
    let open = StreamingUnit::new(&fee_bearing, UnitShape::SessionOpen, 7, 1_700_000_000);
    assert_eq!(
...
fn a_paid_turns_record_names_its_principal() {
    ...
    // KNOWN id — "key-streaming-1" — rather than running behind the open door, where every non-empty
    ...
    assert_eq!(
        inputs.subject,
        Subject::PrincipalId("key-streaming-1".to_string()),
    ...
    let unseen = StreamingUnit::new(&node, UnitShape::Turn, 7, 1_700_000_000);
...
fn an_errored_turn_bills_nothing_though_it_located_a_figure() {
    ...
    let clean = StreamingUnit::new(&node, UnitShape::Turn, 7, 1_700_000_000)
        .charging_through(ungoverned())
        .reporting(TurnUsage {
            audio_tokens_out: 120,
            ..TurnUsage::default()
...
fn an_answered_turn_is_one_that_emitted_something() {
    let node = priced_node(serviceable());
    let silent = StreamingUnit::new(&node, UnitShape::Turn, 7, 1_700_000_000)
        .charging_through(ungoverned())
        .reporting(TurnUsage {
            audio_tokens_in: 90,
            ..TurnUsage::default()
    ...
    let spoken = StreamingUnit::new(&node, UnitShape::Turn, 7, 1_700_000_000)
        .charging_through(ungoverned())
        .reporting(TurnUsage {
            audio_tokens_out: 3,
            ..TurnUsage::default()
...
fn a_turn_opens_accrues_settles_and_lands_on_the_journal() {
    ...
    let key = StreamingUnit::balance(&who);
...
fn a_turn_that_outruns_its_reservation_posts_in_full_and_carries_the_rest() {
    ...
    let figures = durability
        .ledger
        .book()
        .get(&StreamingUnit::balance(&who), window);
...
fn a_client_event_opens_a_turn_and_a_later_one_relays_onto_it() {
    ...
    use busbar_plane_streaming::session::StreamingSessionState;
    ...
    let mut state = PlaneSessionState::new(StreamingSessionState::for_dialect(Dialect::OpenaiRealtime));
    ...
    for shape in [UnitShape::SessionOpen, UnitShape::Turn, UnitShape::ToolCall] {
        assert!(
            <StreamingPlane as busbar_contract::plane::PlaneMeta>::OP_CLASSES
    ...
    // class.
    let mut telephony =
        PlaneSessionState::new(StreamingSessionState::for_dialect(Dialect::TwilioMediaStreams));
...
fn a_credential_is_resolved_against_this_planes_own_audience() {
    ...
    .expect("a memory-buffered journal cannot fail to open");
    let node = StreamingNode::new(StreamingNodeParts {
        plane: StreamingPlane::new(UPSTREAMS),
    ...
    let answer = |credential: Option<&str>| {
        let mut unit = StreamingUnit::new(&node, UnitShape::SessionOpen, 7, 1_700_000_000);
...
#[test]
fn a_streaming_group_capped_at_one_turn_refuses_the_second_and_admits_it_after_the_first_ends() {
    ...
    let turn = || StreamingUnit::new(&node, UnitShape::Turn, 7, 1_700_000_000).charging_through(&chain);
...
#[test]
fn a_streaming_caller_bound_to_an_unconfigured_group_is_refused() {
...
fn a_capped_group_still_opens_a_session() {
    ...
    let open =
        StreamingUnit::new(&node, UnitShape::SessionOpen, 8, 1_700_000_000).charging_through(&chain);
...
fn two_turns_of_one_session_are_handed_the_same_chain() {
    ...
    let first = StreamingUnit::new(&node, UnitShape::Turn, 7, 1_700_000_000).charging_through(&chain);
    let second = StreamingUnit::new(&node, UnitShape::Turn, 7, 1_700_000_000).charging_through(&chain);
