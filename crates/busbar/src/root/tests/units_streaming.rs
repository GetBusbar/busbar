...
fn each_dialect_dials_its_own_composed_endpoint() {
    ...
    let live =
        StreamingUnit::new(&node, UnitShape::SessionOpen, 8, 0).on_dialect(Dialect::GeminiLive);
    assert_eq!(
...
fn a_tool_call_that_minted_no_identifier_is_refused_rather_than_entered() {
    let kernel = Kernel::new();
    let node = node(serviceable());
    let unit = StreamingUnit::new(&node, UnitShape::ToolCall, 7, 1_700_000_000)
        .charging_through(ungoverned());
    let Ended::Settled { end, .. } = run(&kernel, &unit) else {
        panic!("the exit settles it");
    };
...
fn a_client_event_opens_a_turn_and_a_later_one_relays_onto_it() {
    ...
    let mut state =
        PlaneSessionState::new(StreamingSessionState::for_dialect(Dialect::OpenaiRealtime));
    ...
    // class.
    let mut telephony = PlaneSessionState::new(StreamingSessionState::for_dialect(
...
fn a_streaming_group_capped_at_one_turn_refuses_the_second_and_admits_it_after_the_first_ends() {
    ...
    let turn =
        || StreamingUnit::new(&node, UnitShape::Turn, 7, 1_700_000_000).charging_through(&chain);
...
fn a_capped_group_still_opens_a_session() {
    ...
    let open = StreamingUnit::new(&node, UnitShape::SessionOpen, 8, 1_700_000_000)
