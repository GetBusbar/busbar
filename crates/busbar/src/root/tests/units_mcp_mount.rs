...
///
/// The SAME chain type every mounted plane is composed on: one process is one node, and a mount that
/// made counters of its own would count this plane's traffic against an empty one.
fn a_chain() -> Arc<crate::root::data_plane::PlaneChain> {
    Arc::new(
        crate::root::data_plane::PlaneChain::over(&busbar_plane_mcp::surface::SURFACE)
