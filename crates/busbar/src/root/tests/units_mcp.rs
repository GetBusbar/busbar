...
fn the_bound_form_authenticates_through_the_nodes_own_seams() {
    ...
    impl VirtualKeyDirectory for OneKey {
        fn verify(
            ...
            (credential == "tok" && expected_aud == Some("mcp"))
                .then(|| KeyFacts::unrestricted("key-mcp-1", "an approved key"))
        }
...
/// configured here" for every pooled deployment, and the trust unit reads that as unpriced — so a
/// card that prices the pool refuses every request that names it.
#[test]
