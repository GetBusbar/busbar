...
///
/// The two shapes are the same shape, so the conversion is a move. What this pins is that it STAYS
/// a move: a field added on one side and dropped in the `From` is the failure mode a hand-written
/// projection has, and a round trip is what catches it. Every field is set to something
/// DISTINGUISHABLE, because a round trip over defaults would pass while silently dropping half.
#[test]
...
///
/// The legacy key's only money-carrying field was its `group`, and it goes to the cost and ledger
/// views. Asserted over the source because the point is an ABSENCE, and an absence has no value to
/// compare. Comments are stripped so the file's prose stays free to explain the rule.
#[test]
