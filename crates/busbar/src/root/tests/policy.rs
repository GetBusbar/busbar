...
use busbar_kernel::config::groups::{LimitCfg, LimitWindow};
...
fn a_pool_scoped_limit_gets_its_own_row() {
    ...
    cheap.scope = Some(busbar_contract::records::ScopeRef::pool("value"));
    let mut dear = limit(LimitMetric::Budget, 500, Some(LimitWindow::Month));
    dear.scope = Some(busbar_contract::records::ScopeRef::pool("frontier"));
