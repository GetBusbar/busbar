...
/// The kernel's answer, written as the counts a case is about: so many transactions, and — until a
/// case is about the door — no visits. The cell names the COUNTS it means rather than building a
/// `Charge` literal five fields wide at every site, and it goes through the same type the
/// production path carries, so a case cannot quietly price a count the kernel would never hand over.
fn charge_of(transaction: u32) -> busbar_kernel::teller::Charge {
    busbar_kernel::teller::Charge {
        transaction,
        units_allowed: true,
        ..busbar_kernel::teller::Charge::default()
    }
}
...
async fn a_provider_origin_unit_posts_no_flat_fee() {
    ...
    let fee = |origin| {
        ...
        crate::root::kernel::default_fee(&unit.evidence(&record).fee, record.head()).0
    };
    assert_eq!(
...
fn the_leg_carries_the_status_leg_the_plane_declares() {
    ...
    // THE DECLARATION IS READ WHERE THE HEAD IS BUILT. The status half of the old evidence moved
    // onto the answer's head, so this is where the leg either reads the plane or answers for it.
    let head = head_facts(Some(200), Some(busbar_contract::FinishClass::Complete))
        .expect("a leg that saw a status records a head");
    assert_eq!(
        head.at, declared,
        "the leg builds the answer's head from what the plane declares, not from a `None` it \
...
fn a_dead_stream_after_a_good_head_is_disputed_and_bills_what_was_delivered() {
    let (fee, flags) = crate::root::kernel::default_fee(
        &fee_facts(true, OriginKind::Client),
        head_facts(Some(200), Some(busbar_contract::FinishClass::Error)).as_ref(),
    );
    assert_eq!(
        fee, 0,
