...
/// deployment fronts.
///
/// The refusal is at APPROVE, which is before Route and therefore before anything is dialled.
#[test]
...
/// "unset" would hand every agent to a key an operator had deliberately scoped down to none.
#[test]
...
/// so a key granted `pool:probe` holds nothing at all on `agent:probe`.
#[test]
