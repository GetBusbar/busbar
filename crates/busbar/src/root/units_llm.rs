...
use busbar_contract::{FinishClass, LaneId, Registration, StatusLeg, UnitKey};
...
/// apart: the origin the kernel sealed and the instant the unit arrived are both facts about a unit
/// that no longer exists to be asked. They travel together because they are one reading taken at
/// one moment — two arguments would let a later caller take one off one unit and one off another,
/// which is how a posting ends up attributed to a request that did not make it.
struct SealedFacts {
...
impl LlmNode {
    ...
    fn attach_late_accrual(
        ...
        let &SealedFacts { origin, arrived } = sealed;
        let Some(book) = self.book.get() else {
            return response;
        };
...
/// THE ANSWER'S HEAD FOR THIS UNIT, off the one thing this plane knows about its answer: the served
/// status the walk saw. The transport reports no status leg of its own here — the response IS the
/// status — so `at` is the first frame and the finish is decided from the frame the client saw.
///
/// It is a free function because the fee is decided twice over one unit (at the exit and on the
