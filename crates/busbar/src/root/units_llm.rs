    ...
    pub async fn answer_with(
        ...
        match entered {
            ...
            Ok(slot) => {
                ...
                self.attach_late_accrual(response, walk, &principal, charged_at, card)
            }
        }
    }
...
mod tests {
    ...
    ///
    /// `CACHE_READ` is reported as OpenAI reports it — `prompt_tokens` is the total and
    /// `prompt_tokens_details.cached_tokens` is the cached prefix inside it, which the dialect's own
    /// reader subtracts to leave the uncached input. So the wire carries `INPUT + CACHE_READ` and the
    /// unit sees `INPUT` uncached and `CACHE_READ` cached, which is what puts a SECOND priced tier on
    /// every delivered answer. Without one, a rate card that priced every tier at the same figure —
    /// or that read the wrong tier's rate — would price identically to a correct one.
    ///
    /// There is no cache-WRITE figure here and there cannot be on this dialect: the OpenAI chat
    /// grammar has no cache-creation field, so its reader reports `None` for that tier whatever the
    /// upstream says. The card's cache-write price is proven where it is reachable — over
    /// [`card_from_config`] — rather than pretended to here.
    const INPUT: u64 = 11;
    const OUTPUT: u64 = 7;
    const CACHE_READ: u64 = 5;

    /// THE DEPLOYMENT'S CONFIGURED RATES, in micro-units per token, one per reserved tier.
    ///
    /// Four DISTINCT figures, and distinctness is the point rather than realism: the four tiers are
    /// read out of one configured entry in a fixed order and written into four card cells in the
    /// same order, and any two of them sharing a value would let a transposition of the pair price
    /// exactly the same as the correct reading.
    ///
    /// They are also not round multiples of each other, so the total is not reachable by pricing the
    /// wrong quantity at the right rate.
    const RATE_INPUT_UTOK: f64 = 2.0;
    const RATE_OUTPUT_UTOK: f64 = 30.0;
    const RATE_CACHE_READ_UTOK: f64 = 0.25;
    const RATE_CACHE_WRITE_UTOK: f64 = 7.5;

    /// The configured rate entry every rig's lane is priced by, in the engine's own config shape.
    fn rate_entry() -> busbar_core::config::RateEntryCfg {
        busbar_core::config::RateEntryCfg {
            input_utok: RATE_INPUT_UTOK,
            output_utok: RATE_OUTPUT_UTOK,
            cache_read_utok: RATE_CACHE_READ_UTOK,
            cache_write_utok: RATE_CACHE_WRITE_UTOK,
        }
    }

    /// The card names the UNROUTABLE model too, and that is a fixture decision worth stating.
    ///
    /// With a card configured, a model the card does not name is refused by the pricing guard with
    /// the dialect's invalid-request 400 — before the routing that would have found it belongs to no
    /// pool. That guard is real and is somebody else's proof; what it would do HERE is collapse the
    /// unroutable fixture's end onto the malformed one and take a distinct end out of the table. So
    /// the card prices the name, and the fixture goes on reaching the routing refusal it is named
    /// for.
    ///
    /// The figures are deliberately unlike the served lane's: nothing prices against them, and a
    /// reading that did would produce a number no assertion here expects.
    fn unroutable_rate_entry() -> busbar_core::config::RateEntryCfg {
        busbar_core::config::RateEntryCfg {
            input_utok: 1_000.0,
            output_utok: 2_000.0,
            cache_read_utok: 3_000.0,
            cache_write_utok: 4_000.0,
        }
    }

    /// The same configured rates as the ROOT's card — built through the production projection, so a
    /// rig prices against the card a boot would have installed rather than against one written here.
    fn root_card() -> busbar_unit_cost::RateCard {
        let raw = rate_entry().raw_tier_rates();
        card_from_config([(LANE, raw)], FEE_CENTS, true)
    }

    /// What one delivered answer costs, in nano-units, derived from the CONFIGURED figures rather
    /// than read back out of the thing under test.
    ///
    /// Spelled as the four multiplies and the fee, in the scale the card converts to: a configured
    /// micro-unit rate is a thousand nano-units, and the flat fee is cents lifted to nano-units. If
    /// this and the posting agree, they agree for a reason.
    fn one_answer_nanos() -> u128 {
        let nanos = |utok: f64| (utok * 1_000.0) as u128;
        u128::from(INPUT) * nanos(RATE_INPUT_UTOK)
            + u128::from(OUTPUT) * nanos(RATE_OUTPUT_UTOK)
            + u128::from(CACHE_READ) * nanos(RATE_CACHE_READ_UTOK)
            + u128::try_from(FEE_CENTS).expect("the fixture fee is positive")
                * busbar_unit_cost::NANOS_PER_CENT
    }
    ...
    /// The usage object the scripted upstream reports, in the dialect's own grammar. One shape for
    /// the buffered answer and the stream's final chunk alike, because that is what OpenAI sends.
    fn wire_usage() -> serde_json::Value {
        serde_json::json!({
    ...
    struct Rig {
        ...
        charged_at: u64,
        group: String,
        /// THE ROOT'S BOOK for this rig, and the reason the late arm has anywhere to post.
        ///
        /// One per rig rather than one per process: the node the drives below build is a fresh one
        /// each time, so a shared book would make every test's figure the sum of every test that ran
        /// before it — and those run concurrently.
        book: Arc<Mutex<crate::root::durability::Durability>>,
    }
    ...
    async fn rig(fixture: Fixture) -> Rig {
        ...
        // THE DEPLOYMENT'S RATE CARD, on both readings of it. The engine resolves its own spend
        // projection from these figures and the root resolves its card from the SAME ones through
        // the production seam, which is the arrangement a boot produces — and the arrangement the
        // reconciliation identity is a claim about. A rig with no card priced every tier at nothing,
        // so the whole of the pricing was a flat fee and a card edit could not be observed at all.
        let rate_card: std::collections::BTreeMap<String, busbar_core::config::RateEntryCfg> = [
            (LANE.to_string(), rate_entry()),
            (
                Fixture::UnknownModel.model().to_string(),
                unroutable_rate_entry(),
            ),
        ]
        .into_iter()
        .collect();
        let cost =
            busbar_core::cost::CostModel::resolve_parts(Some(&rate_card), FEE_CENTS, &groups);
        gov.hydrate_budgets(&cost, 0).expect("hydrate");
        // The process's one card holder, swapped through the same call the boot's rate-apply seam
        // makes. It is a static, so this is the whole process's card; every rig resolves the same
        // figures, so a test that ran beside another still prices at the rates it asserts against.
        crate::root::kernel::ROOT_CARD.apply(Arc::new(root_card()));
    ...
    impl Rig {
        /// EVERYTHING THE ROOT'S BOOK HOLDS for one principal, in nano-units, across every window.
        ///
        /// Summed over the windows rather than read at one, and that is not laziness: the window a
        /// posting lands in comes off the unit's own pinned arrival epoch, and a test that computed
        /// the same window from its own clock read would be a test that fails at midnight rather
        /// than a test that checks anything. The key is still exact.
        fn posted_nanos(&self, principal: &PrincipalId) -> i128 {
            let want = balance(principal);
            let durability = self.book.lock().unwrap_or_else(|p| p.into_inner());
            durability
                .ledger
                .book()
                .iter()
                .filter(|((key, _), _)| *key == want)
                .map(|(_, totals)| totals.settled)
                .sum()
        }

        /// The principal the door resolves this rig's own key to — the one the postings are keyed by.
        fn principal(&self) -> PrincipalId {
            authenticate::principal_id(&self.gov())
        }
    ...
    /// One field of what a leg left behind, by name.
    ///
    /// A name nothing observed is a PANIC and not an empty string. Reading a missing field as `""`
    /// makes `assert_eq!(field(&o, "typo"), "")` pass, which is how three assertions here came to
    /// hold over a field no leg has ever recorded — the comparison was against the default, not
    /// against an observation. There is no divergence a missing field could be: the two legs record
    /// the same list in the same order (the loop below checks that), so an absent name is a name
    /// that was never observed by anybody, and that is a test bug.
    fn field(o: &Observed, k: &str) -> String {
        o.0.iter()
            ...
            .unwrap_or_else(|| {
                panic!(
                    "no leg observes a field called `{k}`; the ones observed are {:?}",
                    o.0.iter().map(|(f, _)| *f).collect::<Vec<_>>()
                )
            })
    }
    ...
    ///
    /// THE TWO LISTS ARE CHECKED FIRST, and that is not ceremony. `zip` stops at the shorter one, so
    /// a leg that stopped recording its last field — or that recorded a different set in a different
    /// order — would have the missing ones silently dropped from the comparison and pass. A leg's
    /// field list is fixed by `observe`, so any disagreement about it is itself the divergence.
    fn compare(label: &str, legacy: &Observed, looped: &Observed, failures: &mut Vec<String>) {
        let want_names: Vec<&str> = legacy.0.iter().map(|(f, _)| *f).collect();
        let got_names: Vec<&str> = looped.0.iter().map(|(f, _)| *f).collect();
        if want_names != got_names {
            failures.push(format!(
                "{label}: the two legs observed different fields\n  shipped: {want_names:?}\n  \
                 loop:    {got_names:?}"
            ));
            return;
        }
    ...
    ///
    /// The node is BOUND TO THE RIG'S BOOK, which is what the composition root does at boot and what
    /// every drive here used to leave undone. Unbound, `attach_late_accrual` returns the response
    /// untouched and the whole money path behind it — the pricing, the posting, the two ends the
    /// wrapper fires on — is never entered, so a suite of unbound drives is green with that arm
    /// deleted.
    async fn drive(rig: &Rig, fixture: Fixture) -> Response {
        let node = LlmNode::new();
        node.bind_book(Arc::clone(&rig.book));
        let arrival = WalkArrival {
            host: rig.host(),
            gov: rig.gov(),
            proto: PROTO,
    ...
    async fn the_exit_arm_puts_the_loops_posting_on_the_journal() {
        ...
        let node = LlmNode::new();
        let ended =
            end_of_one_unit(&rig, &node, Fixture::BufferedOk, rig.gov(), NATIVE_SEATS).await;
    ...
    fn the_root_card_prices_every_configured_tier_at_its_own_rate() {
        ...
        assert_eq!(
            distinct.len(),
            4,
            "two tiers share a rate; a swap would hide"
        );
    ...
    fn no_configured_card_resolves_to_an_absent_one_and_not_to_a_card_of_zeros() {
        let card = card_from_config([(LANE, rate_entry().raw_tier_rates())], FEE_CENTS, false);
        ...
        let rates = card
            .lane_rates("nobody")
            .expect("no card prices every lane");
    ...
    /// fills after the body is consumed, which is after the exit sealed the end. So the money arrives
    /// through the wrapper the exit attaches, and NOTHING before this test drove it — every drive
    /// bound no book, so the wrapper was never attached and the whole arm could be deleted.
    ///
    /// The amount is computed from the CONFIGURED rates rather than read back through the pricing,
    /// and the four tiers are not one figure: an uncached input count, an output count, a cached
    /// prefix and the flat fee, each at its own rate. A posting that priced the right total from the
    /// wrong tiers is a different number here.
    #[tokio::test(flavor = "multi_thread", worker_threads = 4)]
    async fn a_drained_answer_posts_what_its_report_prices_at_onto_the_book() {
        let rig = rig(Fixture::BufferedOk).await;
        let who = rig.principal();
        assert_eq!(
            rig.posted_nanos(&who),
            0,
            "the book must start empty, or the figure below is somebody else's"
        );

        let resp = drive(&rig, Fixture::BufferedOk).await;
        assert_eq!(resp.status(), StatusCode::OK);
        // THE DRAIN. The wrapper fires on the frame that ends the stream, so the body has to be
        // consumed — a test that dropped the response here would be proving the OTHER end.
        let bytes = axum::body::to_bytes(resp.into_body(), usize::MAX)
            .await
            .expect("the answer's body reads");
        assert!(!bytes.is_empty(), "the client was given an answer");

        let expected = i128::try_from(one_answer_nanos()).expect("the fixture's price is small");
        assert_eq!(
            rig.posted_nanos(&who),
            expected,
            "the drained answer's report must post at the configured rates for all four tiers"
        );
        rig.server.shutdown().await;
        // And it is not the fee alone, which is what a card that priced no token line would post.
        let fee_only = i128::from(FEE_CENTS)
            * i128::try_from(busbar_unit_cost::NANOS_PER_CENT).expect("small");
        assert!(
            expected > fee_only,
            "the fixture must price tokens as well as the flat fee"
        );
    }

    /// **THE CUT.** A client that hangs up mid-answer is still billed for what the tap read.
    ///
    /// The other of the wrapper's two ends, and the one the drain cannot reach. The body is polled
    /// for a single frame and then DROPPED where it stands, which is what axum does when a connection
    /// goes away. The arm has to fire from that drop — and it has to fire AFTER the inner body has
    /// been let go, because the engine's own stream wrapper files its report from its own `Drop` and
    /// an arm that read the cell first would read an empty one.
    #[tokio::test(flavor = "multi_thread", worker_threads = 4)]
    async fn a_stream_cut_after_its_first_frame_still_posts_what_the_tap_read() {
        use http_body_util::BodyExt as _;

        let rig = rig(Fixture::StreamedOk).await;
        let who = rig.principal();
        let resp = drive(&rig, Fixture::StreamedOk).await;
        assert_eq!(resp.status(), StatusCode::OK);

        let mut body = resp.into_body();
        let first = body.frame().await;
        assert!(first.is_some(), "the stream yielded a frame before the cut");
        // THE HANG-UP, spelled as the drop it is.
        drop(body);
        // The engine's stream wrapper reports its partial from a drop of its own; give the runtime
        // the tick that lets those drops run before the book is read.
        tokio::task::yield_now().await;

        let posted = rig.posted_nanos(&who);
        assert!(
            posted > 0,
            "a cut stream posted nothing; the previous release charges this request"
        );
        // The FEE is on it whatever the tap read, because the frame that decided the fee was
        // answered before the cut — so the floor is a fee and not a token.
        let fee = i128::from(FEE_CENTS)
            * i128::try_from(busbar_unit_cost::NANOS_PER_CENT).expect("small");
        assert!(
            posted >= fee,
            "a cut stream must still carry the flat per-request fee: {posted} < {fee}"
        );
        assert!(
            posted <= i128::try_from(one_answer_nanos()).expect("small"),
            "a cut stream cannot cost more than the whole answer would have"
        );
        rig.server.shutdown().await;
    }

    // The ordering inside the wrapper's own drop — inner body released FIRST, arm fired second — is
    // what the assertion above is sensitive to and is not asserted separately. Reversed, the arm
    ...
    async fn the_loop_leaves_the_money_where_the_shipped_plane_leaves_it() {
        ...
        assert_eq!(
            ...
            (INPUT + OUTPUT + CACHE_READ).to_string(),
            "the stream-end tap accrued the reported split"
        );
        assert_eq!(
            field(&streamed, "metering_rows"),
            format!("{LANE}/test in={INPUT} out={OUTPUT} cr={CACHE_READ} cw=0 req=1 billable=1")
        );
    ...
    async fn an_empty_url_model_ends_where_the_shipped_path_model_entry_point_ends_it() {
        ...
        for proto in [GEMINI, BEDROCK] {
            ...
            compare(
                &format!("{proto} on a URL that named no model"),
                &shipped,
                &looped,
                &mut failures,
            );
    ...
    /// The NAMES only; the pointers beside them are driven by
    /// [`every_path_table_row_answers_in_the_dialect_it_is_named_for`].
    #[test]
    ...
    /// THE URL each path-model dialect's client sends for a buffered chat on the fixture's model.
    ///
    /// Spelled here, unlike the body table's endpoints, because these are the URL SPACES the two
    /// dialects parse rather than endpoints either of them renders — there is nothing to read them
    /// off. They are the same URLs `busbar_llm`'s own parse tests drive, which is what keeps this
    /// file's guess and that file's subject the same string.
    fn path_url(proto: &str) -> String {
        let model = Fixture::BufferedOk.model();
        if proto == GEMINI {
    ...
    /// **EVERY ROW OF THE PATH TABLE IS DRIVEN.** The path-axis twin of the body-table drive, and
    /// for the same reason: two names in the right order say nothing about the two pointers beside
    /// them, and a transposition sends every gemini URL through bedrock's parse — which reads no
    /// model out of it, so the surface answers a refusal to every request on both dialects.
    ///
    /// Each row is reached through its OWN pointer with its own dialect's URL, and its answer is
    /// compared against the shipped path-model entry point driven with the facts that URL means.
    #[tokio::test(flavor = "multi_thread", worker_threads = 4)]
    async fn every_path_table_row_answers_in_the_dialect_it_is_named_for() {
        let mut failures: Vec<String> = Vec::new();
        for (name, row) in PATH_INGRESS {
            // LEG 1 — the shipped path-model entry point, with this dialect's own facts.
            let legacy = leg_legacy_path(Fixture::BufferedOk, name).await;

            // LEG 2 — THE ROW, through the pointer the table holds beside this name, over the URL a
            // client of this dialect sends. The row does its own parse; nothing is handed to it.
            let rig = rig(Fixture::BufferedOk).await;
            let url = path_url(name);
            let resp = row(ArrivalRequest {
                host: Arc::new(NoRejectionExpected),
                ...
                uri: url
                    .parse::<axum::http::Uri>()
                    .expect("the dialect's own URL is a URI"),
                headers: json_headers(),
                body: path_body(name),
            })
            .await;
            let looped = observe(&rig, resp).await;
            rig.server.shutdown().await;

            compare(
                &format!("path table row `{name}`"),
                &legacy,
                &looped,
                &mut failures,
            );
        ...
        assert!(
            failures.is_empty(),
            "{} divergence(s) across the {} rows of the path table:\n{}",
            failures.len(),
            PATH_INGRESS.len(),
            failures.join("\n")
        );
    ...
    ///
    /// The NAMES only. What sits beside each name is a bare `fn` pointer, and two rows whose
    /// pointers were swapped have exactly these names in exactly this order — every openai request
    /// answered as anthropic, and the table reading as correct. That half is
    /// [`every_body_table_row_answers_in_the_dialect_it_is_named_for`], which drives them.
    #[test]
    ...
    /// The `ArrivalHost` a table row is handed when it is driven here.
    ///
    /// Every method on it is the pre-routing rejection path — the branch a request whose URL the
    /// dialect names no operation for takes. The drives below send each dialect to its OWN endpoint,
    /// so none of them is reached, and each says so rather than answering something plausible: a
    /// stand-in that returned a 404 would let a row that took the rejection branch pass as a row
    /// that answered.
    struct NoRejectionExpected;

    impl busbar_substrate::ingress::arrival::ArrivalHost for NoRejectionExpected {
        fn finish_rejected(
            &self,
            _ctx: &busbar_substrate::ingress::arrival::ArrivalCtx,
            proto: &str,
            _pool: &str,
            _started: Instant,
            _charged_at: u64,
            ...
            panic!("{proto}: the row rejected a request before routing")
        }
        ...
        fn ingress_error(
            ...
            panic!("{proto}: the row refused before routing: {message}")
        }
        ...
        fn envelope_dialect(
            ...
            panic!("the row asked for an envelope dialect for {path}")
        }
        ...
        fn fallback_not_found(
            ...
            panic!("the row named no operation for its own endpoint {path}")
        }
    ...
    /// One dialect's OWN chat endpoint, read off the dialect's own handler.
    ///
    /// Read rather than written down, and from the same constant the dialect's inbound
    /// `resolve_operation` matches against — a URL spelled here would be this file's guess about
    /// somebody else's surface, and would go stale the day the surface moved.
    fn chat_endpoint(proto: &str) -> String {
        busbar_substrate::handlers::request_handler(proto)
            .unwrap_or_else(|| panic!("{proto} has a request handler"))
            .upstream_path(&busbar_substrate::wire::EgressCtx {
                operation: busbar_api::operation::Operation::CHAT,
                model: POOL,
                stream: false,
                path_base: None,
    ...
    /// **EVERY ROW OF THE TABLE IS DRIVEN**, not only named.
    ///
    /// A `fn`-pointer table is two things — a list of names and a list of targets — and comparing it
    /// to the plane's compares only the first. Transpose two of the targets and the names still
    /// match, in order, and every request on those two dialects is answered in the other's wire
    /// shape: a `model` field the caller never sent, an error envelope no client of that dialect
    /// parses, and a table that reads as correct in every test there was.
    ///
    /// So each row is REACHED THROUGH ITS OWN POINTER, with a request built for the name beside it,
    /// and its answer is compared against the shipped entry point for that same name. A transposed
    /// row answers in the wrong dialect and the bytes say so.
    #[tokio::test(flavor = "multi_thread", worker_threads = 4)]
    async fn every_body_table_row_answers_in_the_dialect_it_is_named_for() {
        let mut failures: Vec<String> = Vec::new();
        for (name, row) in BODY_INGRESS {
            let body = dialect_body(name, Decoded::Named);
            // LEG 1 — the shipped entry point, asked for this dialect BY NAME.
            let legacy =
                leg_legacy_decode(name, busbar_api::operation::Operation::CHAT, body.clone()).await;

            // LEG 2 — THE ROW, through the pointer the table holds beside this name.
            let rig = rig(Fixture::BufferedOk).await;
            let endpoint = chat_endpoint(name);
            let resp = row(ArrivalRequest {
                host: Arc::new(NoRejectionExpected),
                ctx: busbar_substrate::ingress::arrival::ArrivalCtx::new(ArrivalPayload {
                    host: rig.host(),
                ...
                uri: endpoint
                    .parse::<axum::http::Uri>()
                    .expect("the dialect's own endpoint is a URI"),
                headers: json_headers(),
                body,
            })
            .await;
        ...
        assert!(
            failures.is_empty(),
            "{} divergence(s) across the {} rows of the body table:\n{}",
            failures.len(),
            BODY_INGRESS.len(),
            failures.join("\n")
        );
