...
impl Units for LlmUnit<'_> {
    ...
    fn meter(
        ...
        //
        // THE PRICING IS NOT HERE EITHER, and this step names no amount. The Meter step assembles
        // what the unit consumed and hands the report over; the one side that holds a card prices it,
        // once, and that is the late accrual below ([`LateAccrual::post`]) — resolved against the
        // SNAPSHOT this unit was admitted under and at the instant it arrived, through the same
        // `priced_amount` expression, so one report is never priced two ways. There is nothing for
        // this step to spend and no card for it to read.
