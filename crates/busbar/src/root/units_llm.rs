...
///
/// `rates` is the deployment's `rate_card:` as the neutral per-lane raw view, and `per_request_fee`
/// is its flat per-request fee — the SAME two configured figures the previous release's usage
/// projection derives a row's spend from. Read in the composition root, because that is the one
...
/// exactly what the previous release bills for that deployment. Building nothing instead would post
/// nothing for a node that charges a fee.
///
