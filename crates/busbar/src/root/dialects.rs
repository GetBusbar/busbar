...
///
/// One row per dialect crate. `busbar-plane-llm-openai` is the first of the six the plane's ladder
/// was written for; the remaining five are still the plane's own rows and move here, one row at a
/// time, as each is cut out.
pub const LLM: &[DialectEntry] = &[busbar_plane_llm_openai::dialect::ENTRY];
