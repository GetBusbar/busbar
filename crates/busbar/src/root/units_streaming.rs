...
//!
//! `busbar-streaming` is the half of the plane that owns sockets: the telephony carrier, the WebSocket
...
//!
//! 1. [`ProviderDial`] — `busbar_streaming::topology::dial_provider`, which selects the WebSocket
...
//! 2. [`SessionPump`] — `busbar_streaming::runtime::{SessionCore, StreamingSession, UplinkForwarder,
//!    Outbound}`, the per-frame loop over a byte duplex.
//! 3. [`SessionLease`] — `busbar_streaming::runtime::{MeteringPort, MeteringLease, LeaseState,
//!    LeaseCloseGuard}`, the reserve-then-settle object the hold is driven through.
//! 4. [`Carrier`] — `busbar_streaming::topology::telephony` and `busbar_streaming::runtime::carrier`, the
...
///
/// Satisfied by `busbar_streaming::topology::dial_provider`. That function selects the WebSocket
...
///
/// Satisfied by `busbar_streaming::runtime::{SessionCore, StreamingSession, UplinkForwarder, Outbound}`. The
...
/// `busbar_streaming::runtime::GovernedCalls` is declared over there and implemented here, the way that
...
///
/// Satisfied by `busbar_streaming::runtime::{MeteringPort, MeteringLease, LeaseState, LeaseCloseGuard}`.
...
///
/// Satisfied by `busbar_streaming::topology::telephony` and `busbar_streaming::runtime::carrier`. The claim
...
impl std::fmt::Debug for StreamingIo {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("StreamingIo").finish_non_exhaustive()
    }
}
...
/// socket: a client's reply arriving, and the tick beside the pump. Both live in `busbar-streaming`, and
...
/// already does: `busbar-streaming` declares the port, the root implements it, and what crosses is two
...
pub struct NodeCalls {
    node: std::sync::Arc<StreamingNode>,
}
...
impl NodeCalls {
    ...
    #[must_use]
    pub fn new(node: std::sync::Arc<StreamingNode>) -> Self {
        NodeCalls { node }
    }
}

#[cfg(feature = "plane-streaming")]
impl busbar_streaming::runtime::GovernedCalls for NodeCalls {
    fn replied(
        &self,
        session: u64,
        call_id: &str,
        ...
        match self.node.tool_calls.replied(session, correlates) {
            ...
            Err(ReplyRefused::UnknownCall) => Err(busbar_streaming::runtime::ReplyRefusal::UnknownCall),
        }
    }
...
impl std::fmt::Debug for StreamingNode {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("StreamingNode")
...
impl StreamingNode {
    ...
    #[must_use]
    pub fn new(parts: StreamingNodeParts) -> Self {
        StreamingNode {
            plane: parts.plane,
...
pub struct StreamingUnit<'n> {
    /// The node's long-lived half.
    pub node: &'n StreamingNode,
...
impl std::fmt::Debug for StreamingUnit<'_> {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("StreamingUnit")
...
impl<'n> StreamingUnit<'n> {
    /// A unit of one shape, on one session.
    #[must_use]
    pub fn new(node: &'n StreamingNode, shape: UnitShape, session: u64, epoch: u64) -> Self {
        StreamingUnit {
            node,
            shape,
            session,
...
impl Units for StreamingUnit<'_> {
