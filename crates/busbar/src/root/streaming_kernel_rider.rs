...
//! THE DORMANT STREAMING KERNEL-LOOP SESSION RIDER — the fourth WITNESS on the generic bridge, and the
//! first of the two SESSION planes (streaming, duplex) onto the unified kernel loop's session admit.
//!
//! This module onboards the STREAMING session-open onto the unified kernel loop the SAME way MCP/A2A/LLM
...
//! [`admit_streaming_session_via_kernel`] drives the SAME streaming `GauntletPlane` — the `SessionGauntlet`
//! in `busbar-voice`'s `topology/mod.rs`, whose `verify_destination` refuses a denied upstream model and
...
//!
//! MONEY STAYS EXACTLY PLANE-SIDE, AFTER THE GATE. The streaming session's reserve-on-admit
...
//!
//! DORMANT — DUAL-PATH, AUTHORITY NOT FLIPPED. The shipped streaming authority stays
...
//! byte-identical here, but the one-line flip — registering the kernel session runner under the streaming
...
//! `PlaneAnswer` and `PlaneInFlight` carry no `streaming` noun. This module is the ONLY place the streaming
//! rider names streaming, and it does so only to point the neutral bridge at the streaming session gate.
...
/// OPEN a streaming session through the UNIFIED kernel loop and return at the door — the dormant
/// kernel-loop twin of the shipped `run_gauntlet_session` admit the streaming plane reaches through
/// `begin_session` (`topology/mod.rs`) and `accept_gauntlet` (`mount.rs`).
///
/// A one-line delegation to the plane-neutral session bridge: the streaming `SessionGauntlet` rides it with
...
/// nothing else changes, because the streaming gate's `verify_destination` is byte-identical on both loops.
///
...
pub(crate) fn admit_streaming_session_via_kernel(
...
#[cfg(test)]
#[path = "tests/streaming_kernel_rider.rs"]
mod tests;
