...
//! THE A2A PLANE'S MOUNT: one HTTP surface, one loop, one answer.
//!
...
//!
//! A path list in this file would be a second declaration of where this protocol lives, and the one
//! that drifts is the one nobody re-derived. So the claim is asked TWICE, of two things that already
//! know:
//!
//! 1. **The plane's own claim table** says which addresses this protocol occupies. It is walked
//!    generically — an exact path or a segment pattern, matched by the grammar's own shapes — so this
//!    file names no route of this protocol and gains one for free when the plane declares one.
//! 2. **The plane's own decode** says whether these particular bytes are a unit it has. A body it
//!    does not recognise is not a unit of this plane, and it goes straight to the surface that
//!    already answers it — with the 404, the 405 or the document that release pinned.
//!
//! That second question is what keeps a GET of an agent's card, or a REST route the plane declares
//! and does not name an operation for, byte-for-byte what it was: the loop takes the units it can
//! name and hands back everything else untouched.
//!
...
/// `/a2ax` is somebody else's path, and a `starts_with` would take it.
///
...
/// mounted router below serves it. What decides whether the loop takes those bytes is the question
/// after this one — the plane's own decode — and a body it cannot name is handed straight back.
#[must_use]
pub fn claims_the_path(path: &str) -> bool {
    busbar_plane_a2a::claims::CLAIMS
        .iter()
        .any(|claim| selector_matches(&claim.selector, path))
}
...
fn selector_matches(selector: &Selector, path: &str) -> bool {
    match selector {
        Selector::ExactPath(exact) => *exact == path,
...
/// identifier is the empty string, which is an address the router below does not serve.
///
...
///
/// A header value is bytes rather than text, and this is the one place that matters: this protocol's
    ...
    axum::Router::new().fallback(axum::routing::any(
        move |req: axum::http::Request<axum::body::Body>| {
            ...
            async move {
                ...
                // routing already answers, with the document, the 404 or the 405 that release
                // pinned. Manufacturing a status here for bytes this plane never claimed would be
                // the root inventing an answer it has no basis for.
...
/// The node one unit of this plane is answered by.
///
...
impl MountedNode {
    ...
    fn answer(
        ...
        match answer {
            ...
            None => refused_response(&self.leg, arrival, &ended),
        }
    }
}
...
/// helpers the generic driver uses — so a caller refused by the mounted loop and one refused by the
...
/// this protocol's own transport spells for each. That narrowing lives in one place for the same
...
///
/// One literal, and the same one the plane's own declaration names for every operation that answers
/// with a document rather than a run of events. It is here because a refusal is written by the plane
/// and framed by the mount, and the frame is this file's half.
...
/// value of writing the narrowing down once. Each pair is the status this protocol's surface already
/// answers the same condition with.
#[must_use]
fn status_of(outcome: busbar_contract::transport::Outcome) -> u16 {
