...
/// The Route step's record legs: what one answers, why one refuses, and the runner that walks a
/// planned leg against the kernel-held store.
///
/// It moved to [`crate::root::units_mcp_records`] when the structural cap said this file was too
/// big again, and the cap picked a real seam: everything here is the request path's decode, judge
/// and encode; everything there is one closed record vocabulary reached from the Route step.
///
/// Re-exported here, so every caller and every cell reads the same names it always did.
#[allow(unused_imports)]
pub use crate::root::units_mcp_records::{RecordAnswer, RecordLeg, RecordRefusal, Records};
...
// ═════════════════════════════════════════════════════════════════════════════════════════════════
//   THE BOOT SEAL, WHICH IS NOT THIS FILE'S
// ═════════════════════════════════════════════════════════════════════════════════════════════════

/// The boot half of this plane's composition: the checks that run ONCE, before any listener is
/// bound, and the pricing table read off a rate map.
///
/// It moved to [`crate::root::units_mcp_seal`] when the structural cap said this file was too big,
/// and the cap picked the right seam: everything above is the REQUEST path, reached per arrival, and
/// everything there runs once at boot. Two lifetimes in one file is how a reader ends up looking for
/// a per-request cost in a function that runs before the first connection.
///
/// Re-exported here, so every caller and every cell reads the same names it always did.
#[allow(unused_imports)]
