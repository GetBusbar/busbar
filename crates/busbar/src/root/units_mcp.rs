...
/// single registered SERVER, which is the degenerate pool of one. A pool key is not a registration id
/// and matches no [`Server`] — so a view that resolved only the server table would answer "not
/// configured" for every pooled deployment, and the trust unit reads that as UNPRICED and refuses
/// every request the card does price.
///
