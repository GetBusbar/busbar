...
//!
//! `oauth-as` 0.9.2 ships a CIMD *validator* behind a `cimd` feature; the 0.9.1 this tree pins
//! has neither. The document checks in [`materialize`] are therefore busbar's own — real checks,
//! enforced today — and the one call the 0.9.2 bump replaces is marked `TODO` at the site, tied
//! to `chore/1.6.0-oauth-as-0.9.2`. The fetch and the seam are unchanged by that bump: the
//! library validates only, it does not fetch, and it exposes no client-resolution hook.
...
///
/// TODO(chore/1.6.0-oauth-as-0.9.2): `oauth-as` 0.9.2 ships a CIMD document validator behind its
/// `cimd` feature; this tree pins 0.9.1, which has neither. When the bump branch lands, the
/// checks below hand over to (or are cross-checked against) the crate's validator AT THIS CALL
/// SITE — the fetch, the seam and the ceiling are unchanged by that bump. The checks below are
