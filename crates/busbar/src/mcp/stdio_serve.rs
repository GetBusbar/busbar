...
//! [`crate::ingress::protocol::serve`], with the same [`super::envelope::McpWords`], the same
...
pub(crate) async fn session_identity(
    ...
    if let Some(token) = credential {
        use crate::auth::audience::{self, Binding};
    ...
    match crate::auth::resolve_data_plane_identity(&app, verdict) {
        ...
        Err(crate::auth::IdentityRefusal::Denied) => Err(if credential.is_none() {
            format!(
        ...
        Err(crate::auth::IdentityRefusal::NoGrant) => Err(format!(
...
/// plane-local `serve` beside `a2a::grpc::serve`.)
pub(crate) async fn serve_stdio(handle: Arc<AppHandle>) -> i32 {
    let credential = std::env::var(ENV_CREDENTIAL).ok().filter(|c| !c.is_empty());
    let identity = match session_identity(&handle, credential.as_deref()).await {
...
impl<W: AsyncWrite + Unpin + Send + 'static> Session<W> {
    ...
    fn route_reply(&self, line: &[u8]) -> bool {
        ...
        let outcome = match crate::ingress::jsonrpc::read_response(&value, id) {
            Ok(crate::ingress::jsonrpc::Reply::Result(result)) => Ok(result),
            Ok(crate::ingress::jsonrpc::Reply::Error { code, message }) => Err(format!(
