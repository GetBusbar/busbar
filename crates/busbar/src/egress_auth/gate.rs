...
pub(crate) struct Requirement<G> {
    pub(crate) grant: G,
...
#[derive(Clone, Debug, PartialEq, Eq)]
pub(crate) enum EgressRefusal<G> {
...
pub(crate) struct EgressGrant<S> {
    subject: S,
}
...
pub(crate) fn authorise<S: EgressSubject>(
    caller: &VirtualKey,
    subject: S,
    now: u64,
