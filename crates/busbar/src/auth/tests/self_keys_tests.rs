...
use super::super::self_keys::{
    issue_key, resolve_exchange, DeterministicEd25519Keys, ExchangeError, IssuedKey, SelfServeKeys,
};
...
fn identity_from_principal_not_body() {
    ...
    let (p, _pools) = resolve_exchange(&v, &rb).expect("bound principal admitted");
    assert_eq!(
        p.id, "sam",
...
fn resolve_then_issue_via_real_seam() {
    ...
    let rb = bindings("oidc", "eng", Some("team"));
    let v = identified("oidc", principal("sam", &["eng"]));
    let (p, pools) = resolve_exchange(&v, &rb).unwrap();
    let keys = DeterministicEd25519Keys::new(gov.clone(), pools);
...
fn endpoint_is_scheme_agnostic_over_the_trait() {
    let rb = bindings("oidc", "eng", Some("team"));
    let v = identified("oidc", principal("sam", &["eng"]));
    let (p, _pools) = resolve_exchange(&v, &rb).unwrap();
