...
// The WS transport is the edge of the linked plane carrying the `ws-arrivals` axis and is compiled
// only with it, so this witness names it only in a build that carries it.
