...
//
// Several boot-a-real-child integration tests (and the `hook_path` bench) each carried their OWN
// copy of a `free_port()` that bound `127.0.0.1:0`, read the port back, and DROPPED the listener —
// before the rest of fixture setup (writing config/provider YAML, shelling out to
...
// worker threads (an ephemeral `:0` there hands each worker a DIFFERENT port), and every listener's
// boot line prints the CONFIGURED address (`cfg.listen`/`cfg.admin_listen`), never the bound one,
// so there is no line a test could read a dynamically-assigned port back off. The number has to be
// chosen up front, which is exactly the TOCTOU the `free_port()` copies accepted as a known,
// unfixed risk: the OS can (and, by measurement, does) hand this process's freed port to another
// process on a shared box before the child gets to it.
//
// `ReservedPort` does not make that window zero — nothing short of every process on the box
// coordinating could — but the copies were leaving it far WIDER than the race itself needs:
// `free_port()` dropped the listener immediately and only afterward did the fixture write YAML,
// shell out to a `--generate-signing-key` child, or boot a mock upstream. `ReservedPort` holds the
// OS-assigned listener open across all of that; the caller drops it (via `release`, or simply
// letting it go out of scope) only IMMEDIATELY before the `Command::spawn` that binds the number,
// shrinking the window from "however long fixture setup takes" down to a `drop` followed by a
// syscall.
pub struct ReservedPort(std::net::TcpListener);

impl ReservedPort {
    /// Ask the OS for a free loopback port and hold it open. Panics — like every `free_port()` copy
    /// this replaces already did — if the OS has none to give; there is no meaningful fallback for a
    /// test fixture.
    pub fn reserve() -> Self {
