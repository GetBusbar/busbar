...
//! plane-streaming`: the composition root links `busbar-streaming`, the plane OWNS the `streams:` section,
...
/// `DeployCfg` deserialize/validate it without naming a `busbar_streaming` type.
#[test]
fn streaming_decl_owns_streams_and_wires_the_section_hooks() {
    let d = &busbar_streaming::PLANE_DECL;
...
#[test]
fn dup_claim_guard_admits_the_real_streaming_decl() {
    check_owned_config_claims(&[&busbar_streaming::PLANE_DECL], CORE_OWNED_CONCRETE_SECTIONS).expect(
...
fn dup_claim_guard_refuses_a_planted_streams_collision() {
    let err = check_owned_config_claims(
        &[&busbar_streaming::PLANE_DECL, &RIVAL_CLAIMS_STREAMS],
    ...
    assert!(
        err.contains("streams") && err.contains("streaming") && err.contains("rival-streaming"),
...
#[test]
fn registering_streaming_puts_streams_into_config_sections() {
    register_test_plane(&busbar_streaming::PLANE_DECL);
