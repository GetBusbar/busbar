...
//! THE PROTOCOL REGISTRY OVER THE LINKED PLUGINS' DECLARATIONS — and the acceptance test for the
//! whole step.
//!
...
//!
//! These tests exercise the HOST's registry (its fold, its refusals, its boot merge) over the
//! declarations the composition root installs — the LLM plugin's six dialects and the MCP
//! declaration — so they live here, where the host and the linked plugins meet. They moved from the
//! LLM codec's suite (#83a SD-3): the codec's tests prove the codec's own seam and never reach the
//! host.
...
use busbar_contract::protocol::{IngressAuth, ProtocolDecl};
use busbar_contract::SlabBytes;
use busbar_kernel::proto::Registry;
...
fn registered() {
    ...
    REGISTER.call_once(|| busbar_kernel::proto::register_test_protocols(busbar_llm::DECLS));
}

fn builtins() -> Registry {
    Registry::new(busbar_llm::DECLS.iter().copied())
}
...
fn the_absorbed_sweeps_produce_the_sets_they_produced_before() {
    ...
    assert_eq!(
        ...
        &[busbar_llm::gemini::GEMINI_JSON_ARRAY_SHIM_KEY],
...
fn a_declaration_without_a_codec_dispatches_but_is_not_a_provider_protocol() {
    ...
    let d = busbar_kernel::proto::decl_for("mcp").expect("mcp declares itself");
    assert!(d.codec.is_none());
    assert!(d.handler.is_some(), "mcp serves operations");
    assert!(
        busbar_llm::proto_codec::protocol_for("mcp").is_none(),
        "no codec means no cross-dialect translation into or out of it"
    );
    assert!(
        !busbar_kernel::proto::known_protocols().contains(&"mcp"),
...
fn a_protocol_nobody_wrote_costs_a_declaration_and_nothing_else() {
    ...
    assert!(
        busbar_kernel::proto::decl_for("telex").is_none(),
...
fn a_codec_less_declaration_does_not_move_the_operator_visible_list_when_it_is_folded_ahead() {
    ...
    let with_codecs: Vec<&'static ProtocolDecl> = busbar_llm::DECLS
        .iter()
        .copied()
...
fn installed_declarations_are_folded_ahead_of_the_builtins() {
    ...
    let merged = busbar_kernel::proto::merged_boot_decls(&[&EXTRACTED], &[&BUILTIN_A, &BUILTIN_B]);
...
fn a_later_registration_of_a_declared_name_is_skipped_keeping_the_first() {
    ...
    let merged =
        busbar_kernel::proto::merged_boot_decls(&[&INSTALLED_COPY], &[&BUILTIN_COPY, &OTHER]);
...
fn a_registry_with_no_declarations_reports_no_protocols_at_all() {
    ...
    let decls = busbar_kernel::proto::merged_boot_decls(&[], &[]);
...
fn a_url_model_protocol_without_a_registered_arrival_is_caught() {
    ...
    assert_eq!(
        busbar_kernel::proto::first_path_model_without_arrival(decls, &[]),
        Some("telex"),
    ...
    assert_eq!(
        busbar_kernel::proto::first_path_model_without_arrival(decls, &["telex"]),
        None,
    ...
    assert_eq!(
        busbar_kernel::proto::first_path_model_without_arrival(body_model, &[]),
        None,
