...
/// one document, and they are not rows of the eighty-four because the eighty-four are the admin
/// plane's own.
///
...
async fn the_eighty_four_are_split_between_the_loop_and_the_surface() {
    ...
    for template in documented_gets().await {
        ...
        let Some(row) = busbar_plane_admin::verbs::resolve("GET", &path) else {
            // Not a row of the eighty-four. The only documented GETs that can be true of are the
