...
const SINGLE_PLANE_ALLOWLIST: &[(&str, &str, &str)] = &[
    ...
    ("session_meter", "voice", "Neutral single-consumer capability: the count-only live-carrier session meter (open over a kernel-derived budget, raw counts per turn, Live/MustClose back) - the plane-facing face of the kernel's pricing (#43); generic signature, no voice-transport vocabulary. Sole current caller is the voice plane's hosted runtime. Re-review if a second plane meters a live carrier."),
