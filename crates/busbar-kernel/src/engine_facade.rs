...
// R5-store: named from `busbar_kernel::store`, where both already live, rather than through
...
pub use busbar_kernel::store::{Admit, LaneRuntime};
...
// candidate/stage/refusal/order/walk_with FAMILY is already `pub` at `busbar_kernel::failover`.
...
// Already `pub` at `crate::proxy` (re-exported there from `busbar_kernel::egress::engine`);
...
// R5-store: named from `busbar_kernel::topology`, its defining module since the WEDGE 3-PREP
// relocation, rather than through core's `crate::state::…` shim.
pub use busbar_kernel::topology::UpstreamClients;
