...
//! `busbar_kernel::config::groups` and are re-exported below at this historical path, so no
//! caller of `busbar_kernel::config::groups::*` moves. This module keeps the tree VALIDATION and the
