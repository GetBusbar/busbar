...
//! concrete stream IR. In core's test/`test-support` build the plugin's `proto_stream` is
//! `#[path]`-netted back as `crate::proto::stream`, so the factory routes there directly; in
//! production it routes through the installed pointer (the composition root installs it alongside
//! `install_protocols`).
...
///
/// Under core's OWN test binary (`cfg(test)`) it routes to the dialect plugin's concrete factory DIRECTLY
