...
//! through, and the genuinely cross-plane `invoke`/`subscribe` leaves (shared by more than one
//! resident plane). The CONCRETE chat IR and the leaf-op IR specific to one plane
//! (`IrRequest`/`IrResponse`/`IrBlock`/`Embeddings*`/… — everything that names that plane's own wire
//! shape) RELOCATED to the `busbar-llm` plugin at the G6 A4b cutover, so core reads a request only
//! through the neutral projection and never names a concrete plane-family type in production. The
...
//
// The concrete chat IR and that plane's own leaf-op IR (`IrRequest`/`IrResponse`/`IrBlock`/
// `Embeddings*`/… — the five leaf-op IR modules and the `IrFacts for IrRequest` projection) live
// wholly in the `busbar-llm` plugin crate (`crates/busbar-llm/src/ir/*`). Their `#[path]` witness
// re-includes into core (which made `crate::ir::IrRequest` etc. resolve at this module root for core's
// own test binary) were DELETED once Phase 1.6 drained core's own suite of any dependence on the
// witnessed concrete IR: the concrete-IR unit tests moved to `busbar-llm/src/ir/tests/`, beside the
// types they exercise. Production core reads a request only through the neutral projection (`facts`)
// and drives translation through the sealed neutral `IrHandle` (`handle`); it names no concrete
// plane-family type.
