...
/// `busbar_kernel::trust::VerifyGate` directly while an in-core plane consumer keeps naming
...
pub trait PinnedArtifact: Clone + PartialEq + std::fmt::Debug {
...
pub struct Observation<A: PinnedArtifact> {
...
#[derive(Clone, Debug, PartialEq)]
pub enum Sighting<A: PinnedArtifact> {
    /// Never contacted.
    Never,
...
#[derive(Clone, Debug, PartialEq, Eq)]
pub struct Approval<A: PinnedArtifact> {
    pin: Option<A>,
...
impl<A: PinnedArtifact> Approval<A> {
    ...
    pub fn declared(pin: A, capabilities: BTreeMap<String, String>) -> Self {
        Self {
            pin: Some(pin),
    ...
    pub fn state(&self, sighting: &Sighting<A>) -> TrustState {
    ...
    /// compare against.
    pub fn drift(&self, sighting: &Sighting<A>) -> Drift {
        sighting
            .observation()
    ...
    pub fn serves(&self, capability: &str, observed_digest: &str) -> bool {
...
// THE RE-VERIFICATION CADENCE — when to look again at a pinned upstream. Relocated here in Phase-B
// B1 with the trust value families it is generic over; core re-exports it so `crate::trust::reverify`
// resolves unchanged.

/// THE DECLARED PIN, READ ONCE — *"what authenticity root did the operator write down?"*, asked in
/// one order for every plane. Relocated in Phase-B B1; core re-exports it.
pub mod declared;
...
// The verify-on-call gate type, surfaced at the `trust` root so `busbar_kernel::trust::VerifyGate`
