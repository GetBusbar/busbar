...
//! THE FAILOVER SEAM — one selection, one admission, one disposition, in core, for every plugin that
//! has more than one place to send a request.
//!
//! ## Why this is core and not a plugin's
//!
//! Standing architectural rule: *"core needs to be the core. if its used 2x or could be, its core."*
//! Two upstream registrations can be interchangeable deployments of the same thing, and busbar cannot
//! be told they are interchangeable without a shared mechanism to check the claim:
//!
//! > *two upstreams can expose an equivalent capability, and busbar simply cannot be told they are
//! > interchangeable yet.*
//!
//! That is a missing CONFIG VOCABULARY and a missing SELECTION KEY, not a missing mechanism. The
//! mechanism — [`busbar_kernel::store::LaneRuntime::try_admit_breaker`], the ONE circuit breaker, keyed by
//! `(pool, lane)` — is good and is not rewritten here. This module is the seam that lets a plugin
//! reach it.
//!
...
//!
//! This is the hard part and it is not hand-waved. A candidate's identity is pinned by whatever
//! fingerprint the owning plugin computes for it — an approved schema digest, a verified registration
//! card, or an equivalent artifact identity. Two DIFFERENT upstreams exposing "the same" capability
//! have DIFFERENT fingerprints and different pins, so "interchangeable" cannot be allowed to mean
//! "identical artifact by operator assertion".
//!
//! So it does not mean that. **The canonical case this is built for is THE SAME UPSTREAM, DEPLOYED
//! TWICE** — one image in two regions, a hosted instance beside its self-hosted twin. Same image
//! means the same schema and the same description, and therefore THE SAME FINGERPRINT. That
...
//! claim against the fingerprints before it moves a single request, and says so when the claim is
//! false.
//!
//! The honest statement of what is NOT covered: two genuinely different upstreams offering a similar
//! capability have different pins, so they are refused here. That is deliberate. A call that failed
//! over between them could reach a target carrying different instructions than the primary, which is
//! the confused-deputy shape, and it must never be a default.
//!
//! ## THE SAFETY RULE, which is the reason this module has a `Stage` at all
//!
//! A call with side effects is not a read. Retrying an operation against a second deployment may
//! repeat its side effect — and the two deployments being the SAME IMAGE makes that worse, not
//! better, because both are wired to the same downstream. So the seam distinguishes two movements
//! that look identical in a log and are not remotely the same act:
//!
...
//! moving it to an equivalent deployment cannot duplicate anything; that is the case a caller never
...
//! unless the OPERATION ITSELF is declared [`Repeatable::Yes`]. Read-only work is the safe half and
//! an operator names it; anything else is refused by default, with no key to turn the rule off
//! wholesale.
//!
...
//!
//! ## ADDING A NEW PLUGIN COSTS A CANDIDATE TYPE AND NOTHING ELSE
//!
//! That is the acceptance test for this seam, and `tests/failover_tests.rs` declares a candidate type
//! for a plugin busbar does not have and shows it selects, admits, trips and reroutes with NO second
//! breaker, NO second walk and NO error type written for it.

// ## ONE SELECTION LOOP, FOR EVERY PLUGIN THAT FAILS OVER
//
// [`walk_with`] is that loop, and it is the only one. The breaker was already one FSM, one
// `breaker::classify`, one disposition pipeline, no per-plugin state machine — and this is what made
// it true of SELECTION as well. Some resident plugins previously carried their own admission loops;
// this module's own definition of the capability said "the one selection loop" while more than one
// existed. All resident selection now runs through this same function.
//
// WHY THIS DIRECTION. A plugin's own dispatch loop can carry weighting, routing policy, session
// affinity, queueing and an exhaustion policy, and the natural fear was that folding failover in
// would drag all of that into core. It did not, because those are not selection:
//
//   * WEIGHTING / ROUTING POLICY / AFFINITY are ORDER — they decide who is ASKED first, never who is
//     ALLOWED. They are an [`Order`] implementation a plugin supplies, and nothing they return can
//     admit a candidate the breaker refused, because they perform no admission at all.
//   * QUEUEING and an exhaustion policy are what a plugin does AFTER the loop finds nothing — they
//     live outside this loop entirely, so selection itself stays non-blocking.
...
//     verdict decoder and the same single-flight probe CAS. It rides in as the plugin's ADMISSION.
//
// The opposite direction — welding this loop to one plugin's own lane indices and internal weighted-
// lane type — was rejected on the code: doing so would make the loop unusable by any other resident
// plugin's candidate set (which runs on the shared breaker store's own runtime); and it has neither
// the pin check nor the repeat-safety rule, so those two safety rules would have had to be bolted on
// or lost on the plugins that need them most.
//
// MOUNTED. Resident plugins consume this seam directly: a candidate set is walked before a dispatch
// leg, and submission admission walks a pool the same way. Call sites record outcomes through the
// shared breaker store's own signal recorder — the same Stage-2 classifier this module's
// `record_outcome` wraps — rather than through `record_outcome` itself, because a shared lane table
// across pools means the all-cells hard-down write in `record_outcome` would trip every OTHER pool's
// member at the same position. `record_outcome`/`record_success` remain the halves exercised directly
// by `tests/failover_tests.rs`, and keep a narrow dead-code allow saying so.
...
pub fn record_outcome<C: Candidate>(
    ...
    cfg: &busbar_kernel::store::BreakerCfg,
...
pub fn walk_with<'a, C: Candidate, T>(
    pool: &str,
    members: &'a [C],
    attempt: &Attempt<'_>,
    order: &mut dyn Order,
