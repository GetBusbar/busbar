...
// The settled arm carries the frame the encode step wrote, inline. Boxing it would make every
// completed unit on the request path pay one allocation to hand its own bytes back, for the sake of
// a smaller `AlreadySettled` that is built once per lifetime of a hold; the ending is a value that
// crosses one stack frame and is destructured at once, and its size is not what that path is
// measured on.
