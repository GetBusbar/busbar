...
//! only type-checks when there is ONE `busbar_kernel` in the graph, which is exactly what an
//! integration-test target gives (busbar_kernel links as a normal dependency, the same instance the
...
fn the_carried_a2a_verify_gate_prunes_dead_subjects_and_drops_with_the_plane() {
    ...
    let cfg_with_agents = || {
        let mut c = cfg_with_provider_api_key(busbar_kernel::config::SecretRef::env(
    ...
    let removed = build_once(
        cfg_with_provider_api_key(busbar_kernel::config::SecretRef::env(
            "BUSBAR_TEST_NO_SUCH_KEY_A2A_PRUNE",
        )),
        Some(&prior),
    )
...
fn plane_slot_mirrors_the_typed_mcp_and_a2a_fields_when_configured() {
    ...
    busbar_mcp::testkit::install_test_seams();
    busbar_a2a::testkit::install_test_seams();
    let mut cfg = cfg_with_provider_api_key(busbar_kernel::config::SecretRef::env(
...
fn plane_slot_is_none_when_the_plane_is_not_configured() {
    ...
    busbar_mcp::testkit::install_test_seams();
    busbar_a2a::testkit::install_test_seams();
    let cfg = cfg_with_provider_api_key(busbar_kernel::config::SecretRef::env(
...
mod metrics_scrape {
    ...
    async fn mcp_and_a2a_traffic_appear_on_a_real_metrics_scrape() {
        ...
        let llm_counters = lines_for(&exposition, busbar_kernel::metrics::REQUESTS_TOTAL);
        assert!(
