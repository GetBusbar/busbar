...
//! Tests for the A2A relay's SSE frame reader (`sse.rs`): the reader's own battery, carried with it
//! from the neutral crate unchanged, plus the plane-code drift guard for its one diagnostic.
...
/// caller relaying a real event feed got nothing back and had to fail the exchange upstream), and a
