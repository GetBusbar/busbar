...
/// The CODEC's: `busbar-plane-a2a` names the same URL and may not name this crate.
pub(crate) use busbar_plane_a2a::ERROR_INFO_TYPE;

/// The domain every A2A `ErrorInfo` carries. Fixed by the specification, never this deployment's,
/// and the codec's for the reason above.
use busbar_plane_a2a::ERROR_INFO_DOMAIN;
...
/// [`crate::taskstore`] is the change that would give these two a caller; it is not made here.
...
/// `busbar_kernel::ingress::jsonrpc::read` before admission, so an envelope that is JSON and is not a
...
// close. The id now arrives already decided by `busbar_kernel::ingress::jsonrpc::read`: `Envelope::Request`
