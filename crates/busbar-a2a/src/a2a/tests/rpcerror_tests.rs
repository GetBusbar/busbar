...
fn an_id_that_was_never_sent_is_null_rather_than_absent() {
    ...
    // `busbar_kernel::ingress::jsonrpc::read` — the reader both planes share — so what is left for this
...
/// crate — so the table lives in `busbar-plane-a2a` and both halves read it. This enum keeps the
...
fn the_specific_error_table_is_the_codecs_own() {
    for (code, reason) in busbar_plane_a2a::ERRORS {
    ...
    for code in -32099..=-32001i64 {
        if let Some(err) = A2aError::from_code(code) {
            assert!(
                busbar_plane_a2a::ERRORS.iter().any(|(c, _)| *c == code),
