...
// `busbar_kernel::trust::verify`) BEFORE the relay's live trust gate. These prove the plane's half: a drifted
...
// plane-neutrally in `busbar_kernel::trust::verify`'s own batteries.
