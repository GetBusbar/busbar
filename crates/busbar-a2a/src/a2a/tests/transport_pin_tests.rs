...
fn no_path_in_this_crate_obtains_a_pin_by_switching_verification_off() {
    ...
    // over the WHOLE A2A crate rather than one module, because the shortcut is equally available to
    // the module next door and would be equally fatal there. Since the plane extraction the crate is
    // `busbar-a2a`; reached as a sibling of the dual-compiling `busbar-core` so the path resolves in
    // core's test binary too.
    let dir = std::path::Path::new(env!("CARGO_MANIFEST_DIR")).join("../busbar-a2a/src");
