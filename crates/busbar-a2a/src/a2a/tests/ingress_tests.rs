...
//!
//! Every assertion here goes through the real router, not through the handlers directly. A
...
/// `(path, bar)` pairs over public types, read through the engine kit's built-App seam.
fn mounted(app: &std::sync::Arc<dyn EngineAppPlus>) -> Vec<(String, RouteAuth)> {
    app.data_route_table()
}
...
fn no_agents_configured_means_no_route_in_the_table_at_all() {
    ...
    assert!(crate::a2a::runtime(app.as_ref()).is_none());
...
fn a_delegation_only_deployment_mounts_no_receiving_surface() {
    ...
    let app = engine()
        ...
        .build();
    assert!(
        crate::a2a::runtime(app.as_ref()).is_some(),
...
async fn an_anonymous_caller_is_refused_and_told_where_to_get_a_token() {
    ...
    engine().metrics_init();
    let store: std::sync::Arc<dyn busbar_contract::records::RecordStore> = engine().scratch_store();
    let gov = engine()
        .governance(
            store,
            None,
...
fn the_credential_kind_is_conferred_by_an_audience_bound_mount() {
    ...
    assert!(
        bound
            .mount_of("a2a")
            .and_then(|m| bound.admission_for(&m))
            .is_some(),
    ...
    assert!(
        unbound
            .mount_of("a2a")
            .and_then(|m| unbound.admission_for(&m))
            .is_none(),
