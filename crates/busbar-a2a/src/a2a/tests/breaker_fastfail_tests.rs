...
//! binding's card through the REAL router (`relay_harness`, the real router).
//!
...
async fn battery(binding: &str, submission: serde_json::Value) {
    ...
    // Submission 2: fast-fail must be sub-second, carry `503` + `Retry-After`, and leave the
    // recording transport untouched — all judged on outputs below.
    let t0 = Instant::now();
