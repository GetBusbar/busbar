...
impl busbar_kernel::plane_host::GauntletPlane for A2aInvokePlane {
    ...
    /// A2A'S CAPABILITY KEY on the composition-tier host-selection seam (loop unification, #28). The
    /// plane DECLARES its identity here; the LOOP CHOICE is the composition root's — it registers a
    /// kernel-loop runner under this same key ([`crate::PLANE_KEY`]) or leaves it UNSET. With W2.b the
    /// root flips `a2a` on, so this reported key routes the inbound A2A call through the unified kernel
    /// loop; the `drive` above is byte-identical either way, so declaring the key is safe with or
    /// without the flip.
