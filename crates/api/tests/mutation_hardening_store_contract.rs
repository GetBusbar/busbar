...
//! implemented the newer surface actually runs — had no direct test. That is the gap this file
//! closes: flipping a default's `Err(..)` to `Ok(())`, or its `Ok(Vec::new())` to `Ok(vec![...])`,
//! or dropping a step out of a sequential default, produces no compile error and, without the
//! cases below, no test failure either.
//!
