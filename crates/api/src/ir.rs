...
//! THE SUPERSET INTERMEDIATE REPRESENTATION — the vocabulary every protocol plugin reads into and
//! writes out of, and the reason a protocol crate can be written against `busbar-api` ALONE.
//!
//! A reader turns a wire body into an [`IrRequest`]; a writer turns an [`IrResponse`] or an
//! [`IrStreamEvent`] back into a wire body. Neither can be expressed without these types, so a
//! protocol ABI that does not carry the IR is not an ABI a protocol can be built against. That is
//! why this module is here rather than in the engine. (Fidelity contract: `docs/adr/0005-ir-fidelity.md`.)
//!
//! ## What is here, and what deliberately is not
//!
//! Here: the DATA — request/response/stream shapes, block and media vocabulary, tool and citation
//! shapes, usage counts, and the decode state a streaming reader threads through a translation.
//! Every inherent impl on these types is a fact ABOUT the shape (is this block opaque? what is the
//! normalized billable total?), never a policy decision about what the engine should DO with it.
//!
//! Not here, and staying in the engine: the disclosure projection a hook is allowed to see
//! (`ir::facts`), the per-operation `IrReq`/`IrResp` assembly, and the egress preparation that
//! applies lane gating, clamping and cap enforcement. Those are engine policy, and a plugin that
//! could name them could rewrite the rules it is supposed to be governed by.
//!
...
pub enum IrStreamEvent {
    ...
    MessageStop,
    Error(crate::breaker::CanonicalSignal),
}
...
pub enum IrDelta {
    ...
    /// [`IrCitation`] (neutral fields + the byte-exact `raw` escape hatch) rather than
