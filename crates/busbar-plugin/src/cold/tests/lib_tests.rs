...
use super::*;
use busbar_contract::records::{AuditRecord, ScopeRef, VirtualKey};
...
fn sample_credential_secret() -> CredentialSecret {
    CredentialSecret {
        meta: CredentialMeta {
            ...
            secret_form: busbar_contract::records::SecretForm::Recoverable,
            created_at: 42,
            updated_at: 42,
            expires_at: None,
            revoked_at: None,
...
fn request_response_json_roundtrip() {
    let reqs = vec![
            ...
            ledger: busbar_contract::records::UsageLedger {
                ...
                models: vec![busbar_contract::records::ModelTokens {
                    ...
                    usage_units: std::collections::BTreeMap::from([
                        (busbar_contract::records::UNIT_INPUT.to_string(), 7u64),
                        (busbar_contract::records::UNIT_OUTPUT.to_string(), 3),
        ...
        StoreRequest::AddUsage {
            ...
            window_start: 100,
            delta: busbar_contract::records::UsageDelta {
                requests: 1,
                billable_requests: 1,
                models: vec![busbar_contract::records::ModelTokensDelta {
                    model: "gpt-5".into(),
                    usage_units: std::collections::BTreeMap::from([
                        (busbar_contract::records::UNIT_INPUT.to_string(), 7i64),
                        (busbar_contract::records::UNIT_OUTPUT.to_string(), -3),
                    ]),
                }],
            },
