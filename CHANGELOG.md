...
from the hook view, and one refusal status on `POST /keys/{id}/rotate` — an improvement in
substance, registered here because the register lets only a `breaking` entry accept a status
change. Everything else that touches a 1.5.5 config, request or plugin is named above
