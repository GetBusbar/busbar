...
- **Observability** over open standards: opt-in Prometheus `/metrics` (an `export:` instance with `module: prometheus` and a required retention window), OTLP traces, a per-request audit webhook → [Configuration](https://getbusbar.com/docs/configuration/)
