...
#   docker run --rm -v ...same mounts... getbusbar/busbar --list-plugins
# `--list-plugins` inventories the tarballs in plugins.dir and verifies each signature/ABI window,
# printing `VERIFIED (not loaded)` because that is all it did. It never `dlopen`s anything — the
# 1.5.5 image printed `LOADS` for a tarball it could not load, and an operator surface that answers
# a question it never asked is a defect whichever way it guesses.
